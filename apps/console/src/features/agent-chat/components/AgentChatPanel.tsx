...
export function AgentChatPanel({
  ...
  // fires again. `send`/`sendDesignCommand` are held in refs and kept out of
  // the deps: their identity flips whenever `isSending` changes, which would
  // otherwise re-run this mid-turn. See the original panel note (#150) for
  // the full rationale.
  const sendRef = useRef(send);
  sendRef.current = send;
  const sendDesignCommandRef = useRef(sendDesignCommand);
  sendDesignCommandRef.current = sendDesignCommand;
  ...
  useEffect(() => {
    ...
    if (!engagedRef.current) {
      // Through the dedicated generate-design operation (gated on ae:design
      // alone), not `send(DESIGN_COMMAND)` — the whole point of
      // sendDesignCommand existing is that this CTA never rides create-turn's
      // ae:design-OR-ae:resource-config gate. The turn it produces is still
      // identical to a typed `/design`: aep-api classifies the same flow
      // token either way (spec.Service.StartDesignTurn), the wording lives in
      // the agents service, not here.
      sendDesignCommandRef.current();
    }
    onAutoGenerated?.();

...
// and wrapped with the grilling-interview directive (#270) — the console is
// the interactive caller that opts in, so headless users of the same builders
// stay one-shot.
...
export function AgentChatPanel({
  ...
  const { messages, isSending, send, answer } = useAgentChat(org, projectName);
  ...
  // not superseded by a later delivered user message; the composer stays an
  // equally valid answer path, so `awaiting` only softens the placeholder.
  // One O(n) pass per log change — not per card, not per keystroke.
  const answerable = useMemo(() => answerableQuestionIds(messages), [messages]);
  const awaiting = !isSending && answerable.size > 0;
              ...
              item.kind === "message" ? (
                item.message.role === "question" ? (
                  <QuestionCard
                    key={item.message.id}
                    msg={item.message}
                    answerable={answerable.has(item.message.id)}
                    busy={isSending}
                    onAnswer={answer}
                  />
                ) : (

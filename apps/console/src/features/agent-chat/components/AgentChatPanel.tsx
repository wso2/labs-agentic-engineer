...
//
// They belong to a project that HAS a spec. On a brand-new project the agent
// is already interviewing the user about their idea, and off-domain chips
// ("Add a returns-policy section") invited them to talk over it.
const SUGGESTIONS = [
...
export function AgentChatPanel({
  ...
  onClose: () => void;
  /** Generate-design CTA (#159): auto-send the design turn once. There is no
   *  requirements signal — the backend starts the spec interview itself
   *  (#485), so the console has no first turn to send. */
  autoGenerate?: "design";
  ...
  // The empty state is only honest for a project the agent is NOT already
  // working on. On a brand-new project the backend has started the interview
  // (#485), so a greeting plus three off-domain chips invited the user to talk
  // over the very turn they are waiting for. Read only while there is nothing
  // to show — on a thread with history the status query stays disabled.
  const firstRun = useSpecFirstRun(projectName, feed.length === 0);
  ...
  // NOTHING here navigates. Questions are answered on the spec view, and the
  // thread's own banner ("The agent has N questions — Answer them →") is how
  // the user gets there — from any page, on a click they made. The effect this
  // replaces fired for any PENDING question rather than one that arrived live,
  // so simply opening the rail over a parked interview teleported the user out
  ...
  // A turn of THIS panel's own that is still in flight. `engaged` covers the
  // project-wide log; this covers the window before the log has caught up.
  const sendingRef = useRef(isSending);
  sendingRef.current = isSending;
  // The backstop on the one injected command left. The Spec-view CTA already
  // declines to attach `?generate` while an agent is working, but the signal
  // also arrives from a pasted or bookmarked URL — and this is where "may I
  // send this" is actually decided.
  //
  // It covers BOTH ways a send would be destructive: a turn already running
  // (the 409 the user reads as "an agent turn is already running for this
  // project") and an unanswered question form, which the flow skills read as
  // the user's skip valve and answer for them.
  ...
  useEffect(() => {
    ...
    if (!engagedRef.current && !sendingRef.current) {
        ...
        <Box ref={contentRef}>
          {feed.length === 0 ? (
            ...
            >
              ...
              {firstRun.open ? (
                <>
                  <Typography variant="subtitle2">{firstRun.line}</Typography>
                  <Typography variant="body2" color="text.secondary">
                    The conversation about your idea starts here — it appears as
                    the agent works.
                  </Typography>
                </>
              ) : (

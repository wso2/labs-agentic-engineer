...
// The design CTA's own path (generate-design, not create-turn) — a separate
// spy so its tests can pin it apart from ordinary sends/seeds, the same way
// the real hook keeps it a distinct function from `send`.
const mockSendDesignCommand = vi.fn<() => Promise<boolean>>(async () => true);
...
vi.mock("../useAgentChat", () => ({
  useAgentChat: () => ({
    ...
    send: mockSend,
    sendDesignCommand: mockSendDesignCommand,
...
describe("AgentChatPanel — the injected generate is gated on an open exchange", () => {
  ...
  it("sends the design command when nothing is in flight", () => {
    ...
    expect(mockSendDesignCommand).toHaveBeenCalledTimes(1);
  ...
  it("holds the signal — unconsumed — until the thread id resolves", () => {
    ...
    expect(mockSendDesignCommand).not.toHaveBeenCalled();
...
describe("AgentChatPanel — the design CTA", () => {
  ...
  it("fires through the dedicated generate-design path, not a plain send", () => {
    // Through sendDesignCommand (generate-design, gated on ae:design alone),
    // never `send("/design")` — create-turn's own gate (ae:design OR
    // ae:resource-config) must never be what authorizes this CTA.

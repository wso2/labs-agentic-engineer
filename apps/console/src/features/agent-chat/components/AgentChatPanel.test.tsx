...
// There is no requirements signal any more (#485): the backend starts every
// project's `/start` turn at create, so the console has no first turn to inject
// and cannot race the one that is already running. `?generate=design` survives
// because no BE kickoff exists for `/design` — and it stays gated, because a
// design turn fired over a running turn is the same 409 and over an unanswered
// question form is the same silent skip-valve.
describe("AgentChatPanel — the design signal is gated on an open exchange", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  // The console composes no `/start`, on any path: the backend owns the first
  // turn, and a second starter is what produced "an agent turn is already
  // running for this project" for real users.
  it("never sends the start command", () => {
    renderPanel({ autoGenerate: "design", onAutoGenerated: vi.fn() });

    expect(mockSend).not.toHaveBeenCalledWith(START_COMMAND);
  });
...
// The one generation CTA left (#159 design). Requirements have none: the
// backend starts every project's `/start` turn at create (#485), so the console
// composes nothing and cannot race it.

...
const { Route: prototypeRoute } = await import("../../../routes/projects.$projectName_.json-prototype.$component");
...
function renderApp() {
  ...
  prototypeRoute.update({
    id: "/projects/$projectName_/json-prototype/$component",
    path: "/projects/$projectName/json-prototype/$component",
    getParentRoute: () => rootRoute,
...
describe("the prototype review journey", () => {
  it("opens from the Spec rail, previews, annotates two components, and sends both requests as one turn that refreshes once", async () => {
    ...
    expect(router.state.location.pathname).toBe(`/projects/${PROJECT}/json-prototype/${COMPONENT}`);

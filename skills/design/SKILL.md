...
a one-line pointer to `specs/design/`. When the cell declares a
`web-application`, the pointer may add that the user can then **Generate
prototype** in the Prototype stage for a clickable review. The dependency
narration during the turn (the `architecture` skill owns its format) already
carried the play-by-play.

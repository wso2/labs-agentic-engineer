...
|---|---|---|---|
| O-1 | **How `ae-design-agent` joins a Room on `ae-collab`** after the move (same pod, `localhost`). The user's JWT is no longer copied into the turn, and no token is chosen for this call. | It is a trust decision inside TB-4. **The spec is not locked until this is decided.** | [04-flows.md](04-flows.md), [07-identity-and-tokens.md](07-identity-and-tokens.md) |
...
A later wish to merge `ae-collab` and `ae-studio-tools` into one container also needs a new decision. It changes the secret split, the Room token `aud`, and TB-4 and TB-5.

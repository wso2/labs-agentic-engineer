...
so `dockerfile-builder` clones unauthenticated, which only works for public
repos. **Private-repo cloud builds are blocked on #319.** Not a builder limitation — local k3d builds private repos fine once the

...
- **Everything after the OC project + repo is best-effort.** Skills provisioning, the webhook, the
  ...
  the user one question from the `/start` skill; a kickoff that fails — the dispatch, or the first-run
  TURN it created dying afterwards — is surfaced on the spec stage (`status.spec.kickoff`), so the overview
  names it and offers Retry — nothing more.

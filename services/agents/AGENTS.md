...
`openapi.yaml` structure, `prototype.json` model — so validating a spec costs
no round trip), the SSE reader (`streamTurn`), and the published JSON Schema — lives in the workspace package

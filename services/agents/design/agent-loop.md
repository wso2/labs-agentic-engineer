...
| **The model is chosen per turn** (turn body `model`, else `AGENT_MODEL`) | the organization picks one model for every agent, and a change applies from its next turn: the BFF resolves the org's key and model together on each turn and sends both, the route builds the model from them (`buildModel(apiKey, modelId)`), and the manifest attributes usage to that same id. `AGENT_MODEL` is only the default for a caller that names none (the playground, local dev) |
| **Reasoning effort is model-aware** (`modelProviderOptions(modelId)`) | `@ai-sdk/anthropic` forwards `effort` as `output_config.effort` without checking the model, and Haiku 4.5 / Sonnet 4.5 answer it with a 400. Those turns carry no provider options; every other offered model gets `AGENT_REASONING_EFFORT` |

...
| 2 | `aep-api` → `ae-design-agent` | Org kgateway, TLS only | Design turns; the source of the SSE that `aep-api` copies to the browser | CP → DP service token, `aud` = org + `ae-design-agent` | `ae-design-agent`: signature against aep-api JWKS, `aud`, `exp`, token org = this pod's org. |
...
`localhost` calls carry no extra token.

| Pod | From → to | What |
|---|---|---|
| `ae-studio` | `ae-collab` → `ae-studio-tools` | Files API: `files/bundle`, `files/apply`, seed, flush. `ae-studio-tools` commits and pushes. |
| `ae-studio` | `ae-studio-tools` → `ae-design-agent` | Snapshots written to a shared named emptyDir. `ae-design-agent` reads them. |
| `ae-studio` | `ae-design-agent` → `ae-collab` | Join a Room for a Room-mode turn. **Not yet decided:** how `ae-design-agent` proves itself to `ae-collab`. See [12-gaps-and-open-items.md](12-gaps-and-open-items.md). |
| coding Job | `ae-coding-agent` → `ae-coding-tools` | Git, GitHub and platform actions for this run. `ae-coding-tools` never returns a secret value. |

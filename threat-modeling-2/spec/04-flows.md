...
|---|---|---|---|---|---|
| 1 | Browser → `aep-api` | The console's own web server, then `aep-api` inside the control plane. A public gateway route for the console is planned. | Console REST, turn start, design-turn SSE (`GET …/turns/{id}/stream`), Room token request | Platform IdP user JWT | `aep-api` itself: signature against the Platform IdP JWKS, `iss=platform-idp`, `aud`, `exp`. Then it authorizes user, org and Room. The web server checks nothing. |
...
| 10 | `aep-api` → SM API | Control plane | Write gitpat, org HMAC, Default key, Coding agent token | The user's Platform IdP JWT, passed through (WSO2 Cloud). Writes with no user on the request: open (O-3). | SM API writes vault through the OpenChoreo Secret API. It returns keys and `secretReferenceName` only. |
...
|---|---|---|
| Turn start, design-turn SSE, other console REST, Room token request | Console web server, then `aep-api` (flow 1) | Platform IdP user JWT |

...
|---|---|---|---|
| **TB-1** Internet → `aep-api` gateway | 1 user REST and SSE; 6 webhook events; 7a run platform calls; 12 design agent MCP tool calls | `jwt-auth` `iss=platform-idp`; `aep-api` authorizes user, org and Room; for 6: redact, dedup on delivery id; for 12: `/internal/v1/mcp` accepts only the publisher client token, org from `ouHandle` | none |

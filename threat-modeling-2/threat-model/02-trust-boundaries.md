...
| **TB-3** Control plane ↔ dataplane | Calls in both directions; secret values read into the dataplane | No private network path. Every call goes through a public gateway and carries a token. | GAP-2, O-10 | AE-03, AE-04, AE-06 |
...
| **TB-8** Dataplane → internet (egress) | Model calls, GitHub, calls back to the API | Only DNS and public ports 80/443. Private ranges, cloud metadata and the Kubernetes API are blocked. | none | AE-03, AE-04, AE-05, AE-06 |

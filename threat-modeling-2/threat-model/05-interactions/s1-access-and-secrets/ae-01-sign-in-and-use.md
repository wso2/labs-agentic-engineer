...
| :---- | :---- | :---- |
| User's browser (console app) | Platform IdP (sign-in); console web server | Agentic Engineer API; Postgres; OpenChoreo (through the platform API) |
...
- Login token (JWT): issuer `platform-idp`, audience (who the token is for) `APP_FACTORY_CONSOLE`, the org (`ouHandle`, `ouId`), the user, and the user's permissions (`scope`, `ae:*` keys). Lives 1 hour; the browser refreshes it.
...
| AE-01-2 | Tampering | A malicious org member puts another org's name in a request to read or change that org's data. | No | **Implemented:** the API takes the org only from the login token and scopes every record to it. Platform calls carry the user's token, and the platform limits them to that org. |
| AE-01-3 | Repudiation | A user denies starting a build or deploy. | No | **Implemented:** builds, deploys and run steps are stored with the user who started them. |
| AE-01-4 | Information disclosure | A Developer reads data meant for Admins, such as usage and cost, or the org's AI keys. | No | **Implemented:** the permission check refuses views the role does not allow. Secret values are never returned; the console shows only a short prefix and the last four characters. |
...
- **Planned:** console calls also go through the public gateway, so the token is checked there too and the gateway's traffic limits apply.
- **Planned (PR #778):** WSO2 Cloud sign-in issues the `ae-admin` and `ae-developer` roles and their `ae:*` permissions, and the console asks for them.

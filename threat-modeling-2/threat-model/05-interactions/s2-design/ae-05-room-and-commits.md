...
| Access Control and Authorization | Permission check, then one Room per token | Joining a Room needs `ae:design-view`, and every shipped role also holds `ae:design`. The token opens only one Room of one org. |
...
| AE-05-3 | Repudiation | A user denies making a spec change. | No | **By design:** each commit lists the people in the Room as co-authors. Git keeps the history of every change. |

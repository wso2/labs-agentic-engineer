...
The spec is written in stages — requirements, design, then, optionally, for
each `web-application` the design declares, a read-only **prototype**
...
prototype is a review step beside the design's `wireframes.dsl`, which stays the
web application's screen contract: Build asks for the wireframes, not the
prototype, and the coding run builds its screens from them —

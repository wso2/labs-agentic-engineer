...
| Types | 161 declarations (32 records, 129 other), not listed here — read one with `type` |

## Next

- `bal-library ops ballerinax/postgresql <path>` — navigate a client's operations
- `bal-library type ballerinax/postgresql <Name> [--deps]` — read a declaration whole
- `bal-library api ballerinax/postgresql` — every declaration, when nothing above answered

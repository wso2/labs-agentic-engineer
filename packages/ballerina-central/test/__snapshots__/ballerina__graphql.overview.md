...
| Source | central |
| Clients | `Client` |
| Module functions | 2, listed below |
| Errors | 9, listed below |
| Types | 60 declarations (47 records, 5 unions, 8 other), not listed here — read one with `type` |

## Next

- `bal-library ops ballerina/graphql <path>` — navigate a client's operations
- `bal-library type ballerina/graphql <Name> [--deps]` — read a declaration whole
- `bal-library api ballerina/graphql` — every declaration, when nothing above answered

// ============================================================
// Library: ballerinax/github
// [GitHub](https://github.com/) is a widely used platform for version control and collaboration, allowing developers to work together on projects from anywhere. It hosts a vast array of both open-source and private projects, providing a suite of development tools for collaborative software development.
// ============================================================
import ballerinax/github;

// --- Types ---


type AccessSelectedUsersBody record {
    # The usernames of the organization members whose codespaces be billed to the organization
    string[] selectedUsernames;
};


type AccessSelectedUsersBody1 record {
    # The usernames of the organization members whose codespaces should not be billed to the organization
    string[] selectedUsernames;
};


type ActionsBillingUsage record {
    # The total paid GitHub Actions minutes used
    int totalPaidMinutesUsed;
    # The amount of free GitHub Actions minutes available
    int includedMinutes;
    # The sum of the free and paid GitHub Actions minutes used
    int totalMinutesUsed;
    # Breakdown of GitHub Actions minutes used by operating system.
    ActionsBillingUsageMinutesUsedBreakdown minutesUsedBreakdown;
};


type ActionsBillingUsageMinutesUsedBreakdown record {
    # Total minutes used on Windows 4 core runner machines
    int windows4Core?;
    # Total minutes used on Ubuntu 64 core runner machines
    int ubuntu64Core?;
    # Total minutes used on macOS runner machines
    int mACOS?;
    # Total minutes used on macOS 12 core runner machines
    int macos12Core?;
    # Total minutes used on Ubuntu runner machines
    int uBUNTU?;
    # Total minutes used on Ubuntu 8 core runner machines
    int ubuntu8Core?;
    # Total minutes used on all runner machines
    int total?;
    # Total minutes used on Windows 64 core runner machines
    int windows64Core?;
    # Total minutes used on Ubuntu 32 core runner machines
    int ubuntu32Core?;
    # Total minutes used on Windows runner machines
    int wINDOWS?;
    # Total minutes used on Ubuntu 16 core runner machines
    int ubuntu16Core?;
    # Total minutes used on Windows 32 core runner machines
    int windows32Core?;
    # Total minutes used on Ubuntu 4 core runner machines
    int ubuntu4Core?;
    # Total minutes used on Windows 8 core runner machines
    int windows8Core?;
    # Total minutes used on Windows 16 core runner machines
    int windows16Core?;
};

# Repository actions caches

type ActionsCacheList record {
    # Total number of caches
    int totalCount;
    # Array of caches
    ActionsCacheListActionsCaches[] actionsCaches;
};


type ActionsCacheListActionsCaches record {
    # The Git ref associated with the cache entry.
    string ref?;
    # The size of the cache entry in bytes.
    int sizeInBytes?;
    # The timestamp indicating when the cache entry was created.
    string createdAt?;
    # The unique identifier of the cache entry.
    int id?;
    # The timestamp indicating when the cache entry was last accessed.
    string lastAccessedAt?;
    # The version identifier of the cache entry.
    string version?;
    # The cache key used to identify the cache entry.
    string 'key?;
};

# GitHub Actions Cache Usage by repository

type ActionsCacheUsageByRepository record {
    # The repository owner and name for the cache usage being shown
    string fullName;
    # The number of active caches in the repository
    int activeCachesCount;
    # The sum of the size in bytes of all the active cache items in the repository
    int activeCachesSizeInBytes;
};

# GitHub Actions Cache Usage by repository

type ActionsCacheUsageByRepositoryResponse record {
    # Total number of repositories with Actions cache usage data.
    int totalCount;
    # List of repositories and their Actions cache usage details.
    ActionsCacheUsageByRepository[] repositoryCacheUsages;
};


type ActionsCacheUsageOrgEnterprise record {
    # The total size in bytes of all active cache items across all repositories of an enterprise or an organization
    int totalActiveCachesSizeInBytes;
    # The count of active caches across all repositories of an enterprise or an organization
    int totalActiveCachesCount;
};

# Represents the Queries record for the operation: actions/delete-actions-cache-by-key

type ActionsDeleteActionsCacheByKeyQueries record {
    # The full Git reference for narrowing down the cache. The ref for a branch should be formatted as refs/heads/<branch name>. To reference a pull request use refs/pull/<number>/merge
    string ref?;
    # A key for identifying the cache
    string 'key;
};

# Represents the Queries record for the operation: actions/get-actions-cache-list

type ActionsGetActionsCacheListQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The full Git reference for narrowing down the cache. The ref for a branch should be formatted as refs/heads/<branch name>. To reference a pull request use refs/pull/<number>/merge
    string ref?;
    # Page number of the results to fetch
    int page = 1;
    # The property to sort the results by. created_at means when the cache was created. last_accessed_at means when the cache was last accessed. size_in_bytes is the size of the cache in bytes
        "created_at"|"last_accessed_at"|"size_in_bytes"  sort = "last_accessed_at";
    # An explicit key or prefix for identifying the cache
    string 'key?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: actions/get-actions-cache-usage-by-repo-for-org

type ActionsGetActionsCacheUsageByRepoForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type ActionsGetDefaultWorkflowPermissions record {
    # The default permissions granted to the GITHUB_TOKEN in workflows.
    ActionsDefaultWorkflowPermissions defaultWorkflowPermissions;
    # Whether GitHub Actions can approve pull request reviews.
    ActionsCanApprovePullRequestReviews canApprovePullRequestReviews;
};

# Represents the Queries record for the operation: actions/get-workflow-run-attempt

type ActionsGetWorkflowRunAttemptQueries record {
    # If true pull requests are omitted from the response (empty array)
    boolean excludePullRequests = false;
};

# Represents the Queries record for the operation: actions/get-workflow-run

type ActionsGetWorkflowRunQueries record {
    # If true pull requests are omitted from the response (empty array)
    boolean excludePullRequests = false;
};

# Represents the Queries record for the operation: actions/list-artifacts-for-repo

type ActionsListArtifactsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name field of an artifact. When specified, only artifacts with this name will be returned
    string name?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-environment-secrets

type ActionsListEnvironmentSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-environment-variables

type ActionsListEnvironmentVariablesQueries record {
    # The number of results per page (max 30)
    int perPage = 10;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-jobs-for-workflow-run-attempt

type ActionsListJobsForWorkflowRunAttemptQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-jobs-for-workflow-run

type ActionsListJobsForWorkflowRunQueries record {
    # Filters jobs by their completed_at timestamp. latest returns jobs from the most recent execution of the workflow run. all returns all jobs for a workflow run, including from old executions of the workflow run
        "latest"|"all"  filter = "latest";
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-org-secrets

type ActionsListOrgSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-org-variables

type ActionsListOrgVariablesQueries record {
    # The number of results per page (max 30)
    int perPage = 10;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-repo-organization-secrets

type ActionsListRepoOrganizationSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-repo-organization-variables

type ActionsListRepoOrganizationVariablesQueries record {
    # The number of results per page (max 30)
    int perPage = 10;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-repo-secrets

type ActionsListRepoSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-repo-variables

type ActionsListRepoVariablesQueries record {
    # The number of results per page (max 30)
    int perPage = 10;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-repo-workflows

type ActionsListRepoWorkflowsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-selected-repos-for-org-secret

type ActionsListSelectedReposForOrgSecretQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-selected-repos-for-org-variable

type ActionsListSelectedReposForOrgVariableQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-selected-repositories-enabled-github-actions-organization

type ActionsListSelectedRepositoriesEnabledGithubActionsOrganizationQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-self-hosted-runners-for-org

type ActionsListSelfHostedRunnersForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name of a self-hosted runner
    string name?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-self-hosted-runners-for-repo

type ActionsListSelfHostedRunnersForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name of a self-hosted runner
    string name?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-workflow-run-artifacts

type ActionsListWorkflowRunArtifactsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name field of an artifact. When specified, only artifacts with this name will be returned
    string name?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: actions/list-workflow-runs-for-repo

type ActionsListWorkflowRunsForRepoQueries record {
    # Returns someone's workflow runs. Use the login for the user who created the push associated with the check suite or workflow run
    string actor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Returns workflow runs with the check_suite_id that you specify
    int checkSuiteId?;
    # Returns workflow runs created within the given date-time range. For more information on the syntax, see "[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates)."
    string created?;
    # If true pull requests are omitted from the response (empty array)
    boolean excludePullRequests = false;
    # Page number of the results to fetch
    int page = 1;
    # Returns workflow run triggered by the event you specify. For example, push, pull_request or issue. For more information, see "[Events that trigger workflows](https://docs.github.com/actions/automating-your-workflow-with-github-actions/events-that-trigger-workflows)."
    string event?;
    # Returns workflow runs associated with a branch. Use the name of the branch of the push
    string branch?;
    # Only returns workflow runs that are associated with the specified head_sha
    string headSha?;
    # Returns workflow runs with the check run status or conclusion that you specify. For example, a conclusion can be success or a status can be in_progress. Only GitHub can set a status of waiting or requested
        "completed"|"action_required"|"cancelled"|"failure"|"neutral"|"skipped"|"stale"|"success"|"timed_out"|"in_progress"|"queued"|"requested"|"waiting"|"pending"  status?;
};

# Represents the Queries record for the operation: actions/list-workflow-runs

type ActionsListWorkflowRunsQueries record {
    # Returns someone's workflow runs. Use the login for the user who created the push associated with the check suite or workflow run
    string actor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Returns workflow runs with the check_suite_id that you specify
    int checkSuiteId?;
    # Returns workflow runs created within the given date-time range. For more information on the syntax, see "[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates)."
    string created?;
    # If true pull requests are omitted from the response (empty array)
    boolean excludePullRequests = false;
    # Page number of the results to fetch
    int page = 1;
    # Returns workflow run triggered by the event you specify. For example, push, pull_request or issue. For more information, see "[Events that trigger workflows](https://docs.github.com/actions/automating-your-workflow-with-github-actions/events-that-trigger-workflows)."
    string event?;
    # Returns workflow runs associated with a branch. Use the name of the branch of the push
    string branch?;
    # Only returns workflow runs that are associated with the specified head_sha
    string headSha?;
    # Returns workflow runs with the check run status or conclusion that you specify. For example, a conclusion can be success or a status can be in_progress. Only GitHub can set a status of waiting or requested
        "completed"|"action_required"|"cancelled"|"failure"|"neutral"|"skipped"|"stale"|"success"|"timed_out"|"in_progress"|"queued"|"requested"|"waiting"|"pending"  status?;
};

# Actions OIDC subject customization for a repository

type ActionsOIDCSubjectCustomizationForARepository record {
    # Array of unique strings. Each claim key can only contain alphanumeric characters and underscores
    string[] includeClaimKeys?;
    # Whether to use the default template or not. If true, the include_claim_keys field is ignored
    boolean useDefault;
};


type ActionsOrganizationPermissions record {
    # The policy that controls which repositories can use GitHub Actions.
    EnabledRepositories enabledRepositories;
    # The policy that controls which actions and reusable workflows are allowed.
    AllowedActions allowedActions?;
    # The API URL to use to get or set the selected repositories that are allowed to run GitHub Actions, when enabled_repositories is set to selected
    string selectedRepositoriesUrl?;
    # The API URL to get or set the allowed actions for the organization.
    SelectedActionsUrl selectedActionsUrl?;
};


type ActionsPermissionsBody record {
    # The policy for which repositories are permitted to use Actions.
    EnabledRepositories enabledRepositories;
    # The policy for which Actions and reusable workflows are allowed to run.
    AllowedActions allowedActions?;
};


type ActionsPermissionsBody1 record {
    # The actions and reusable workflows permitted to run in the organization.
    AllowedActions allowedActions?;
    # Whether GitHub Actions is enabled for the organization.
    ActionsEnabled enabled;
};

# The public key used for setting Actions Secrets

type ActionsPublicKey record {
    # The identifier for the key
    string keyId;
    # The date and time the public key was created.
    string createdAt?;
    # The unique identifier of the public key.
    int id?;
    # A descriptive title for the public key.
    string title?;
    # The Base64 encoded public key
    string 'key;
    # The API URL for the public key.
    string url?;
};


type ActionsRepositoryPermissions record {
    # The permissions policy that controls which actions are allowed to run.
    AllowedActions allowedActions?;
    # The API URL to retrieve the list of selected allowed actions.
    SelectedActionsUrl selectedActionsUrl?;
    # Indicates whether GitHub Actions is enabled for the repository.
    ActionsEnabled enabled;
};

# Set secrets for GitHub Actions

type ActionsSecret record {
    # The date and time the secret was last updated.
    string updatedAt;
    # The name of the secret
    string name;
    # The date and time the secret was created.
    string createdAt;
};

# Set secrets for GitHub Actions

type ActionsSecretResponse record {
    # The total number of Actions secrets available.
    int totalCount;
    # The list of Actions secrets.
    ActionsSecret[] secrets;
};


type ActionsSetDefaultWorkflowPermissions record {
    # The default permissions granted to the GITHUB_TOKEN for workflows.
    ActionsDefaultWorkflowPermissions defaultWorkflowPermissions?;
    # Indicates whether GitHub Actions can approve pull request reviews.
    ActionsCanApprovePullRequestReviews canApprovePullRequestReviews?;
};


type ActionsVariable record {
    # The date and time at which the variable was last updated, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # The name of the variable
    string name;
    # The date and time at which the variable was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string createdAt;
    # The value of the variable
    string value;
};


type ActionsVariableResponse record {
    # The list of Actions variables returned in the response.
    ActionsVariable[] variables;
    # The total number of Actions variables available.
    int totalCount;
};


type ActionsVariablesBody record {
    # An array of repository ids that can access the organization variable. You can only provide a list of repository ids when the visibility is set to selected
    int[] selectedRepositoryIds?;
    # The type of repositories in the organization that can access the variable. selected means only the repositories specified by selected_repository_ids can access the variable
        "all"|"private"|"selected"  visibility;
    # The name of the variable
    string name;
    # The value of the variable
    string value;
};


type ActionsVariablesBody1 record {
    # The name of the variable
    string name;
    # The value of the variable
    string value;
};


type ActionsWorkflowAccessToRepository record {
    # Defines the level of access that workflows outside of the repository have to actions and reusable workflows within the
repository.

none means the access is only possible from workflows in this repository. user level access allows sharing across user owned private repos only. organization level access allows sharing across the organization
        "none"|"user"|"organization"  accessLevel;
};

# Activity

type Activity record {
    # The user who performed the activity.
    NullableSimpleUser? actor;
    # The full Git reference, formatted as refs/heads/<branch name>
    string ref;
    # The SHA of the commit before the activity
    string before;
    # The type of the activity that was performed
        "push"|"force_push"|"branch_deletion"|"branch_creation"|"pr_merge"|"merge_queue_merge"  activityType;
    # The unique identifier of the activity.
    int id;
    # The SHA of the commit after the activity
    string after;
    # The GraphQL node identifier of the activity.
    string nodeId;
    # The time when the activity occurred
    string timestamp;
};

# Represents the Queries record for the operation: activity/list-events-for-authenticated-user

type ActivityListEventsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-notifications-for-authenticated-user

type ActivityListNotificationsForAuthenticatedUserQueries record {
    # If true, show notifications marked as read
    boolean all = false;
    # The number of results per page (max 50)
    int perPage = 50;
    # Only show notifications updated before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string before?;
    # If true, only shows notifications in which the user is directly participating or mentioned
    boolean participating = false;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: activity/list-org-events-for-authenticated-user

type ActivityListOrgEventsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-public-events-for-repo-network

type ActivityListPublicEventsForRepoNetworkQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-public-events-for-user

type ActivityListPublicEventsForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-public-events

type ActivityListPublicEventsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-public-org-events

type ActivityListPublicOrgEventsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-received-events-for-user

type ActivityListReceivedEventsForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-received-public-events-for-user

type ActivityListReceivedPublicEventsForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-repo-events

type ActivityListRepoEventsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-repo-notifications-for-authenticated-user

type ActivityListRepoNotificationsForAuthenticatedUserQueries record {
    # If true, show notifications marked as read
    boolean all = false;
    # The number of results per page (max 100)
    int perPage = 30;
    # Only show notifications updated before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string before?;
    # If true, only shows notifications in which the user is directly participating or mentioned
    boolean participating = false;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: activity/list-repos-starred-by-authenticated-user

type ActivityListReposStarredByAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by. created means when the repository was starred. updated means when the repository was last pushed to
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: activity/list-repos-starred-by-user

type ActivityListReposStarredByUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by. created means when the repository was starred. updated means when the repository was last pushed to
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: activity/list-repos-watched-by-user

type ActivityListReposWatchedByUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-stargazers-for-repo

type ActivityListStargazersForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-watched-repos-for-authenticated-user

type ActivityListWatchedReposForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: activity/list-watchers-for-repo

type ActivityListWatchersForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Actor

type Actor record {
    # The display name shown for the actor in GitHub events.
    string displayLogin?;
    # URL of the actor's avatar image.
    string avatarUrl;
    # Unique identifier of the actor.
    int id;
    # The username of the actor.
    string login;
    # Gravatar ID associated with the actor's email.
    string? gravatarId;
    # API URL for the actor's GitHub profile.
    string url;
};

# Added to Project Issue Event

type AddedToProjectIssueEvent record {
    # The user who triggered this issue event.
    SimpleUser actor;
    # API URL for the commit associated with this event.
    string? commitUrl;
    # The GitHub App that performed this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The date and time when this event occurred.
    string createdAt;
    # Unique numeric identifier for this issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # API URL for this issue event.
    string url;
    # The project card associated with this event.
    RemovedFromProjectIssueEventProjectCard projectCard?;
    # The GraphQL node identifier for this issue event.
    string nodeId;
};


type AlertsalertNumberBody record {
    # An optional comment explaining why the alert was dismissed.
    CodeScanningAlertDismissedComment? dismissedComment?;
    # The desired state to set for the code scanning alert.
    CodeScanningAlertSetState state;
    # The reason for dismissing or closing the alert.
    CodeScanningAlertDismissedReason? dismissedReason?;
};


type AlertsalertNumberBody1 record {
    # An optional comment associated with dismissing the alert
    string dismissedComment?;
    # The state of the Dependabot alert.
A dismissed_reason must be provided when setting the state to dismissed
        "dismissed"|"open"  state;
    # **Required when state is dismissed.** A reason for dismissing the alert
        "fix_started"|"inaccurate"|"no_bandwidth"|"not_used"|"tolerable_risk"  dismissedReason?;
};


type AlertsalertNumberBody2 record {
    # An optional comment explaining the resolution of the secret scanning alert.
    SecretScanningAlertResolutionComment? resolutionComment?;
    # The state to set for the secret scanning alert.
    SecretScanningAlertState state;
    # The reason for resolving the secret scanning alert.
    SecretScanningAlertResolution? resolution?;
};

# Api Overview

type ApiOverview record {
    # SSH key fingerprints used to verify GitHub's host keys.
    ApiOverviewSshKeyFingerprints sshKeyFingerprints?;
    # IP ranges used by GitHub's source importer service.
    string[] importer?;
    # Indicates whether password authentication is verifiable.
    boolean verifiablePasswordAuthentication;
    # Domain names used by various GitHub services.
    ApiOverviewDomains domains?;
    # IP ranges used by GitHub Packages.
    string[] packages?;
    # IP ranges used by the GitHub Enterprise importer.
    string[] githubEnterpriseImporter?;
    # Public SSH keys used by GitHub.
    string[] sshKeys?;
    # IP ranges used for Git operations on GitHub.
    string[] git?;
    # IP ranges used by GitHub Pages.
    string[] pages?;
    # IP ranges used by GitHub's web interface.
    string[] web?;
    # IP ranges used by GitHub's API.
    string[] api?;
    # IP ranges used by GitHub webhooks.
    string[] hooks?;
    # IP ranges used by GitHub Actions.
    string[] actions?;
    # IP ranges used by Dependabot.
    string[] dependabot?;
};


type ApiOverviewDomains record {
    # The list of domains used for GitHub website traffic.
    string[] website?;
    # The list of domains used by GitHub Copilot.
    string[] copilot?;
    # The list of domains used by GitHub Codespaces.
    string[] codespaces?;
    # The list of domains used by GitHub Packages.
    string[] packages?;
};


type ApiOverviewSshKeyFingerprints record {
    # The SHA256 fingerprint of the RSA SSH host key.
    string sHA256RSA?;
    # The SHA256 fingerprint of the DSA SSH host key.
    string sHA256DSA?;
    # The SHA256 fingerprint of the ECDSA SSH host key.
    string sHA256ECDSA?;
    # The SHA256 fingerprint of the Ed25519 SSH host key.
    string sHA256ED25519?;
};

# The permissions granted to the user access token

type AppPermissions record {
    # The level of permission to grant the access token to view and manage secret scanning alerts
        "read"|"write"  secretScanningAlerts?;
    # The level of permission to grant the access token to search repositories, list collaborators, and access repository metadata
        "read"|"write"  metadata?;
    # The level of permission to grant the access token to view and manage announcement banners for an organization
        "read"|"write"  organizationAnnouncementBanners?;
    # The level of permission to grant the access token for organization packages published to GitHub Packages
        "read"|"write"  organizationPackages?;
    # The level of permission to grant the access token for managing repository environments
        "read"|"write"  environments?;
    # The level of permission to grant the access token to manage team discussions and related comments
        "read"|"write"  teamDiscussions?;
    # The level of permission to grant the access token for repository creation, deletion, settings, teams, and collaborators creation
        "read"|"write"  administration?;
    # The level of permission to grant the access token for viewing an organization's plan
        "read"  organizationPlan?;
    # The level of permission to grant the access token to manage Dependabot alerts
        "read"|"write"  vulnerabilityAlerts?;
    # The level of permission to grant the access token for viewing and managing fine-grained personal access token requests to an organization
        "read"|"write"  organizationPersonalAccessTokens?;
    # The level of permission to grant the access token for viewing and managing fine-grained personal access tokens that have been approved by an organization
        "read"|"write"  organizationPersonalAccessTokenRequests?;
    # The level of permission to grant the access token to update GitHub Actions workflow files
        "write"  workflows?;
    # The level of permission to grant the access token for issues and related comments, assignees, labels, and milestones
        "read"|"write"  issues?;
    # The level of permission to grant the access token for pull requests and related comments, assignees, labels, milestones, and merges
        "read"|"write"  pullRequests?;
    # The level of permission to grant the access token to view and manage users blocked by the organization
        "read"|"write"  organizationUserBlocking?;
    # The level of permission to grant the access token to manage just a single file
        "read"|"write"  singleFile?;
    # The level of permission to grant the access token to retrieve Pages statuses, configuration, and builds, as well as create new builds
        "read"|"write"  pages?;
    # The level of permission to grant the access token to manage repository projects, columns, and cards
        "read"|"write"|"admin"  repositoryProjects?;
    # The level of permission to grant the access token for organization teams and members
        "read"|"write"  members?;
    # The level of permission to grant the access token for custom repository roles management. This property is in beta and is subject to change
        "read"|"write"  organizationCustomRoles?;
    # The level of permission to grant the access token to manage access to an organization
        "read"|"write"  organizationAdministration?;
    # The level of permission to grant the access token to manage organization secrets
        "read"|"write"  organizationSecrets?;
    # The level of permission to grant the access token to manage the post-receive hooks for an organization
        "read"|"write"  organizationHooks?;
    # The level of permission to grant the access token for packages published to GitHub Packages
        "read"|"write"  packages?;
    # The level of permission to grant the access token to manage repository secrets
        "read"|"write"  secrets?;
    # The level of permission to grant the access token for deployments and deployment statuses
        "read"|"write"  deployments?;
    # The level of permission to grant the access token for checks on code
        "read"|"write"  checks?;
    # The level of permission to grant the access token to manage organization projects and projects beta (where available)
        "read"|"write"|"admin"  organizationProjects?;
    # The level of permission to grant the access token to view and manage security events like code scanning alerts
        "read"|"write"  securityEvents?;
    # The level of permission to grant the access token for repository contents, commits, branches, downloads, releases, and merges
        "read"|"write"  contents?;
    # The level of permission to grant the access token to manage the post-receive hooks for a repository
        "read"|"write"  repositoryHooks?;
    # The level of permission to grant the access token for commit statuses
        "read"|"write"  statuses?;
    # The level of permission to grant the access token for GitHub Actions workflows, workflow runs, and artifacts
        "read"|"write"  actions?;
    # The level of permission to grant the access token to view and manage GitHub Actions self-hosted runners available to an organization
        "read"|"write"  organizationSelfHostedRunners?;
};

# Represents the Queries record for the operation: apps/list-accounts-for-plan

type AppsListAccountsForPlanQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # To return the oldest accounts first, set to asc. Ignored without the sort parameter
        "asc"|"desc"  direction?;
};

# Represents the Queries record for the operation: apps/list-accounts-for-plan-stubbed

type AppsListAccountsForPlanStubbedQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # To return the oldest accounts first, set to asc. Ignored without the sort parameter
        "asc"|"desc"  direction?;
};

# Represents the Queries record for the operation: apps/list-installation-repos-for-authenticated-user

type AppsListInstallationReposForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-installation-requests-for-authenticated-app

type AppsListInstallationRequestsForAuthenticatedAppQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-installations-for-authenticated-user

type AppsListInstallationsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-installations

type AppsListInstallationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter to only return outdated installations.
    string outdated?;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: apps/list-plans

type AppsListPlansQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-plans-stubbed

type AppsListPlansStubbedQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-repos-accessible-to-installation

type AppsListReposAccessibleToInstallationQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-subscriptions-for-authenticated-user

type AppsListSubscriptionsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-subscriptions-for-authenticated-user-stubbed

type AppsListSubscriptionsForAuthenticatedUserStubbedQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: apps/list-webhook-deliveries

type AppsListWebhookDeliveriesQueries record {
    # Used for pagination: the starting delivery from which the page of deliveries is fetched. Refer to the link header for the next and previous page cursors
    string cursor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter to only return redeliveries.
    boolean redelivery?;
};

# An artifact

type Artifact record {
    # The URL to download the artifact as a zip archive.
    string archiveDownloadUrl;
    # Whether or not the artifact has expired
    boolean expired;
    # The date and time when the artifact expires.
    string? expiresAt;
    # The workflow run associated with this artifact.
    ArtifactWorkflowRun? workflowRun?;
    # The date and time the artifact was last updated.
    string? updatedAt;
    # The name of the artifact
    string name;
    # The size in bytes of the artifact
    int sizeInBytes;
    # The date and time the artifact was created.
    string? createdAt;
    # The unique numeric identifier for the artifact.
    int id;
    # The API URL for this artifact.
    string url;
    # The GraphQL node ID for the artifact.
    string nodeId;
};

# An artifact

type ArtifactResponse record {
    # The total number of artifacts available.
    int totalCount;
    # The list of artifact objects returned.
    Artifact[] artifacts;
};


type ArtifactWorkflowRun record {
    # The branch from which the workflow run was triggered.
    string headBranch?;
    # The unique identifier of the repository where the workflow ran.
    int repositoryId?;
    # The unique identifier of the head repository for the workflow run.
    int headRepositoryId?;
    # The unique identifier of the workflow run.
    int id?;
    # The SHA of the head commit for the workflow run.
    string headSha?;
};


type AssetsassetIdBody record {
    # The file name of the asset
    string name?;
    # An alternate short description of the asset. Used in place of the filename
    string label?;
    # The state of the release asset.
    string state?;
};

# Assigned Issue Event

type AssignedIssueEvent record {
    # The user who triggered the assignment event.
    SimpleUser actor;
    # API URL for the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed the assignment event.
    Integration performedViaGithubApp;
    # The user who assigned the issue.
    SimpleUser assigner;
    # The date and time the event was created.
    string createdAt;
    # The unique identifier of the event.
    int id;
    # The user who was assigned to the issue.
    SimpleUser assignee;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # API URL for the event.
    string url;
    # The GraphQL node identifier of the event.
    string nodeId;
};

# Authentication Token

type AuthenticationToken record {
    # Describe whether all repositories have been selected or there's a selection involved
        "all"|"selected"  repositorySelection?;
    # The single file path this token grants access to.
    string? singleFile?;
    # The time this token expires
    string expiresAt;
    # The repositories this token has access to
    Repository[] repositories?;
    # The permissions granted to this authentication token.
    record {} permissions?;
    # The token used for authentication
    string token;
};

# The authorization for an OAuth app, GitHub App, or a Personal Access Token

type Authorization record {
    # The OAuth application associated with this authorization.
    AuthorizationApp app;
    # A note to remind you what the authorization is for.
    string? note;
    # A URL to remind you what the authorization is for.
    string? noteUrl;
    # The timestamp indicating when the authorization was created.
    string createdAt;
    # The URL of the authorization resource.
    string url;
    # The OAuth access token string.
    string token;
    # The hashed version of the OAuth access token.
    string? hashedToken;
    # The timestamp indicating when the token expires.
    string? expiresAt;
    # The timestamp indicating when the authorization was last updated.
    string updatedAt;
    # The scoped installation associated with this authorization.
    NullableScopedInstallation? installation?;
    # The last eight characters of the token for identification.
    string? tokenLastEight;
    # A unique string to distinguish an authorization from others with the same note.
    string? fingerprint;
    # The unique identifier of the authorization.
    int id;
    # A list of scopes that this authorization is in
    string[]? scopes;
    # The user associated with this authorization.
    NullableSimpleUser? user?;
};


type AuthorizationApp record {
    # The name of the OAuth application.
    string name;
    # The client ID of the OAuth application.
    string clientId;
    # The URL of the OAuth application's homepage.
    string url;
};


type AuthorsauthorIdBody record {
    # The new Git author name
    string name?;
    # The new Git author email
    string email?;
};

# An autolink reference

type Autolink record {
    # The prefix of a key that is linkified
    string keyPrefix;
    # A template for the target URL that is generated if a key was found
    string urlTemplate;
    # The unique identifier of the autolink reference.
    int id;
    # Whether this autolink reference matches alphanumeric characters. If false, this autolink reference only matches numeric characters
    boolean isAlphanumeric;
};

# The status of auto merging a pull request

type AutoMerge record {
    # Title for the merge commit message
    string commitTitle;
    # The user who enabled auto-merge on the pull request.
    SimpleUser enabledBy;
    # Commit message for the merge commit
    string commitMessage;
    # The merge method to use
        "merge"|"squash"|"rebase"  mergeMethod;
};

# Base Gist

type BaseGist record {
    # The user who owns the gist.
    SimpleUser owner?;
    # List of gist forks.
    anydata[] forks?;
    # API URL for listing gist commits.
    string commitsUrl;
    # The number of comments on the gist.
    int comments;
    # API URL for listing gist forks.
    string forksUrl;
    # The Git push URL for the gist.
    string gitPushUrl;
    # The date and time the gist was created.
    string createdAt;
    # A short description of the gist.
    string? description;
    # Whether the gist content is truncated due to size.
    boolean truncated?;
    # List of gist revision history entries.
    anydata[] history?;
    # API URL for the gist.
    string url;
    # Whether the gist is public.
    boolean 'public;
    # The date and time the gist was last updated.
    string updatedAt;
    # URL of the gist page on GitHub.
    string htmlUrl;
    # The Git pull URL for the gist.
    string gitPullUrl;
    # API URL for listing gist comments.
    string commentsUrl;
    # The files included in the gist.
    record {} files;
    # The unique identifier of the gist.
    string id;
    # The user associated with the gist, if applicable.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the gist.
    string nodeId;
};


type BaseGistFiles record {
    # The name of the gist file.
    string filename?;
    # The size of the gist file in bytes.
    int size?;
    # The detected programming language of the gist file.
    string language?;
    # The MIME type of the gist file.
    string 'type?;
    # The URL to retrieve the raw contents of the gist file.
    string rawUrl?;
};


type BillingSelectedTeamsBody record {
    # List of team names within the organization to which to grant access to GitHub Copilot
    string[] selectedTeams;
};


type BillingSelectedTeamsBody1 record {
    # The names of teams from which to revoke access to GitHub Copilot
    string[] selectedTeams;
};


type BillingSelectedUsersBody record {
    # The usernames of the organization members to be granted access to GitHub Copilot
    string[] selectedUsernames;
};


type BillingSelectedUsersBody1 record {
    # The usernames of the organization members for which to revoke access to GitHub Copilot
    string[] selectedUsernames;
};

# Blob

type Blob record {
    # The size of the blob content in bytes.
    int? size;
    # The encoding used for the blob content, such as base64 or utf-8.
    string encoding;
    # The SHA hash of the blob object.
    string sha;
    # The syntax-highlighted HTML content of the blob.
    string highlightedContent?;
    # The encoded content of the blob.
    string content;
    # The API URL for this blob object.
    string url;
    # The GraphQL node ID for the blob.
    string nodeId;
};

# Branch Protection

type BranchProtection record {
    # The pull request review requirements for the protected branch.
    ProtectedBranchPullRequestReview requiredPullRequestReviews?;
    # The commit signature requirements for the protected branch.
    ProtectedBranchRequiredSignatures requiredSignatures?;
    # The required status checks that must pass before merging.
    ProtectedBranchRequiredStatusCheck requiredStatusChecks?;
    # Indicates whether forked repositories can be synced with this branch.
    BranchProtectionAllowForkSyncing allowForkSyncing?;
    # The user, team, and app restrictions for pushing to the branch.
    BranchRestrictionPolicy restrictions?;
    # Indicates whether a linear commit history is required.
    BranchProtectionRequiredLinearHistory requiredLinearHistory?;
    # Indicates whether branch protections are enforced for administrators.
    ProtectedBranchAdminEnforced enforceAdmins?;
    # The API URL for this branch protection configuration.
    string url?;
    # Indicates whether branch protection is enabled.
    boolean enabled?;
    # Indicates whether force pushes are permitted to the branch.
    BranchProtectionRequiredLinearHistory allowForcePushes?;
    # Indicates whether the branch is locked against changes.
    BranchProtectionLockBranch lockBranch?;
    # Indicates whether branch creations matching this rule are blocked.
    BranchProtectionRequiredLinearHistory blockCreations?;
    # Indicates whether all conversations must be resolved before merging.
    BranchProtectionRequiredLinearHistory requiredConversationResolution?;
    # The name of the protected branch.
    string name?;
    # Indicates whether the branch can be deleted.
    BranchProtectionRequiredLinearHistory allowDeletions?;
    # The API URL for the branch protection resource.
    string protectionUrl?;
};

# Whether users can pull changes from upstream when the branch is locked. Set to true to allow fork syncing. Set to false to prevent fork syncing

type BranchProtectionAllowForkSyncing record {
    # Whether fork syncing is enabled for the locked branch.
    boolean enabled = false;
};


type BranchProtectionBody record {
    # Whether to set the branch as read-only. If this is true, users will not be able to push to the branch. Default: false
    boolean lockBranch = false;
    # Settings for required pull request reviews before merging.
    ReposownerrepobranchesbranchprotectionRequiredPullRequestReviews? requiredPullRequestReviews;
    # If set to true, the restrictions branch protection settings which limits who can push will also block pushes which create new branches, unless the push is initiated by a user, team, or app which has the ability to push. Set to true to restrict new branch creation. Default: false
    boolean blockCreations?;
    # Requires all conversations on code to be resolved before a pull request can be merged into a branch that matches this rule. Set to false to disable. Default: false
    boolean requiredConversationResolution?;
    # Settings for required status checks before merging.
    ReposownerrepobranchesbranchprotectionRequiredStatusChecks? requiredStatusChecks;
    # Whether users can pull changes from upstream when the branch is locked. Set to true to allow fork syncing. Set to false to prevent fork syncing. Default: false
    boolean allowForkSyncing = false;
    # Restrictions on who can push to the protected branch.
    ReposownerrepobranchesbranchprotectionRestrictions? restrictions;
    # Enforces a linear commit Git history, which prevents anyone from pushing merge commits to a branch. Set to true to enforce a linear commit history. Set to false to disable a linear commit Git history. Your repository must allow squash merging or rebase merging before you can enable a linear commit history. Default: false. For more information, see "[Requiring a linear commit history](https://docs.github.com/github/administering-a-repository/requiring-a-linear-commit-history)" in the GitHub Help documentation
    boolean requiredLinearHistory?;
    # Enforce all configured restrictions for administrators. Set to true to enforce required status checks for repository administrators. Set to null to disable
    boolean? enforceAdmins;
    # Allows deletion of the protected branch by anyone with write access to the repository. Set to false to prevent deletion of the protected branch. Default: false. For more information, see "[Enabling force pushes to a protected branch](https://docs.github.com/github/administering-a-repository/enabling-force-pushes-to-a-protected-branch)" in the GitHub Help documentation
    boolean allowDeletions?;
    # Permits force pushes to the protected branch by anyone with write access to the repository. Set to true to allow force pushes. Set to false or null to block force pushes. Default: false. For more information, see "[Enabling force pushes to a protected branch](https://docs.github.com/github/administering-a-repository/enabling-force-pushes-to-a-protected-branch)" in the GitHub Help documentation."
    boolean? allowForcePushes?;
};

# Whether to set the branch as read-only. If this is true, users will not be able to push to the branch

type BranchProtectionLockBranch record {
    # Whether the branch is set as read-only.
    boolean enabled = false;
};


type BranchProtectionRequiredLinearHistory record {
    # Whether required linear history is enabled for the branch.
    boolean enabled?;
};


type BranchRenameBody record {
    # The new name of the branch
    string newName;
};

# Branch Restriction Policy

type BranchRestrictionPolicy record {
    # API URL for the teams allowed to push to the branch.
    string teamsUrl;
    # The list of teams allowed to push to the restricted branch.
    BranchRestrictionPolicyTeams[] teams;
    # API URL for the users allowed to push to the branch.
    string usersUrl;
    # API URL for the branch restriction policy.
    string url;
    # API URL for the apps allowed to push to the branch.
    string appsUrl;
    # The list of users allowed to push to the restricted branch.
    RepositoryTemplateRepositoryOwner[] users;
    # The list of apps allowed to push to the restricted branch.
    BranchRestrictionPolicyApps[] apps;
};


type BranchRestrictionPolicyApps record {
    # The owner of the GitHub App.
    BranchRestrictionPolicyOwner owner?;
    # The external URL associated with the GitHub App.
    string externalUrl?;
    # The date and time the app was last updated.
    string updatedAt?;
    # The permissions granted to the GitHub App.
    BranchRestrictionPolicyPermissions permissions?;
    # The HTML URL for the GitHub App's page.
    string htmlUrl?;
    # The name of the GitHub App.
    string name?;
    # A short description of the GitHub App.
    string description?;
    # The date and time the app was created.
    string createdAt?;
    # The unique numeric identifier for the GitHub App.
    int id?;
    # The URL-friendly name of the GitHub App.
    string slug?;
    # The list of events the GitHub App subscribes to.
    string[] events?;
    # The GraphQL node ID for the GitHub App.
    string nodeId?;
};


type BranchRestrictionPolicyOwner record {
    # API URL listing the owner's repositories.
    string reposUrl?;
    # API URL template for the owner's gists.
    string gistsUrl?;
    # API URL template for the organization's members.
    string membersUrl?;
    # API URL template for users this owner follows.
    string followingUrl?;
    # The description of the organization.
    string description?;
    # API URL template for repositories starred by the owner.
    string starredUrl?;
    # The owner's GitHub login username.
    string login?;
    # API URL for the owner's followers.
    string followersUrl?;
    # The type of GitHub account.
    string 'type?;
    # API URL for the owner.
    string url?;
    # API URL template for the organization's public members.
    string publicMembersUrl?;
    # API URL for the owner's repository subscriptions.
    string subscriptionsUrl?;
    # API URL template for the organization's issues.
    string issuesUrl?;
    # API URL for events received by the owner.
    string receivedEventsUrl?;
    # URL of the owner's avatar image.
    string avatarUrl?;
    # API URL template for events performed by the owner.
    string eventsUrl?;
    # URL of the owner's profile page on GitHub.
    string htmlUrl?;
    # Whether the owner is a GitHub site administrator.
    boolean siteAdmin?;
    # The unique numeric identifier of the owner.
    int id?;
    # API URL for the organization's webhooks.
    string hooksUrl?;
    # The Gravatar identifier for the owner's avatar.
    string gravatarId?;
    # The GraphQL node identifier of the owner.
    string nodeId?;
    # API URL for the owner's organization memberships.
    string organizationsUrl?;
};


type BranchRestrictionPolicyPermissions record {
    # Permission level granted for repository metadata.
    string metadata?;
    # Permission level granted for a single specified file.
    string singleFile?;
    # Permission level granted for repository contents.
    string contents?;
    # Permission level granted for repository issues.
    string issues?;
};


type BranchRestrictionPolicyTeams record {
    # The parent team of this team, if any.
    string? parent?;
    # The URL to list repositories accessible to this team.
    string repositoriesUrl?;
    # The URL to list members of this team.
    string membersUrl?;
    # A description of the team.
    string? description?;
    # The privacy level of the team (secret or closed).
    string privacy?;
    # The default permission level for this team.
    string permission?;
    # The API URL of the team resource.
    string url?;
    # The notification setting for members of this team.
    string notificationSetting?;
    # The URL to view the team on GitHub.
    string htmlUrl?;
    # The name of the team.
    string name?;
    # The unique identifier of the team.
    int id?;
    # The URL-friendly name of the team.
    string slug?;
    # The GraphQL node identifier of the team.
    string nodeId?;
};

# Branch Short

type BranchShort record {
    # Indicates whether the branch is protected.
    boolean protected;
    # The name of the branch.
    string name;
    # The latest commit on the branch.
    BranchShortCommit 'commit;
};


type BranchShortCommit record {
    # The SHA hash of the commit.
    string sha;
    # API URL for the commit.
    string url;
};

# Branch With Protection

type BranchWithProtection record {
    # Whether the branch has branch protection rules enabled.
    boolean protected;
    # Hypermedia links related to the branch.
    BranchWithProtectionLinks links;
    # The number of approving reviews required before merging.
    int requiredApprovingReviewCount?;
    # The name of the branch.
    string name;
    # The latest commit on the branch.
    Commit 'commit;
    # The branch name pattern used for protection rules.
    string pattern?;
    # The branch protection settings applied to the branch.
    BranchProtection protection;
    # API URL for the branch protection settings.
    string protectionUrl;
};


type BranchWithProtectionLinks record {
    # The API URL for the branch resource.
    string self;
    # The URL to view the branch on GitHub.
    string html;
};


type CardIdMovesBody record {
    # The unique identifier of the column the card should be moved to
    int columnId?;
    # The position of the card in a column. Can be one of: top, bottom, or after:<card_id> to place after the specified card
    string position;
};


type CardscardIdBody record {
    # The project card's note
    string? note?;
    # Whether or not the card is archived
    boolean archived?;
};

# Check Annotation

type CheckAnnotation record {
    # The file path to which this annotation applies.
    string path;
    # The starting column number of the annotated code.
    int? startColumn;
    # The severity level of the annotation (notice, warning, or failure).
    string? annotationLevel;
    # The URL to the annotated file blob on GitHub.
    string blobHref;
    # Additional raw details providing extra information for the annotation.
    string? rawDetails;
    # The starting line number of the annotated code.
    int startLine;
    # The title of the annotation.
    string? title;
    # The message describing the annotation.
    string? message;
    # The ending line number of the annotated code.
    int endLine;
    # The ending column number of the annotated code.
    int? endColumn;
};

# Check Automated Security Fixes

type CheckAutomatedSecurityFixes record {
    # Whether automated security fixes are paused for the repository
    boolean paused;
    # Whether automated security fixes are enabled for the repository
    boolean enabled;
};

# A check performed on the code of a given code change

type CheckRun record {
    # The GitHub App that created this check run.
    NullableIntegration? app;
    # A reference identifier for the check run provided by the app.
    string? externalId;
    # The URL where details about the check run are displayed.
    string? detailsUrl;
    # The SHA of the commit that is being checked
    string headSha;
    # The API URL for this check run.
    string url;
    # The final conclusion result of the completed check run.
        "success"|"failure"|"neutral"|"cancelled"|"skipped"|"timed_out"|"action_required"? conclusion;
    # The output summary and annotations for this check run.
    CheckRunOutput output;
    # The date and time the check run completed.
    string? completedAt;
    # Pull requests that are open with a head_sha or head_branch that matches the check. The returned pull requests do not necessarily indicate pull requests that triggered the check
    PullRequestMinimal[] pullRequests;
    # The HTML URL of the check run on GitHub.
    string? htmlUrl;
    # The name of the check
    string name;
    # The date and time the check run started.
    string? startedAt;
    # The id of the check
    int id;
    # The check suite this check run belongs to.
    CheckRunCheckSuite? checkSuite;
    # The GraphQL node identifier of the check run.
    string nodeId;
    # The phase of the lifecycle that the check is currently in
        "queued"|"in_progress"|"completed"  status;
    # The deployment associated with this check run.
    DeploymentSimple deployment?;
};


type CheckRunCheckSuite record {
    # The unique identifier of the check suite.
    int id;
};


type CheckRunOutput record {
    # A brief summary of the check run results.
    string? summary;
    # API URL to retrieve annotations for the check run output.
    string annotationsUrl;
    # Detailed information about the check run results.
    string? text;
    # The title of the check run output.
    string? title;
    # The total number of annotations associated with the check run.
    int annotationsCount;
};

# A check performed on the code of a given code change

type CheckRunResponse record {
    # List of check runs associated with the code change.
    CheckRun[] checkRuns;
    # The total number of check runs in the response.
    int totalCount;
};

# Represents the Queries record for the operation: checks/list-annotations

type ChecksListAnnotationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: checks/list-for-ref

type ChecksListForRefQueries record {
    # Filters check runs by their completed_at timestamp. latest returns the most recent check runs
        "latest"|"all"  filter = "latest";
    # The number of results per page (max 100)
    int perPage = 30;
    # Returns check runs with the specified name
    string checkName?;
    # Page number of the results to fetch
    int page = 1;
    # Filter check runs by the app that created them, using the app's ID.
    int appId?;
    # Returns check runs with the specified status
        "queued"|"in_progress"|"completed"  status?;
};

# Represents the Queries record for the operation: checks/list-for-suite

type ChecksListForSuiteQueries record {
    # Filters check runs by their completed_at timestamp. latest returns the most recent check runs
        "latest"|"all"  filter = "latest";
    # The number of results per page (max 100)
    int perPage = 30;
    # Returns check runs with the specified name
    string checkName?;
    # Page number of the results to fetch
    int page = 1;
    # Returns check runs with the specified status
        "queued"|"in_progress"|"completed"  status?;
};

# Represents the Queries record for the operation: checks/list-suites-for-ref

type ChecksListSuitesForRefQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Returns check runs with the specified name
    string checkName?;
    # Page number of the results to fetch
    int page = 1;
    # Filters check suites by GitHub App id
    int appId?;
};

# A suite of checks performed on the code of a given code change

type CheckSuite record {
    # The GitHub App that created this check suite.
    NullableIntegration? app;
    # The commit at the head of the branch being checked.
    SimpleCommit headCommit;
    # The name of the branch the check suite is associated with.
    string? headBranch;
    # The SHA of the commit before the head commit in the push.
    string? before;
    # The timestamp indicating when the check suite was created.
    string? createdAt;
    # The repository this check suite belongs to.
    MinimalRepository repository;
    # The SHA of the head commit that is being checked
    string headSha;
    # The API URL of the check suite resource.
    string? url;
    # The final conclusion of the check suite after completion.
        "success"|"failure"|"neutral"|"cancelled"|"skipped"|"timed_out"|"action_required"|"startup_failure"|"stale"? conclusion;
    # The pull requests associated with this check suite.
    PullRequestMinimal[]? pullRequests;
    # The timestamp indicating when the check suite was last updated.
    string? updatedAt;
    # The total number of check runs in this check suite.
    int latestCheckRunsCount;
    # Whether the check suite can be manually rerequested.
    boolean rerequestable?;
    # Whether individual check runs in the suite can be rerequested.
    boolean runsRerequestable?;
    # The unique identifier of the check suite.
    int id;
    # The SHA of the commit after the head commit in the push.
    string? after;
    # The URL to list all check runs in this check suite.
    string checkRunsUrl;
    # The GraphQL node identifier of the check suite.
    string nodeId;
    # The phase of the lifecycle that the check suite is currently in. Statuses of waiting, requested, and pending are reserved for GitHub Actions check suites
        "queued"|"in_progress"|"completed"? status;
};

# Check suite configuration preferences for a repository

type CheckSuitePreference record {
    # The auto-trigger preferences for check suites on the repository.
    CheckSuitePreferencePreferences preferences;
    # The repository to which the check suite preferences apply.
    MinimalRepository repository;
};


type CheckSuitePreferencePreferences record {
    # The list of auto-trigger check settings per GitHub App.
    CheckSuitePreferencePreferencesAutoTriggerChecks[] autoTriggerChecks?;
};


type CheckSuitePreferencePreferencesAutoTriggerChecks record {
    # The unique identifier of the GitHub App.
    int appId;
    # Whether auto-trigger is enabled for the specified app's check suites.
    boolean setting;
};

# A suite of checks performed on the code of a given code change

type CheckSuiteResponse record {
    # The total number of check suites returned.
    int totalCount;
    # The list of check suites.
    CheckSuite[] checkSuites;
};


type CheckSuitesPreferencesBody record {
    # Enables or disables automatic creation of CheckSuite events upon pushes to the repository. Enabled by default
    ReposownerrepocheckSuitespreferencesAutoTriggerChecks[] autoTriggerChecks?;
};

# A GitHub Classroom classroom

type Classroom record {
    # Whether classroom is archived
    boolean archived;
    # The GitHub organization associated with the classroom.
    SimpleClassroomOrganization organization;
    # The name of the classroom
    string name;
    # Unique identifier of the classroom
    int id;
    # The URL of the classroom on GitHub Classroom
    string url;
};

# A GitHub Classroom accepted assignment

type ClassroomAcceptedAssignment record {
    # Count of student commits
    int commitCount;
    # Whether an accepted assignment has been submitted
    boolean submitted;
    # The classroom assignment associated with this accepted assignment.
    SimpleClassroomAssignment assignment;
    # Most recent grade
    string grade;
    # The list of students who accepted the assignment.
    SimpleClassroomUser[] students;
    # Unique identifier of the repository
    int id;
    # Whether a submission passed
    boolean passing;
    # The repository created for the accepted assignment.
    SimpleClassroomRepository repository;
};

# A GitHub Classroom assignment

type ClassroomAssignment record {
    # The selected editor for the assignment
    string editor;
    # Whether feedback pull request will be created when a student accepts the assignment
    boolean feedbackPullRequestsEnabled;
    # Whether the invitation link is enabled. Visiting an enabled invitation link will accept the assignment
    boolean invitationsEnabled;
    # Whether students are admins on created repository when a student accepts the assignment
    boolean studentsAreRepoAdmins;
    # The number of students that have accepted the assignment
    int accepted;
    # The programming language used in the assignment
    string language;
    # The classroom to which this assignment belongs.
    Classroom classroom;
    # Assignment title
    string title;
    # Whether it's a group assignment or individual assignment
        "individual"|"group"  'type;
    # The link that a student can use to accept the assignment
    string inviteLink;
    # The repository used as starter code for the assignment.
    SimpleClassroomRepository starterCodeRepository;
    # The number of students that have submitted the assignment
    int submitted;
    # The maximum allowable teams for the assignment
    int? maxTeams;
    # Whether an accepted assignment creates a public repository
    boolean publicRepo;
    # The maximum allowable members per team
    int? maxMembers;
    # Unique identifier of the repository
    int id;
    # The number of students that have passed the assignment
    int passing;
    # The time at which the assignment is due
    string? deadline;
    # Sluggified name of the assignment
    string slug;
};

# Grade for a student or groups GitHub Classroom assignment

type ClassroomAssignmentGrade record {
    # URL of the assignment
    string assignmentUrl;
    # Timestamp of the student's assignment submission
    string submissionTimestamp;
    # Roster identifier of the student
    string rosterIdentifier;
    # If a group assignment, name of the group the student is in
    string groupName?;
    # URL of the student's assignment repository
    string studentRepositoryUrl;
    # Number of points awarded to the student
    int pointsAwarded;
    # GitHub username of the student
    string githubUsername;
    # Name of the assignment
    string assignmentName;
    # Number of points available for the assignment
    int pointsAvailable;
    # URL of the starter code for the assignment
    string starterCodeUrl;
    # Name of the student's assignment repository
    string studentRepositoryName;
};

# Represents the Queries record for the operation: classroom/list-accepted-assigments-for-an-assignment

type ClassroomListAcceptedAssigmentsForAnAssignmentQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: classroom/list-assignments-for-a-classroom

type ClassroomListAssignmentsForAClassroomQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: classroom/list-classrooms

type ClassroomListClassroomsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type ClientIdGrantBody record {
    # The OAuth access token used to authenticate to the GitHub API
    string accessToken;
};


type ClientIdTokenBody record {
    # The access_token of the OAuth or GitHub application
    string accessToken;
};

# Clone Traffic

type CloneTraffic record {
    # List of clone traffic data points over time.
    Traffic[] clones;
    # The total number of clones over the reporting period.
    int count;
    # The number of unique cloners over the reporting period.
    int uniques;
};

# Code Of Conduct

type CodeOfConduct record {
    # The HTML URL to view the code of conduct on GitHub.
    string? htmlUrl;
    # The display name of the code of conduct.
    string name;
    # The full text content of the code of conduct.
    string body?;
    # The unique key identifier for the code of conduct.
    string 'key;
    # The API URL for the code of conduct.
    string url;
};

# Code of Conduct Simple

type CodeOfConductSimple record {
    # The URL of the code of conduct page on GitHub.
    string? htmlUrl;
    # The name of the code of conduct.
    string name;
    # The API URL for the code of conduct.
    string url;
    # The unique key identifying the code of conduct.
    string 'key;
};

# A list of errors found in a repo's CODEOWNERS file

type CodeownersErrors record {
    # List of errors found in the repository's CODEOWNERS file.
    CodeownersErrorsErrors[] errors;
};


type CodeownersErrorsErrors record {
    # The path of the file where the error occured
    string path;
    # The line number where this errors occurs
    int line;
    # The type of error
    string kind;
    # Suggested action to fix the error. This will usually be null, but is provided for some common errors
    string? suggestion?;
    # The column number where this errors occurs
    int column;
    # The contents of the line where the error occurs
    string 'source?;
    # A human-readable description of the error, combining information from multiple fields, laid out for display in a monospaced typeface (for example, a command-line setting)
    string message;
};


type CodeScanningAlert record {
    # The URL to retrieve all instances of this code scanning alert.
    AlertInstancesUrl instancesUrl;
    # Timestamp when the code scanning alert was created.
    AlertCreatedAt createdAt;
    # Optional comment provided when the alert was dismissed.
    CodeScanningAlertDismissedComment? dismissedComment?;
    # The code scanning rule that triggered this alert.
    CodeScanningAlertRule rule;
    # The API URL for this code scanning alert.
    AlertUrl url;
    # The analysis tool that generated this code scanning alert.
    CodeScanningAnalysisTool tool;
    # The unique number identifying this alert within the repository.
    AlertNumber number;
    # Timestamp when the code scanning alert was last updated.
    AlertUpdatedAt updatedAt?;
    # The HTML URL to view the alert on GitHub.
    AlertHtmlUrl htmlUrl;
    # Timestamp when the code scanning alert was fixed.
    AlertFixedAt? fixedAt?;
    # The most recent instance of this code scanning alert.
    CodeScanningAlertInstance mostRecentInstance;
    # The state of the code scanning alert.
    CodeScanningAlertState state;
    # The user who dismissed the code scanning alert.
    NullableSimpleUser? dismissedBy;
    # The reason provided for dismissing the alert.
    CodeScanningAlertDismissedReason? dismissedReason;
    # Timestamp when the code scanning alert was dismissed.
    AlertDismissedAt? dismissedAt;
};


type CodeScanningAlertInstance record {
    # Classifications that have been applied to the file that triggered the alert.
For example identifying it as documentation, or a generated file
    CodeScanningAlertClassification[] classifications?;
    # The Git reference associated with this alert instance.
    CodeScanningRef ref?;
    # The environment in which the alert instance was detected.
    CodeScanningAlertEnvironment environment?;
    # The SHA of the commit where the alert instance was detected.
    string commitSha?;
    # The URL of the alert instance page on GitHub.
    string htmlUrl?;
    # The code location where the alert was detected.
    CodeScanningAlertLocation location?;
    # The state of the code scanning alert instance.
    CodeScanningAlertState state?;
    # The analysis key that identifies the analysis configuration.
    CodeScanningAnalysisAnalysisKey analysisKey?;
    # The category of the code scanning analysis that produced this alert.
    CodeScanningAnalysisCategory category?;
    # The message associated with the code scanning alert instance.
    CodeScanningAlertInstanceMessage message?;
};


type CodeScanningAlertInstanceMessage record {
    # The message text describing the code scanning alert.
    string text?;
};


type CodeScanningAlertItems record {
    # The API URL to list all instances of this alert.
    AlertInstancesUrl instancesUrl;
    # The date and time the alert was created.
    AlertCreatedAt createdAt;
    # Optional comment provided when the alert was dismissed.
    CodeScanningAlertDismissedComment? dismissedComment?;
    # The code scanning rule that triggered this alert.
    CodeScanningAlertRuleSummary rule;
    # The API URL for this code scanning alert.
    AlertUrl url;
    # The code scanning tool that generated this alert.
    CodeScanningAnalysisTool tool;
    # The unique number identifying this alert within the repository.
    AlertNumber number;
    # The date and time the alert was last updated.
    AlertUpdatedAt updatedAt?;
    # The HTML URL of the alert on GitHub.
    AlertHtmlUrl htmlUrl;
    # The date and time the alert was fixed.
    AlertFixedAt? fixedAt?;
    # The most recent instance of this alert.
    CodeScanningAlertInstance mostRecentInstance;
    # The current state of the code scanning alert.
    CodeScanningAlertState state;
    # The user who dismissed this alert.
    NullableSimpleUser? dismissedBy;
    # The reason provided when the alert was dismissed.
    CodeScanningAlertDismissedReason? dismissedReason;
    # The date and time the alert was dismissed.
    AlertDismissedAt? dismissedAt;
};

# Describe a region within a file for the alert

type CodeScanningAlertLocation record {
    # The file path where the alert was detected.
    string path?;
    # The column number where the alert region begins.
    int startColumn?;
    # The line number where the alert region begins.
    int startLine?;
    # The line number where the alert region ends.
    int endLine?;
    # The column number where the alert region ends.
    int endColumn?;
};


type CodeScanningAlertRule record {
    # The severity of the alert
        "none"|"note"|"warning"|"error"? severity?;
    # Detailed documentation for the rule as GitHub Flavored Markdown
    string? help?;
    # The security severity of the alert
        "low"|"medium"|"high"|"critical"? securitySeverityLevel?;
    # description of the rule used to detect the alert
    string fullDescription?;
    # The name of the rule used to detect the alert
    string name?;
    # A short description of the rule used to detect the alert
    string description?;
    # A unique identifier for the rule used to detect the alert
    string? id?;
    # A link to the documentation for the rule used to detect the alert
    string? helpUri?;
    # A set of tags applicable for the rule
    string[]? tags?;
};


type CodeScanningAlertRuleSummary record {
    # The severity of the alert
        "none"|"note"|"warning"|"error"? severity?;
    # The name of the rule used to detect the alert
    string name?;
    # A short description of the rule used to detect the alert
    string description?;
    # A unique identifier for the rule used to detect the alert
    string? id?;
    # A set of tags applicable for the rule
    string[]? tags?;
};


type CodeScanningAnalysis record {
    # Indicates whether this analysis can be deleted.
    boolean deletable;
    # The date and time the analysis was created.
    CodeScanningAnalysisCreatedAt createdAt;
    # The total number of rules used in the analysis
    int rulesCount;
    # The unique key identifying the analysis configuration.
    CodeScanningAnalysisAnalysisKey analysisKey;
    # Any error message generated during the analysis.
    string 'error;
    # The API URL for this code scanning analysis.
    CodeScanningAnalysisUrl url;
    # The code scanning tool used to perform the analysis.
    CodeScanningAnalysisTool tool;
    # The Git reference associated with the analysis.
    CodeScanningRef ref;
    # The SHA of the commit the analysis was performed on.
    CodeScanningAnalysisCommitSha commitSha;
    # The environment variables used in the analysis.
    CodeScanningAnalysisEnvironment environment;
    # The total number of results in the analysis
    int resultsCount;
    # Warning generated when processing the analysis
    string warning;
    # The SARIF ID associated with the uploaded analysis file.
    CodeScanningAnalysisSarifId sarifId;
    # Unique identifier for this analysis
    int id;
    # The category grouping for this analysis.
    CodeScanningAnalysisCategory category?;
};

# Successful deletion of a code scanning analysis

type CodeScanningAnalysisDeletion record {
    # Next deletable analysis in chain, without last analysis deletion confirmation
    string? nextAnalysisUrl;
    # Next deletable analysis in chain, with last analysis deletion confirmation
    string? confirmDeleteUrl;
};


type CodeScanningAnalysisTool record {
    # The name of the code scanning analysis tool.
    CodeScanningAnalysisToolName name?;
    # The globally unique identifier of the code scanning tool.
    CodeScanningAnalysisToolGuid? guid?;
    # The version of the code scanning analysis tool.
    CodeScanningAnalysisToolVersion? version?;
};

# A CodeQL database

type CodeScanningCodeqlDatabase record {
    # The MIME type of the CodeQL database file
    string contentType;
    # The size of the CodeQL database file in bytes
    int size;
    # The date and time at which the CodeQL database was last updated, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # The commit SHA of the repository at the time the CodeQL database was created
    string? commitOid?;
    # The user who uploaded the CodeQL database.
    SimpleUser uploader;
    # The name of the CodeQL database
    string name;
    # The date and time at which the CodeQL database was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string createdAt;
    # The language of the CodeQL database
    string language;
    # The ID of the CodeQL database
    int id;
    # The URL at which to download the CodeQL database. The Accept header must be set to the value of the content_type property
    string url;
};

# Configuration for code scanning default setup

type CodeScanningDefaultSetup record {
    # The frequency of the periodic analysis
        "weekly"? schedule?;
    # CodeQL query suite to be used
        "default"|"extended"  querySuite?;
    # Languages to be analyzed
    ("c-cpp"|"csharp"|"go"|"java-kotlin"|"javascript-typescript"|"javascript"|"python"|"ruby"|"typescript"|"swift")[] languages?;
    # Timestamp of latest configuration update
    string? updatedAt?;
    # Code scanning default setup has been configured or not
        "configured"|"not-configured"  state?;
};

# Configuration for code scanning default setup

type CodeScanningDefaultSetupUpdate record {
    # CodeQL query suite to be used
        "default"|"extended"  querySuite?;
    # CodeQL languages to be analyzed
    ("c-cpp"|"csharp"|"go"|"java-kotlin"|"javascript-typescript"|"python"|"ruby"|"swift")[] languages?;
    # Whether code scanning default setup has been configured or not
        "configured"|"not-configured"  state;
};

# You can use run_url to track the status of the run. This includes a property status and conclusion.
# You should not rely on this always being an actions workflow run object

type CodeScanningDefaultSetupUpdateResponse record {
    # ID of the corresponding run
    int runId?;
    # URL of the corresponding run
    string runUrl?;
};

# Represents the Queries record for the operation: code-scanning/delete-analysis

type CodeScanningDeleteAnalysisQueries record {
    # Allow deletion if the specified analysis is the last in a set. If you attempt to delete the final analysis in a set without setting this parameter to true, you'll get a 400 response with the message: Analysis is last of its type and deletion may result in the loss of historical alert data. Please specify confirm_delete
    string? confirmDelete?;
};

# Represents the Queries record for the operation: code-scanning/list-alert-instances

type CodeScanningListAlertInstancesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The Git reference for the results you want to list. The ref for a branch can be formatted either as refs/heads/<branch name> or simply <branch name>. To reference a pull request use refs/pull/<number>/merge
    CodeScanningRef ref?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: code-scanning/list-alerts-for-org

type CodeScanningListAlertsForOrgQueries record {
    # The name of a code scanning tool. Only results by this tool will be listed. You can specify the tool by using either tool_name or tool_guid, but not both
    CodeScanningAnalysisToolName toolName?;
    # If specified, only code scanning alerts with this severity will be returned
    CodeScanningAlertSeverity severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # Page number of the results to fetch
    int page = 1;
    # If specified, only code scanning alerts with this state will be returned
    CodeScanningAlertStateQuery state?;
    # The property by which to sort the results
        "created"|"updated"  sort = "created";
    # The GUID of a code scanning tool. Only results by this tool will be listed. Note that some code scanning tools may not include a GUID in their analysis data. You can specify the tool by using either tool_guid or tool_name, but not both
    CodeScanningAnalysisToolGuid? toolGuid?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: code-scanning/list-alerts-for-repo

type CodeScanningListAlertsForRepoQueries record {
    # The name of a code scanning tool. Only results by this tool will be listed. You can specify the tool by using either tool_name or tool_guid, but not both
    CodeScanningAnalysisToolName toolName?;
    # If specified, only code scanning alerts with this severity will be returned
    CodeScanningAlertSeverity severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # The Git reference for the results you want to list. The ref for a branch can be formatted either as refs/heads/<branch name> or simply <branch name>. To reference a pull request use refs/pull/<number>/merge
    CodeScanningRef ref?;
    # Page number of the results to fetch
    int page = 1;
    # The property by which to sort the results
        "created"|"updated"  sort = "created";
    # If specified, only code scanning alerts with this state will be returned
    CodeScanningAlertStateQuery state?;
    # The GUID of a code scanning tool. Only results by this tool will be listed. Note that some code scanning tools may not include a GUID in their analysis data. You can specify the tool by using either tool_guid or tool_name, but not both
    CodeScanningAnalysisToolGuid? toolGuid?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: code-scanning/list-recent-analyses

type CodeScanningListRecentAnalysesQueries record {
    # The name of a code scanning tool. Only results by this tool will be listed. You can specify the tool by using either tool_name or tool_guid, but not both
    CodeScanningAnalysisToolName toolName?;
    # The number of results per page (max 100)
    int perPage = 30;
    # The Git reference for the analyses you want to list. The ref for a branch can be formatted either as refs/heads/<branch name> or simply <branch name>. To reference a pull request use refs/pull/<number>/merge
    CodeScanningRef ref?;
    # Filter analyses belonging to the same SARIF upload
    CodeScanningAnalysisSarifId sarifId?;
    # Page number of the results to fetch
    int page = 1;
    # The property by which to sort the results
        "created"  sort = "created";
    # The GUID of a code scanning tool. Only results by this tool will be listed. Note that some code scanning tools may not include a GUID in their analysis data. You can specify the tool by using either tool_guid or tool_name, but not both
    CodeScanningAnalysisToolGuid? toolGuid?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};


type CodeScanningOrganizationAlertItems record {
    # URL for listing all instances of this code scanning alert.
    AlertInstancesUrl instancesUrl;
    # Timestamp indicating when the code scanning alert was created.
    AlertCreatedAt createdAt;
    # Optional comment provided when the alert was dismissed.
    CodeScanningAlertDismissedComment? dismissedComment?;
    # Details of the code scanning rule that triggered this alert.
    CodeScanningAlertRule rule;
    # The repository where this code scanning alert was detected.
    SimpleRepository repository;
    # API URL for accessing this code scanning alert.
    AlertUrl url;
    # The code analysis tool that generated this alert.
    CodeScanningAnalysisTool tool;
    # Unique number identifying this alert within the repository.
    AlertNumber number;
    # Timestamp indicating when the code scanning alert was last updated.
    AlertUpdatedAt updatedAt?;
    # HTML URL for viewing this alert in the GitHub UI.
    AlertHtmlUrl htmlUrl;
    # Timestamp indicating when the code scanning alert was fixed.
    AlertFixedAt? fixedAt?;
    # The most recent instance of this code scanning alert.
    CodeScanningAlertInstance mostRecentInstance;
    # Current state of the code scanning alert.
    CodeScanningAlertState state;
    # The user who dismissed this code scanning alert.
    NullableSimpleUser? dismissedBy;
    # Reason provided for dismissing this code scanning alert.
    CodeScanningAlertDismissedReason? dismissedReason;
    # Timestamp indicating when this code scanning alert was dismissed.
    AlertDismissedAt? dismissedAt;
};


type CodeScanningSarifsBody record {
    # The name of the tool used to generate the code scanning analysis. If this parameter is not used, the tool name defaults to "API". If the uploaded SARIF contains a tool GUID, this will be available for filtering using the tool_guid parameter of operations such as GET /repos/{owner}/{repo}/code-scanning/alerts
    string toolName?;
    # The SHA of the commit the SARIF analysis was performed on.
    CodeScanningAnalysisCommitSha commitSha;
    # The Git reference associated with the uploaded SARIF analysis.
    CodeScanningRef ref;
    # The SARIF data containing the code scanning analysis results.
    CodeScanningAnalysisSarifFile sarif;
    # The base directory used in the analysis, as it appears in the SARIF file.
This property is used to convert file paths from absolute to relative, so that alerts can be mapped to their correct location in the repository
    string checkoutUri?;
    # The time that the analysis run began. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string startedAt?;
    # Whether the SARIF file will be validated according to the code scanning specifications.
This parameter is intended to help integrators ensure that the uploaded SARIF files are correctly rendered by code scanning
    boolean validate?;
};


type CodeScanningSarifsReceipt record {
    # The identifier for the uploaded SARIF analysis.
    CodeScanningAnalysisSarifId id?;
    # The REST API URL for checking the status of the upload
    string url?;
};


type CodeScanningSarifsStatus record {
    # pending files have not yet been processed, while complete means results from the SARIF have been stored. failed files have either not been processed at all, or could only be partially processed
        "pending"|"complete"|"failed"  processingStatus?;
    # The REST API URL for getting the analyses associated with the upload
    string? analysesUrl?;
    # Any errors that ocurred during processing of the delivery
    string[]? errors?;
};

# Code Search Result Item

type CodeSearchResultItem record {
    # Line numbers in the file where the search match was found.
    string[] lineNumbers?;
    # The programming language of the file.
    string? language?;
    # The repository containing the matched file.
    MinimalRepository repository;
    # The SHA hash of the file blob.
    string sha;
    # The API URL for this file.
    string url;
    # The size of the file in bytes.
    int fileSize?;
    # The timestamp when the file was last modified.
    string lastModifiedAt?;
    # The path to the file within the repository.
    string path;
    # The relevance score of this search result.
    decimal score;
    # The HTML URL to view the file on GitHub.
    string htmlUrl;
    # Text fragments showing where the search terms matched.
    SearchResultTextMatches textMatches?;
    # The name of the file.
    string name;
    # The Git API URL for the file blob.
    string gitUrl;
};

# Code Search Result Item

type CodeSearchResultItemResponse record {
    # The total number of code search results found.
    int totalCount;
    # Indicates whether the search results are incomplete.
    boolean incompleteResults;
    # The list of code search result items returned.
    CodeSearchResultItem[] items;
};

# A codespace

type Codespace record {
    # UUID identifying this codespace's environment
    string? environmentId;
    # Whether or not a codespace has a pending async operation. This would mean that the codespace is temporarily unavailable. The only thing that you can do with a codespace in this state is delete it
    boolean? pendingOperation?;
    # The user to whom charges for this codespace are billed.
    SimpleUser billableOwner;
    # API URL to start this codespace
    string startUrl;
    # The timestamp when this codespace was created.
    string createdAt;
    # API URL to stop this codespace
    string stopUrl;
    # The repository associated with this codespace.
    MinimalRepository repository;
    # Last known time this codespace was started
    string lastUsedAt;
    # Whether the codespace was created from a prebuild
    boolean? prebuild;
    # The timestamp when this codespace was last updated.
    string updatedAt;
    # When a codespace will be auto-deleted based on the "retention_period_minutes" and "last_used_at"
    string? retentionExpiresAt?;
    # Unique numeric identifier for this codespace.
    int id;
    # State of this codespace
        "Unknown"|"Created"|"Queued"|"Provisioning"|"Available"|"Awaiting"|"Unavailable"|"Deleted"|"Moved"|"Shutdown"|"Archived"|"Starting"|"ShuttingDown"|"Failed"|"Exporting"|"Updating"|"Rebuilding"  state;
    # The current git status of the codespace's working directory.
    CodespaceGitStatus gitStatus;
    # API URL to publish this codespace to a new repository
    string? publishUrl?;
    # API URL to access available alternate machine types for this codespace
    string machinesUrl;
    # The user who owns this codespace.
    SimpleUser owner;
    # Runtime constraints applied to this codespace.
    CodespaceRuntimeConstraints runtimeConstraints?;
    # The text to display to a user when a codespace has been stopped for a potentially actionable reason
    string? lastKnownStopNotice?;
    # List of recently opened folders in this codespace.
    string[] recentFolders;
    # Duration in minutes after codespace has gone idle in which it will be deleted. Must be integer minutes between 0 and 43200 (30 days)
    int? retentionPeriodMinutes?;
    # Display name for this codespace
    string? displayName?;
    # API URL for this codespace
    string url;
    # Path to devcontainer.json from repo root used to create Codespace
    string? devcontainerPath?;
    # Text to show user when codespace is disabled by a pending operation
    string? pendingOperationDisabledReason?;
    # URL to access this codespace on the web
    string webUrl;
    # The machine type used by this codespace.
    NullableCodespaceMachine? machine;
    # Automatically generated name of this codespace
    string name;
    # API URL for the Pull Request associated with this codespace, if any
    string? pullsUrl;
    # The initally assigned location of a new codespace
        "EastUs"|"SouthEastAsia"|"WestEurope"|"WestUs2"  location;
    # Text to show user when codespace idle timeout minutes has been overriden by an organization policy
    string? idleTimeoutNotice?;
    # The number of minutes of inactivity after which this codespace will be automatically stopped
    int? idleTimeoutMinutes;
};

# Codespace default attributes

type CodespaceDefault record {
    # The path to the devcontainer configuration file for the codespace.
    string? devcontainerPath;
    # The geographic location where the codespace will be created.
    string location;
};

# Codespace default attributes

type CodespaceDefaultResponse record {
    # The default codespace configuration attributes.
    CodespaceDefault defaults?;
    # The user billed for the codespace usage.
    SimpleUser billableOwner?;
};

# An export of a codespace. Also, latest export details for a codespace can be fetched with id = latest

type CodespaceExportDetails record {
    # Completion time of the last export operation
    string? completedAt?;
    # Web url for the exported branch
    string? htmlUrl?;
    # Url for fetching export details
    string exportUrl?;
    # State of the latest export
    string? state?;
    # Id for the export details
    string id?;
    # Name of the exported branch
    string? branch?;
    # Git commit SHA of the exported branch
    string? sha?;
};

# Details about the codespace's git repository

type CodespaceGitStatus record {
    # The number of commits the local repository is behind the remote
    int behind?;
    # The current branch (or SHA if in detached HEAD state) of the local repository
    string ref?;
    # The number of commits the local repository is ahead of the remote
    int ahead?;
    # Whether the local repository has unpushed changes
    boolean hasUnpushedChanges?;
    # Whether the local repository has uncommitted changes
    boolean hasUncommittedChanges?;
};

# A description of the machine powering a codespace

type CodespaceMachine record {
    # How many cores are available to the codespace
    int cpus;
    # The name of the machine
    string name;
    # Whether a prebuild is currently available when creating a codespace for this machine and repository. If a branch was not specified as a ref, the default branch will be assumed. Value will be "null" if prebuilds are not supported or prebuild availability could not be determined. Value will be "none" if no prebuild is available. Latest values "ready" and "in_progress" indicate the prebuild availability status
        "none"|"ready"|"in_progress"? prebuildAvailability;
    # The operating system of the machine
    string operatingSystem;
    # The display name of the machine includes cores, memory, and storage
    string displayName;
    # How much memory is available to the codespace
    int memoryInBytes;
    # How much storage is available to the codespace
    int storageInBytes;
};

# A description of the machine powering a codespace

type CodespaceMachineResponse record {
    # The total number of available codespace machines.
    int totalCount;
    # The list of machine types available for codeapces.
    CodespaceMachine[] machines;
};


type CodespaceNamePublishBody record {
    # Whether the new repository should be private
    boolean 'private = false;
    # A name for the new repository
    string name?;
};

# A codespace

type CodespaceResponse record {
    # The total number of codespaces returned.
    int totalCount;
    # The list of codespaces for the user or organization.
    Codespace[] codespaces;
};


type CodespaceRuntimeConstraints record {
    # The privacy settings a user can select from when forwarding a port
    string[]? allowedPortPrivacySettings?;
};


type CodespacesAccessBody record {
    # Which users can access codespaces in the organization. disabled means that no users can access codespaces in the organization
        "disabled"|"selected_members"|"all_members"|"all_members_and_outside_collaborators"  visibility;
    # The usernames of the organization members who should have access to codespaces in the organization. Required when visibility is selected_members. The provided list of usernames will replace any existing value
    string[] selectedUsernames?;
};

# Represents the Queries record for the operation: codespaces/check-permissions-for-devcontainer

type CodespacesCheckPermissionsForDevcontainerQueries record {
    # Path to the devcontainer.json configuration to use for the permission check
    string devcontainerPath;
    # The git reference that points to the location of the devcontainer configuration to use for the permission check. The value of ref will typically be a branch name (heads/BRANCH_NAME). For more information, see "[Git References](https://git-scm.com/book/en/v2/Git-Internals-Git-References)" in the Git documentation
    string ref;
};


type CodespacescodespaceNameBody record {
    # A valid machine to transition this codespace to
    string machine?;
    # Recently opened folders inside the codespace. It is currently used by the clients to determine the folder path to load the codespace in
    string[] recentFolders?;
    # Display name for this codespace
    string displayName?;
};

# Represents the Queries record for the operation: codespaces/get-codespaces-for-user-in-org

type CodespacesGetCodespacesForUserInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-devcontainers-in-repository-for-authenticated-user

type CodespacesListDevcontainersInRepositoryForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-for-authenticated-user

type CodespacesListForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # ID of the Repository to filter on
    int repositoryId?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-in-organization

type CodespacesListInOrganizationQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-in-repository-for-authenticated-user

type CodespacesListInRepositoryForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-org-secrets

type CodespacesListOrgSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-repo-secrets

type CodespacesListRepoSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-secrets-for-authenticated-user

type CodespacesListSecretsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: codespaces/list-selected-repos-for-org-secret

type CodespacesListSelectedReposForOrgSecretQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Secrets for a GitHub Codespace

type CodespacesOrgSecret record {
    # The date and time at which the secret was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # The type of repositories in the organization that the secret is visible to
        "all"|"private"|"selected"  visibility;
    # The name of the secret
    string name;
    # The API URL at which the list of repositories this secret is visible to can be retrieved
    string selectedRepositoriesUrl?;
    # The date and time at which the secret was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string createdAt;
};

# Secrets for a GitHub Codespace

type CodespacesOrgSecretResponse record {
    # The total number of organization-level Codespace secrets.
    int totalCount;
    # The list of Codespace secrets for the organization.
    CodespacesOrgSecret[] secrets;
};

# Permission check result for a given devcontainer config

type CodespacesPermissionsCheckForDevcontainer record {
    # Whether the user has accepted the permissions defined by the devcontainer config
    boolean accepted;
};

# Represents the Queries record for the operation: codespaces/pre-flight-with-repo-for-authenticated-user

type CodespacesPreFlightWithRepoForAuthenticatedUserQueries record {
    # The branch or commit to check for a default devcontainer path. If not specified, the default branch will be checked
    string ref?;
    # An alternative IP for default location auto-detection, such as when proxying a request
    string clientIp?;
};

# The public key used for setting Codespaces secrets

type CodespacesPublicKey record {
    # The identifier for the key
    string keyId;
    # The date and time the public key was created.
    string createdAt?;
    # The unique numeric identifier for the public key.
    int id?;
    # The title or label associated with the public key.
    string title?;
    # The Base64 encoded public key
    string 'key;
    # The API URL for the public key resource.
    string url?;
};

# Represents the Queries record for the operation: codespaces/repo-machines-for-authenticated-user

type CodespacesRepoMachinesForAuthenticatedUserQueries record {
    # The branch or commit to check for prebuild availability and devcontainer restrictions
    string ref?;
    # The location to check for available machines. Assigned by IP if not provided
    string location?;
    # IP for location auto-detection when proxying a request
    string clientIp?;
};

# Secrets for a GitHub Codespace

type CodespacesSecret record {
    # The date and time at which the secret was last updated, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # The type of repositories in the organization that the secret is visible to
        "all"|"private"|"selected"  visibility;
    # The name of the secret
    string name;
    # The API URL at which the list of repositories this secret is visible to can be retrieved
    string selectedRepositoriesUrl;
    # The date and time at which the secret was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string createdAt;
};

# Secrets for a GitHub Codespace

type CodespacesSecretResponse record {
    # The total number of Codespaces secrets available.
    int totalCount;
    # List of secrets for the GitHub Codespace.
    CodespacesSecret[] secrets;
};

# The public key used for setting user Codespaces' Secrets

type CodespacesUserPublicKey record {
    # The identifier for the key
    string keyId;
    # The Base64 encoded public key
    string 'key;
};

# A codespace

type CodespaceWithFullRepository record {
    # UUID identifying this codespace's environment
    string? environmentId;
    # Whether or not a codespace has a pending async operation. This would mean that the codespace is temporarily unavailable. The only thing that you can do with a codespace in this state is delete it
    boolean? pendingOperation?;
    # The user to whom the codespace usage is billed.
    SimpleUser billableOwner;
    # API URL to start this codespace
    string startUrl;
    # The date and time when the codespace was created.
    string createdAt;
    # API URL to stop this codespace
    string stopUrl;
    # The full repository associated with this codespace.
    FullRepository repository;
    # Last known time this codespace was started
    string lastUsedAt;
    # Whether the codespace was created from a prebuild
    boolean? prebuild;
    # The date and time when the codespace was last updated.
    string updatedAt;
    # When a codespace will be auto-deleted based on the "retention_period_minutes" and "last_used_at"
    string? retentionExpiresAt?;
    # Unique numeric identifier for the codespace.
    int id;
    # State of this codespace
        "Unknown"|"Created"|"Queued"|"Provisioning"|"Available"|"Awaiting"|"Unavailable"|"Deleted"|"Moved"|"Shutdown"|"Archived"|"Starting"|"ShuttingDown"|"Failed"|"Exporting"|"Updating"|"Rebuilding"  state;
    # The current Git status of the codespace repository.
    CodespaceGitStatus gitStatus;
    # API URL to publish this codespace to a new repository
    string? publishUrl?;
    # API URL to access available alternate machine types for this codespace
    string machinesUrl;
    # The user who owns this codespace.
    SimpleUser owner;
    # The runtime constraints applied to this codespace.
    CodespaceRuntimeConstraints runtimeConstraints?;
    # List of recently opened folders within the codespace.
    string[] recentFolders;
    # Duration in minutes after codespace has gone idle in which it will be deleted. Must be integer minutes between 0 and 43200 (30 days)
    int? retentionPeriodMinutes?;
    # Display name for this codespace
    string? displayName?;
    # API URL for this codespace
    string url;
    # Path to devcontainer.json from repo root used to create Codespace
    string? devcontainerPath?;
    # Text to show user when codespace is disabled by a pending operation
    string? pendingOperationDisabledReason?;
    # URL to access this codespace on the web
    string webUrl;
    # The machine type allocated for this codespace.
    NullableCodespaceMachine? machine;
    # Automatically generated name of this codespace
    string name;
    # API URL for the Pull Request associated with this codespace, if any
    string? pullsUrl;
    # The initally assigned location of a new codespace
        "EastUs"|"SouthEastAsia"|"WestEurope"|"WestUs2"  location;
    # Text to show user when codespace idle timeout minutes has been overriden by an organization policy
    string? idleTimeoutNotice?;
    # The number of minutes of inactivity after which this codespace will be automatically stopped
    int? idleTimeoutMinutes;
};

# Collaborator

type Collaborator record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # API URL template to check who the user is following
    string followingUrl;
    # API URL template for repositories the user has starred
    string starredUrl;
    # The username of the user
    string login;
    # API URL to list the user's followers
    string followersUrl;
    # The type of the account
    string 'type;
    # API URL for the user
    string url;
    # The role name assigned to the collaborator
    string roleName;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # API URL for events received by the user
    string receivedEventsUrl;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # The permission levels granted to the collaborator on the repository.
    NullableCollaboratorPermissions permissions?;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The display name of the user
    string? name?;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The unique identifier of the user
    int id;
    # The Gravatar ID of the user
    string? gravatarId;
    # The publicly visible email address of the user
    string? email?;
    # The GraphQL node identifier of the user
    string nodeId;
    # API URL to list the user's organizations
    string organizationsUrl;
};


type CollaboratorsusernameBody record {
    # The permission to grant the collaborator
        "read"|"write"|"admin"  permission = "write";
};


type CollaboratorsusernameBody1 record {
    # The permission to grant the collaborator. **Only valid on organization-owned repositories.** We accept the following permissions to be set: pull, triage, push, maintain, admin and you can also specify a custom repository role name, if the owning organization has defined any
    string permission = "push";
};


type ColumnIdMovesBody record {
    # The position of the column in a project. Can be one of: first, last, or after:<column_id> to place after the specified column
    string position;
};


type ColumnscolumnIdBody record {
    # Name of the project column
    string name;
};


type CombinedBillingUsage record {
    # Numbers of days left in billing cycle
    int daysLeftInBillingCycle;
    # Estimated storage space (GB) used in billing cycle
    int estimatedPaidStorageForMonth;
    # Estimated sum of free and paid storage space (GB) used in billing cycle
    int estimatedStorageForMonth;
};

# Combined Commit Status

type CombinedCommitStatus record {
    # API URL for the commit this status is associated with.
    string commitUrl;
    # The total number of statuses for this commit.
    int totalCount;
    # The list of individual commit statuses.
    SimpleCommitStatus[] statuses;
    # The combined state of all statuses for the commit.
    string state;
    # The repository containing the commit.
    MinimalRepository repository;
    # The SHA of the commit.
    string sha;
    # API URL for this combined commit status.
    string url;
};


type CommentIdReactionsBody record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the commit comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};


type CommentIdReactionsBody1 record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the issue comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};


type CommentIdReactionsBody2 record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the pull request review comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};


type CommentIdRepliesBody record {
    # The text of the review comment
    string body;
};


type CommentNumberReactionsBody record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the team discussion comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};


type CommentscommentIdBody record {
    # The contents of the comment
    string body;
};


type CommentscommentIdBody1 record {
    # The contents of the comment
    string body;
};


type CommentscommentIdBody2 record {
    # The text of the reply to the review comment
    string body;
};

# Commit

type Commit record {
    # The GitHub user who committed the commit.
    NullableSimpleUser? committer;
    # Statistics about additions and deletions in the commit.
    CommitStats stats?;
    # The GitHub user who authored the commit.
    NullableSimpleUser? author;
    # URL of the commit page on GitHub
    string htmlUrl;
    # API URL for the commit's comments
    string commentsUrl;
    # The raw Git commit data including message and authorship.
    CommitCommit 'commit;
    # The list of files changed in the commit
    DiffEntry[] files?;
    # The SHA hash of the commit
    string sha;
    # API URL for the commit
    string url;
    # The GraphQL node identifier of the commit
    string nodeId;
    # The parent commits of this commit
    CommitParents[] parents;
};

# Commit Activity

type CommitActivity record {
    # The total number of commits in the week.
    int total;
    # The Unix timestamp representing the start of the week.
    int week;
    # The number of commits per day of the week, Sunday through Saturday.
    int[] days;
};

# Commit Comment

type CommitComment record {
    # The line number in the file the comment applies to.
    int? line;
    # The date and time the comment was created.
    string createdAt;
    # The content of the commit comment.
    string body;
    # API URL for the commit comment.
    string url;
    # The commenter's association with the repository.
    AuthorAssociation authorAssociation;
    # The relative file path the comment is associated with.
    string? path;
    # The date and time the comment was last updated.
    string updatedAt;
    # URL of the commit comment on GitHub.
    string htmlUrl;
    # Reaction counts for the commit comment.
    ReactionRollup reactions?;
    # The unique identifier of the commit comment.
    int id;
    # The line index in the diff the comment applies to.
    int? position;
    # The SHA of the commit the comment is associated with.
    string commitId;
    # The user who created the commit comment.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the commit comment.
    string nodeId;
};

# The Git commit data

type CommitCommit record {
    # The number of comments on this commit.
    int commentCount;
    # The Git user information for the committer.
    NullableGitUser? committer;
    # The Git user information for the author.
    NullableGitUser? author;
    # The Git tree object associated with this commit.
    CommitCommitTree tree;
    # The commit message.
    string message;
    # The API URL for this commit object.
    string url;
    # The signature verification details for this commit.
    Verification verification?;
};


type CommitCommitTree record {
    # The SHA hash of the commit tree object.
    string sha;
    # The API URL of the commit tree resource.
    string url;
};

# Commit Comparison

type CommitComparison record {
    # The base commit used as the starting point for comparison.
    Commit baseCommit;
    # Number of commits the head is behind the base.
    int behindBy;
    # The URL to view the diff between the two commits.
    string diffUrl;
    # Number of commits the head is ahead of the base.
    int aheadBy;
    # The common ancestor commit of the two compared commits.
    Commit mergeBaseCommit;
    # The API URL for this commit comparison.
    string url;
    # The total number of commits between the two references.
    int totalCommits;
    # The URL to download the patch for this comparison.
    string patchUrl;
    # The HTML URL to view the comparison on GitHub.
    string htmlUrl;
    # The list of commits between the base and head.
    Commit[] commits;
    # The list of files changed between the two commits.
    DiffEntry[] files?;
    # The permanent URL for this commit comparison.
    string permalinkUrl;
    # The relationship status between the base and head commits.
        "diverged"|"ahead"|"behind"|"identical"  status;
};


type CommitParents record {
    # The URL to view the parent commit on GitHub.
    string htmlUrl?;
    # The SHA hash of the parent commit.
    string sha;
    # The API URL for the parent commit resource.
    string url;
};

# Commit Search Result Item

type CommitSearchResultItem record {
    # The search relevance score for this commit result.
    decimal score;
    # The Git user who committed the changes.
    NullableGitUser? committer;
    # The GitHub user who authored the commit.
    NullableSimpleUser? author;
    # The URL to view this commit on GitHub.
    string htmlUrl;
    # Text fragments that matched the search query within this commit.
    SearchResultTextMatches textMatches?;
    # The API URL to retrieve comments for this commit.
    string commentsUrl;
    # Detailed commit data including message, tree, and author information.
    CommitSearchResultItemCommit 'commit;
    # The repository where this commit was made.
    MinimalRepository repository;
    # The SHA hash uniquely identifying this commit.
    string sha;
    # The API URL for this commit resource.
    string url;
    # List of parent commits for this commit.
    CommitSearchResultItemParents[] parents;
    # The unique GraphQL node identifier for this commit.
    string nodeId;
};


type CommitSearchResultItemCommit record {
    # The number of comments on the commit.
    int commentCount;
    # The Git user who committed the change.
    NullableGitUser? committer;
    # The author information for the commit.
    CommitSearchResultItemCommitAuthor author;
    # The Git tree object associated with the commit.
    CommitSearchResultItemCommitTree tree;
    # The commit message describing the changes made.
    string message;
    # The API URL for the commit resource.
    string url;
    # The GPG signature verification status of the commit.
    Verification verification?;
};


type CommitSearchResultItemCommitAuthor record {
    # The date and time when the commit was authored.
    string date;
    # The name of the commit author.
    string name;
    # The email address of the commit author.
    string email;
};


type CommitSearchResultItemCommitTree record {
    # The SHA of the Git tree object for the commit.
    string sha;
    # The API URL for the Git tree object.
    string url;
};


type CommitSearchResultItemParents record {
    # The HTML URL of the parent commit on GitHub.
    string htmlUrl?;
    # The SHA hash of the parent commit.
    string sha?;
    # The API URL of the parent commit.
    string url?;
};

# Commit Search Result Item

type CommitSearchResultItemResponse record {
    # The total number of commits matching the search query.
    int totalCount;
    # Whether the search results are incomplete due to a timeout.
    boolean incompleteResults;
    # The list of commit search result items.
    CommitSearchResultItem[] items;
};


type CommitShaCommentsBody record {
    # Relative path of the file to comment on
    string path?;
    # **Deprecated**. Use **position** parameter instead. Line number in the file to comment on
    int line?;
    # Line index in the diff to comment on
    int position?;
    # The contents of the comment
    string body;
};

# The commit statistics showing additions, deletions, and total changes

type CommitStats record {
    # The total number of lines changed in the commit.
    int total?;
    # The number of lines added in the commit.
    int additions?;
    # The number of lines deleted in the commit.
    int deletions?;
};

# Community Profile

type CommunityProfile record {
    # The overall community health score as a percentage.
    int healthPercentage;
    # The date and time when the community profile was last updated.
    string? updatedAt;
    # URL to the repository's documentation.
    string? documentation;
    # The description of the repository.
    string? description;
    # Community health files present in the repository.
    CommunityProfileFiles files;
    # Indicates whether content reporting is enabled for the repository.
    boolean contentReportsEnabled?;
};


type CommunityProfileFiles record {
    # The issue template community health file for the repository.
    NullableCommunityHealthFile? issueTemplate;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # The code of conduct file community health file for the repository.
    NullableCommunityHealthFile? codeOfConductFile;
    # The contributing guidelines community health file for the repository.
    NullableCommunityHealthFile? contributing;
    # The README community health file for the repository.
    NullableCommunityHealthFile? readme;
    # The pull request template community health file for the repository.
    NullableCommunityHealthFile? pullRequestTemplate;
    # The code of conduct associated with the repository.
    NullableCodeOfConductSimple? codeOfConduct;
};

# Provides a set of configurations for controlling the behaviours when communicating with a remote HTTP endpoint.

type ConnectionConfig record {
    # Configurations related to client authentication
    http:BearerTokenConfig auth; // Special Agent Note: BearerTokenConfig FROM ballerina/http package
    # The HTTP version understood by the client
    http:HttpVersion httpVersion = http:HTTP_2_0; // Special Agent Note: HttpVersion FROM ballerina/http package
    # Configurations related to HTTP/1.x protocol
    http:ClientHttp1Settings http1Settings = {}; // Special Agent Note: ClientHttp1Settings FROM ballerina/http package
    # Configurations related to HTTP/2 protocol
    http:ClientHttp2Settings http2Settings = {}; // Special Agent Note: ClientHttp2Settings FROM ballerina/http package
    # The maximum time to wait (in seconds) for a response before closing the connection
    decimal timeout = 30;
    # The choice of setting `forwarded`/`x-forwarded` header
    string forwarded = "disable";
    # Configurations associated with Redirection
    http:FollowRedirects followRedirects?; // Special Agent Note: FollowRedirects FROM ballerina/http package
    # Configurations associated with request pooling
    http:PoolConfiguration poolConfig?; // Special Agent Note: PoolConfiguration FROM ballerina/http package
    # HTTP caching related configurations
    http:CacheConfig cache = {}; // Special Agent Note: CacheConfig FROM ballerina/http package
    # Specifies the way of handling compression (`accept-encoding`) header
    http:Compression compression = http:COMPRESSION_AUTO; // Special Agent Note: Compression FROM ballerina/http package
    # Configurations associated with the behaviour of the Circuit Breaker
    http:CircuitBreakerConfig circuitBreaker?; // Special Agent Note: CircuitBreakerConfig FROM ballerina/http package
    # Configurations associated with retrying
    http:RetryConfig retryConfig?; // Special Agent Note: RetryConfig FROM ballerina/http package
    # Configurations associated with cookies
    http:CookieConfig cookieConfig?; // Special Agent Note: CookieConfig FROM ballerina/http package
    # Configurations associated with inbound response size limits
    http:ResponseLimitConfigs responseLimits = {}; // Special Agent Note: ResponseLimitConfigs FROM ballerina/http package
    # SSL/TLS-related options
    http:ClientSecureSocket secureSocket?; // Special Agent Note: ClientSecureSocket FROM ballerina/http package
    # Proxy server related options
    http:ProxyConfig proxy?; // Special Agent Note: ProxyConfig FROM ballerina/http package
    # Provides settings related to client socket configuration
    http:ClientSocketConfig socketConfig = {}; // Special Agent Note: ClientSocketConfig FROM ballerina/http package
    # Enables the inbound payload validation functionality which provided by the constraint package. Enabled by default
    boolean validation = true;
    # Enables relaxed data binding on the client side. When enabled, `nil` values are treated as optional, 
and absent fields are handled as `nilable` types. Enabled by default.
    boolean laxDataBinding = true;
};


type ContainerMetadata record {
    # The list of tags associated with the container package.
    string[] tags;
};


type ContentDirectoryInner record {
    # The path to the content item within the repository.
    string path;
    # The size of the content item in bytes.
    int size;
    # Hypermedia links related to this content item.
    ContentDirectoryInnerLinks links;
    # The HTML URL to view this content item on GitHub.
    string? htmlUrl;
    # The name of the file or directory.
    string name;
    # The URL to download the raw content of the file.
    string? downloadUrl;
    # The type of content item (file, dir, symlink, or submodule).
        "dir"|"file"|"submodule"|"symlink"  'type;
    # The Git API URL for this content item's blob or tree.
    string? gitUrl;
    # The SHA hash of this content item.
    string sha;
    # The Base64-encoded content of the file, if applicable.
    string content?;
    # The API URL for this content item.
    string url;
};

# Hypermedia links related to this content item

type ContentDirectoryInnerLinks record {
    # URL to the Git API endpoint for this content item.
    string? git;
    # URL to the API endpoint for this content item.
    string self;
    # URL to the HTML web page for this content item.
    string? html;
};

# Content File

type ContentFile record {
    # Hypermedia links for navigating related content resources.
    ContentSymlinkLinks links;
    # Git URL of the submodule repository, if applicable.
    string submoduleGitUrl?;
    # Type of the content, always 'file' for this schema.
        "file"  'type;
    # Encoding format used for the file content, such as base64.
    string encoding;
    # SHA hash of the file blob.
    string sha;
    # Base64-encoded content of the file.
    string content;
    # API URL for accessing this file's content.
    string url;
    # Symlink target path if the file is a symbolic link.
    string target?;
    # Path to the file within the repository.
    string path;
    # Size of the file in bytes.
    int size;
    # HTML URL for viewing this file in the GitHub UI.
    string? htmlUrl;
    # Name of the file.
    string name;
    # Direct URL for downloading the raw file content.
    string? downloadUrl;
    # API URL for accessing the underlying Git blob object.
    string? gitUrl;
};


type ContentspathBody record {
    # The person that committed the file on behalf of the author.
    ReposownerrepocontentspathCommitter committer?;
    # The author of the file being created or updated.
    ReposownerrepocontentspathAuthor author?;
    # The commit message
    string message;
    # **Required if you are updating a file**. The blob SHA of the file being replaced
    string sha?;
    # The branch name. Default: the repository’s default branch
    string branch?;
    # The new file content, using Base64 encoding
    string content;
};


type ContentspathBody1 record {
    # The person who committed the file deletion.
    ReposownerrepocontentspathCommitter1 committer?;
    # The author of the file deletion commit.
    ReposownerrepocontentspathAuthor1 author?;
    # The commit message
    string message;
    # The blob SHA of the file being deleted
    string sha;
    # The branch name. Default: the repository’s default branch
    string branch?;
};

# An object describing a submodule

type ContentSubmodule record {
    # The path of the submodule within the repository.
    string path;
    # The size of the submodule content in bytes.
    int size;
    # The Git URL of the submodule repository.
    string submoduleGitUrl;
    # Links to related resources for the submodule.
    ContentSymlinkLinks links;
    # The URL to view the submodule on GitHub.
    string? htmlUrl;
    # The name of the submodule.
    string name;
    # The URL to download the submodule content.
    string? downloadUrl;
    # The content type, always 'submodule' for submodule entries.
        "submodule"  'type;
    # The API URL to retrieve the Git object for the submodule.
    string? gitUrl;
    # The SHA hash identifying the submodule commit.
    string sha;
    # The API URL of the submodule resource.
    string url;
};

# An object describing a symlink

type ContentSymlink record {
    # The file path of the symlink within the repository.
    string path;
    # The size of the symlink target path in bytes.
    int size;
    # Hypermedia links related to the symlink content.
    ContentSymlinkLinks links;
    # The URL to view the symlink on GitHub.
    string? htmlUrl;
    # The name of the symlink file.
    string name;
    # The URL to download the symlink target file.
    string? downloadUrl;
    # The content type, always 'symlink' for symlink objects.
        "symlink"  'type;
    # The API URL for the Git blob associated with the symlink.
    string? gitUrl;
    # The SHA hash of the symlink object.
    string sha;
    # The API URL for the symlink content.
    string url;
    # The path the symlink points to.
    string target;
};


type ContentSymlinkLinks record {
    # API URL for the symlink's Git object.
    string? git;
    # API URL for this symlink content resource.
    string self;
    # URL of the symlink on GitHub.
    string? html;
};

# Content Traffic

type ContentTraffic record {
    # The path of the content page receiving traffic.
    string path;
    # The total number of views for the content page.
    int count;
    # The number of unique visitors to the content page.
    int uniques;
    # The title of the content page.
    string title;
};

# Contributor

type Contributor record {
    # API URL template for the user's gists.
    string gistsUrl?;
    # API URL to list the user's repositories.
    string reposUrl?;
    # API URL template to check who the user is following.
    string followingUrl?;
    # API URL template for repositories the user has starred.
    string starredUrl?;
    # The username of the contributor.
    string login?;
    # API URL to list the user's followers.
    string followersUrl?;
    # The type of the account.
    string 'type;
    # API URL for the contributor.
    string url?;
    # API URL to list repositories the user is watching.
    string subscriptionsUrl?;
    # API URL for events received by the user.
    string receivedEventsUrl?;
    # The total number of contributions made by the contributor.
    int contributions;
    # URL of the user's avatar image.
    string avatarUrl?;
    # API URL template for the user's events.
    string eventsUrl?;
    # URL of the user's GitHub profile page.
    string htmlUrl?;
    # Whether the user is a GitHub site administrator.
    boolean siteAdmin?;
    # The display name of the contributor.
    string name?;
    # The unique identifier of the contributor.
    int id?;
    # The Gravatar ID of the user.
    string? gravatarId?;
    # The publicly visible email address of the contributor.
    string email?;
    # The GraphQL node identifier of the contributor.
    string nodeId?;
    # API URL to list the user's organizations.
    string organizationsUrl?;
};

# Contributor Activity

type ContributorActivity record {
    # The total number of commits made by the contributor.
    int total;
    # Weekly breakdown of the contributor's commit activity.
    ContributorActivityWeeks[] weeks;
    # The contributor whose activity is being reported.
    NullableSimpleUser? author;
};


type ContributorActivityWeeks record {
    # The number of lines added during the week.
    int a?;
    # The number of commits made during the week.
    int c?;
    # The number of lines deleted during the week.
    int d?;
    # The start of the week as a Unix timestamp.
    int w?;
};

# Converted Note to Issue Issue Event

type ConvertedNoteToIssueIssueEvent record {
    # The user who triggered the converted note to issue event.
    SimpleUser actor;
    # The URL of the commit associated with this event.
    string? commitUrl;
    # The GitHub App that performed this event, if any.
    Integration performedViaGithubApp;
    # The timestamp when the event was created.
    string createdAt;
    # The unique identifier of the issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL of the issue event.
    string url;
    # The project card that was converted to an issue.
    RemovedFromProjectIssueEventProjectCard projectCard?;
    # The GraphQL node ID of the issue event.
    string nodeId;
};

# Represents the Queries record for the operation: copilot/list-copilot-seats

type CopilotListCopilotSeatsQueries record {
    # The number of results per page (max 100)
    int perPage = 50;
    # Page number of the results to fetch
    int page = 1;
};

# Information about the seat breakdown and policies set for an organization with a Copilot for Business subscription

type CopilotOrganizationDetails record {
    # Breakdown of Copilot seat usage across the organization.
    CopilotSeatBreakdown seat_breakdown;
    # The organization policy for allowing or disallowing Copilot to make suggestions that match public code.
        "allow"|"block"|"unconfigured"|"unknown"  public_code_suggestions;
    # The organization policy for allowing or disallowing organization members to use Copilot Chat within their editor.
        "enabled"|"disabled"|"unconfigured"  copilot_chat?;
    # The mode of assigning new seats.
        "assign_all"|"assign_selected"|"disabled"|"unconfigured"  seat_management_setting;
};

# The breakdown of Copilot for Business seats for the organization

type CopilotSeatBreakdown record {
    # The number of seats that have not used Copilot during the current billing cycle
    int inactiveThisCycle?;
    # The total number of seats being billed for the organization as of the current billing cycle
    int total?;
    # Seats added during the current billing cycle
    int addedThisCycle?;
    # The number of seats that have been assigned to users that have not yet accepted an invitation to this organization
    int pendingInvitation?;
    # The number of seats that have used Copilot during the current billing cycle
    int activeThisCycle?;
    # The number of seats that are pending cancellation at the end of the current billing cycle
    int pendingCancellation?;
};

# The total number of seat assignments cancelled

type CopilotSeatCancelled record {
    # The number of Copilot seat assignments that were cancelled.
    int seatsCancelled;
};

# The total number of seat assignments created

type CopilotSeatCreated record {
    # The number of Copilot seat assignments successfully created.
    int seatsCreated;
};

# Information about a Copilot for Business seat assignment for a user, team, or organization

type CopilotSeatDetails record {
    # Timestamp of when the assignee's GitHub Copilot access was last updated, in ISO 8601 format
    string updatedAt?;
    # Last editor that was used by the user for a GitHub Copilot completion
    string? lastActivityEditor?;
    # Timestamp of user's last GitHub Copilot activity, in ISO 8601 format
    string? lastActivityAt?;
    # The team that granted access to GitHub Copilot to the assignee. This will be null if the user was assigned a seat individually
    Team? assigningTeam?;
    # Timestamp of when the assignee was last granted access to GitHub Copilot, in ISO 8601 format
    string createdAt;
    # The pending cancellation date for the seat, in YYYY-MM-DD format. This will be null unless the assignee's Copilot access has been canceled during the current billing cycle. If the seat has been cancelled, this corresponds to the start of the organization's next billing cycle
    string? pendingCancellationDate?;
    # The assignee that has been granted access to GitHub Copilot
    SimpleUser|Team|Organization assignee;
};

# Information about a Copilot for Business seat assignment for a user, team, or organization

type CopilotSeatDetailsResponse record {
    # Total number of Copilot For Business seats for the organization currently being billed
    int totalSeats?;
    # List of Copilot seat assignments for the organization.
    CopilotSeatDetails[] seats?;
};

# A GitHub App that is providing a custom deployment protection rule

type CustomDeploymentRuleApp record {
    # The URL for the endpoint to get details about the app
    string integrationUrl;
    # The unique identifier of the deployment protection rule integration
    int id;
    # The slugified name of the deployment protection rule integration
    string slug;
    # The node ID for the deployment protection rule integration
    string nodeId;
};

# A GitHub App that is providing a custom deployment protection rule

type CustomDeploymentRuleAppResponse record {
    # List of available custom deployment protection rule integrations for this environment.
    CustomDeploymentRuleApp[] availableCustomDeploymentProtectionRuleIntegrations?;
    # The total number of custom deployment protection rule integrations available for this environment
    int totalCount?;
};

# Demilestoned Issue Event

type DemilestonedIssueEvent record {
    # The user who triggered the demilestone event.
    SimpleUser actor;
    # API URL for the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed the demilestone event.
    NullableIntegration? performedViaGithubApp;
    # The milestone that was removed from the issue.
    MilestonedIssueEventMilestone milestone;
    # The date and time the event was created.
    string createdAt;
    # The unique identifier of the event.
    int id;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # API URL for the event.
    string url;
    # The GraphQL node identifier of the event.
    string nodeId;
};

# A Dependabot alert

type DependabotAlert record {
    # The vulnerable dependency associated with this alert.
    DependabotAlertWithRepositoryDependency dependency;
    # The security advisory details for this alert.
    DependabotAlertSecurityAdvisory securityAdvisory;
    # The specific vulnerability details for this alert.
    DependabotAlertSecurityVulnerability securityVulnerability;
    # The date and time the alert was created.
    AlertCreatedAt createdAt;
    # An optional comment associated with the alert's dismissal
    string? dismissedComment;
    # The date and time the alert was automatically dismissed.
    AlertAutoDismissedAt? autoDismissedAt?;
    # The API URL for this Dependabot alert.
    AlertUrl url;
    # The unique number identifying this Dependabot alert in the repository.
    AlertNumber number;
    # The date and time the alert was last updated.
    AlertUpdatedAt updatedAt;
    # The URL of the Dependabot alert on GitHub.
    AlertHtmlUrl htmlUrl;
    # The date and time the alert was fixed.
    AlertFixedAt? fixedAt;
    # The state of the Dependabot alert
        "auto_dismissed"|"dismissed"|"fixed"|"open"  state;
    # The user who dismissed the alert.
    NullableSimpleUser? dismissedBy;
    # The reason that the alert was dismissed
        "fix_started"|"inaccurate"|"no_bandwidth"|"not_used"|"tolerable_risk"? dismissedReason;
    # The date and time the alert was dismissed.
    AlertDismissedAt? dismissedAt;
};

# Details for the vulnerable package

type DependabotAlertPackage record {
    # The package's language or package management ecosystem
    string ecosystem;
    # The unique package name within its ecosystem
    string name;
};

# Details for the GitHub Security Advisory

type DependabotAlertSecurityAdvisory record {
    # A short, plain text summary of the advisory
    string summary;
    # The severity of the advisory
        "low"|"medium"|"high"|"critical"  severity;
    # Links to additional advisory information
    DependabotAlertSecurityAdvisoryReferences[] references;
    # Values that identify this advisory among security information sources
    DependabotAlertSecurityAdvisoryIdentifiers[] identifiers;
    # A long-form Markdown-supported description of the advisory
    string description;
    # Details for the advisory pertaining to Common Weakness Enumeration
    DependabotAlertSecurityAdvisoryCwes[] cwes;
    # The unique GitHub Security Advisory ID assigned to the advisory
    string ghsaId;
    # The time that the advisory was last modified in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # The unique CVE ID assigned to the advisory
    string? cveId;
    # Vulnerable version range information for the advisory
    DependabotAlertSecurityVulnerability[] vulnerabilities;
    # The time that the advisory was withdrawn in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string? withdrawnAt;
    # The time that the advisory was published in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string publishedAt;
    # The CVSS score and vector string for the advisory.
    DependabotAlertSecurityAdvisoryCvss cvss;
};

# Details for the advisory pertaining to the Common Vulnerability Scoring System

type DependabotAlertSecurityAdvisoryCvss record {
    # The overall CVSS score of the advisory
    decimal score;
    # The full CVSS vector string for the advisory
    string? vectorString;
};

# A CWE weakness assigned to the advisory

type DependabotAlertSecurityAdvisoryCwes record {
    # The unique CWE ID
    string cweId;
    # The short, plain text name of the CWE
    string name;
};

# An advisory identifier

type DependabotAlertSecurityAdvisoryIdentifiers record {
    # The type of advisory identifier
        "CVE"|"GHSA"  'type;
    # The value of the advisory identifer
    string value;
};

# A link to additional advisory information

type DependabotAlertSecurityAdvisoryReferences record {
    # The URL of the reference
    string url;
};

# Details pertaining to one vulnerable version range for the advisory

type DependabotAlertSecurityVulnerability record {
    # The severity of the vulnerability
        "low"|"medium"|"high"|"critical"  severity;
    # The earliest version of the package that resolves this vulnerability.
    DependabotAlertSecurityVulnerabilityFirstPatchedVersion? firstPatchedVersion;
    # The package ecosystem and name affected by this vulnerability.
    DependabotAlertPackage package;
    # Conditions that identify vulnerable versions of this vulnerability's package
    string vulnerableVersionRange;
};

# Details pertaining to the package version that patches this vulnerability

type DependabotAlertSecurityVulnerabilityFirstPatchedVersion record {
    # The package version that patches this vulnerability
    string identifier;
};

# A Dependabot alert

type DependabotAlertWithRepository record {
    # The vulnerable dependency that triggered the alert.
    DependabotAlertWithRepositoryDependency dependency;
    # The security advisory associated with the alert.
    DependabotAlertSecurityAdvisory securityAdvisory;
    # The specific vulnerability details from the security advisory.
    DependabotAlertSecurityVulnerability securityVulnerability;
    # The date and time the alert was created.
    AlertCreatedAt createdAt;
    # An optional comment associated with the alert's dismissal
    string? dismissedComment;
    # The repository where the vulnerable dependency was detected.
    SimpleRepository repository;
    # The date and time the alert was automatically dismissed.
    AlertAutoDismissedAt? autoDismissedAt?;
    # The API URL for the Dependabot alert.
    AlertUrl url;
    # The number that uniquely identifies the alert within the repository.
    AlertNumber number;
    # The date and time the alert was last updated.
    AlertUpdatedAt updatedAt;
    # The URL of the alert's page on GitHub.
    AlertHtmlUrl htmlUrl;
    # The date and time the vulnerability was fixed.
    AlertFixedAt? fixedAt;
    # The state of the Dependabot alert
        "auto_dismissed"|"dismissed"|"fixed"|"open"  state;
    # The user who dismissed the alert.
    NullableSimpleUser? dismissedBy;
    # The reason that the alert was dismissed
        "fix_started"|"inaccurate"|"no_bandwidth"|"not_used"|"tolerable_risk"? dismissedReason;
    # The date and time the alert was dismissed.
    AlertDismissedAt? dismissedAt;
};

# Details for the vulnerable dependency

type DependabotAlertWithRepositoryDependency record {
    # The package associated with the vulnerable dependency.
    DependabotAlertPackage package?;
    # The full path to the dependency manifest file, relative to the root of the repository
    string manifestPath?;
    # The execution scope of the vulnerable dependency
        "development"|"runtime"? scope?;
};

# Represents the Queries record for the operation: dependabot/list-alerts-for-enterprise

type DependabotListAlertsForEnterpriseQueries record {
    # A comma-separated list of severities. If specified, only alerts with these severities will be returned.

Can be: low, medium, high, critical
    string severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of package names. If specified, only alerts for these packages will be returned
    string package?;
    # A comma-separated list of ecosystems. If specified, only alerts for these ecosystems will be returned.

Can be: composer, go, maven, npm, nuget, pip, pub, rubygems, rust
    string ecosystem?;
    # **Deprecated**. The number of results per page (max 100), starting from the last matching result.
This parameter must not be used in combination with first.
Instead, use per_page in combination with before to fetch the last page of results
    int last?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # The scope of the vulnerable dependency. If specified, only alerts with this scope will be returned
        "development"|"runtime"  scope?;
    # A comma-separated list of states. If specified, only alerts with these states will be returned.

Can be: auto_dismissed, dismissed, fixed, open
    string state?;
    # The property by which to sort the results.
created means when the alert was created.
updated means when the alert's state last changed
        "created"|"updated"  sort = "created";
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # **Deprecated**. The number of results per page (max 100), starting from the first matching result.
This parameter must not be used in combination with last.
Instead, use per_page in combination with after to fetch the first page of results
    int first = 30;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: dependabot/list-alerts-for-org

type DependabotListAlertsForOrgQueries record {
    # A comma-separated list of severities. If specified, only alerts with these severities will be returned.

Can be: low, medium, high, critical
    string severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of package names. If specified, only alerts for these packages will be returned
    string package?;
    # A comma-separated list of ecosystems. If specified, only alerts for these ecosystems will be returned.

Can be: composer, go, maven, npm, nuget, pip, pub, rubygems, rust
    string ecosystem?;
    # **Deprecated**. The number of results per page (max 100), starting from the last matching result.
This parameter must not be used in combination with first.
Instead, use per_page in combination with before to fetch the last page of results
    int last?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # The scope of the vulnerable dependency. If specified, only alerts with this scope will be returned
        "development"|"runtime"  scope?;
    # A comma-separated list of states. If specified, only alerts with these states will be returned.

Can be: auto_dismissed, dismissed, fixed, open
    string state?;
    # The property by which to sort the results.
created means when the alert was created.
updated means when the alert's state last changed
        "created"|"updated"  sort = "created";
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # **Deprecated**. The number of results per page (max 100), starting from the first matching result.
This parameter must not be used in combination with last.
Instead, use per_page in combination with after to fetch the first page of results
    int first = 30;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: dependabot/list-alerts-for-repo

type DependabotListAlertsForRepoQueries record {
    # A comma-separated list of severities. If specified, only alerts with these severities will be returned.

Can be: low, medium, high, critical
    string severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of package names. If specified, only alerts for these packages will be returned
    string package?;
    # A comma-separated list of ecosystems. If specified, only alerts for these ecosystems will be returned.

Can be: composer, go, maven, npm, nuget, pip, pub, rubygems, rust
    string ecosystem?;
    # **Deprecated**. The number of results per page (max 100), starting from the last matching result.
This parameter must not be used in combination with first.
Instead, use per_page in combination with before to fetch the last page of results
    int last?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # A comma-separated list of full manifest paths. If specified, only alerts for these manifests will be returned
    string manifest?;
    # The property by which to sort the results.
created means when the alert was created.
updated means when the alert's state last changed
        "created"|"updated"  sort = "created";
    # The scope of the vulnerable dependency. If specified, only alerts with this scope will be returned
        "development"|"runtime"  scope?;
    # A comma-separated list of states. If specified, only alerts with these states will be returned.

Can be: auto_dismissed, dismissed, fixed, open
    string state?;
    # **Deprecated**. Page number of the results to fetch. Use cursor-based pagination with before or after instead
    int page = 1;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # **Deprecated**. The number of results per page (max 100), starting from the first matching result.
This parameter must not be used in combination with last.
Instead, use per_page in combination with after to fetch the first page of results
    int first = 30;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: dependabot/list-org-secrets

type DependabotListOrgSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: dependabot/list-repo-secrets

type DependabotListRepoSecretsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: dependabot/list-selected-repos-for-org-secret

type DependabotListSelectedReposForOrgSecretQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# The public key used for setting Dependabot Secrets

type DependabotPublicKey record {
    # The identifier for the key
    string keyId;
    # The Base64 encoded public key
    string 'key;
};

# Set secrets for Dependabot

type DependabotSecret record {
    # The timestamp when the Dependabot secret was last updated.
    string updatedAt;
    # The name of the secret
    string name;
    # The timestamp when the Dependabot secret was created.
    string createdAt;
};

# Set secrets for Dependabot

type DependabotSecretResponse record {
    # The total number of Dependabot secrets available.
    int totalCount;
    # The list of Dependabot secrets.
    DependabotSecret[] secrets;
};


type Dependency record {
    # Additional metadata key/value pairs associated with this dependency.
    Metadata metadata?;
    # Package-url (PURL) of dependency. See https://github.com/package-url/purl-spec for more details
    string packageUrl?;
    # A notation of whether the dependency is required for the primary build artifact (runtime) or is only used for development. Future versions of this specification may allow for more granular scopes
        "runtime"|"development"  scope?;
    # A notation of whether a dependency is requested directly by this manifest or is a dependency of another dependency
        "direct"|"indirect"  relationship?;
    # Array of package-url (PURLs) of direct child dependencies
    string[] dependencies?;
};


type DependencyGraphDiffInner record {
    # The license associated with the dependency.
    string? license;
    # The package ecosystem of the dependency.
    string ecosystem;
    # The package URL (PURL) identifying the dependency.
    string? packageUrl;
    # The path to the manifest file containing the dependency.
    string manifest;
    # Where the dependency is utilized. development means that the dependency is only utilized in the development environment. runtime means that the dependency is utilized at runtime and in the development environment
        "unknown"|"runtime"|"development"  scope;
    # The name of the dependency package.
    string name;
    # List of known vulnerabilities associated with this dependency.
    DependencyGraphDiffInnerVulnerabilities[] vulnerabilities;
    # Whether the dependency was added or removed in the diff.
        "added"|"removed"  changeType;
    # The URL of the source repository for the dependency.
    string? sourceRepositoryUrl;
    # The version of the dependency.
    string version;
};


type DependencyGraphDiffInnerVulnerabilities record {
    # Severity level of the vulnerability (e.g., low, medium, high, critical).
    string severity;
    # URL to the GitHub Security Advisory for this vulnerability.
    string advisoryUrl;
    # GitHub Security Advisory identifier for this vulnerability.
    string advisoryGhsaId;
    # Brief description summarizing the security advisory.
    string advisorySummary;
};

# Represents the Queries record for the operation: dependency-graph/diff-range

type DependencyGraphDiffRangeQueries record {
    # The full path, relative to the repository root, of the dependency manifest file
    string name?;
};

# A schema for the SPDX JSON format returned by the Dependency Graph

type DependencyGraphSpdxSbom record {
    # The SPDX-formatted software bill of materials for the repository.
    DependencyGraphSpdxSbomSbom sbom;
};


type DependencyGraphSpdxSbomSbom record {
    # The license under which the SPDX document is licensed
    string dataLicense;
    # The namespace for the SPDX document
    string documentNamespace;
    # The version of the SPDX specification that this document conforms to
    string spdxVersion;
    # The SPDX identifier for the SPDX document
    string sPDXID;
    # The name of the SPDX document
    string name;
    # The name of the repository that the SPDX document describes
    string[] documentDescribes;
    # List of packages described in the SPDX SBOM document.
    DependencyGraphSpdxSbomSbomPackages[] packages;
    # Metadata about when and how the SPDX document was created.
    DependencyGraphSpdxSbomSbomCreationInfo creationInfo;
};


type DependencyGraphSpdxSbomSbomCreationInfo record {
    # The date and time the SPDX document was created
    string created;
    # The tools that were used to generate the SPDX document
    string[] creators;
};


type DependencyGraphSpdxSbomSbomExternalRefs record {
    # A locator for the particular external resource this reference refers to
    string referenceLocator;
    # The category of reference to an external resource this reference refers to
    string referenceType;
    # The category of reference to an external resource this reference refers to
    string referenceCategory;
};


type DependencyGraphSpdxSbomSbomPackages record {
    # Whether the package's file content has been subjected to
analysis during the creation of the SPDX document
    boolean filesAnalyzed?;
    # The license of the package as determined while creating the SPDX document
    string licenseConcluded?;
    # A unique SPDX identifier for the package
    string sPDXID?;
    # The distribution source of this package, or NOASSERTION if this was not determined
    string supplier?;
    # The name of the package
    string name?;
    # External references providing additional information about the package.
    DependencyGraphSpdxSbomSbomExternalRefs[] externalRefs?;
    # The location where the package can be downloaded,
or NOASSERTION if this has not been determined
    string downloadLocation?;
    # The license of the package as declared by its author, or NOASSERTION if this information
was not available when the SPDX document was created
    string licenseDeclared?;
    # The version of the package. If the package does not have an exact version specified,
a version range is given
    string versionInfo?;
};

# An SSH key granting access to a single repository

type DeployKey record {
    # Whether the deploy key has read-only access to the repository.
    boolean readOnly;
    # The username of the user who added the deploy key.
    string? addedBy?;
    # The timestamp when the deploy key was last used.
    string? lastUsed?;
    # Whether the deploy key has been verified.
    boolean verified;
    # The timestamp when the deploy key was created.
    string createdAt;
    # The unique identifier of the deploy key.
    int id;
    # The display name of the deploy key.
    string title;
    # The public SSH key string.
    string 'key;
    # The API URL for this deploy key.
    string url;
};

# A request for a specific ref(branch,sha,tag) to be deployed

type Deployment record {
    # The user who created the deployment.
    NullableSimpleUser? creator;
    # API URL for the deployment's statuses.
    string statusesUrl;
    # A short description of the deployment.
    string? description;
    # The date and time when the deployment was created.
    string createdAt;
    # The SHA of the commit being deployed.
    string sha;
    # API URL for this deployment.
    string url;
    # The ref to deploy. This can be a branch, tag, or sha
    string ref;
    # Name for the target deployment environment
    string environment;
    # Parameter to specify a task to execute
    string task;
    # The date and time when the deployment was last updated.
    string updatedAt;
    # The GitHub App that performed the deployment.
    NullableIntegration? performedViaGithubApp?;
    # Additional metadata supplied by the deployer as a JSON object.
    record {}|string payload;
    # Specifies if the given environment is will no longer exist at some point in the future. Default: false
    boolean transientEnvironment?;
    # The original environment targeted by this deployment.
    string originalEnvironment?;
    # Unique identifier of the deployment
    int id;
    # API URL for the repository associated with this deployment.
    string repositoryUrl;
    # The GraphQL node identifier of the deployment.
    string nodeId;
    # Specifies if the given environment is one that end-users directly interact with. Default: false
    boolean productionEnvironment?;
};

# Details of a deployment branch or tag policy

type DeploymentBranchPolicy record {
    # The name pattern that branches or tags must match in order to deploy to the environment
    string name?;
    # The unique identifier of the branch or tag policy
    int id?;
    # Whether this rule targets a branch or tag
        "branch"|"tag"  'type?;
    # The global node ID of the deployment branch or tag policy.
    string nodeId?;
};


type DeploymentBranchPolicyNamePattern record {
    # The name pattern that branches must match in order to deploy to the environment.

Wildcard characters will not match /. For example, to match branches that begin with release/ and contain an additional single slash, use release/*/*.
For more information about pattern matching syntax, see the [Ruby File.fnmatch documentation](https://ruby-doc.org/core-2.5.1/File.html#method-c-fnmatch)
    string name;
};


type DeploymentBranchPolicyNamePatternWithType record {
    # The name pattern that branches or tags must match in order to deploy to the environment.

Wildcard characters will not match /. For example, to match branches that begin with release/ and contain an additional single slash, use release/*/*.
For more information about pattern matching syntax, see the [Ruby File.fnmatch documentation](https://ruby-doc.org/core-2.5.1/File.html#method-c-fnmatch)
    string name;
    # Whether this rule targets a branch or tag
        "branch"|"tag"  'type?;
};

# Details of a deployment branch or tag policy

type DeploymentBranchPolicyResponse record {
    # The number of deployment branch policies for the environment
    int totalCount;
    # List of deployment branch or tag policies for the environment.
    DeploymentBranchPolicy[] branchPolicies;
};

# The type of deployment branch policy for this environment. To allow all branches to deploy, set to null

type DeploymentBranchPolicySettings record {
    # Whether only branches that match the specified name patterns can deploy to this environment.  If custom_branch_policies is true, protected_branches must be false; if custom_branch_policies is false, protected_branches must be true
    boolean customBranchPolicies;
    # Whether only branches with branch protection rules can deploy to this environment. If protected_branches is true, custom_branch_policies must be false; if protected_branches is false, custom_branch_policies must be true
    boolean protectedBranches;
};


type DeploymentIdStatusesBody record {
    # Name for the target deployment environment, which can be changed when setting a deploy status. For example, production, staging, or qa. If not defined, the environment of the previous status on the deployment will be used, if it exists. Otherwise, the environment of the deployment will be used
    string environment?;
    # Sets the URL for accessing your environment. Default: ""
    string environmentUrl = "";
    # The target URL to associate with this status. This URL should contain output to keep the user updated while the task is running or serve as historical information for what happened in the deployment. **Note:** It's recommended to use the log_url parameter, which replaces target_url
    string targetUrl = "";
    # The full URL of the deployment's output. This parameter replaces target_url. We will continue to accept target_url to support legacy uses, but we recommend replacing target_url with log_url. Setting log_url will automatically set target_url to the same value. Default: ""
    string logUrl = "";
    # Adds a new inactive status to all prior non-transient, non-production environment deployments with the same repository and environment name as the created status's deployment. An inactive status is only added to deployments that had a success state. Default: true
    boolean autoInactive?;
    # A short description of the status. The maximum description length is 140 characters
    string description = "";
    # The state of the status. When you set a transient deployment to inactive, the deployment will be shown as destroyed in GitHub
        "error"|"failure"|"inactive"|"in_progress"|"queued"|"pending"|"success"  state;
};

# Deployment protection rule

type DeploymentProtectionRule record {
    # The GitHub App that provides the deployment protection rule.
    CustomDeploymentRuleApp app;
    # The unique identifier for the deployment protection rule
    int id;
    # Whether the deployment protection rule is enabled for the environment
    boolean enabled;
    # The node ID for the deployment protection rule
    string nodeId;
};

# Deployment protection rule

type DeploymentProtectionRuleResponse record {
    # The number of enabled custom deployment protection rules for this environment
    int totalCount?;
    # The list of enabled custom deployment protection rules.
    DeploymentProtectionRule[] customDeploymentProtectionRules?;
};

# A deployment created as the result of an Actions check run from a workflow that references an environment

type DeploymentSimple record {
    # The API URL for listing the deployment's statuses.
    string statusesUrl;
    # A short description of the deployment.
    string? description;
    # The date and time the deployment was created, in ISO 8601 format.
    string createdAt;
    # The API URL for the deployment.
    string url;
    # Name for the target deployment environment
    string environment;
    # Parameter to specify a task to execute
    string task;
    # The date and time the deployment was last updated, in ISO 8601 format.
    string updatedAt;
    # The GitHub App that triggered the deployment, if applicable.
    NullableIntegration? performedViaGithubApp?;
    # Specifies if the given environment is will no longer exist at some point in the future. Default: false
    boolean transientEnvironment?;
    # The original target environment for the deployment.
    string originalEnvironment?;
    # Unique identifier of the deployment
    int id;
    # The API URL for the repository associated with the deployment.
    string repositoryUrl;
    # The GraphQL node identifier of the deployment.
    string nodeId;
    # Specifies if the given environment is one that end-users directly interact with. Default: false
    boolean productionEnvironment?;
};

# The status of a deployment

type DeploymentStatus record {
    # The user who created this deployment status.
    NullableSimpleUser? creator;
    # Deprecated: the URL to associate with this status
    string targetUrl = "";
    # The URL of the deployment associated with this status.
    string deploymentUrl;
    # A short description of the status
    string description = "";
    # The timestamp indicating when the deployment status was created.
    string createdAt;
    # The API URL of the deployment status resource.
    string url;
    # The environment of the deployment that the status is for
    string environment = "";
    # The URL for accessing your environment
    string environmentUrl = "";
    # The timestamp indicating when the deployment status was last updated.
    string updatedAt;
    # The GitHub App that triggered this deployment status.
    NullableIntegration? performedViaGithubApp?;
    # The URL to associate with this status
    string logUrl = "";
    # The unique identifier of the deployment status.
    int id;
    # The state of the status
        "error"|"failure"|"inactive"|"pending"|"success"|"queued"|"in_progress"  state;
    # The URL of the repository associated with this deployment status.
    string repositoryUrl;
    # The GraphQL node identifier of the deployment status.
    string nodeId;
};

# Dev Containers

type Devcontainers record {
    # The file path to the dev container configuration.
    string path;
    # The internal name identifier of the dev container.
    string name?;
    # The human-readable display name of the dev container.
    string displayName?;
};

# Dev Containers

type DevcontainersResponse record {
    # List of dev container configurations found in the repository.
    Devcontainers[] devcontainers;
    # The total number of dev container configurations.
    int totalCount;
};

# Diff Entry

type DiffEntry record {
    # The patch content showing the diff changes for the file.
    string patch?;
    # The path of the file in the repository.
    string filename;
    # The number of lines added in the file.
    int additions;
    # The number of lines deleted from the file.
    int deletions;
    # The total number of lines changed in the file.
    int changes;
    # The previous path of the file before renaming.
    string previousFilename?;
    # The SHA hash identifying the file blob.
    string sha;
    # URL to the file blob on GitHub.
    string blobUrl;
    # URL to the raw file content.
    string rawUrl;
    # The change status of the file in the diff.
        "added"|"removed"|"modified"|"renamed"|"copied"|"changed"|"unchanged"  status;
    # URL to retrieve the file contents via the API.
    string contentsUrl;
};


type DiscussionNumberCommentsBody record {
    # The discussion comment's body text
    string body;
};


type DiscussionNumberReactionsBody record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the team discussion
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};


type DiscussionsdiscussionNumberBody record {
    # The discussion post's title
    string title?;
    # The discussion post's body text
    string body?;
};


type DockerMetadata record {
    # The list of tags associated with the Docker container image.
    string[] tag?;
};

# Email

type Email record {
    # The visibility of the email address (public or private).
    string? visibility;
    # Whether the email address has been verified.
    boolean verified;
    # The email address.
    string email;
    # Whether this is the primary email address for the account.
    boolean primary;
};


type EmailVisibilityBody record {
    # Denotes whether an email is publicly visible
        "public"|"private"  visibility;
};

# An object without any properties

type EmptyObject record {
};

# An enterprise on GitHub

type Enterprise record {
    # The enterprise's website URL
    string? websiteUrl?;
    # The date and time the enterprise was last updated.
    string? updatedAt;
    # The URL of the enterprise's avatar image.
    string avatarUrl;
    # The GitHub web URL for the enterprise's page.
    string htmlUrl;
    # The name of the enterprise
    string name;
    # A short description of the enterprise
    string? description?;
    # The date and time the enterprise was created.
    string? createdAt;
    # Unique identifier of the enterprise
    int id;
    # The slug url identifier for the enterprise
    string slug;
    # The GraphQL node ID of the enterprise.
    string nodeId;
};

# Details of a deployment environment

type Environment record {
    # The time that the environment was last updated, in ISO 8601 format
    string updatedAt;
    # URL to view the environment on GitHub.
    string htmlUrl;
    # The name of the environment
    string name;
    # Built-in deployment protection rules for the environment
    EnvironmentProtectionRules[] protectionRules?;
    # The branch policy settings controlling which branches can deploy.
    DeploymentBranchPolicySettings? deploymentBranchPolicy?;
    # The time that the environment was created, in ISO 8601 format
    string createdAt;
    # The id of the environment
    int id;
    # API URL for this deployment environment.
    string url;
    # The GraphQL node identifier for this environment.
    string nodeId;
};

# An entry in the reviews log for environment deployments

type EnvironmentApprovals record {
    # The list of environments that were approved or rejected
    EnvironmentApprovalsEnvironments[] environments;
    # The comment submitted with the deployment review
    string comment;
    # Whether deployment to the environment(s) was approved or rejected or pending (with comments)
        "approved"|"rejected"|"pending"  state;
    # The user who approved or rejected the deployment.
    SimpleUser user;
};


type EnvironmentApprovalsEnvironments record {
    # The time that the environment was last updated, in ISO 8601 format
    string updatedAt?;
    # The URL of the environment page on GitHub.
    string htmlUrl?;
    # The name of the environment
    string name?;
    # The time that the environment was created, in ISO 8601 format
    string createdAt?;
    # The id of the environment
    int id?;
    # The API URL for the environment.
    string url?;
    # The GraphQL node identifier of the environment.
    string nodeId?;
};


type EnvironmentNameDeploymentProtectionRulesBody record {
    # The ID of the custom app that will be enabled on the environment
    int integrationId?;
};

# Details of a deployment environment

type EnvironmentResponse record {
    # The list of deployment environments in the repository.
    Environment[] environments?;
    # The number of environments in this repository
    int totalCount?;
};


type EnvironmentsenvironmentNameBody record {
    # Prevents the actor who triggered the deployment from approving it.
    PreventSelfReview preventSelfReview?;
    # The branch policy that governs which branches can deploy to this environment.
    DeploymentBranchPolicySettings? deploymentBranchPolicy?;
    # The time in minutes to wait before allowing deployments to proceed.
    WaitTimer waitTimer?;
    # The people or teams that may review jobs that reference the environment. You can list up to six users or teams as reviewers. The reviewers must have at least read access to the repository. Only one of the required reviewers needs to approve the job for it to proceed
    ReposownerrepoenvironmentsenvironmentNameReviewers[]? reviewers?;
};

# Event

type Event record {
    # The user or application that triggered the event.
    Actor actor;
    # Whether the event is publicly visible.
    boolean 'public;
    # The organization associated with the event, if applicable.
    Actor org?;
    # The event-specific payload data.
    EventPayload payload;
    # The repository where the event occurred.
    EventRepo repo;
    # The date and time the event was created.
    string? createdAt;
    # The unique identifier of the event.
    string id;
    # The type of GitHub event.
    string? 'type;
};


type EventPayload record {
    # The list of wiki pages affected by the event.
    EventPayloadPages[] pages?;
    # The issue associated with the event.
    Issue issue?;
    # The action that triggered the event.
    string action?;
    # The issue comment associated with the event.
    IssueComment comment?;
};


type EventPayloadPages record {
    # A summary of the wiki page change.
    string? summary?;
    # The name of the wiki page that was changed.
    string pageName?;
    # The URL of the wiki page on GitHub.
    string htmlUrl?;
    # The action performed on the wiki page.
    string action?;
    # The title of the wiki page.
    string title?;
    # The SHA of the commit associated with the wiki page change.
    string sha?;
};


type EventRepo record {
    # The full name of the repository associated with the event.
    string name;
    # The unique numeric identifier of the repository.
    int id;
    # The API URL for the repository associated with the event.
    string url;
};

# Feed

type Feed record {
    # URL for the feed of public security advisories.
    string securityAdvisoriesUrl?;
    # Hypermedia links for the feed resources.
    FeedLinks links;
    # URL for the authenticated user's activity feed.
    string currentUserUrl?;
    # URL for the authenticated user's organization activity feed.
    string currentUserOrganizationUrl?;
    # List of URLs for each of the authenticated user's organization feeds.
    string[] currentUserOrganizationUrls?;
    # URL template for a given user's public activity feed.
    string userUrl;
    # A feed of discussions for a given repository
    string repositoryDiscussionsUrl?;
    # URL for the authenticated user's activity feed as an actor.
    string currentUserActorUrl?;
    # A feed of discussions for a given repository and category
    string repositoryDiscussionsCategoryUrl?;
    # URL for the authenticated user's public activity feed.
    string currentUserPublicUrl?;
    # URL for the authenticated user's timeline feed.
    string timelineUrl;
};


type FeedLinks record {
    # The feed link for repository discussions.
    LinkWithType repositoryDiscussions?;
    # The feed link for the current user's organization activity.
    LinkWithType currentUserOrganization?;
    # The feed links for all of the current user's organizations.
    LinkWithType[] currentUserOrganizations?;
    # The feed link for the current user's activity as an actor.
    LinkWithType currentUserActor?;
    # The feed link for the authenticated user's timeline.
    LinkWithType timeline;
    # The feed link for the user's public activity.
    LinkWithType user;
    # The feed link for GitHub security advisories.
    LinkWithType securityAdvisories?;
    # The feed link for the current user's public activity.
    LinkWithType currentUserPublic?;
    # The feed link for a specific repository discussion category.
    LinkWithType repositoryDiscussionsCategory?;
    # The feed link for the current authenticated user's activity.
    LinkWithType currentUser?;
};

# File Commit

type FileCommit record {
    # The commit object created as a result of the file operation.
    FileCommitCommit 'commit;
    # The file content metadata after the commit operation.
    FileCommitContent? content;
};


type FileCommitCommit record {
    # Details about the person who committed the file change.
    FileCommitCommitAuthor committer?;
    # Details about the person who authored the file change.
    FileCommitCommitAuthor author?;
    # The HTML URL to view the commit on GitHub.
    string htmlUrl?;
    # The git tree object associated with this commit.
    FileCommitCommitTree tree?;
    # The commit message describing the file change.
    string message?;
    # The SHA hash of the commit object.
    string sha?;
    # The API URL for the commit.
    string url?;
    # Signature verification details for the commit.
    FileCommitCommitVerification verification?;
    # The GraphQL node ID of the commit.
    string nodeId?;
    # List of parent commits for this commit.
    CommitSearchResultItemParents[] parents?;
};


type FileCommitCommitAuthor record {
    # The date and time the author made the commit.
    string date?;
    # The name of the commit author.
    string name?;
    # The email address of the commit author.
    string email?;
};


type FileCommitCommitTree record {
    # The SHA of the tree object.
    string sha?;
    # API URL for the tree object.
    string url?;
};


type FileCommitCommitVerification record {
    # The reason for the verification status of the commit signature.
    string reason?;
    # The GPG signature of the commit.
    string? signature?;
    # The signed payload used to verify the commit signature.
    string? payload?;
    # Whether the commit signature was successfully verified.
    boolean verified?;
};


type FileCommitContent record {
    # The file path relative to the repository root.
    string path?;
    # The size of the file in bytes.
    int size?;
    # Hypermedia links related to this file content resource.
    FileCommitContentLinks links?;
    # URL to view this file on GitHub.
    string htmlUrl?;
    # The name of the file.
    string name?;
    # URL to download the raw file contents.
    string downloadUrl?;
    # API URL for this file's git object.
    string gitUrl?;
    # The type of content, such as file or directory.
    string 'type?;
    # The SHA hash of the file blob.
    string sha?;
    # API URL for this file content resource.
    string url?;
};


type FileCommitContentLinks record {
    # The API URL for the git blob object of the file.
    string git?;
    # The API URL for the file content resource.
    string self?;
    # The HTML URL to view the file on GitHub.
    string html?;
};

# Full Repository

type FullRepository record {
    # The parent repository, if this repository is a fork.
    Repository parent?;
    # Whether forking is allowed on the repository
    boolean allowForking?;
    # Whether anonymous git access is allowed
    boolean anonymousAccessEnabled = true;
    # API URL for the authenticated user's subscription to the repository
    string subscriptionUrl;
    # API URL template for listing repository branches
    string branchesUrl;
    # API URL template for accessing issue comments
    string issueCommentUrl;
    # Whether rebase merging is allowed on pull requests
    boolean allowRebaseMerge?;
    # API URL for listing repository watchers
    string subscribersUrl;
    # The permissions the authenticated user has on this repository.
    FullRepositoryPermissions permissions?;
    # A temporary token for cloning the repository
    string? tempCloneToken?;
    # API URL template for listing repository releases
    string releasesUrl;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # The number of users watching the repository
    int subscribersCount;
    # The unique identifier of the repository
    int id;
    # Whether the repository has discussions enabled
    boolean hasDiscussions;
    # The number of forks of the repository
    int forks;
    # API URL template for accessing Git references
    string gitRefsUrl;
    # The SSH URL for cloning the repository
    string sshUrl;
    # The full name of the repository in owner/name format
    string fullName;
    # The size of the repository. Size is calculated hourly. When a repository is initially created, the size is 0
    int size;
    # The template repository from which this repository was created
    NullableRepository? templateRepository?;
    # API URL for listing programming languages used
    string languagesUrl;
    # URL of the repository page on GitHub
    string htmlUrl;
    # API URL template for listing repository collaborators
    string collaboratorsUrl;
    # The HTTPS URL for cloning the repository
    string cloneUrl;
    # The default branch of the repository
    string defaultBranch;
    # API URL for listing repository webhooks
    string hooksUrl;
    # API URL template for accessing Git trees
    string treesUrl;
    # Whether the repository has downloads enabled
    boolean hasDownloads?;
    # The date the repository was created
    string createdAt;
    # The number of watchers on the repository
    int watchers;
    # API URL for listing repository deployments
    string deploymentsUrl;
    # API URL template for listing repository deploy keys
    string keysUrl;
    # Whether the repository is archived
    boolean archived;
    # Whether the repository has the wiki enabled
    boolean hasWiki;
    # The date the repository was last updated
    string updatedAt;
    # Returns whether or not this repository disabled
    boolean disabled;
    # API URL template for comparing two commits
    string compareUrl;
    # API URL template for accessing Git commits
    string gitCommitsUrl;
    # The list of topics associated with the repository
    string[] topics?;
    # Whether the pull request branch can be updated from the base branch
    boolean allowUpdateBranch?;
    # API URL template for accessing Git tags
    string gitTagsUrl;
    # API URL for performing merge operations
    string mergesUrl;
    # API URL for the repository
    string url;
    # API URL template for accessing repository contents
    string contentsUrl;
    # API URL template for listing repository issues
    string issuesUrl;
    # Whether the PR title is used as the default squash commit message
    boolean useSquashPrTitleAsDefault?;
    # The organization that owns the repository, if applicable.
    NullableSimpleUser? organization?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # API URL template for listing repository assignees
    string assigneesUrl;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # The number of open issues in the repository
    int openIssues;
    # The GraphQL node identifier of the repository
    string nodeId;
    # The number of stars on the repository
    int stargazersCount;
    # Whether the repository is a template repository
    boolean isTemplate?;
    # The date of the most recent push to the repository
    string pushedAt;
    # The primary programming language of the repository
    string? language;
    # The ultimate source repository in a fork network.
    Repository 'source?;
    # API URL template for listing repository labels
    string labelsUrl;
    # The Subversion URL for the repository
    string svnUrl;
    # The name of the master branch
    string masterBranch?;
    # API URL template for downloading repository archives
    string archiveUrl;
    # Whether merge commits are allowed on pull requests
    boolean allowMergeCommit?;
    # API URL for listing repository forks
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility?;
    # API URL template for listing commit statuses
    string statusesUrl;
    # The number of repositories in the fork network
    int networkCount;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # Whether auto-merge is allowed on pull requests
    boolean allowAutoMerge?;
    # The name of the repository
    string name;
    # API URL template for listing pull requests
    string pullsUrl;
    # API URL for listing repository tags
    string tagsUrl;
    # Whether the repository is private
    boolean 'private;
    # API URL for listing repository contributors
    string contributorsUrl;
    # API URL template for listing repository notifications
    string notificationsUrl;
    # The number of open issues in the repository
    int openIssuesCount;
    # A short description of the repository
    string? description;
    # Whether the repository has projects enabled
    boolean hasProjects;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # API URL template for listing commit comments
    string commentsUrl;
    # API URL for listing users who starred the repository
    string stargazersUrl;
    # Whether to delete branches on merge
    boolean deleteBranchOnMerge?;
    # The Git protocol URL for the repository
    string gitUrl;
    # Whether the repository has GitHub Pages enabled
    boolean hasPages;
    # The user or organization that owns the repository.
    SimpleUser owner;
    # Whether squash merging is allowed on pull requests
    boolean allowSquashMerge?;
    # API URL template for listing repository commits
    string commitsUrl;
    # API URL template for accessing repository blobs
    string blobsUrl;
    # API URL for listing repository downloads
    string downloadsUrl;
    # Whether the repository has issues enabled
    boolean hasIssues;
    # Whether commit sign-off is required for web-based commits
    boolean webCommitSignoffRequired?;
    # The code of conduct for the repository
    CodeOfConductSimple codeOfConduct?;
    # The URL of the mirror for the repository
    string? mirrorUrl;
    # API URL template for listing repository milestones
    string milestonesUrl;
    # API URL for listing teams with access to the repository
    string teamsUrl;
    # The security and analysis settings for the repository
    SecurityAndAnalysis? securityAndAnalysis?;
    # Whether the repository is a fork
    boolean 'fork;
    # API URL for listing repository events
    string eventsUrl;
    # API URL template for listing issue events
    string issueEventsUrl;
    # The number of watchers on the repository
    int watchersCount;
    # The URL of the repository's homepage
    string? homepage;
    # The number of forks of the repository
    int forksCount;
};

# The permissions the authenticated user has on the repository

type FullRepositoryPermissions record {
    # Indicates whether the user has pull (read) permission.
    boolean pull;
    # Indicates whether the user has maintain permission on the repository.
    boolean maintain?;
    # Indicates whether the user has admin permission on the repository.
    boolean admin;
    # Indicates whether the user has triage permission on the repository.
    boolean triage?;
    # Indicates whether the user has push (write) permission.
    boolean push;
};

# Gist

type Gist record {
    # The user who owns the gist.
    NullableSimpleUser? owner?;
    # List of forks of this gist.
    anydata[] forks?;
    # The API URL for the gist's commit history.
    string commitsUrl;
    # The number of comments on the gist.
    int comments;
    # The API URL for the gist's forks.
    string forksUrl;
    # The Git push URL for the gist.
    string gitPushUrl;
    # The timestamp when the gist was created.
    string createdAt;
    # The description of the gist.
    string? description;
    # Whether the gist content is truncated due to size limits.
    boolean truncated?;
    # The list of revision history entries for the gist.
    anydata[] history?;
    # The API URL for the gist.
    string url;
    # Whether the gist is publicly accessible.
    boolean 'public;
    # The timestamp when the gist was last updated.
    string updatedAt;
    # The URL of the gist page on GitHub.
    string htmlUrl;
    # The Git pull URL for the gist.
    string gitPullUrl;
    # The API URL for the gist's comments.
    string commentsUrl;
    # The files contained within the gist.
    record {} files;
    # The unique identifier of the gist.
    string id;
    # The user associated with the gist.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the gist.
    string nodeId;
};

# A comment made to a gist

type GistComment record {
    # The commenter's association with the gist's repository.
    AuthorAssociation authorAssociation;
    # The date and time when the comment was last updated.
    string updatedAt;
    # The date and time when the comment was created.
    string createdAt;
    # Unique numeric identifier for the comment.
    int id;
    # The comment text
    string body;
    # The user who created the comment.
    NullableSimpleUser? user;
    # API URL for this gist comment.
    string url;
    # The GraphQL node identifier for this comment.
    string nodeId;
};

# Gist Commit

type GistCommit record {
    # The date and time the gist revision was committed.
    string committedAt;
    # Statistics about additions, deletions, and total changes in this revision.
    GistHistoryChangeStatus changeStatus;
    # The commit SHA identifying this version of the gist.
    string version;
    # The user who committed this version of the gist.
    NullableSimpleUser? user;
    # The API URL for this gist commit.
    string url;
};

# Gist History

type GistHistory record {
    # The date and time the gist version was committed.
    string committedAt?;
    # The change status showing additions and deletions for this version.
    GistHistoryChangeStatus changeStatus?;
    # The user who made this change to the gist.
    NullableSimpleUser? user?;
    # The commit SHA identifier for this version of the gist.
    string version?;
    # The API URL for this specific gist history entry.
    string url?;
};


type GistHistoryChangeStatus record {
    # The total number of lines changed in the gist revision.
    int total?;
    # The number of lines added in the gist revision.
    int additions?;
    # The number of lines deleted in the gist revision.
    int deletions?;
};


type GistIdCommentsBody record {
    # The comment text
    string body;
};


type GistsBody record {
    # Whether the gist is public or private.
    boolean|"true"|"false"  'public?;
    # Description of the gist
    string description?;
    # Names and content for the files that make up the gist
    record {} files;
};


type GistsFiles record {
    # Content of the file
    string content;
};


type GistsgistIdBody record {
    # The description of the gist
    string description?;
    # The gist files to be updated, renamed, or deleted. Each key must match the current filename
(including extension) of the targeted gist file. For example: hello.py.

To delete a file, set the whole file to null. For example: hello.py : null. The file will also be
deleted if the specified object does not contain at least one of content or filename
    record {} files?;
};


type GistsgistIdFiles record {
    # The new filename for the file
    string? filename?;
    # The new content of the file
    string content?;
};

# Gist Simple

type GistSimple record {
    # The list of forks of this gist.
    GistSimpleForks[]? forks?;
    # The user who owns the gist.
    SimpleUser owner?;
    # The API URL to retrieve the gist's commit history.
    string commitsUrl?;
    # The number of comments on the gist.
    int comments?;
    # The API URL to retrieve the gist's forks.
    string forksUrl?;
    # The Git URL used to push changes to the gist.
    string gitPushUrl?;
    # The timestamp when the gist was created.
    string createdAt?;
    # A brief description of the gist.
    string? description?;
    # Indicates whether the gist content is truncated.
    boolean truncated?;
    # The original gist this gist was forked from.
    Gist? forkOf?;
    # The list of historical versions of the gist.
    GistHistory[]? history?;
    # The API URL of the gist.
    string url?;
    # Indicates whether the gist is publicly visible.
    boolean 'public?;
    # The timestamp when the gist was last updated.
    string updatedAt?;
    # The browser-accessible URL of the gist.
    string htmlUrl?;
    # The Git URL used to pull the gist contents.
    string gitPullUrl?;
    # The API URL to retrieve comments on the gist.
    string commentsUrl?;
    # The files contained within the gist.
    record {} files?;
    # The unique identifier of the gist.
    string id?;
    # The username associated with the gist.
    string? user?;
    # The GraphQL node ID of the gist.
    string nodeId?;
};


type GistSimpleFiles record {
    # The name of the file in the gist.
    string filename?;
    # The size of the file in bytes.
    int size?;
    # Indicates whether the file content is truncated.
    boolean truncated?;
    # The detected programming language of the file.
    string language?;
    # The MIME type of the file.
    string 'type?;
    # The URL to access the raw file content.
    string rawUrl?;
    # The text content of the gist file.
    string content?;
};


type GistSimpleForks record {
    # The timestamp when the gist fork was last updated.
    string updatedAt?;
    # The timestamp when the gist fork was created.
    string createdAt?;
    # The unique identifier of the gist fork.
    string id?;
    # The user who forked the gist.
    PublicUser user?;
    # The API URL of the gist fork.
    string url?;
};

# Represents the Queries record for the operation: gists/list-comments

type GistsListCommentsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: gists/list-commits

type GistsListCommitsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: gists/list-forks

type GistsListForksQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: gists/list-for-user

type GistsListForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: gists/list-public

type GistsListPublicQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: gists/list

type GistsListQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: gists/list-starred

type GistsListStarredQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};


type GitBlobsBody record {
    # The encoding used for content. Currently, "utf-8" and "base64" are supported
    string encoding = "utf-8";
    # The new blob's content
    string content;
};

# Low-level Git commit operations within a repository

type GitCommit record {
    # The person who committed the changes to the repository.
    GitCommitCommitter committer;
    # The person who originally authored the commit.
    GitCommitCommitter author;
    # URL of the commit page on GitHub
    string htmlUrl;
    # The Git tree object this commit points to.
    GitCommitTree tree;
    # Message describing the purpose of the commit
    string message;
    # SHA for the commit
    string sha;
    # API URL for the commit
    string url;
    # Signature verification details for this commit.
    GitCommitVerification verification;
    # The GraphQL node identifier of the commit
    string nodeId;
    # The parent commits of this commit
    GitCommitParents[] parents;
};

# Identifying information for the git-user

type GitCommitCommitter record {
    # Timestamp of the commit
    string date;
    # Name of the git user
    string name;
    # Git email address of the user
    string email;
};


type GitCommitParents record {
    # The HTML URL to view the parent commit on GitHub.
    string htmlUrl;
    # SHA for the commit
    string sha;
    # The API URL for the parent commit.
    string url;
};


type GitCommitsBody record {
    # The person who committed the code to the repository.
    ReposownerrepogitcommitsCommitter committer?;
    # The [PGP signature](https://en.wikipedia.org/wiki/Pretty_Good_Privacy) of the commit. GitHub adds the signature to the gpgsig header of the created commit. For a commit signature to be verifiable by Git or GitHub, it must be an ASCII-armored detached PGP signature over the string commit as it would be written to the object database. To pass a signature parameter, you need to first manually create a valid PGP signature, which can be complicated. You may find it easier to [use the command line](https://git-scm.com/book/id/v2/Git-Tools-Signing-Your-Work) to create signed commits
    string signature?;
    # The person who originally authored the commit.
    ReposownerrepogitcommitsAuthor author?;
    # The SHA of the tree object this commit points to
    string tree;
    # The commit message
    string message;
    # The SHAs of the commits that were the parents of this commit. If omitted or empty, the commit will be written as a root commit. For a single parent, an array of one SHA should be provided; for a merge commit, an array of more than one should be provided
    string[] parents?;
};

# The tree object referenced by the commit

type GitCommitTree record {
    # SHA for the commit
    string sha;
    # The API URL for the tree object referenced by the commit.
    string url;
};

# The GPG signature verification data for the commit

type GitCommitVerification record {
    # The reason for the verification status of the signature.
    string reason;
    # The GPG signature string for the commit.
    string? signature;
    # The signed data payload used to verify the signature.
    string? payload;
    # Indicates whether the commit signature is verified.
    boolean verified;
};

# Represents the Queries record for the operation: git/get-tree

type GitGetTreeQueries record {
    # Setting this parameter to any value returns the objects or subtrees referenced by the tree specified in :tree_sha. For example, setting recursive to any of the following will enable returning objects or subtrees: 0, 1, "true", and "false". Omit this parameter to prevent recursively returning objects or subtrees
    string recursive?;
};

# Gitignore Template

type GitignoreTemplate record {
    # The name of the gitignore template.
    string name;
    # The gitignore template content with ignore patterns.
    string 'source;
};

# Git references within a repository

type GitRef record {
    # The fully qualified name of the Git reference.
    string ref;
    # API URL for this Git reference.
    string url;
    # The GraphQL node identifier of the Git reference.
    string nodeId;
    # The Git object this reference points to.
    GitRefObject 'object;
};


type GitRefObject record {
    # The type of Git object the reference points to.
    string 'type;
    # SHA for the reference
    string sha;
    # The API URL for the Git reference object.
    string url;
};


type GitRefsBody record {
    # The name of the fully qualified reference (ie: refs/heads/master). If it doesn't start with 'refs' and have at least two slashes, it will be rejected
    string ref;
    # The SHA1 value for this reference
    string sha;
};

# Metadata for a Git tag

type GitTag record {
    # The person who created the tag.
    GitTagTagger tagger;
    # Name of the tag
    string tag;
    # Message describing the purpose of the tag
    string message;
    # The SHA hash identifying the tag object.
    string sha;
    # URL for the tag
    string url;
    # The verification details of the tag signature.
    Verification verification?;
    # The GraphQL node identifier of the tag.
    string nodeId;
    # The Git object that this tag points to.
    GitTagObject 'object;
};


type GitTagObject record {
    # The type of Git object the tag points to.
    string 'type;
    # The SHA of the Git object the tag points to.
    string sha;
    # The API URL for the tagged Git object.
    string url;
};


type GitTagsBody record {
    # The person creating the tag, including name, email, and date.
    ReposownerrepogittagsTagger tagger?;
    # The tag's name. This is typically a version (e.g., "v0.0.1")
    string tag;
    # The tag message
    string message;
    # The type of the object we're tagging. Normally this is a commit but it can also be a tree or a blob
        "commit"|"tree"|"blob"  'type;
    # The SHA of the git object this is tagging
    string 'object;
};


type GitTagTagger record {
    # The date and time when the tag was created.
    string date;
    # The name of the person who created the tag.
    string name;
    # The email address of the person who created the tag.
    string email;
};

# The hierarchy between files in a Git repository

type GitTree record {
    # Objects specifying a tree structure
    GitTreeTree[] tree;
    # Indicates whether the tree was truncated due to size limits.
    boolean truncated;
    # The SHA1 hash of the Git tree object.
    string sha;
    # The API URL for this Git tree.
    string url;
};


type GitTreesBody record {
    # The SHA1 of an existing Git tree object which will be used as the base for the new tree. If provided, a new Git tree object will be created from entries in the Git tree object pointed to by base_tree and entries defined in the tree parameter. Entries defined in the tree parameter will overwrite items from base_tree with the same path. If you're creating new changes on a branch, then normally you'd set base_tree to the SHA1 of the Git tree object of the current latest commit on the branch you're working on.
If not provided, GitHub will create a new Git tree object from only the entries defined in the tree parameter. If you create a new commit pointing to such a tree, then all files which were a part of the parent commit's tree and were not defined in the tree parameter will be listed as deleted by the new commit
    string baseTree?;
    # Objects (of path, mode, type, and sha) specifying a tree structure
    ReposownerrepogittreesTree[] tree;
};


type GitTreeTree record {
    # File mode of the tree node, representing permissions and type.
    string mode?;
    # Path of the file or directory within the repository tree.
    string path?;
    # Size in bytes of the file represented by this tree node.
    int size?;
    # Type of the tree node, such as blob, tree, or commit.
    string 'type?;
    # SHA1 checksum hash of the tree node object.
    string sha?;
    # API URL for retrieving this tree node's details.
    string url?;
};

# A GitHub Security Advisory

type GlobalAdvisory record {
    # A short summary of the advisory
    string summary;
    # The severity of the advisory
        "critical"|"high"|"medium"|"low"|"unknown"  severity;
    # The URL of the advisory's source code
    string? sourceCodeLocation;
    # A list of reference URLs related to the advisory.
    string[]? references;
    # The API URL for the repository advisory
    string? repositoryAdvisoryUrl;
    # A list of identifiers such as CVE or GHSA IDs for the advisory.
    GlobalAdvisoryIdentifiers[]? identifiers;
    # A detailed description of what the advisory entails
    string? description;
    # The type of advisory
        "reviewed"|"unreviewed"|"malware"  'type;
    # The Common Weakness Enumeration (CWE) entries associated with the advisory.
    GlobalAdvisoryCwes[]? cwes;
    # The date and time when the advisory was published in the National Vulnerability Database, in ISO 8601 format.
This field is only populated when the advisory is imported from the National Vulnerability Database
    string? nvdPublishedAt;
    # The API URL for the advisory
    string url;
    # The GitHub Security Advisory ID
    string ghsaId;
    # The date and time of when the advisory was last updated, in ISO 8601 format
    string updatedAt;
    # The Common Vulnerabilities and Exposures (CVE) ID
    string? cveId;
    # The users who contributed to the advisory
    GlobalAdvisoryCredits[]? credits;
    # The URL for the advisory
    string htmlUrl;
    # The date and time of when the advisory was reviewed by GitHub, in ISO 8601 format
    string? githubReviewedAt;
    # The date and time of when the advisory was withdrawn, in ISO 8601 format
    string? withdrawnAt;
    # The products and respective version ranges affected by the advisory
    GlobalAdvisoryVulnerabilities[]? vulnerabilities;
    # The date and time of when the advisory was published, in ISO 8601 format
    string publishedAt;
    # The CVSS score and vector string for the advisory.
    GlobalAdvisoryCvss? cvss;
};


type GlobalAdvisoryCredits record {
    # The type of credit assigned for the security advisory contribution.
    SecurityAdvisoryCreditTypes 'type;
    # The user receiving credit for the security advisory.
    SimpleUser user;
};


type GlobalAdvisoryCvss record {
    # The CVSS score
    decimal? score;
    # The CVSS vector
    string? vectorString;
};


type GlobalAdvisoryCwes record {
    # The Common Weakness Enumeration (CWE) identifier
    string cweId;
    # The name of the CWE
    string name;
};


type GlobalAdvisoryIdentifiers record {
    # The type of identifier
        "CVE"|"GHSA"  'type;
    # The identifier value
    string value;
};

# The name of the package affected by the vulnerability

type GlobalAdvisoryPackage record {
    # The package ecosystem in which the vulnerability exists.
    SecurityAdvisoryEcosystems ecosystem;
    # The unique package name within its ecosystem
    string? name;
};


type GlobalAdvisoryVulnerabilities record {
    # The package version that resolve the vulnerability
    string? firstPatchedVersion;
    # The package affected by this vulnerability.
    GlobalAdvisoryPackage? package;
    # The functions in the package that are affected by the vulnerability
    string[]? vulnerableFunctions;
    # The range of the package versions affected by the vulnerability
    string? vulnerableVersionRange;
};

# A unique encryption key

type GpgKey record {
    # The public key value of the GPG key.
    string publicKey;
    # The unique identifier string of the GPG key.
    string keyId;
    # Timestamp when the GPG key was created.
    string createdAt;
    # Indicates whether the GPG key has been revoked.
    boolean revoked;
    # The raw key data string of the GPG key.
    string? rawKey;
    # List of email addresses associated with the GPG key.
    GpgKeyEmails[] emails;
    # Indicates whether this key can be used to certify other keys.
    boolean canCertify;
    # Timestamp when the GPG key expires, or null if it never expires.
    string? expiresAt;
    # Indicates whether this key can encrypt communications.
    boolean canEncryptComms;
    # The display name associated with the GPG key.
    string? name?;
    # Indicates whether this key can encrypt storage.
    boolean canEncryptStorage;
    # Indicates whether this key can be used to create signatures.
    boolean canSign;
    # The unique numeric identifier of the GPG key.
    int id;
    # List of subkeys associated with this GPG key.
    GpgKeySubkeys[] subkeys;
    # The ID of the primary GPG key this key is associated with.
    int? primaryKeyId;
};


type GpgKeyEmails record {
    # Indicates whether the email address is verified for the GPG key.
    boolean verified?;
    # The email address associated with the GPG key.
    string email?;
};


type GpgKeySubkeys record {
    # The public key data for the GPG subkey.
    string publicKey?;
    # The identifier of the GPG subkey.
    string keyId?;
    # Timestamp when the GPG subkey was created.
    string createdAt?;
    # Whether the GPG subkey has been revoked.
    boolean revoked?;
    # The raw key data for the GPG subkey.
    string? rawKey?;
    # Email addresses associated with the GPG subkey.
    GpgKeyEmails[] emails?;
    # Whether the GPG subkey can certify other keys.
    boolean canCertify?;
    # Timestamp when the GPG subkey expires.
    string? expiresAt?;
    # Whether the GPG subkey can encrypt communications.
    boolean canEncryptComms?;
    # Whether the GPG subkey can encrypt storage.
    boolean canEncryptStorage?;
    # Whether the GPG subkey can create signatures.
    boolean canSign?;
    # Unique identifier of the GPG subkey.
    int id?;
    # List of subkeys associated with this GPG subkey.
    anydata[] subkeys?;
    # Identifier of the primary GPG key that owns this subkey.
    int primaryKeyId?;
};

# Webhooks for repositories

type Hook record {
    # The URL used to test the webhook delivery.
    string testUrl;
    # Determines whether the hook is actually triggered on pushes
    boolean active;
    # The date and time the webhook was created.
    string createdAt;
    # The type of webhook.
    string 'type;
    # The API URL for the webhook.
    string url;
    # The date and time the webhook was last updated.
    string updatedAt;
    # The name of a valid service, use 'web' for a webhook
    string name;
    # Unique identifier of the webhook
    int id;
    # The last response received from the webhook endpoint.
    HookResponse lastResponse;
    # Configuration settings for the webhook.
    HookConfig config;
    # The URL used to ping the webhook.
    string pingUrl;
    # Determines what events the hook is triggered for. Default: ['push']
    string[] events;
    # The API URL listing deliveries for this webhook.
    string deliveriesUrl?;
};


type HookConfig record {
    # The password used for authentication with the webhook endpoint.
    string password?;
    # The media type used to serialize webhook payloads.
    WebhookConfigContentType contentType?;
    # Determines whether SSL verification is bypassed for webhook delivery.
    WebhookConfigInsecureSsl insecureSsl?;
    # The digest value used for webhook payload verification.
    string digest?;
    # The subdomain used for the webhook service integration.
    string subdomain?;
    # The secret token used to sign webhook payloads.
    WebhookConfigSecret secret?;
    # The email address associated with the webhook configuration.
    string email?;
    # The chat room associated with the webhook integration.
    string room?;
    # The URL to which webhook payloads are delivered.
    WebhookConfigUrl url?;
    # The authentication token for the webhook service integration.
    string token?;
};


type HookConfigBody record {
    # The media type used to serialize the webhook payload.
    WebhookConfigContentType contentType?;
    # Whether SSL verification is skipped when delivering payloads.
    WebhookConfigInsecureSsl insecureSsl?;
    # The shared secret used to generate the HMAC signature of webhook payloads.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads are delivered.
    WebhookConfigUrl url?;
};

# Delivery made by a webhook

type HookDelivery record {
    # The request that was sent for this webhook delivery.
    HookDeliveryRequest request;
    # Status code received when delivery was made
    int statusCode;
    # The id of the GitHub App installation associated with this event
    int? installationId;
    # Whether the delivery is a redelivery
    boolean redelivery;
    # The URL target of the delivery
    string url?;
    # Time spent delivering
    decimal duration;
    # The response received for this webhook delivery.
    HookDeliveryResponse response;
    # Unique identifier for the event (shared with all deliveries for all webhooks that subscribe to this event)
    string guid;
    # The type of activity for the event that triggered the delivery
    string? action;
    # The id of the repository associated with this event
    int? repositoryId;
    # Unique identifier of the delivery
    int id;
    # The event that triggered the delivery
    string event;
    # Time when the delivery was delivered
    string deliveredAt;
    # Description of the status of the attempted delivery
    string status;
};

# Delivery made by a webhook, without request and response information

type HookDeliveryItem record {
    # Time spent delivering
    decimal duration;
    # Status code received when delivery was made
    int statusCode;
    # Unique identifier for the event (shared with all deliveries for all webhooks that subscribe to this event)
    string guid;
    # The type of activity for the event that triggered the delivery
    string? action;
    # The id of the GitHub App installation associated with this event
    int? installationId;
    # The id of the repository associated with this event
    int? repositoryId;
    # Unique identifier of the webhook delivery
    int id;
    # The event that triggered the delivery
    string event;
    # Time when the webhook delivery occurred
    string deliveredAt;
    # Whether the webhook delivery is a redelivery
    boolean redelivery;
    # Describes the response returned after attempting the delivery
    string status;
};


type HookDeliveryRequest record {
    # The request headers sent with the webhook delivery
    record {}? headers;
    # The webhook payload
    record {}? payload;
};


type HookDeliveryResponse record {
    # The response headers received when the delivery was made
    record {}? headers;
    # The response payload received
    record {}? payload;
};


type HookIdConfigBody record {
    # The media type used to serialize the webhook payload.
    WebhookConfigContentType contentType?;
    # Whether to disable SSL verification for webhook delivery.
    WebhookConfigInsecureSsl insecureSsl?;
    # The secret used to sign webhook payloads for verification.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads will be delivered.
    WebhookConfigUrl url?;
};


type HookResponse record {
    # The HTTP response code from the webhook delivery.
    int? code;
    # The response message from the webhook delivery.
    string? message;
    # The response status string from the webhook delivery.
    string? status;
};


type HookshookIdBody record {
    # The name of the webhook.
    string name?;
    # Determines if notifications are sent when the webhook is triggered. Set to true to send notifications
    boolean active = true;
    # The configuration settings for the webhook.
    OrgsorghookshookIdConfig config?;
    # Determines what [events](https://docs.github.com/webhooks/event-payloads) the hook is triggered for
    string[] events = ["push"];
};


type HookshookIdBody1 record {
    # Determines a list of events to be removed from the list of events that the Hook triggers for
    string[] removeEvents?;
    # Determines if notifications are sent when the webhook is triggered. Set to true to send notifications
    boolean active = true;
    # Determines a list of events to be added to the list of events that the Hook triggers for
    string[] addEvents?;
    # Configuration object for the webhook including URL and content type.
    ReposownerrepohookshookIdConfig config?;
    # Determines what [events](https://docs.github.com/webhooks/event-payloads) the hook is triggered for. This replaces the entire array of events
    string[] events = ["push"];
};

# Hovercard

type Hovercard record {
    # List of contextual information displayed in the hovercard.
    HovercardContexts[] contexts;
};


type HovercardContexts record {
    # The context message displayed in the hovercard.
    string message;
    # The octicon icon name used for the hovercard context.
    string octicon;
};

# A repository import from an external source

type Import record {
    # The name of the import step that failed.
    string? failedStep?;
    # The error message if the import encountered a failure.
    string? errorMessage?;
    # The percentage of the push phase that has completed.
    int? pushPercent?;
    # The number of large files identified during the import.
    int largeFilesCount?;
    # The URL of the originating repository
    string vcsUrl;
    # The version control system of the source repository.
    string? vcs;
    # The root path of the source version control repository.
    string svcRoot?;
    # The REST API URL to retrieve the list of import authors.
    string authorsUrl;
    # A status message describing the current state of the import.
    string message?;
    # Available project choices when multiple projects are detected.
    ImportProjectChoices[] projectChoices?;
    # The REST API URL of the import.
    string url;
    # The total number of commits imported from the source repository.
    int? commitCount?;
    # Whether large files are stored using Git Large File Storage.
    boolean useLfs?;
    # The TFVC project name being imported.
    string tfvcProject?;
    # The number of authors identified in the import source.
    int? authorsCount?;
    # The GitHub web URL of the imported repository.
    string htmlUrl;
    # The percentage of the import phase that has completed.
    int? importPercent?;
    # Whether the import source contains large files.
    boolean hasLargeFiles?;
    # A human-readable description of the current import status.
    string? statusText?;
    # The REST API URL of the destination repository.
    string repositoryUrl;
    # The root path of the SVN repository being imported.
    string svnRoot?;
    # The total size in kilobytes of large files in the import.
    int largeFilesSize?;
    # The current status of the import process.
        "auth"|"error"|"none"|"detecting"|"choose"|"auth_failed"|"importing"|"mapping"|"waiting_to_push"|"pushing"|"complete"|"setup"|"unknown"|"detection_found_multiple"|"detection_found_nothing"|"detection_needs_auth"  status;
};


type ImportLfsBody record {
    # Whether to store large files during the import. opt_in means large files will be stored using Git LFS. opt_out means large files will be removed during the import
        "opt_in"|"opt_out"  useLfs;
};


type ImportProjectChoices record {
    # The human-readable name of the importable project.
    string humanName?;
    # The TFVC project name for Team Foundation Version Control imports.
    string tfvcProject?;
    # The version control system type of the importable project.
    string vcs?;
};

# Installation

type Installation record {
    # The API URL to create access tokens for the installation.
    string accessTokensUrl;
    # The API URL listing repositories accessible to the installation.
    string repositoriesUrl;
    # The type of account the installation targets, such as User or Organization.
    string targetType;
    # The single file path the installation is granted access to.
    string? singleFileName;
    # The date and time the installation was created.
    string createdAt;
    # The ID of the user or organization this token is being scoped to
    int targetId;
    # The contact email address associated with the installation.
    string? contactEmail?;
    # Describe whether all repositories have been selected or there's a selection involved
        "all"|"selected"  repositorySelection;
    # The URL-friendly slug identifier for the GitHub App.
    string appSlug;
    # The user who suspended the installation, if applicable.
    NullableSimpleUser? suspendedBy;
    # The date and time the installation was last updated.
    string updatedAt;
    # The permissions granted to the installation.
    AppPermissions permissions;
    # The GitHub web URL for the GitHub App's installation page.
    string htmlUrl;
    # Indicates whether the installation has access to multiple single files.
    boolean hasMultipleSingleFiles?;
    # The ID of the installation
    int id;
    # The ID of the GitHub App associated with this installation.
    int appId;
    # The list of file paths the installation is granted single-file access to.
    string[] singleFilePaths?;
    # The account (user or organization) where the app is installed.
    SimpleUser|Enterprise? account;
    # The list of events the installation is subscribed to.
    string[] events;
    # The date and time the installation was suspended.
    string? suspendedAt;
};


type InstallationIdAccessTokensBody record {
    # List of repository IDs that the token should have access to
    int[] repositoryIds?;
    # List of repository names that the token should have access to
    string[] repositories?;
    # The permissions granted to the installation access token.
    AppPermissions permissions?;
};

# Installation

type InstallationResponse record {
    # The total number of installations returned.
    int totalCount;
    # The list of GitHub App installations.
    Installation[] installations;
};

# Authentication token for a GitHub App installed on a user or org

type InstallationToken record {
    # Indicates whether the token applies to all or selected repositories.
        "all"|"selected"  repositorySelection?;
    # The path of the single file the token has access to.
    string singleFile?;
    # The date and time when the installation token expires.
    string expiresAt;
    # The list of repositories the token has access to.
    Repository[] repositories?;
    # The permissions granted to the installation token.
    AppPermissions permissions?;
    # Indicates whether the token grants access to multiple single files.
    boolean hasMultipleSingleFiles?;
    # The list of file paths the token has single-file access to.
    string[] singleFilePaths?;
    # The authentication token string for the installation.
    string token;
};

# GitHub apps are a new way to extend GitHub. They can be installed directly on organizations and user accounts and granted access to specific repositories. They come with granular permissions and built-in webhooks. GitHub apps are first class actors within GitHub

type Integration record {
    # The user or organization that owns the GitHub app.
    NullableSimpleUser? owner;
    # The number of installations associated with the GitHub app
    int installationsCount?;
    # A short description of the GitHub app.
    string? description;
    # The date and time the GitHub app was created.
    string createdAt;
    # The client ID used for OAuth authentication.
    string clientId?;
    # The external URL linked to the GitHub app.
    string externalUrl;
    # The date and time the GitHub app was last updated.
    string updatedAt;
    # The set of permissions granted to the GitHub app.
    IntegrationPermissions permissions;
    # The URL of the GitHub app's GitHub profile page.
    string htmlUrl;
    # The name of the GitHub app
    string name;
    # The secret used to secure webhook payloads.
    string? webhookSecret?;
    # The private key PEM used to sign access token requests.
    string pem?;
    # Unique identifier of the GitHub app
    int id;
    # The client secret used for OAuth authentication.
    string clientSecret?;
    # The slug name of the GitHub app
    string slug?;
    # The list of events for the GitHub app
    string[] events;
    # The GraphQL node identifier of the GitHub app.
    string nodeId;
};

# Request to install an integration on a target

type IntegrationInstallationRequest record {
    # The user who requested the integration installation.
    SimpleUser requester;
    # The date and time the installation request was created.
    string createdAt;
    # Unique identifier of the request installation
    int id;
    # The target account for the integration installation request.
    SimpleUser|Enterprise account;
    # The GraphQL node identifier of the installation request.
    string nodeId?;
};

# The set of permissions for the GitHub app

type IntegrationPermissions record {
    # Permission level granted for issues.
    string issues?;
    # Permission level granted for check runs and check suites.
    string checks?;
    # Permission level granted for repository metadata.
    string metadata?;
    # Permission level granted for repository contents.
    string contents?;
    # Permission level granted for deployments.
    string deployments?;
    # Rest field
    string ;
};

# Limit interactions to a specific type of user for a specified duration

type InteractionLimit record {
    # The type of user interaction to restrict.
    InteractionGroup 'limit;
    # The duration for which the interaction limit is active.
    InteractionExpiry expiry?;
};

# Interaction limit settings

type InteractionLimitResponse record {
    # The date and time when the interaction limit expires.
    string expiresAt;
    # The origin context where the interaction limit is applied.
    string origin;
    # The type of interaction limit currently active.
    InteractionGroup 'limit;
};


type InteractionLimitResponseAnyAnyOf2 record {
};


type InvitationsinvitationIdBody record {
    # The permissions that the associated user will have on the repository. Valid values are read, write, maintain, triage, and admin
        "read"|"write"|"maintain"|"triage"|"admin"  permissions?;
};

# Issues are a great way to keep track of tasks, enhancements, and bugs for your projects

type Issue record {
    # The HTML-rendered body of the issue
    string bodyHtml?;
    # The plain text body of the issue
    string bodyText?;
    # The users assigned to the issue
    SimpleUser[]? assignees?;
    # The date the issue was created
    string createdAt;
    # Title of the issue
    string title;
    # Contents of the issue
    string? body?;
    # The repository this issue belongs to.
    Repository repository?;
    # The user who closed the issue
    NullableSimpleUser? closedBy?;
    # API URL template for the issue's labels
    string labelsUrl;
    # The association of the author with the repository
    AuthorAssociation authorAssociation;
    # Number uniquely identifying the issue within its repository
    int number;
    # The date the issue was last updated
    string updatedAt;
    # The GitHub App that triggered the event
    NullableIntegration? performedViaGithubApp?;
    # Whether the issue is a draft
    boolean draft?;
    # API URL for the issue's comments
    string commentsUrl;
    # The reason the issue conversation was locked
    string? activeLockReason?;
    # The unique identifier of the issue
    int id;
    # API URL for the repository containing the issue
    string repositoryUrl;
    # State of the issue; either 'open' or 'closed'
    string state;
    # Whether the issue conversation is locked
    boolean locked;
    # API URL for the issue's timeline events
    string timelineUrl?;
    # The reason for the current state
        "completed"|"reopened"|"not_planned"? stateReason?;
    # Pull request details linked to this issue, if applicable.
    IssuePullRequest pullRequest?;
    # The number of comments on the issue
    int comments;
    # The date the issue was closed
    string? closedAt;
    # URL for the issue
    string url;
    # Labels to associate with this issue; pass one or more label names to replace the set of labels on this issue; send an empty array to clear all labels from the issue; note that the labels are silently dropped for users without push access to the repository
    IssueLabels[] labels;
    # The milestone associated with this issue.
    NullableMilestone? milestone;
    # API URL for the issue's events
    string eventsUrl;
    # URL of the issue page on GitHub
    string htmlUrl;
    # Reaction counts aggregated for the issue.
    ReactionRollup reactions?;
    # The primary user assigned to the issue.
    NullableSimpleUser? assignee;
    # The user who created the issue.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the issue
    string nodeId;
};

# Comments provide a way for people to collaborate on an issue

type IssueComment record {
    # API URL for the associated issue
    string issueUrl;
    # The HTML-rendered body of the comment
    string bodyHtml?;
    # The plain text body of the comment
    string bodyText?;
    # The date the comment was created
    string createdAt;
    # Contents of the issue comment
    string body?;
    # URL for the issue comment
    string url;
    # The association of the comment author with the repository
    AuthorAssociation authorAssociation;
    # The date the comment was last updated
    string updatedAt;
    # The GitHub App that created the comment
    NullableIntegration? performedViaGithubApp?;
    # URL of the comment on GitHub
    string htmlUrl;
    # Reaction counts aggregated by emoji for the comment.
    ReactionRollup reactions?;
    # Unique identifier of the issue comment
    int id;
    # The user who created the issue comment.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the comment
    string nodeId;
};

# Issue Event

type IssueEvent record {
    # The team requested for review in this event.
    Team requestedTeam?;
    # Details of the dismissed review associated with this event.
    IssueEventDismissedReview dismissedReview?;
    # The issue associated with this event.
    NullableIssue? issue?;
    # The reason the issue was locked.
    string? lockReason?;
    # The user who performed the assignment action.
    NullableSimpleUser? assigner?;
    # The date and time the event was created.
    string createdAt;
    # The user requested as a reviewer in this event.
    NullableSimpleUser? requestedReviewer?;
    # The label added or removed in this event.
    IssueEventLabel label?;
    # The API URL for this issue event.
    string url;
    # The project card associated with this event.
    IssueEventProjectCard projectCard?;
    # The user who triggered this event.
    NullableSimpleUser? actor;
    # The role of the actor in relation to the repository.
    AuthorAssociation authorAssociation?;
    # The API URL of the commit associated with this event.
    string? commitUrl;
    # The user who requested the review.
    NullableSimpleUser? reviewRequester?;
    # The milestone added or removed in this event.
    IssueEventMilestone milestone?;
    # The GitHub app that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp?;
    # The rename details if the issue was renamed.
    IssueEventRename rename?;
    # The unique identifier of the issue event.
    int id;
    # The user assigned or unassigned in this event.
    NullableSimpleUser? assignee?;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The GraphQL node identifier of this event.
    string nodeId;
};


type IssueEventDismissedReview record {
    # The unique identifier of the dismissed pull request review.
    int reviewId;
    # The state of the review at the time it was dismissed.
    string state;
    # The SHA of the commit at which the review was dismissed.
    string? dismissalCommitId?;
    # The message provided when the review was dismissed.
    string? dismissalMessage;
};

# Issue Event Label

type IssueEventLabel record {
    # The hex color code of the label.
    string? color;
    # The name of the label.
    string? name;
};

# Issue Event Milestone

type IssueEventMilestone record {
    # The title of the milestone associated with the issue event.
    string title;
};

# Issue Event Project Card

type IssueEventProjectCard record {
    # The unique identifier of the project containing the card.
    int projectId;
    # The name of the project column containing the card.
    string columnName;
    # API URL for the project associated with the card.
    string projectUrl;
    # The unique identifier of the project card.
    int id;
    # The name of the column the card was moved from.
    string previousColumnName?;
    # API URL for the project card.
    string url;
};

# Issue Event Rename

type IssueEventRename record {
    # The previous name of the issue before renaming.
    string 'from;
    # The new name of the issue after renaming.
    string to;
};


type IssueNumberAssigneesBody record {
    # Usernames of people to assign this issue to. _NOTE: Only users with push access can add assignees to an issue. Assignees are silently ignored otherwise._
    string[] assignees?;
};


type IssueNumberAssigneesBody1 record {
    # Usernames of assignees to remove from an issue. _NOTE: Only users with push access can remove assignees from an issue. Assignees are silently ignored otherwise._
    string[] assignees?;
};


type IssueNumberLockBody record {
    # The reason for locking the issue or pull request conversation. Lock will fail if you don't use one of these reasons:  
* off-topic  
* too heated  
* resolved  
* spam
        "off-topic"|"too heated"|"resolved"|"spam"  lockReason?;
};


type IssueNumberReactionsBody record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the issue
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
};

# Pull request metadata if this issue is a pull request

type IssuePullRequest record {
    # The URL to download the pull request as a patch file.
    string? patchUrl;
    # The date and time the pull request was merged.
    string? mergedAt?;
    # The GitHub web URL of the associated pull request.
    string? htmlUrl;
    # The URL to download the pull request as a diff file.
    string? diffUrl;
    # The REST API URL of the associated pull request.
    string? url;
};

# Issue Search Result Item

type IssueSearchResultItem record {
    # The HTML-rendered body of the issue.
    string bodyHtml?;
    # The plain-text body of the issue.
    string bodyText?;
    # The list of users assigned to the issue.
    SimpleUser[]? assignees?;
    # The date and time the issue was created.
    string createdAt;
    # The title of the issue.
    string title;
    # The body content of the issue.
    string? body?;
    # The repository containing the issue.
    Repository repository?;
    # API URL for listing labels on this issue.
    string labelsUrl;
    # The author's association with the repository.
    AuthorAssociation authorAssociation;
    # The issue number within the repository.
    int number;
    # The relevance score of the issue in search results.
    decimal score;
    # The date and time the issue was last updated.
    string updatedAt;
    # The GitHub App that triggered the issue action, if any.
    NullableIntegration? performedViaGithubApp?;
    # Whether the associated pull request is a draft.
    boolean draft?;
    # API URL for listing comments on the issue.
    string commentsUrl;
    # The reason the issue is locked, if applicable.
    string? activeLockReason?;
    # API URL for the repository containing the issue.
    string repositoryUrl;
    # The unique identifier of the issue.
    int id;
    # The state of the issue, either open or closed.
    string state;
    # Whether the issue is locked.
    boolean locked;
    # API URL for the issue's timeline events.
    string timelineUrl?;
    # The reason the issue was set to its current state.
    string? stateReason?;
    # Pull request metadata if the issue is linked to one.
    IssueSearchResultItemPullRequest pullRequest?;
    # The number of comments on the issue.
    int comments;
    # The date and time the issue was closed.
    string? closedAt;
    # API URL for the issue.
    string url;
    # The list of labels applied to the issue.
    IssueSearchResultItemLabels[] labels;
    # The milestone associated with the issue.
    NullableMilestone? milestone;
    # API URL for listing events on the issue.
    string eventsUrl;
    # URL of the issue page on GitHub.
    string htmlUrl;
    # Text fragments matching the search query.
    SearchResultTextMatches textMatches?;
    # Reaction counts for the issue.
    ReactionRollup reactions?;
    # The primary user assigned to the issue.
    NullableSimpleUser? assignee;
    # The user who created the issue.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the issue.
    string nodeId;
};


type IssueSearchResultItemLabels record {
    # Whether this is a default label provided by GitHub.
    boolean default?;
    # The hexadecimal color code of the label.
    string color?;
    # The name of the label.
    string name?;
    # A short description of the label.
    string? description?;
    # The unique identifier of the label.
    int id?;
    # The API URL for the label.
    string url?;
    # The GraphQL node identifier of the label.
    string nodeId?;
};


type IssueSearchResultItemPullRequest record {
    # The URL to download the patch for this pull request.
    string? patchUrl;
    # The timestamp when this pull request was merged.
    string? mergedAt?;
    # The HTML URL to view this pull request on GitHub.
    string? htmlUrl;
    # The URL to view the diff for this pull request.
    string? diffUrl;
    # The API URL for this pull request.
    string? url;
};

# Issue Search Result Item

type IssueSearchResultItemResponse record {
    # The total number of issues matching the search query.
    int totalCount;
    # Indicates whether the search results are incomplete.
    boolean incompleteResults;
    # The list of issues matching the search query.
    IssueSearchResultItem[] items;
};


type IssuesissueNumberBody record {
    # The reason for the state change. Ignored unless state is changed
        "completed"|"not_planned"|"reopened"? stateReason?;
    # The milestone to associate with this issue, or null to remove.
    string|int? milestone?;
    # Usernames to assign to this issue. Pass one or more user logins to _replace_ the set of assignees on this issue. Send an empty array ([]) to clear all assignees from the issue. Only users with push access can set assignees for new issues. Without push access to the repository, assignee changes are silently dropped
    string[] assignees?;
    # Username to assign to this issue. **This field is deprecated.**
    string? assignee?;
    # The open or closed state of the issue
        "open"|"closed"  state?;
    # The title of the issue
    string|int? title?;
    # The contents of the issue
    string? body?;
    # Labels to associate with this issue. Pass one or more labels to _replace_ the set of labels on this issue. Send an empty array ([]) to clear all labels from the issue. Only users with push access can set labels for issues. Without push access to the repository, label changes are silently dropped
    ReposownerrepoissuesissueNumberLabels[] labels?;
};

# Represents the Queries record for the operation: issues/list-assignees

type IssuesListAssigneesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-comments-for-repo

type IssuesListCommentsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # Either asc or desc. Ignored without the sort parameter
        "asc"|"desc"  direction?;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: issues/list-comments

type IssuesListCommentsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: issues/list-events-for-repo

type IssuesListEventsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-events-for-timeline

type IssuesListEventsForTimelineQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-events

type IssuesListEventsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-for-authenticated-user

type IssuesListForAuthenticatedUserQueries record {
    # Indicates which sorts of issues to return. assigned means issues assigned to you. created means issues created by you. mentioned means issues mentioning you. subscribed means issues you're subscribed to updates for. all or repos means all issues you can see, regardless of participation or creation
        "assigned"|"created"|"mentioned"|"subscribed"|"repos"|"all"  filter = "assigned";
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the issues to return
        "open"|"closed"|"all"  state = "open";
    # What to sort results by
        "created"|"updated"|"comments"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # A list of comma separated label names. Example: bug,ui,@high
    string labels?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: issues/list-for-org

type IssuesListForOrgQueries record {
    # Indicates which sorts of issues to return. assigned means issues assigned to you. created means issues created by you. mentioned means issues mentioning you. subscribed means issues you're subscribed to updates for. all or repos means all issues you can see, regardless of participation or creation
        "assigned"|"created"|"mentioned"|"subscribed"|"repos"|"all"  filter = "assigned";
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the issues to return
        "open"|"closed"|"all"  state = "open";
    # What to sort results by
        "created"|"updated"|"comments"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # A list of comma separated label names. Example: bug,ui,@high
    string labels?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: issues/list-for-repo

type IssuesListForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The user that created the issue
    string creator?;
    # If an integer is passed, it should refer to a milestone by its number field. If the string * is passed, issues with any milestone are accepted. If the string none is passed, issues without milestones are returned
    string milestone?;
    # Indicates the state of the issues to return
        "open"|"closed"|"all"  state = "open";
    # Can be the name of a user. Pass in none for issues with no assigned user, and * for issues assigned to any user
    string assignee?;
    # What to sort results by
        "created"|"updated"|"comments"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # A user that's mentioned in the issue
    string mentioned?;
    # A list of comma separated label names. Example: bug,ui,@high
    string labels?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: issues/list-labels-for-milestone

type IssuesListLabelsForMilestoneQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-labels-for-repo

type IssuesListLabelsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-labels-on-issue

type IssuesListLabelsOnIssueQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: issues/list-milestones

type IssuesListMilestonesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The state of the milestone. Either open, closed, or all
        "open"|"closed"|"all"  state = "open";
    # What to sort results by. Either due_on or completeness
        "due_on"|"completeness"  sort = "due_on";
    # Page number of the results to fetch
    int page = 1;
    # The direction of the sort. Either asc or desc
        "asc"|"desc"  direction = "asc";
};

# Represents the Queries record for the operation: issues/list

type IssuesListQueries record {
    # Indicates which sorts of issues to return. assigned means issues assigned to you. created means issues created by you. mentioned means issues mentioning you. subscribed means issues you're subscribed to updates for. all or repos means all issues you can see, regardless of participation or creation
        "assigned"|"created"|"mentioned"|"subscribed"|"repos"|"all"  filter = "assigned";
    # The number of results per page (max 100)
    int perPage = 30;
    # If true, include issues from collaborating repositories.
    boolean collab?;
    # If true, include issues from repositories owned by the authenticated user.
    boolean owned?;
    # Indicates the state of the issues to return
        "open"|"closed"|"all"  state = "open";
    # What to sort results by
        "created"|"updated"|"comments"  sort = "created";
    # If true, include issues from organization repositories.
    boolean orgs?;
    # Page number of the results to fetch
    int page = 1;
    # A list of comma separated label names. Example: bug,ui,@high
    string labels?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
    # If true, include pull requests in the results.
    boolean pulls?;
};


type JitConfig record {
    # The runner associated with this just-in-time configuration.
    Runner runner;
    # The base64 encoded runner configuration
    string encodedJitConfig;
};

# Information of a job execution in a workflow run

type Job record {
    # The ID of the runner to which this job has been assigned. (If a runner hasn't yet been assigned, this will be null.)
    int? runnerId;
    # The id of the associated workflow run
    int runId;
    # The name of the workflow
    string? workflowName;
    # The name of the current branch
    string? headBranch;
    # The name of the runner to which this job has been assigned. (If a runner hasn't yet been assigned, this will be null.)
    string? runnerName;
    # The name of the runner group to which this job has been assigned. (If a runner hasn't yet been assigned, this will be null.)
    string? runnerGroupName;
    # The time that the job created, in ISO 8601 format
    string createdAt;
    # Steps in this job
    JobSteps[] steps?;
    # The SHA of the commit that is being run
    string headSha;
    # The API URL for this workflow job.
    string url;
    # The API URL for the check run associated with this job.
    string checkRunUrl;
    # Labels for the workflow job. Specified by the "runs_on" attribute in the action's workflow file
    string[] labels;
    # The outcome of the job
        "success"|"failure"|"neutral"|"cancelled"|"skipped"|"timed_out"|"action_required"? conclusion;
    # The time that the job finished, in ISO 8601 format
    string? completedAt;
    # The API URL for the workflow run associated with this job.
    string runUrl;
    # The HTML URL for viewing this job on GitHub.
    string? htmlUrl;
    # The name of the job
    string name;
    # Attempt number of the associated workflow run, 1 for first attempt and higher if the workflow was re-run
    int runAttempt?;
    # The time that the job started, in ISO 8601 format
    string startedAt;
    # The id of the job
    int id;
    # The ID of the runner group to which this job has been assigned. (If a runner hasn't yet been assigned, this will be null.)
    int? runnerGroupId;
    # The GraphQL node ID for the job.
    string nodeId;
    # The phase of the lifecycle that the job is currently in
        "queued"|"in_progress"|"completed"  status;
};


type JobIdRerunBody record {
    # Whether to enable debug logging for the re-run
    boolean enableDebugLogging = false;
};

# Information of a job execution in a workflow run

type JobResponse record {
    # The total number of jobs in the workflow run.
    int totalCount;
    # The list of jobs in the workflow run.
    Job[] jobs;
};


type JobSteps record {
    # The outcome of the job
    string? conclusion;
    # The sequential step number within the job.
    int number;
    # The time that the job finished, in ISO 8601 format
    string? completedAt?;
    # The name of the job
    string name;
    # The time that the step started, in ISO 8601 format
    string? startedAt?;
    # The phase of the lifecycle that the job is currently in
        "queued"|"in_progress"|"completed"  status;
};

# Key

type Key record {
    # Whether the SSH key grants read-only access to the repository.
    boolean readOnly;
    # Whether the SSH key has been verified.
    boolean verified;
    # The date and time the key was created.
    string createdAt;
    # The unique identifier of the SSH key.
    int id;
    # The display title of the SSH key.
    string title;
    # The public SSH key value.
    string 'key;
    # The API URL for the SSH key.
    string url;
};

# Key Simple

type KeySimple record {
    # The unique identifier of the SSH key.
    int id;
    # The public SSH key value.
    string 'key;
};

# Color-coded labels help you categorize and filter your issues (just like labels in Gmail)

type Label record {
    # Whether this label comes by default in a new repository
    boolean default;
    # 6-character hex code, without the leading #, identifying the color
    string color;
    # The name of the label
    string name;
    # Optional description of the label, such as its purpose
    string? description;
    # Unique identifier for the label
    int id;
    # URL for the label
    string url;
    # The GraphQL node ID of the label.
    string nodeId;
};

# Labeled Issue Event

type LabeledIssueEvent record {
    # The user who triggered this labeled event.
    SimpleUser actor;
    # API URL of the commit associated with this event.
    string? commitUrl;
    # The GitHub App that triggered this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The timestamp when this event was created.
    string createdAt;
    # Unique numeric identifier for this event.
    int id;
    # The label that was applied in this event.
    UnlabeledIssueEventLabel label;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # API URL for this event.
    string url;
    # Global node identifier for this event.
    string nodeId;
};

# Label Search Result Item

type LabelSearchResultItem record {
    # Search relevance score for this label result.
    decimal score;
    # Indicates whether this is a default label for new repositories.
    boolean default;
    # Hex color code associated with the label.
    string color;
    # Text match metadata for highlighted search result snippets.
    SearchResultTextMatches textMatches?;
    # Name of the label.
    string name;
    # Short description of the label's purpose.
    string? description;
    # Unique numeric identifier of the label.
    int id;
    # API URL for accessing this label.
    string url;
    # Global GraphQL node identifier for the label.
    string nodeId;
};

# Label Search Result Item

type LabelSearchResultItemResponse record {
    # The total number of labels matching the search query.
    int totalCount;
    # Whether the search results are incomplete due to a timeout.
    boolean incompleteResults;
    # The list of label search result items.
    LabelSearchResultItem[] items;
};


type LabelsLabelsOneOf112 record {
    # The hexadecimal color code for the label.
    string? color?;
    # The name of the label.
    string name?;
    # A short description of the label.
    string? description?;
    # The unique identifier of the label.
    int id?;
};


type LabelsLabelsOneOf12 record {
    # The hexadecimal color code for the label.
    string? color?;
    # The name of the label.
    string name?;
    # A short description of the label's purpose.
    string? description?;
    # The unique identifier of the label.
    int id?;
};


type LabelsLabelsOneOf122 record {
    # Indicates whether this is a default label for the repository.
    boolean default?;
    # The hex color code of the label.
    string? color?;
    # The name of the label.
    string name?;
    # A brief description of the label.
    string? description?;
    # The unique identifier of the label.
    int id?;
    # The API URL of the label.
    string url?;
    # The GraphQL node identifier of the label.
    string nodeId?;
};


type LabelsLabelsOneOf132 record {
    # Indicates whether this is a default label for the repository.
    boolean default?;
    # The hexadecimal color code for the label.
    string? color?;
    # The name of the label.
    string name?;
    # A short description of the label.
    string? description?;
    # The unique numeric identifier of the label.
    int id?;
    # The URL of the label resource.
    string url?;
    # The GraphQL node identifier for the label.
    string nodeId?;
};


type LabelsnameBody record {
    # The [hexadecimal color code](http://www.color-hex.com/) for the label, without the leading #
    string color?;
    # A short description of the label. Must be 100 characters or fewer
    string description?;
    # The new name of the label. Emoji can be added to label names, using either native emoji or colon-style markup. For example, typing :strawberry: will render the emoji ![:strawberry:](https://github.githubassets.com/images/icons/emoji/unicode/1f353.png ":strawberry:"). For a full list of available emoji and codes, see "[Emoji cheat sheet](https://github.com/ikatyang/emoji-cheat-sheet)."
    string newName?;
};

# Language

type Language record {
    # Rest field
    int ;
};

# License

type License record {
    # Indicates whether the license is featured on GitHub.
    boolean featured;
    # Instructions on how to implement the license in a project.
    string implementation;
    # A brief description of the license.
    string description;
    # The full text body of the license.
    string body;
    # The SPDX identifier for the license.
    string? spdxId;
    # The API URL for the license resource.
    string? url;
    # List of permissions granted by this license.
    string[] permissions;
    # The URL to the human-readable license page.
    string htmlUrl;
    # The full name of the license.
    string name;
    # List of conditions required by this license.
    string[] conditions;
    # The unique key identifier for the license.
    string 'key;
    # The GraphQL node ID for the license.
    string nodeId;
    # List of limitations imposed by this license.
    string[] limitations;
};

# License Content

type LicenseContent record {
    # Hypermedia links for navigating related resources.
    ContentSymlinkLinks links;
    # The type of the content object.
    string 'type;
    # The encoding used for the content field.
    string encoding;
    # The SHA hash of the file content.
    string sha;
    # The API URL for the license content.
    string url;
    # The Base64-encoded content of the license file.
    string content;
    # The path of the license file within the repository.
    string path;
    # The license information associated with this content.
    NullableLicenseSimple? license;
    # The size of the file in bytes.
    int size;
    # The HTML URL to view the license file on GitHub.
    string? htmlUrl;
    # The filename of the license file.
    string name;
    # The URL to download the raw license file content.
    string? downloadUrl;
    # The API URL for the git blob object of the license file.
    string? gitUrl;
};

# Represents the Queries record for the operation: licenses/get-all-commonly-used

type LicensesGetAllCommonlyUsedQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter to only return featured licenses.
    boolean featured?;
    # Page number of the results to fetch
    int page = 1;
};

# License Simple

type LicenseSimple record {
    # URL of the license page on GitHub.
    string htmlUrl?;
    # The full name of the license.
    string name;
    # The SPDX identifier for the license.
    string? spdxId;
    # The unique key identifier for the license.
    string 'key;
    # API URL for the license.
    string? url;
    # The GraphQL node identifier of the license.
    string nodeId;
};

# Hypermedia Link

type Link record {
    # The URL of the hyperlink.
    string href;
};

# Hypermedia Link with Type

type LinkWithType record {
    # The URL of the hypermedia link.
    string href;
    # The media type of the hypermedia link.
    string 'type;
};

# Locked Issue Event

type LockedIssueEvent record {
    # The user who locked the issue.
    SimpleUser actor;
    # API URL for the commit associated with this event.
    string? commitUrl;
    # The GitHub App that triggered the lock event.
    NullableIntegration? performedViaGithubApp;
    # The reason the issue was locked.
    string? lockReason;
    # The date and time when the issue was locked.
    string createdAt;
    # The unique identifier of this event.
    int id;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # API URL for this event.
    string url;
    # The GraphQL node identifier of this event.
    string nodeId;
};


type Manifest record {
    # Additional metadata associated with the dependency manifest.
    Metadata metadata?;
    # The file details associated with the dependency manifest.
    ManifestFile file?;
    # The name of the manifest
    string name;
    # A collection of resolved package dependencies
    record {} resolved?;
};


type ManifestConversions record {
    NullableSimpleUser|() owner;
    int installationsCount;
    string|() description;
    string createdAt;
    string clientId;
    string externalUrl;
    string updatedAt;
    IntegrationPermissions permissions;
    string htmlUrl;
    string name;
    string|() webhookSecret;
    string pem;
    int id;
    string clientSecret;
    string slug;
    string[] events;
    string nodeId;
    string client_id;
    string client_secret;
    string? webhook_secret;
};


type ManifestFile record {
    # The path of the manifest file relative to the root of the Git repository
    string sourceLocation?;
};


type MarkdownBody record {
    # The rendering mode
        "markdown"|"gfm"  mode = "markdown";
    # The repository context to use when creating references in gfm mode.  For example, setting context to octo-org/octo-repo will change the text #42 into an HTML link to issue 42 in the octo-org/octo-repo repository
    string context?;
    # The Markdown text to render in HTML
    string text;
};


type MarketplaceAccount record {
    # The billing email address for the organization account.
    string? organizationBillingEmail?;
    # The unique identifier of the marketplace account.
    int id;
    # The type of account, such as User or Organization.
    string 'type;
    # The login username of the marketplace account.
    string login;
    # The API URL for the marketplace account.
    string url;
    # The email address associated with the marketplace account.
    string? email?;
    # The GraphQL node identifier of the marketplace account.
    string nodeId?;
};

# Marketplace Listing Plan

type MarketplaceListingPlan record {
    # Indicates whether this plan offers a free trial period.
    boolean hasFreeTrial;
    # The API URL to list accounts subscribed to this plan.
    string accountsUrl;
    # A description of the Marketplace listing plan.
    string description;
    # The API URL for this Marketplace listing plan.
    string url;
    # The name of the unit for per-unit pricing plans.
    string? unitName;
    # The unique plan number for this Marketplace listing.
    int number;
    # The yearly price of the plan in US cents.
    int yearlyPriceInCents;
    # The name of the Marketplace listing plan.
    string name;
    # The unique identifier of this Marketplace listing plan.
    int id;
    # The monthly price of the plan in US cents.
    int monthlyPriceInCents;
    # The current state of the Marketplace listing plan.
    string state;
    # The pricing model used for this plan (FREE, FLAT_RATE, or PER_UNIT).
        "FREE"|"FLAT_RATE"|"PER_UNIT"  priceModel;
    # A list of feature bullet points describing this plan.
    string[] bullets;
};

# Marketplace Purchase

type MarketplacePurchase record {
    # A pending change to the user's Marketplace subscription.
    MarketplacePurchaseMarketplacePendingChange? marketplacePendingChange?;
    # The billing email address of the purchasing organization.
    string organizationBillingEmail?;
    # The unique identifier of the Marketplace purchaser account.
    int id;
    # The details of the current Marketplace purchase subscription.
    MarketplacePurchaseMarketplacePurchase marketplacePurchase;
    # The type of account making the Marketplace purchase.
    string 'type;
    # The username of the account making the Marketplace purchase.
    string login;
    # The REST API URL of the purchasing account.
    string url;
    # The email address of the account making the Marketplace purchase.
    string? email?;
};


type MarketplacePurchaseMarketplacePendingChange record {
    # Indicates whether the Marketplace plan is currently installed.
    boolean isInstalled?;
    # The date when the pending plan change takes effect.
    string effectiveDate?;
    # Unique identifier of the pending Marketplace change.
    int id?;
    # The Marketplace listing plan associated with the pending change.
    MarketplaceListingPlan plan?;
    # The number of units for the pending Marketplace plan change.
    int? unitCount?;
};


type MarketplacePurchaseMarketplacePurchase record {
    # Indicates whether the GitHub App is installed for this purchase.
    boolean isInstalled?;
    # The date and time when the free trial period ends.
    string? freeTrialEndsOn?;
    # Indicates whether the account is currently on a free trial.
    boolean onFreeTrial?;
    # The date and time when the purchase was last updated.
    string updatedAt?;
    # The billing cycle frequency for the Marketplace plan.
    string billingCycle?;
    # The Marketplace listing plan associated with this purchase.
    MarketplaceListingPlan plan?;
    # The number of units purchased for the Marketplace plan.
    int? unitCount?;
    # The date and time of the next scheduled billing.
    string? nextBillingDate?;
};


type MembershipsusernameBody record {
    # The role to give the user in the organization. Can be one of:  
* admin - The user will become an owner of the organization.  
* member - The user will become a non-owner member of the organization
        "admin"|"member"  role = "member";
};


type MembershipsusernameBody1 record {
    # The role that this user should have in the team
        "member"|"maintainer"  role = "member";
};

# Merged branch response message

type MergedBranchResponse record {
    # Response message indicating the result of the branch merge.
    string message?;
};

# Results of a successful merge upstream request

type MergedUpstream record {
    # The name of the base branch that was synced upstream.
    string baseBranch?;
    # The type of merge operation performed during the upstream sync.
        "merge"|"fast-forward"|"none"  mergeType?;
    # A message describing the result of the upstream merge.
    string message?;
};

# User-defined metadata to store domain-specific information limited to 8 keys with scalar values

type Metadata record {
};

# Represents the Queries record for the operation: meta/get-octocat

type MetaGetOctocatQueries record {
    # The words to show in Octocat's speech bubble
    string s?;
};

# A migration

type Migration record {
    # The user or organization that owns the migration.
    NullableSimpleUser? owner;
    # The URL to download the migration archive.
    string archiveUrl?;
    # Whether Git data is excluded from the migration.
    boolean excludeGitData;
    # The timestamp when the migration was created.
    string createdAt;
    # Whether releases are excluded from the migration.
    boolean excludeReleases;
    # Whether owner projects are excluded from the migration.
    boolean excludeOwnerProjects;
    # Whether repositories are locked during the migration.
    boolean lockRepositories;
    # The API URL for this migration.
    string url;
    # Whether metadata is excluded from the migration.
    boolean excludeMetadata;
    # The timestamp when the migration was last updated.
    string updatedAt;
    # The repositories included in the migration. Only returned for export migrations
    Repository[] repositories;
    # The unique GUID identifier for the migration.
    string guid;
    # Whether attachments are excluded from the migration.
    boolean excludeAttachments;
    # Whether only organization metadata is included in the migration.
    boolean orgMetadataOnly;
    # Exclude related items from being returned in the response in order to improve performance of the request. The array can include any of: "repositories"
    string[] exclude?;
    # The unique identifier of the migration.
    int id;
    # The current state of the migration.
    string state;
    # The GraphQL node identifier of the migration.
    string nodeId;
};

# Represents the Queries record for the operation: migrations/get-commit-authors

type MigrationsGetCommitAuthorsQueries record {
    # A user ID. Only return users with an ID greater than this ID
    int since?;
};

# Represents the Queries record for the operation: migrations/get-status-for-authenticated-user

type MigrationsGetStatusForAuthenticatedUserQueries record {
    # Exclude attributes from the API response to improve performance.
    string[] exclude?;
};

# Represents the Queries record for the operation: migrations/get-status-for-org

type MigrationsGetStatusForOrgQueries record {
    # Exclude attributes from the API response to improve performance
    ("repositories")[] exclude?;
};

# Represents the Queries record for the operation: migrations/list-for-authenticated-user

type MigrationsListForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: migrations/list-for-org

type MigrationsListForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Exclude attributes from the API response to improve performance
    ("repositories")[] exclude?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: migrations/list-repos-for-authenticated-user

type MigrationsListReposForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: migrations/list-repos-for-org

type MigrationsListReposForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# A collection of related issues and pull requests

type Milestone record {
    # The user who created the milestone.
    NullableSimpleUser? creator;
    # The date the milestone was closed
    string? closedAt;
    # A description of the milestone
    string? description;
    # The date the milestone was created
    string createdAt;
    # The title of the milestone
    string title;
    # The number of closed issues in the milestone
    int closedIssues;
    # API URL for the milestone
    string url;
    # The due date for the milestone
    string? dueOn;
    # API URL for issues with this milestone
    string labelsUrl;
    # The number of the milestone
    int number;
    # The date the milestone was last updated
    string updatedAt;
    # URL of the milestone page on GitHub
    string htmlUrl;
    # The unique identifier of the milestone
    int id;
    # The state of the milestone
        "open"|"closed"  state = "open";
    # The number of open issues in the milestone
    int openIssues;
    # The GraphQL node identifier of the milestone
    string nodeId;
};

# Milestoned Issue Event

type MilestonedIssueEvent record {
    # The user who triggered the milestoned event.
    SimpleUser actor;
    # The API URL of the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp;
    # The milestone added to the issue.
    MilestonedIssueEventMilestone milestone;
    # Timestamp when the event was created.
    string createdAt;
    # The unique numeric identifier for the event.
    int id;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # The API URL for this event.
    string url;
    # The GraphQL node ID for the event.
    string nodeId;
};


type MilestonedIssueEventMilestone record {
    # The title of the milestone associated with the issue event.
    string title;
};


type MilestonesmilestoneNumberBody record {
    # A description of the milestone
    string description?;
    # The state of the milestone. Either open or closed
        "open"|"closed"  state = "open";
    # The title of the milestone
    string title?;
    # The milestone due date. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string dueOn?;
};

# Minimal Repository

type MinimalRepository record {
    # Whether forking is allowed on the repository
    boolean allowForking?;
    # The number of stars on the repository
    int stargazersCount?;
    # Whether the repository is a template repository
    boolean isTemplate?;
    # The date of the most recent push to the repository
    string? pushedAt?;
    # API URL for the authenticated user's subscription to the repository
    string subscriptionUrl;
    # The primary programming language of the repository
    string? language?;
    # API URL template for listing repository branches
    string branchesUrl;
    # API URL template for accessing issue comments
    string issueCommentUrl;
    # API URL template for listing repository labels
    string labelsUrl;
    # API URL for listing repository watchers
    string subscribersUrl;
    # The permissions the authenticated user has on this repository.
    MinimalRepositoryPermissions permissions?;
    # A temporary token for cloning the repository
    string tempCloneToken?;
    # API URL template for listing repository releases
    string releasesUrl;
    # The Subversion URL for the repository
    string svnUrl?;
    # The number of users watching the repository
    int subscribersCount?;
    # The unique identifier of the repository
    int id;
    # Whether the repository has discussions enabled
    boolean hasDiscussions?;
    # The number of forks of the repository
    int forks?;
    # API URL template for downloading repository archives
    string archiveUrl;
    # API URL template for accessing Git references
    string gitRefsUrl;
    # API URL for listing repository forks
    string forksUrl;
    # The visibility of the repository
    string visibility?;
    # API URL template for listing commit statuses
    string statusesUrl;
    # The number of repositories in the fork network
    int networkCount?;
    # The SSH URL for cloning the repository
    string sshUrl?;
    # The role name assigned to the user for this repository
    string roleName?;
    # The license applied to the repository.
    MinimalRepositoryLicense? license?;
    # The full name of the repository in owner/name format
    string fullName;
    # The size of the repository. Size is calculated hourly. When a repository is initially created, the size is 0
    int size?;
    # API URL for listing programming languages used
    string languagesUrl;
    # URL of the repository page on GitHub
    string htmlUrl;
    # API URL template for listing repository collaborators
    string collaboratorsUrl;
    # The HTTPS URL for cloning the repository
    string cloneUrl?;
    # The name of the repository
    string name;
    # API URL template for listing pull requests
    string pullsUrl;
    # The default branch of the repository
    string defaultBranch?;
    # API URL for listing repository webhooks
    string hooksUrl;
    # API URL template for accessing Git trees
    string treesUrl;
    # API URL for listing repository tags
    string tagsUrl;
    # Whether the repository is private
    boolean 'private;
    # API URL for listing repository contributors
    string contributorsUrl;
    # Whether the repository has downloads enabled
    boolean hasDownloads?;
    # API URL template for listing repository notifications
    string notificationsUrl;
    # The number of open issues in the repository
    int openIssuesCount?;
    # A short description of the repository
    string? description;
    # The date the repository was created
    string? createdAt?;
    # The number of watchers on the repository
    int watchers?;
    # API URL for listing repository deployments
    string deploymentsUrl;
    # API URL template for listing repository deploy keys
    string keysUrl;
    # Whether the repository has projects enabled
    boolean hasProjects?;
    # Whether the repository is archived
    boolean archived?;
    # Whether the repository has the wiki enabled
    boolean hasWiki?;
    # The date the repository was last updated
    string? updatedAt?;
    # API URL template for listing commit comments
    string commentsUrl;
    # API URL for listing users who starred the repository
    string stargazersUrl;
    # Whether the repository is disabled
    boolean disabled?;
    # Whether to delete branches on merge
    boolean deleteBranchOnMerge?;
    # The Git protocol URL for the repository
    string gitUrl?;
    # Whether the repository has GitHub Pages enabled
    boolean hasPages?;
    # The account that owns the repository.
    SimpleUser owner;
    # API URL template for listing repository commits
    string commitsUrl;
    # API URL template for comparing two commits
    string compareUrl;
    # API URL template for accessing Git commits
    string gitCommitsUrl;
    # The list of topics associated with the repository
    string[] topics?;
    # API URL template for accessing repository blobs
    string blobsUrl;
    # API URL template for accessing Git tags
    string gitTagsUrl;
    # API URL for performing merge operations
    string mergesUrl;
    # API URL for listing repository downloads
    string downloadsUrl;
    # Whether the repository has issues enabled
    boolean hasIssues?;
    # The code of conduct for the repository
    CodeOfConduct codeOfConduct?;
    # Whether commit sign-off is required for web-based commits
    boolean webCommitSignoffRequired?;
    # API URL for the repository
    string url;
    # API URL template for accessing repository contents
    string contentsUrl;
    # The URL of the mirror for the repository
    string? mirrorUrl?;
    # API URL template for listing repository milestones
    string milestonesUrl;
    # API URL for listing teams with access to the repository
    string teamsUrl;
    # The security and analysis settings for the repository
    SecurityAndAnalysis? securityAndAnalysis?;
    # Whether the repository is a fork
    boolean 'fork;
    # API URL template for listing repository issues
    string issuesUrl;
    # API URL for listing repository events
    string eventsUrl;
    # API URL template for listing issue events
    string issueEventsUrl;
    # API URL template for listing repository assignees
    string assigneesUrl;
    # The number of open issues in the repository
    int openIssues?;
    # The number of watchers on the repository
    int watchersCount?;
    # The GraphQL node identifier of the repository
    string nodeId;
    # The URL of the repository's homepage
    string? homepage?;
    # The number of forks of the repository
    int forksCount?;
};

# The license information for the repository

type MinimalRepositoryLicense record {
    # The full name of the repository's license.
    string name?;
    # The SPDX identifier for the license.
    string spdxId?;
    # The unique key identifier for the license.
    string 'key?;
    # The API URL for the license details.
    string? url?;
    # The GraphQL node ID of the license.
    string nodeId?;
};

# The permissions the authenticated user has on the repository

type MinimalRepositoryPermissions record {
    # Indicates whether the user has pull (read) permission.
    boolean pull?;
    # Indicates whether the user has maintain permission.
    boolean maintain?;
    # Indicates whether the user has admin permission.
    boolean admin?;
    # Indicates whether the user has triage permission.
    boolean triage?;
    # Indicates whether the user has push (write) permission.
    boolean push?;
};

# Minimal Repository

type MinimalRepositoryResponse record {
    # The list of minimal repository objects returned.
    MinimalRepository[] repositories;
    # The total number of repositories in the response.
    int totalCount;
};

# Moved Column in Project Issue Event

type MovedColumnInProjectIssueEvent record {
    # The user who triggered the move column event.
    SimpleUser actor;
    # The URL of the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp;
    # The date and time the event was created.
    string createdAt;
    # The unique identifier of the event.
    int id;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # The API URL for the event.
    string url;
    # The project card that was moved to a different column.
    RemovedFromProjectIssueEventProjectCard projectCard?;
    # The GraphQL node identifier of the event.
    string nodeId;
};


type NameRepositoriesBody record {
    # The IDs of the repositories that can access the organization variable
    int[] selectedRepositoryIds;
};


type NotificationRead record {
    # A message describing the result of marking notifications as read.
    string message?;
    # The URL to check the status of the mark-as-read operation.
    string url?;
};


type NotificationsBody record {
    # Whether the notification has been read
    boolean read?;
    # Describes the last point that notifications were checked. Anything updated since this time will not be marked as read. If you omit this parameter, all notifications are marked as read. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ. Default: The current timestamp
    string lastReadAt?;
};

# Thread

type NotificationThread record {
    # The reason the user is receiving the notification.
    string reason;
    # The date and time the notification was last updated.
    string updatedAt;
    # Whether the notification has been read.
    boolean unread;
    # The subject of the notification thread.
    NotificationThreadSubject subject;
    # API URL for managing the thread subscription.
    string subscriptionUrl;
    # The unique identifier of the notification thread.
    string id;
    # The repository associated with the notification thread.
    MinimalRepository repository;
    # The date and time the thread was last marked as read.
    string? lastReadAt;
    # API URL for the notification thread.
    string url;
};


type NotificationThreadSubject record {
    # API URL of the latest comment on the notification subject.
    string latestCommentUrl;
    # The title of the notification subject.
    string title;
    # The type of resource this notification subject refers to.
    string 'type;
    # API URL for the notification subject resource.
    string url;
};

# Code of Conduct Simple

type NullableCodeOfConductSimple record {
    # The HTML URL to view the code of conduct on GitHub.
    string? htmlUrl;
    # The name of the code of conduct.
    string name;
    # The API URL for this code of conduct.
    string url;
    # The unique key identifying this code of conduct.
    string 'key;
};

# A description of the machine powering a codespace

type NullableCodespaceMachine record {
    # How many cores are available to the codespace
    int cpus;
    # The name of the machine
    string name;
    # Whether a prebuild is currently available when creating a codespace for this machine and repository. If a branch was not specified as a ref, the default branch will be assumed. Value will be "null" if prebuilds are not supported or prebuild availability could not be determined. Value will be "none" if no prebuild is available. Latest values "ready" and "in_progress" indicate the prebuild availability status
        "none"|"ready"|"in_progress"? prebuildAvailability;
    # The operating system of the machine
    string operatingSystem;
    # The display name of the machine includes cores, memory, and storage
    string displayName;
    # How much memory is available to the codespace
    int memoryInBytes;
    # How much storage is available to the codespace
    int storageInBytes;
};

# Collaborator

type NullableCollaborator record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # API URL template to check who the user is following
    string followingUrl;
    # API URL template for repositories the user has starred
    string starredUrl;
    # The username of the user
    string login;
    # API URL to list the user's followers
    string followersUrl;
    # The type of the account
    string 'type;
    # API URL for the user
    string url;
    # The role name assigned to the collaborator
    string roleName;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # API URL for events received by the user
    string receivedEventsUrl;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # The permission levels granted to the collaborator on the repository.
    NullableCollaboratorPermissions permissions?;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The display name of the user
    string? name?;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The unique identifier of the user
    int id;
    # The Gravatar ID of the user
    string? gravatarId;
    # The publicly visible email address of the user
    string? email?;
    # The GraphQL node identifier of the user
    string nodeId;
    # API URL to list the user's organizations
    string organizationsUrl;
};

# The permissions the collaborator has on the repository

type NullableCollaboratorPermissions record {
    # Indicates whether the collaborator has pull (read) permission.
    boolean pull;
    # Indicates whether the collaborator has maintain permission.
    boolean maintain?;
    # Indicates whether the collaborator has admin permission.
    boolean admin;
    # Indicates whether the collaborator has triage permission.
    boolean triage?;
    # Indicates whether the collaborator has push (write) permission.
    boolean push;
};


type NullableCommunityHealthFile record {
    # The HTML URL of the community health file on GitHub.
    string htmlUrl;
    # The API URL of the community health file.
    string url;
};

# Metaproperties for Git author/committer information

type NullableGitUser record {
    # The date and time associated with the Git author or committer.
    string date?;
    # The name of the Git author or committer.
    string name?;
    # The email address of the Git author or committer.
    string email?;
};

# GitHub apps are a new way to extend GitHub. They can be installed directly on organizations and user accounts and granted access to specific repositories. They come with granular permissions and built-in webhooks. GitHub apps are first class actors within GitHub

type NullableIntegration record {
    # The user or organization that owns the GitHub app.
    NullableSimpleUser? owner;
    # The number of installations associated with the GitHub app
    int installationsCount?;
    # Short description of the GitHub app.
    string? description;
    # Timestamp when the GitHub app was created.
    string createdAt;
    # The OAuth client ID of the GitHub app.
    string clientId?;
    # External URL for the GitHub app's homepage.
    string externalUrl;
    # Timestamp when the GitHub app was last updated.
    string updatedAt;
    # The set of permissions granted to the GitHub app.
    IntegrationPermissions permissions;
    # URL to the GitHub app's web page on GitHub.
    string htmlUrl;
    # The name of the GitHub app
    string name;
    # The secret used to secure webhook payloads from the GitHub app.
    string? webhookSecret?;
    # The private key PEM used to sign access tokens for the GitHub app.
    string pem?;
    # Unique identifier of the GitHub app
    int id;
    # The OAuth client secret of the GitHub app.
    string clientSecret?;
    # The slug name of the GitHub app
    string slug?;
    # The list of events for the GitHub app
    string[] events;
    # Global Node ID of the GitHub app.
    string nodeId;
};

# Issues are a great way to keep track of tasks, enhancements, and bugs for your projects

type NullableIssue record {
    # The HTML-rendered body of the issue
    string bodyHtml?;
    # The plain text body of the issue
    string bodyText?;
    # The users assigned to the issue
    SimpleUser[]? assignees?;
    # The date the issue was created
    string createdAt;
    # Title of the issue
    string title;
    # Contents of the issue
    string? body?;
    # The repository in which the issue exists.
    Repository repository?;
    # The user who closed the issue
    NullableSimpleUser? closedBy?;
    # API URL template for the issue's labels
    string labelsUrl;
    # The association of the author with the repository
    AuthorAssociation authorAssociation;
    # Number uniquely identifying the issue within its repository
    int number;
    # The date the issue was last updated
    string updatedAt;
    # The GitHub App that triggered the event
    NullableIntegration? performedViaGithubApp?;
    # Whether the issue is a draft
    boolean draft?;
    # API URL for the issue's comments
    string commentsUrl;
    # The reason the issue conversation was locked
    string? activeLockReason?;
    # The unique identifier of the issue
    int id;
    # API URL for the repository containing the issue
    string repositoryUrl;
    # State of the issue; either 'open' or 'closed'
    string state;
    # Whether the issue conversation is locked
    boolean locked;
    # API URL for the issue's timeline events
    string timelineUrl?;
    # The reason for the current state
        "completed"|"reopened"|"not_planned"? stateReason?;
    # Pull request metadata if the issue is linked to a pull request.
    IssuePullRequest pullRequest?;
    # The number of comments on the issue
    int comments;
    # The date the issue was closed
    string? closedAt;
    # URL for the issue
    string url;
    # Labels to associate with this issue; pass one or more label names to replace the set of labels on this issue; send an empty array to clear all labels from the issue; note that the labels are silently dropped for users without push access to the repository
    NullableIssueLabels[] labels;
    # The milestone associated with the issue.
    NullableMilestone? milestone;
    # API URL for the issue's events
    string eventsUrl;
    # URL of the issue page on GitHub
    string htmlUrl;
    # Reaction summary counts for the issue.
    ReactionRollup reactions?;
    # The primary user assigned to the issue.
    NullableSimpleUser? assignee;
    # The user who created the issue.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the issue
    string nodeId;
};

# License Simple

type NullableLicenseSimple record {
    # The URL to the human-readable license page.
    string htmlUrl?;
    # The full name of the license.
    string name;
    # The SPDX identifier for the license.
    string? spdxId;
    # The unique key identifier for the license.
    string 'key;
    # The API URL for the license resource.
    string? url;
    # The GraphQL node ID for the license.
    string nodeId;
};

# A collection of related issues and pull requests

type NullableMilestone record {
    # The user who created the milestone.
    NullableSimpleUser? creator;
    # The date the milestone was closed
    string? closedAt;
    # A description of the milestone
    string? description;
    # The date the milestone was created
    string createdAt;
    # The title of the milestone
    string title;
    # The number of closed issues in the milestone
    int closedIssues;
    # API URL for the milestone
    string url;
    # The due date for the milestone
    string? dueOn;
    # API URL for issues with this milestone
    string labelsUrl;
    # The number of the milestone
    int number;
    # The date the milestone was last updated
    string updatedAt;
    # URL of the milestone page on GitHub
    string htmlUrl;
    # The unique identifier of the milestone
    int id;
    # The state of the milestone
        "open"|"closed"  state = "open";
    # The number of open issues in the milestone
    int openIssues;
    # The GraphQL node identifier of the milestone
    string nodeId;
};

# Minimal Repository

type NullableMinimalRepository record {
    # Whether forking is allowed on the repository
    boolean allowForking?;
    # The number of stars on the repository
    int stargazersCount?;
    # Whether the repository is a template repository
    boolean isTemplate?;
    # The date of the most recent push to the repository
    string? pushedAt?;
    # API URL for the authenticated user's subscription to the repository
    string subscriptionUrl;
    # The primary programming language of the repository
    string? language?;
    # API URL template for listing repository branches
    string branchesUrl;
    # API URL template for accessing issue comments
    string issueCommentUrl;
    # API URL template for listing repository labels
    string labelsUrl;
    # API URL for listing repository watchers
    string subscribersUrl;
    # The permissions the authenticated user has on the repository.
    MinimalRepositoryPermissions permissions?;
    # A temporary token for cloning the repository
    string tempCloneToken?;
    # API URL template for listing repository releases
    string releasesUrl;
    # The Subversion URL for the repository
    string svnUrl?;
    # The number of users watching the repository
    int subscribersCount?;
    # The unique identifier of the repository
    int id;
    # Whether the repository has discussions enabled
    boolean hasDiscussions?;
    # The number of forks of the repository
    int forks?;
    # API URL template for downloading repository archives
    string archiveUrl;
    # API URL template for accessing Git references
    string gitRefsUrl;
    # API URL for listing repository forks
    string forksUrl;
    # The visibility of the repository
    string visibility?;
    # API URL template for listing commit statuses
    string statusesUrl;
    # The number of repositories in the fork network
    int networkCount?;
    # The SSH URL for cloning the repository
    string sshUrl?;
    # The role name assigned to the user for this repository
    string roleName?;
    # The license associated with the repository.
    NullableMinimalRepositoryLicense? license?;
    # The full name of the repository in owner/name format
    string fullName;
    # The size of the repository. Size is calculated hourly. When a repository is initially created, the size is 0
    int size?;
    # API URL for listing programming languages used
    string languagesUrl;
    # URL of the repository page on GitHub
    string htmlUrl;
    # API URL template for listing repository collaborators
    string collaboratorsUrl;
    # The HTTPS URL for cloning the repository
    string cloneUrl?;
    # The name of the repository
    string name;
    # API URL template for listing pull requests
    string pullsUrl;
    # The default branch of the repository
    string defaultBranch?;
    # API URL for listing repository webhooks
    string hooksUrl;
    # API URL template for accessing Git trees
    string treesUrl;
    # API URL for listing repository tags
    string tagsUrl;
    # Whether the repository is private
    boolean 'private;
    # API URL for listing repository contributors
    string contributorsUrl;
    # Whether the repository has downloads enabled
    boolean hasDownloads?;
    # API URL template for listing repository notifications
    string notificationsUrl;
    # The number of open issues in the repository
    int openIssuesCount?;
    # A short description of the repository
    string? description;
    # The date the repository was created
    string? createdAt?;
    # The number of watchers on the repository
    int watchers?;
    # API URL for listing repository deployments
    string deploymentsUrl;
    # API URL template for listing repository deploy keys
    string keysUrl;
    # Whether the repository has projects enabled
    boolean hasProjects?;
    # Whether the repository is archived
    boolean archived?;
    # Whether the repository has the wiki enabled
    boolean hasWiki?;
    # The date the repository was last updated
    string? updatedAt?;
    # API URL template for listing commit comments
    string commentsUrl;
    # API URL for listing users who starred the repository
    string stargazersUrl;
    # Whether the repository is disabled
    boolean disabled?;
    # Whether to delete branches on merge
    boolean deleteBranchOnMerge?;
    # The Git protocol URL for the repository
    string gitUrl?;
    # Whether the repository has GitHub Pages enabled
    boolean hasPages?;
    # The user or organization that owns the repository.
    SimpleUser owner;
    # API URL template for listing repository commits
    string commitsUrl;
    # API URL template for comparing two commits
    string compareUrl;
    # API URL template for accessing Git commits
    string gitCommitsUrl;
    # The list of topics associated with the repository
    string[] topics?;
    # API URL template for accessing repository blobs
    string blobsUrl;
    # API URL template for accessing Git tags
    string gitTagsUrl;
    # API URL for performing merge operations
    string mergesUrl;
    # API URL for listing repository downloads
    string downloadsUrl;
    # Whether the repository has issues enabled
    boolean hasIssues?;
    # The code of conduct for the repository
    CodeOfConduct codeOfConduct?;
    # Whether commit sign-off is required for web-based commits
    boolean webCommitSignoffRequired?;
    # API URL for the repository
    string url;
    # API URL template for accessing repository contents
    string contentsUrl;
    # The URL of the mirror for the repository
    string? mirrorUrl?;
    # API URL template for listing repository milestones
    string milestonesUrl;
    # API URL for listing teams with access to the repository
    string teamsUrl;
    # The security and analysis settings for the repository
    SecurityAndAnalysis? securityAndAnalysis?;
    # Whether the repository is a fork
    boolean 'fork;
    # API URL template for listing repository issues
    string issuesUrl;
    # API URL for listing repository events
    string eventsUrl;
    # API URL template for listing issue events
    string issueEventsUrl;
    # API URL template for listing repository assignees
    string assigneesUrl;
    # The number of open issues in the repository
    int openIssues?;
    # The number of watchers on the repository
    int watchersCount?;
    # The GraphQL node identifier of the repository
    string nodeId;
    # The URL of the repository's homepage
    string? homepage?;
    # The number of forks of the repository
    int forksCount?;
};

# The license information for the repository

type NullableMinimalRepositoryLicense record {
    # The full name of the license.
    string name?;
    # The SPDX identifier of the license.
    string spdxId?;
    # The lowercase license key identifier.
    string 'key?;
    # API URL for the license.
    string url?;
    # The GraphQL node identifier of the license.
    string nodeId?;
};

# A repository on GitHub

type NullableRepository record {
    # Whether to allow forking this repo
    boolean allowForking?;
    # Whether anonymous git access is enabled for this repository
    boolean anonymousAccessEnabled?;
    # API URL for managing the repository subscription.
    string subscriptionUrl;
    # API URL template for the repository's branches.
    string branchesUrl;
    # API URL template for the repository's issue comments.
    string issueCommentUrl;
    # Whether to allow rebase merges for pull requests
    boolean allowRebaseMerge = true;
    # The permissions the authenticated user has on this repository.
    TeamRepositoryPermissions permissions?;
    # API URL for the repository's subscribers list.
    string subscribersUrl;
    # Temporary token used for cloning the repository.
    string tempCloneToken?;
    # API URL template for the repository's releases.
    string releasesUrl;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # Number of users watching the repository.
    int subscribersCount?;
    # Unique identifier of the repository
    int id;
    # Whether discussions are enabled
    boolean hasDiscussions = false;
    # Number of forks of the repository.
    int forks;
    # API URL template for the repository's git refs.
    string gitRefsUrl;
    # SSH URL used to clone the repository.
    string sshUrl;
    # Full repository name including owner, e.g., 'owner/repo'.
    string fullName;
    # The size of the repository. Size is calculated hourly. When a repository is initially created, the size is 0
    int size;
    # The template repository used to create this repository.
    NullableRepositoryTemplateRepository? templateRepository?;
    # API URL for the repository's language breakdown.
    string languagesUrl;
    # URL to the repository's GitHub web page.
    string htmlUrl;
    # API URL template for the repository's collaborators.
    string collaboratorsUrl;
    # HTTPS URL used to clone the repository.
    string cloneUrl;
    # The default branch of the repository
    string defaultBranch;
    # API URL for the repository's webhooks.
    string hooksUrl;
    # API URL template for the repository's git trees.
    string treesUrl;
    # Whether downloads are enabled
    boolean hasDownloads = true;
    # Timestamp when the repository was created.
    string? createdAt;
    # Number of users watching the repository.
    int watchers;
    # API URL for the repository's deployments.
    string deploymentsUrl;
    # API URL template for the repository's deploy keys.
    string keysUrl;
    # Whether the repository is archived
    boolean archived = false;
    # Whether the wiki is enabled
    boolean hasWiki = true;
    # Timestamp when the repository was last updated.
    string? updatedAt;
    # Returns whether or not this repository disabled
    boolean disabled;
    # API URL template for comparing two refs in the repository.
    string compareUrl;
    # API URL template for the repository's git commits.
    string gitCommitsUrl;
    # List of topics associated with the repository.
    string[] topics?;
    # Whether or not a pull request head branch that is behind its base branch can always be updated even if it is not required to be up to date before merging
    boolean allowUpdateBranch = false;
    # API URL template for the repository's git tags.
    string gitTagsUrl;
    # API URL for the repository's branch merges.
    string mergesUrl;
    # Timestamp when the repository was starred by the authenticated user.
    string starredAt?;
    # API URL for the repository.
    string url;
    # API URL template for the repository's file contents.
    string contentsUrl;
    # API URL template for the repository's issues.
    string issuesUrl;
    # Whether a squash merge commit can use the pull request title as default. **This property has been deprecated. Please use squash_merge_commit_title instead
    boolean useSquashPrTitleAsDefault = false;
    # The organization that owns the repository, if applicable.
    NullableSimpleUser? organization?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # API URL template for the repository's assignees.
    string assigneesUrl;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # Number of open issues and pull requests in the repository.
    int openIssues;
    # Global Node ID of the repository.
    string nodeId;
    # Number of users who have starred the repository.
    int stargazersCount;
    # Whether this repository acts as a template that can be used to generate new repositories
    boolean isTemplate = false;
    # Timestamp of the most recent push to the repository.
    string? pushedAt;
    # Primary programming language used in the repository.
    string? language;
    # API URL template for the repository's labels.
    string labelsUrl;
    # SVN URL used to access the repository.
    string svnUrl;
    # The name of the repository's master branch.
    string masterBranch?;
    # API URL template for downloading an archive of the repository.
    string archiveUrl;
    # Whether to allow merge commits for pull requests
    boolean allowMergeCommit = true;
    # API URL for the repository's forks.
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility = "public";
    # API URL template for commit statuses in the repository.
    string statusesUrl;
    # Number of repositories in the repository's network.
    int networkCount?;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # Whether to allow Auto-merge to be used on pull requests
    boolean allowAutoMerge = false;
    # The name of the repository
    string name;
    # API URL template for the repository's pull requests.
    string pullsUrl;
    # API URL for the repository's tags.
    string tagsUrl;
    # Whether the repository is private or public
    boolean 'private = false;
    # API URL for the repository's contributors.
    string contributorsUrl;
    # API URL template for the repository's notifications.
    string notificationsUrl;
    # Number of open issues in the repository.
    int openIssuesCount;
    # Short description of the repository.
    string? description;
    # Whether projects are enabled
    boolean hasProjects = true;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # API URL template for the repository's comments.
    string commentsUrl;
    # API URL for users who have starred the repository.
    string stargazersUrl;
    # Whether to delete head branches when pull requests are merged
    boolean deleteBranchOnMerge = false;
    # Git URL used to access the repository.
    string gitUrl;
    # Whether GitHub Pages is enabled for the repository.
    boolean hasPages;
    # The user or organization that owns the repository.
    SimpleUser owner;
    # Whether to allow squash merges for pull requests
    boolean allowSquashMerge = true;
    # API URL template for the repository's commits.
    string commitsUrl;
    # API URL template for the repository's git blobs.
    string blobsUrl;
    # API URL for the repository's downloads.
    string downloadsUrl;
    # Whether issues are enabled
    boolean hasIssues = true;
    # Whether to require contributors to sign off on web-based commits
    boolean webCommitSignoffRequired = false;
    # URL of the repository's mirror, if it is a mirror.
    string? mirrorUrl;
    # API URL template for the repository's milestones.
    string milestonesUrl;
    # API URL for the repository's teams.
    string teamsUrl;
    # Whether the repository is a fork of another repository.
    boolean 'fork;
    # API URL for the repository's events.
    string eventsUrl;
    # API URL template for the repository's issue events.
    string issueEventsUrl;
    # Number of users watching the repository.
    int watchersCount;
    # URL of the repository's homepage or website.
    string? homepage;
    # Number of forks of the repository.
    int forksCount;
};


type NullableRepositoryTemplateRepository record {
    # The number of stars on the repository.
    int stargazersCount?;
    # Whether this repository acts as a template for generating new repositories.
    boolean isTemplate?;
    # The date of the most recent push to the repository.
    string pushedAt?;
    # API URL for the authenticated user's subscription to the repository.
    string subscriptionUrl?;
    # The primary programming language of the repository.
    string language?;
    # API URL template for listing repository branches.
    string branchesUrl?;
    # API URL template for accessing issue comments.
    string issueCommentUrl?;
    # Whether to allow rebase merges for pull requests.
    boolean allowRebaseMerge?;
    # API URL template for listing repository labels.
    string labelsUrl?;
    # API URL for listing repository watchers.
    string subscribersUrl?;
    # The permissions the authenticated user has on this repository.
    RepositoryTemplateRepositoryPermissions permissions?;
    # A temporary token for cloning the repository.
    string tempCloneToken?;
    # API URL template for listing repository releases.
    string releasesUrl?;
    # The Subversion URL for the repository.
    string svnUrl?;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # The number of users watching the repository.
    int subscribersCount?;
    # The unique identifier of the repository.
    int id?;
    # API URL template for downloading repository archives.
    string archiveUrl?;
    # Whether to allow merge commits for pull requests.
    boolean allowMergeCommit?;
    # API URL template for accessing Git references.
    string gitRefsUrl?;
    # API URL for listing repository forks.
    string forksUrl?;
    # The repository visibility: public, private, or internal.
    string visibility?;
    # API URL template for listing commit statuses.
    string statusesUrl?;
    # The number of repositories in the fork network.
    int networkCount?;
    # The SSH URL for cloning the repository.
    string sshUrl?;
    # The full name of the repository in owner/name format.
    string fullName?;
    # The size of the repository in kilobytes.
    int size?;
    # Whether to allow auto-merge on pull requests.
    boolean allowAutoMerge?;
    # API URL for listing programming languages used.
    string languagesUrl?;
    # URL of the repository page on GitHub.
    string htmlUrl?;
    # API URL template for listing repository collaborators.
    string collaboratorsUrl?;
    # The HTTPS URL for cloning the repository.
    string cloneUrl?;
    # The name of the repository.
    string name?;
    # API URL template for listing pull requests.
    string pullsUrl?;
    # The default branch of the repository.
    string defaultBranch?;
    # API URL for listing repository webhooks.
    string hooksUrl?;
    # API URL template for accessing Git trees.
    string treesUrl?;
    # API URL for listing repository tags.
    string tagsUrl?;
    # Whether the repository is private or public.
    boolean 'private?;
    # API URL for listing repository contributors.
    string contributorsUrl?;
    # Whether downloads are enabled.
    boolean hasDownloads?;
    # API URL template for listing repository notifications.
    string notificationsUrl?;
    # The number of open issues in the repository.
    int openIssuesCount?;
    # A short description of the repository.
    string description?;
    # The date the repository was created.
    string createdAt?;
    # API URL for listing repository deployments.
    string deploymentsUrl?;
    # API URL template for listing repository deploy keys.
    string keysUrl?;
    # Whether projects are enabled.
    boolean hasProjects?;
    # Whether the repository is archived.
    boolean archived?;
    # Whether the wiki is enabled.
    boolean hasWiki?;
    # The date the repository was last updated.
    string updatedAt?;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # API URL template for listing commit comments.
    string commentsUrl?;
    # API URL for listing users who starred the repository.
    string stargazersUrl?;
    # Whether the repository is disabled.
    boolean disabled?;
    # Whether to delete head branches when pull requests are merged.
    boolean deleteBranchOnMerge?;
    # The Git protocol URL for the repository.
    string gitUrl?;
    # Whether the repository has GitHub Pages enabled.
    boolean hasPages?;
    # The account that owns the template repository.
    RepositoryTemplateRepositoryOwner owner?;
    # Whether to allow squash merges for pull requests.
    boolean allowSquashMerge?;
    # API URL template for listing repository commits.
    string commitsUrl?;
    # API URL template for comparing two commits.
    string compareUrl?;
    # API URL template for accessing Git commits.
    string gitCommitsUrl?;
    # The list of topics associated with the repository.
    string[] topics?;
    # API URL template for accessing repository blobs.
    string blobsUrl?;
    # Whether pull request head branches can be updated even if not required.
    boolean allowUpdateBranch?;
    # API URL template for accessing Git tags.
    string gitTagsUrl?;
    # API URL for performing merge operations.
    string mergesUrl?;
    # API URL for listing repository downloads.
    string downloadsUrl?;
    # Whether issues are enabled.
    boolean hasIssues?;
    # API URL for the repository.
    string url?;
    # API URL template for accessing repository contents.
    string contentsUrl?;
    # The URL of the mirror for the repository.
    string mirrorUrl?;
    # API URL template for listing repository milestones.
    string milestonesUrl?;
    # API URL for listing teams with access to the repository.
    string teamsUrl?;
    # Whether the repository is a fork.
    boolean 'fork?;
    # API URL template for listing repository issues.
    string issuesUrl?;
    # API URL for listing repository events.
    string eventsUrl?;
    # Whether to use the pull request title as the default squash merge commit title.
    boolean useSquashPrTitleAsDefault?;
    # API URL template for listing issue events.
    string issueEventsUrl?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # API URL template for listing repository assignees.
    string assigneesUrl?;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # The number of watchers on the repository.
    int watchersCount?;
    # The GraphQL node identifier of the repository.
    string nodeId?;
    # The URL of the repository's homepage.
    string homepage?;
    # The number of forks of the repository.
    int forksCount?;
};


type NullableScopedInstallation record {
    # Describe whether all repositories have been selected or there's a selection involved
        "all"|"selected"  repositorySelection;
    # API URL for listing repositories accessible to this installation.
    string repositoriesUrl;
    # The permissions granted to this installation.
    AppPermissions permissions;
    # The path of the single file the installation is restricted to.
    string? singleFileName;
    # Whether the installation has access to multiple single files.
    boolean hasMultipleSingleFiles?;
    # The list of file paths the installation is restricted to.
    string[] singleFilePaths?;
    # The user or organization account associated with this installation.
    SimpleUser account;
};

# A commit

type NullableSimpleCommit record {
    # The user who committed the changes to the repository.
    SimpleCommitCommitter? committer;
    # SHA for the commit's tree
    string treeId;
    # The user who authored the commit.
    SimpleCommitAuthor? author;
    # SHA for the commit
    string id;
    # Message describing the purpose of the commit
    string message;
    # Timestamp of the commit
    string timestamp;
};

# A GitHub user

type NullableSimpleUser record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # API URL template to check who the user is following
    string followingUrl;
    # API URL template for repositories the user has starred
    string starredUrl;
    # The username of the user
    string login;
    # API URL to list the user's followers
    string followersUrl;
    # The type of the account
    string 'type;
    # The time the user starred the resource
    string starredAt?;
    # API URL for the user
    string url;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # API URL for events received by the user
    string receivedEventsUrl;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The display name of the user
    string? name?;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The unique identifier of the user
    int id;
    # The Gravatar ID of the user
    string? gravatarId;
    # The publicly visible email address of the user
    string? email?;
    # The GraphQL node identifier of the user
    string nodeId;
    # API URL to list the user's organizations
    string organizationsUrl;
};

# Groups of organization members that gives permissions on specified repositories

type NullableTeamSimple record {
    # The API URL listing repositories accessible to the team.
    string repositoriesUrl;
    # The API URL template listing members of the team.
    string membersUrl;
    # Description of the team
    string? description;
    # The level of privacy this team should have
    string privacy?;
    # Permission that the team will have for its repositories
    string permission;
    # URL for the team
    string url;
    # The notification setting the team has set
    string notificationSetting?;
    # Distinguished Name (DN) that team maps to within LDAP environment
    string ldapDn?;
    # The GitHub web URL for the team's page.
    string htmlUrl;
    # Name of the team
    string name;
    # Unique identifier of the team
    int id;
    # The URL-friendly identifier for the team.
    string slug;
    # The GraphQL node ID of the team.
    string nodeId;
};

# Actions OIDC Subject customization

type OidcCustomSub record {
    # Array of unique strings. Each claim key can only contain alphanumeric characters and underscores
    string[] includeClaimKeys;
};

# Actions OIDC subject customization for a repository

type OidcCustomSubRepo record {
    # Array of unique strings. Each claim key can only contain alphanumeric characters and underscores
    string[] includeClaimKeys?;
    # Whether to use the default template or not. If true, the include_claim_keys field is ignored
    boolean useDefault;
};

# GitHub account for managing multiple users, teams, and repositories

type Organization record {
    # API URL for listing the organization's repositories
    string reposUrl;
    # Specifies if repository projects are enabled for repositories that belong to this org
    boolean hasRepositoryProjects;
    # API URL template for listing the organization's members
    string membersUrl;
    # A short description of the organization
    string? description;
    # The date the organization was created
    string createdAt;
    # Unique login name of the organization
    string login;
    # Display blog url for the organization
    string blog?;
    # The type of the account
    string 'type;
    # API URL template for listing public members
    string publicMembersUrl;
    # The date the organization was last updated
    string updatedAt;
    # Display company name for the organization
    string company?;
    # The unique identifier of the organization
    int id;
    # The number of public repositories in the organization
    int publicRepos;
    # The subscription plan associated with the organization.
    OrganizationPlan plan?;
    # Display email for the organization
    string email?;
    # Whether the organization's domain is verified
    boolean isVerified?;
    # The number of public gists in the organization
    int publicGists;
    # URL for the organization
    string url;
    # API URL for listing the organization's issues
    string issuesUrl;
    # The number of followers of the organization
    int followers;
    # URL of the organization's avatar image
    string avatarUrl;
    # API URL for listing the organization's events
    string eventsUrl;
    # Specifies if organization projects are enabled for this org
    boolean hasOrganizationProjects;
    # URL of the organization's GitHub profile page
    string htmlUrl;
    # The number of accounts the organization is following
    int following;
    # Display name for the organization
    string name?;
    # Display location for the organization
    string location?;
    # API URL for listing the organization's webhooks
    string hooksUrl;
    # The GraphQL node identifier of the organization
    string nodeId;
};

# Secrets for GitHub Actions for an organization

type OrganizationActionsSecret record {
    # The timestamp indicating when the secret was last updated.
    string updatedAt;
    # Visibility of a secret
        "all"|"private"|"selected"  visibility;
    # The name of the secret
    string name;
    # The URL to list repositories with access to this secret.
    string selectedRepositoriesUrl?;
    # The timestamp indicating when the secret was created.
    string createdAt;
};

# Secrets for GitHub Actions for an organization

type OrganizationActionsSecretResponse record {
    # The total number of Actions secrets available to the organization.
    int totalCount;
    # List of Actions secrets configured for the organization.
    OrganizationActionsSecret[] secrets;
};

# Organization variable for GitHub Actions

type OrganizationActionsVariable record {
    # The date and time at which the variable was last updated, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string updatedAt;
    # Visibility of a variable
        "all"|"private"|"selected"  visibility;
    # The name of the variable
    string name;
    # API URL to list repositories that can access this variable.
    string selectedRepositoriesUrl?;
    # The date and time at which the variable was created, in ISO 8601 format':' YYYY-MM-DDTHH:MM:SSZ
    string createdAt;
    # The value of the variable
    string value;
};

# Organization variable for GitHub Actions

type OrganizationActionsVariableResponse record {
    # The list of organization-level Actions variables.
    OrganizationActionsVariable[] variables;
    # The total number of organization Actions variables.
    int totalCount;
};

# Secrets for GitHub Dependabot for an organization

type OrganizationDependabotSecret record {
    # The date and time the secret was last updated.
    string updatedAt;
    # Visibility of a secret
        "all"|"private"|"selected"  visibility;
    # The name of the secret
    string name;
    # The API URL to list repositories with access to this secret.
    string selectedRepositoriesUrl?;
    # The date and time the secret was created.
    string createdAt;
};

# Secrets for GitHub Dependabot for an organization

type OrganizationDependabotSecretResponse record {
    # The total number of Dependabot secrets in the organization.
    int totalCount;
    # The list of Dependabot secrets for the organization.
    OrganizationDependabotSecret[] secrets;
};

# Organization Full

type OrganizationFull record {
    # API URL for listing the organization's repositories
    string reposUrl;
    # Whether members can create internal repositories
    boolean membersCanCreateInternalRepositories?;
    # An optional URL string to display to contributors who are blocked from pushing a secret
    string? secretScanningPushProtectionCustomLink?;
    # Whether members can create public GitHub Pages sites
    boolean membersCanCreatePublicPages?;
    # The URL of the organization's blog or website
    string blog?;
    # The type of the account
    string 'type;
    # API URL template for listing public members
    string publicMembersUrl;
    # The number of private gists
    int? privateGists?;
    # The default permission level for organization repositories
    string? defaultRepositoryPermission?;
    # The billing email address for the organization
    string? billingEmail?;
    # The total disk usage in kilobytes
    int? diskUsage?;
    # The number of collaborators on private repositories.

This field may be null if the number of private repositories is over 50,000
    int? collaborators?;
    # The unique identifier of the organization
    int id;
    # Whether secret scanning push protection is automatically enabled for new repositories and repositories
transferred to this organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean secretScanningPushProtectionEnabledForNewRepositories?;
    # The subscription plan associated with the organization.
    TeamOrganizationPlan plan?;
    # Whether members can create private GitHub Pages sites
    boolean membersCanCreatePrivatePages?;
    # Whether members can create repositories
    boolean? membersCanCreateRepositories?;
    # Whether members can create private repositories
    boolean membersCanCreatePrivateRepositories?;
    # The number of public gists in the organization
    int publicGists;
    # The number of followers of the organization
    int followers;
    # Whether the organization has projects enabled
    boolean hasOrganizationProjects;
    # The number of accounts the organization is following
    int following;
    # URL of the organization's GitHub profile page
    string htmlUrl;
    # The display name of the organization
    string name?;
    # API URL for listing the organization's webhooks
    string hooksUrl;
    # Whether dependabot security updates are automatically enabled for new repositories and repositories transferred
to this organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean dependabotSecurityUpdatesEnabledForNewRepositories?;
    # Whether GitHub Advanced Security is automatically enabled for new repositories and repositories transferred to
this organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean dependabotAlertsEnabledForNewRepositories?;
    # Whether repositories can have projects enabled
    boolean hasRepositoryProjects;
    # API URL template for listing the organization's members
    string membersUrl;
    # The Twitter username of the organization
    string? twitterUsername?;
    # A short description of the organization
    string? description;
    # Whether GitHub Advanced Security is enabled for new repositories and repositories transferred to this organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean advancedSecurityEnabledForNewRepositories?;
    # The date the organization was created
    string createdAt;
    # The login name of the organization
    string login;
    # Whether dependency graph is automatically enabled for new repositories and repositories transferred to this
organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean dependencyGraphEnabledForNewRepositories?;
    # The total number of private repositories
    int totalPrivateRepos?;
    # Whether secret scanning is automatically enabled for new repositories and repositories transferred to this
organization.

This field is only visible to organization owners or members of a team with the security manager role
    boolean secretScanningEnabledForNewRepositories?;
    # The date the organization was last updated
    string updatedAt;
    # The types of repositories members can create
    string membersAllowedRepositoryCreationType?;
    # Whether members can fork private repositories
    boolean? membersCanForkPrivateRepositories?;
    # The company name of the organization
    string? company?;
    # The number of owned private repositories
    int ownedPrivateRepos?;
    # The number of public repositories in the organization
    int publicRepos;
    # The publicly visible email of the organization
    string? email?;
    # Whether two-factor authentication is required for members
    boolean? twoFactorRequirementEnabled?;
    # The date the organization was archived
    string? archivedAt;
    # Whether the organization's domain is verified
    boolean isVerified?;
    # Whether commit sign-off is required for web-based commits
    boolean webCommitSignoffRequired?;
    # API URL for the organization
    string url;
    # Whether members can create public repositories
    boolean membersCanCreatePublicRepositories?;
    # API URL for listing the organization's issues
    string issuesUrl;
    # URL of the organization's avatar image
    string avatarUrl;
    # API URL for listing the organization's events
    string eventsUrl;
    # Whether members can create GitHub Pages sites
    boolean membersCanCreatePages?;
    # The geographic location of the organization
    string location?;
    # Whether a custom link is shown to contributors who are blocked from pushing a secret by push protection
    boolean secretScanningPushProtectionCustomLinkEnabled?;
    # The GraphQL node identifier of the organization
    string nodeId;
};

# Organization Invitation

type OrganizationInvitation record {
    # The source of the invitation, e.g., member or team.
    string invitationSource?;
    # The role assigned to the invited user in the organization.
    string role;
    # The timestamp when the invitation was created.
    string createdAt;
    # The user who sent the organization invitation.
    SimpleUser inviter;
    # The unique identifier of the organization invitation.
    int id;
    # The timestamp when the invitation failed, if applicable.
    string? failedAt?;
    # The GitHub username of the invited user.
    string? login;
    # The URL listing teams associated with this invitation.
    string invitationTeamsUrl;
    # The email address of the invited user.
    string? email;
    # The reason the invitation failed, if applicable.
    string? failedReason?;
    # The number of teams the invited user is being added to.
    int teamCount;
    # The GraphQL node ID of the organization invitation.
    string nodeId;
};

# The billing plan for the organization

type OrganizationPlan record {
    # The number of private repositories allowed under this plan.
    int privateRepos?;
    # The number of seats currently in use.
    int filledSeats?;
    # The name of the billing plan.
    string name?;
    # The total number of seats available under this plan.
    int seats?;
    # The total storage space allocated under this plan in bytes.
    int space?;
};

# Minimal representation of an organization programmatic access grant for enumerations

type OrganizationProgrammaticAccessGrant record {
    # The user who owns the fine-grained personal access token.
    SimpleUser owner;
    # Type of repository selection requested
        "none"|"all"|"subset"  repositorySelection;
    # URL to the list of repositories the fine-grained personal access token can access. Only follow when repository_selection is subset
    string repositoriesUrl;
    # Date and time when the associated fine-grained personal access token expires
    string? tokenExpiresAt;
    # The permissions granted to the fine-grained personal access token.
    OrganizationProgrammaticAccessGrantPermissions permissions;
    # Date and time when the associated fine-grained personal access token was last used for authentication
    string? tokenLastUsedAt;
    # Unique identifier of the fine-grained personal access token. The pat_id used to get details about an approved fine-grained personal access token
    int id;
    # Whether the associated fine-grained personal access token has expired
    boolean tokenExpired;
    # Date and time when the fine-grained personal access token was approved to access the organization
    string accessGrantedAt;
};

# Permissions requested, categorized by type of permission

type OrganizationProgrammaticAccessGrantPermissions record {
    # Other miscellaneous permissions granted by the access token.
    record {} other?;
    # Organization-level permissions granted by the access token.
    record {} organization?;
    # Repository-level permissions granted by the access token.
    record {} repository?;
};

# Minimal representation of an organization programmatic access grant request for enumerations

type OrganizationProgrammaticAccessGrantRequest record {
    # The GitHub user who owns the fine-grained personal access token.
    SimpleUser owner;
    # Reason for requesting access
    string? reason;
    # Type of repository selection requested
        "none"|"all"|"subset"  repositorySelection;
    # URL to the list of repositories requested to be accessed via fine-grained personal access token. Should only be followed when repository_selection is subset
    string repositoriesUrl;
    # Date and time when the associated fine-grained personal access token expires
    string? tokenExpiresAt;
    # The permissions requested by the fine-grained personal access token.
    OrganizationProgrammaticAccessGrantPermissions permissions;
    # Date and time when the associated fine-grained personal access token was last used for authentication
    string? tokenLastUsedAt;
    # Date and time when the request for access was created
    string createdAt;
    # Unique identifier of the request for access via fine-grained personal access token. The pat_request_id used to review PAT requests
    int id;
    # Whether the associated fine-grained personal access token has expired
    boolean tokenExpired;
};


type OrganizationSecretScanningAlert record {
    # The type of secret that secret scanning detected
    string secretType?;
    # The user who bypassed push protection for this alert.
    NullableSimpleUser? pushProtectionBypassedBy?;
    # The time that push protection was bypassed in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string? pushProtectionBypassedAt?;
    # The time the alert was created in ISO 8601 format.
    AlertCreatedAt createdAt?;
    # User-friendly name for the detected secret, matching the secret_type.
For a list of built-in patterns, see "[Secret scanning patterns](https://docs.github.com/code-security/secret-scanning/secret-scanning-patterns#supported-secrets-for-advanced-security)."
    string secretTypeDisplayName?;
    # The secret that was detected
    string secret?;
    # The repository where the secret was detected.
    SimpleRepository repository?;
    # The resolution status of the secret scanning alert.
    SecretScanningAlertResolution? resolution?;
    # The REST API URL of the secret scanning alert.
    AlertUrl url?;
    # The unique number identifying the alert within the repository.
    AlertNumber number?;
    # The user who resolved the secret scanning alert.
    NullableSimpleUser? resolvedBy?;
    # The time the alert was last updated in ISO 8601 format.
    NullableAlertUpdatedAt? updatedAt?;
    # The REST API URL of the code locations for this alert
    string locationsUrl?;
    # The comment that was optionally added when this alert was closed
    string? resolutionComment?;
    # The time that the alert was resolved in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string? resolvedAt?;
    # The GitHub web URL of the secret scanning alert.
    AlertHtmlUrl htmlUrl?;
    # Whether push protection was bypassed for the detected secret
    boolean? pushProtectionBypassed?;
    # The state of the secret scanning alert.
    SecretScanningAlertState state?;
};

# A GitHub organization

type OrganizationSimple record {
    # API URL to retrieve issues for the organization.
    string issuesUrl;
    # API URL to retrieve repositories for the organization.
    string reposUrl;
    # URL of the organization's avatar image.
    string avatarUrl;
    # API URL to retrieve events for the organization.
    string eventsUrl;
    # API URL template to retrieve members of the organization.
    string membersUrl;
    # A short description of the organization.
    string? description;
    # Unique numeric identifier for the organization.
    int id;
    # API URL to retrieve webhooks for the organization.
    string hooksUrl;
    # The login name of the organization.
    string login;
    # API URL for this organization.
    string url;
    # The GraphQL node identifier for the organization.
    string nodeId;
    # API URL template to retrieve public members of the organization.
    string publicMembersUrl;
};

# Org Hook

type OrgHook record {
    # The timestamp indicating when the webhook was last updated.
    string updatedAt;
    # The name of the webhook.
    string name;
    # Whether the webhook is active and will receive events.
    boolean active;
    # The timestamp indicating when the webhook was created.
    string createdAt;
    # The unique identifier of the webhook.
    int id;
    # The type of the webhook.
    string 'type;
    # The URL used to ping the webhook.
    string pingUrl;
    # The configuration settings for the webhook.
    OrgHookConfig config;
    # The URL of the webhook resource.
    string url;
    # The URL to list webhook deliveries.
    string deliveriesUrl?;
    # The list of events the webhook is subscribed to.
    string[] events;
};


type OrgHookConfig record {
    # The media type used to serialize payloads sent to the webhook.
    string contentType?;
    # Indicates whether SSL verification is skipped for webhook delivery.
    string insecureSsl?;
    # The shared secret used to sign webhook payloads.
    string secret?;
    # The URL to which webhook payloads are delivered.
    string url?;
};


type OrgHooksBody record {
    # Must be passed as "web"
    string name;
    # Determines if notifications are sent when the webhook is triggered. Set to true to send notifications
    boolean active = true;
    # Configuration settings for the webhook, including URL and content type.
    OrgsorghooksConfig config;
    # Determines what [events](https://docs.github.com/webhooks/event-payloads) the hook is triggered for. Set to ["*"] to receive all possible events
    string[] events = ["push"];
};


type OrgInvitationsBody record {
    # The role for the new member. 
* admin - Organization owners with full administrative rights to the organization and complete access to all repositories and teams.  
* direct_member - Non-owner organization members with ability to see other members and join teams by invitation.  
* billing_manager - Non-owner organization members with ability to manage the billing settings of your organization
        "admin"|"direct_member"|"billing_manager"  role = "direct_member";
    # Specify IDs for the teams you want to invite new members to
    int[] teamIds?;
    # **Required unless you provide email**. GitHub user ID for the person you are inviting
    int inviteeId?;
    # **Required unless you provide invitee_id**. Email address of the person you are inviting, which can be an existing GitHub user
    string email?;
};

# Org Membership

type OrgMembership record {
    # The API URL of the organization.
    string organizationUrl;
    # The user's membership type in the organization
        "admin"|"member"|"billing_manager"  role;
    # The permissions the user has within the organization.
    OrgMembershipPermissions permissions?;
    # The organization the membership belongs to.
    OrganizationSimple organization;
    # The state of the member in the organization. The pending state indicates the user has not yet accepted an invitation
        "active"|"pending"  state;
    # The user associated with this membership.
    NullableSimpleUser? user;
    # The API URL for this membership.
    string url;
};


type OrgMembershipPermissions record {
    # Indicates whether the member can create repositories in the organization.
    boolean canCreateRepository;
};


type OrgMigrationsBody record {
    # Indicates whether metadata should be excluded and only git source should be included for the migration
    boolean excludeMetadata = false;
    # A list of arrays indicating which repositories should be migrated
    string[] repositories;
    # Indicates whether the repository git data should be excluded from the migration
    boolean excludeGitData = false;
    # Indicates whether attachments should be excluded from the migration (to reduce migration archive file size)
    boolean excludeAttachments = false;
    # Indicates whether releases should be excluded from the migration (to reduce migration archive file size)
    boolean excludeReleases = false;
    # Indicates whether projects owned by the organization or users should be excluded. from the migration
    boolean excludeOwnerProjects = false;
    # Indicates whether this should only include organization metadata (repositories array should be empty and will ignore other flags)
    boolean orgMetadataOnly = false;
    # Exclude related items from being returned in the response in order to improve performance of the request
    ("repositories")[] exclude?;
    # Indicates whether repositories should be locked (to prevent manipulation) while migrating data
    boolean lockRepositories = false;
};


type OrgPersonalAccessTokenRequestsBody record {
    # Reason for approving or denying the requests. Max 1024 characters
    string? reason?;
    # Unique identifiers of the requests for access via fine-grained personal access token. Must be formed of between 1 and 100 pat_request_id values
    int[] patRequestIds?;
    # Action to apply to the requests
        "approve"|"deny"  action;
};


type OrgPersonalAccessTokensBody record {
    # The IDs of the fine-grained personal access tokens
    int[] patIds;
    # Action to apply to the fine-grained personal access token
        "revoke"  action;
};


type OrgProjectsBody record {
    # The name of the project
    string name;
    # The description of the project
    string body?;
};


type OrgReposBody record {
    # Pass true to create an initial commit with empty README
    boolean autoInit = false;
    # Desired language or platform [.gitignore template](https://github.com/github/gitignore) to apply. Use the name of the template without the extension. For example, "Haskell"
    string gitignoreTemplate?;
    # Either true to allow squash-merging pull requests, or false to prevent squash-merging
    boolean allowSquashMerge = true;
    # Either true to allow merging pull requests with a merge commit, or false to prevent merging pull requests with merge commits
    boolean allowMergeCommit = true;
    # Whether the repository is private
    boolean 'private = false;
    # Whether downloads are enabled
    boolean hasDownloads = true;
    # The visibility of the repository
        "public"|"private"  visibility?;
    # Either true to make this repo available as a template repository or false to prevent it
    boolean isTemplate = false;
    # A short description of the repository
    string description?;
    # The id of the team that will be granted access to this repository. This is only valid when creating a repository in an organization
    int teamId?;
    # Either true to allow rebase-merging pull requests, or false to prevent rebase-merging
    boolean allowRebaseMerge = true;
    # Either true to enable issues for this repository or false to disable them
    boolean hasIssues = true;
    # Either true to enable projects for this repository or false to disable them. **Note:** If you're creating a repository in an organization that has disabled repository projects, the default is false, and if you pass true, the API returns an error
    boolean hasProjects = true;
    # Choose an [open source license template](https://choosealicense.com/) that best suits your needs, and then use the [license keyword](https://docs.github.com/articles/licensing-a-repository/#searching-github-by-license-type) as the license_template string. For example, "mit" or "mpl-2.0"
    string licenseTemplate?;
    # Either true to enable the wiki for this repository or false to disable it
    boolean hasWiki = true;
    # Either true to allow auto-merge on pull requests, or false to disallow auto-merge
    boolean allowAutoMerge = false;
    # Either true to allow squash-merge commits to use pull request title, or false to use commit message. **This property has been deprecated. Please use squash_merge_commit_title instead
    boolean useSquashPrTitleAsDefault = false;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # The name of the repository
    string name;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # Either true to allow automatically deleting head branches when pull requests are merged, or false to prevent automatic deletion. **The authenticated user must be an organization owner to set this property to true.**
    boolean deleteBranchOnMerge = false;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # A URL with more information about the repository
    string homepage?;
};


type OrgRulesetsBody record {
    # The actors that can bypass the rules in this ruleset
    RepositoryRulesetBypassActor[] bypassActors?;
    # The name of the ruleset
    string name;
    # The enforcement level of the ruleset.
    RepositoryRuleEnforcement enforcement;
    # An array of rules within the ruleset
    RepositoryRule[] rules?;
    # The conditions that determine which repositories the ruleset applies to.
    OrgRulesetConditions conditions?;
    # The target of the ruleset
        "branch"|"tag"  target?;
};

# Represents the Queries record for the operation: orgs/list-app-installations

type OrgsListAppInstallationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-blocked-users

type OrgsListBlockedUsersQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-failed-invitations

type OrgsListFailedInvitationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-for-authenticated-user

type OrgsListForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-for-user

type OrgsListForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-invitation-teams

type OrgsListInvitationTeamsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-memberships-for-authenticated-user

type OrgsListMembershipsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the memberships to return. If not specified, the API returns both active and pending memberships
        "active"|"pending"  state?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-members

type OrgsListMembersQueries record {
    # Filter members returned in the list. 2fa_disabled means that only members without [two-factor authentication](https://github.com/blog/1614-two-factor-authentication) enabled will be returned. This options is only available for organization owners
        "2fa_disabled"|"all"  filter = "all";
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter members returned by their role
        "all"|"admin"|"member"  role = "all";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-outside-collaborators

type OrgsListOutsideCollaboratorsQueries record {
    # Filter the list of outside collaborators. 2fa_disabled means that only outside collaborators without [two-factor authentication](https://github.com/blog/1614-two-factor-authentication) enabled will be returned
        "2fa_disabled"|"all"  filter = "all";
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-pat-grant-repositories

type OrgsListPatGrantRepositoriesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-pat-grant-request-repositories

type OrgsListPatGrantRequestRepositoriesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-pat-grant-requests

type OrgsListPatGrantRequestsQueries record {
    # A list of owner usernames to use to filter the results
    string[] owner?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Only show fine-grained personal access tokens used before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string lastUsedBefore?;
    # Only show fine-grained personal access tokens used after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string lastUsedAfter?;
    # The permission to use to filter the results
    string permission?;
    # Page number of the results to fetch
    int page = 1;
    # The property by which to sort the results
        "created_at"  sort = "created_at";
    # The name of the repository to use to filter the results
    string repository?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: orgs/list-pat-grants

type OrgsListPatGrantsQueries record {
    # A list of owner usernames to use to filter the results
    string[] owner?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Only show fine-grained personal access tokens used before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string lastUsedBefore?;
    # Only show fine-grained personal access tokens used after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string lastUsedAfter?;
    # The permission to use to filter the results
    string permission?;
    # Page number of the results to fetch
    int page = 1;
    # The property by which to sort the results
        "created_at"  sort = "created_at";
    # The name of the repository to use to filter the results
    string repository?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: orgs/list-pending-invitations

type OrgsListPendingInvitationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter invitations by their invitation source
        "all"|"member"|"scim"  invitationSource = "all";
    # Filter invitations by their member role
        "all"|"admin"|"direct_member"|"billing_manager"|"hiring_manager"  role = "all";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list-public-members

type OrgsListPublicMembersQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: orgs/list

type OrgsListQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # An organization ID. Only return organizations with an ID greater than this ID
    int since?;
};

# Represents the Queries record for the operation: orgs/list-webhook-deliveries

type OrgsListWebhookDeliveriesQueries record {
    # Used for pagination: the starting delivery from which the page of deliveries is fetched. Refer to the link header for the next and previous page cursors
    string cursor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter to only return redeliveries.
    boolean redelivery?;
};

# Represents the Queries record for the operation: orgs/list-webhooks

type OrgsListWebhooksQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type OrgsorgBody record {
    # Whether Dependabot security updates is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean dependabotSecurityUpdatesEnabledForNewRepositories?;
    # Whether Dependabot alerts is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean dependabotAlertsEnabledForNewRepositories?;
    # Whether organization members can create internal repositories, which are visible to all enterprise members. You can only allow members to create internal repositories if your organization is associated with an enterprise account using GitHub Enterprise Cloud or GitHub Enterprise Server 2.20+. For more information, see "[Restricting repository creation in your organization](https://docs.github.com/github/setting-up-and-managing-organizations-and-teams/restricting-repository-creation-in-your-organization)" in the GitHub Help documentation
    boolean membersCanCreateInternalRepositories?;
    # If secret_scanning_push_protection_custom_link_enabled is true, the URL that will be displayed to contributors who are blocked from pushing a secret
    string secretScanningPushProtectionCustomLink?;
    # Whether repositories that belong to the organization can use repository projects
    boolean hasRepositoryProjects?;
    # The Twitter username of the company
    string twitterUsername?;
    # Whether organization members can create public GitHub Pages sites. Existing published sites will not be impacted
    boolean membersCanCreatePublicPages = true;
    # The description of the company
    string description?;
    # Whether GitHub Advanced Security is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean advancedSecurityEnabledForNewRepositories?;
    # The URL of the organization's blog or website.
    string blog?;
    # Whether dependency graph is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean dependencyGraphEnabledForNewRepositories?;
    # Default permission level members have for organization repositories
        "read"|"write"|"admin"|"none"  defaultRepositoryPermission = "read";
    # Whether secret scanning is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean secretScanningEnabledForNewRepositories?;
    # Billing email address. This address is not publicized
    string billingEmail?;
    # Specifies which types of repositories non-admin organization members can create. private is only available to repositories that are part of an organization on GitHub Enterprise Cloud. 
**Note:** This parameter is deprecated and will be removed in the future. Its return value ignores internal repositories. Using this parameter overrides values set in members_can_create_repositories. See the parameter deprecation notice in the operation description for details
        "all"|"private"|"none"  membersAllowedRepositoryCreationType?;
    # Whether organization members can fork private organization repositories
    boolean membersCanForkPrivateRepositories = false;
    # The company name
    string company?;
    # Whether secret scanning push protection is automatically enabled for new repositories.

To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."

You can check which security and analysis features are currently enabled by using a GET /orgs/{org} request
    boolean secretScanningPushProtectionEnabledForNewRepositories?;
    # Whether organization members can create private GitHub Pages sites. Existing published sites will not be impacted
    boolean membersCanCreatePrivatePages = true;
    # The publicly visible email address
    string email?;
    # Whether of non-admin organization members can create repositories. **Note:** A parameter can override this parameter. See members_allowed_repository_creation_type in this table for details
    boolean membersCanCreateRepositories = true;
    # Whether organization members can create private repositories, which are visible to organization members with permission. For more information, see "[Restricting repository creation in your organization](https://docs.github.com/github/setting-up-and-managing-organizations-and-teams/restricting-repository-creation-in-your-organization)" in the GitHub Help documentation
    boolean membersCanCreatePrivateRepositories?;
    # Whether contributors to organization repositories are required to sign off on commits they make through GitHub's web interface
    boolean webCommitSignoffRequired = false;
    # Whether organization members can create public repositories, which are visible to anyone. For more information, see "[Restricting repository creation in your organization](https://docs.github.com/github/setting-up-and-managing-organizations-and-teams/restricting-repository-creation-in-your-organization)" in the GitHub Help documentation
    boolean membersCanCreatePublicRepositories?;
    # Whether an organization can use organization projects
    boolean hasOrganizationProjects?;
    # Whether organization members can create GitHub Pages sites. Existing published sites will not be impacted
    boolean membersCanCreatePages = true;
    # The shorthand name of the company
    string name?;
    # The location
    string location?;
    # Whether a custom link is shown to contributors who are blocked from pushing a secret by push protection
    boolean secretScanningPushProtectionCustomLinkEnabled?;
};


type OrgsorgBody1 record {
    # The state that the membership should be in. Only "active" will be accepted
        "active"  state;
};

# Key/value pairs to provide settings for this webhook

type OrgsorghooksConfig record {
    # Password credential used for webhook authentication.
    string password?;
    # The media type used to serialize payloads sent to this webhook.
    WebhookConfigContentType contentType?;
    # Determines whether SSL verification is skipped for webhook delivery.
    WebhookConfigInsecureSsl insecureSsl?;
    # Secret token used to generate the HMAC hex digest signature for webhook payloads.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads will be delivered.
    WebhookConfigUrl url;
    # Username credential used for webhook authentication.
    string username?;
};

# Key/value pairs to provide settings for this webhook

type OrgsorghookshookIdConfig record {
    # The media type used to serialize payloads sent to this webhook.
    WebhookConfigContentType contentType?;
    # Determines whether SSL verification is skipped for webhook delivery.
    WebhookConfigInsecureSsl insecureSsl?;
    # Secret token used to generate the HMAC hex digest signature for webhook payloads.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads will be delivered.
    WebhookConfigUrl url;
};


type OrgTeamsBody record {
    # List GitHub IDs for organization members who will become team maintainers
    string[] maintainers?;
    # The ID of a team to set as the parent team
    int parentTeamId?;
    # The name of the team
    string name;
    # The full name (e.g., "organization-name/repository-name") of repositories to add the team to
    string[] repoNames?;
    # The description of the team
    string description?;
    # The level of privacy this team should have. The options are:  
**For a non-nested team:**  
* secret - only visible to organization owners and members of this team.  
* closed - visible to all members of this organization.  
Default: secret  
**For a parent or child team:**  
* closed - visible to all members of this organization.  
Default for child team: closed
        "secret"|"closed"  privacy?;
    # **Deprecated**. The permission that new repositories will be added to the team with when none is specified
        "pull"|"push"  permission = "pull";
    # The notification setting the team has chosen. The options are:  
* notifications_enabled - team members receive notifications when the team is @mentioned.  
* notifications_disabled - no one receives notifications.  
Default: notifications_enabled
        "notifications_enabled"|"notifications_disabled"  notificationSetting?;
};


type OutsideCollaboratorsusernameBody record {
    # When set to true, the request will be performed asynchronously. Returns a 202 status code when the job is successfully queued
    boolean async = false;
};


type OwnerrepoBody record {
    # The permission to grant the team on this repository. We accept the following permissions to be set: pull, triage, push, maintain, admin and you can also specify a custom repository role name, if the owning organization has defined any. If no permission is specified, the team's permission attribute will be used to determine what permission to grant the team on this repository
    string permission = "push";
};


type OwnerrepoBody1 record {
    # Either true to make the repository private or false to make it public. Default: false.  
**Note**: You will get a 422 error if the organization restricts [changing repository visibility](https://docs.github.com/articles/repository-permission-levels-for-an-organization#changing-the-visibility-of-repositories) to organization owners and a non-owner tries to change the value of private
    boolean 'private = false;
    # Either true to allow private forks, or false to prevent private forks
    boolean allowForking?;
    # Either true to make this repo available as a template repository or false to prevent it
    boolean isTemplate = false;
    # A short description of the repository
    string description?;
    # Either true to allow rebase-merging pull requests, or false to prevent rebase-merging
    boolean allowRebaseMerge = true;
    # Either true to enable projects for this repository or false to disable them. **Note:** If you're creating a repository in an organization that has disabled repository projects, the default is false, and if you pass true, the API returns an error
    boolean hasProjects = true;
    # Whether to archive this repository. false will unarchive a previously archived repository
    boolean archived = false;
    # Either true to enable the wiki for this repository or false to disable it
    boolean hasWiki = true;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # Either true to allow automatically deleting head branches when pull requests are merged, or false to prevent automatic deletion
    boolean deleteBranchOnMerge = false;
    # Either true to allow squash-merging pull requests, or false to prevent squash-merging
    boolean allowSquashMerge = true;
    # Either true to allow merging pull requests with a merge commit, or false to prevent merging pull requests with merge commits
    boolean allowMergeCommit = true;
    # The visibility of the repository
        "public"|"private"  visibility?;
    # Either true to always allow a pull request head branch that is behind its base branch to be updated even if it is not required to be up to date before merging, or false otherwise
    boolean allowUpdateBranch = false;
    # Either true to enable issues for this repository or false to disable them
    boolean hasIssues = true;
    # Either true to require contributors to sign off on web-based commits, or false to not require contributors to sign off on web-based commits
    boolean webCommitSignoffRequired = false;
    # Security and analysis settings to enable or disable for the repository.
    ReposownerrepoSecurityAndAnalysis? securityAndAnalysis?;
    # Either true to allow auto-merge on pull requests, or false to disallow auto-merge
    boolean allowAutoMerge = false;
    # Either true to allow squash-merge commits to use pull request title, or false to use commit message. **This property has been deprecated. Please use squash_merge_commit_title instead
    boolean useSquashPrTitleAsDefault = false;
    # The name of the repository
    string name?;
    # Updates the default branch for this repository
    string defaultBranch?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # A URL with more information about the repository
    string homepage?;
};


type OwnerrepoBody2 record {
    # The permission to grant the team on this repository. If no permission is specified, the team's permission attribute will be used to determine what permission to grant the team on this repository
        "pull"|"push"|"admin"  permission?;
};

# A software package

type Package record {
    # The user or organization that owns the package.
    NullableSimpleUser? owner?;
    # The visibility level of the package, either public or private.
        "private"|"public"  visibility;
    # The date and time when the package was last updated.
    string updatedAt;
    # The URL to view the package on GitHub.
    string htmlUrl;
    # The name of the package
    string name;
    # The date and time when the package was created.
    string createdAt;
    # Unique identifier of the package
    int id;
    # The package registry type, such as npm or container.
        "npm"|"maven"|"rubygems"|"docker"|"nuget"|"container"  packageType;
    # The repository associated with the package.
    NullableMinimalRepository? repository?;
    # The number of versions of the package
    int versionCount;
    # The API URL for the package resource.
    string url;
};


type PackagesBillingUsage record {
    # Free storage space (GB) for GitHub Packages
    int includedGigabytesBandwidth;
    # Sum of the free and paid storage space (GB) for GitHuub Packages
    int totalGigabytesBandwidthUsed;
    # Total paid storage space (GB) for GitHuub Packages
    int totalPaidGigabytesBandwidthUsed;
};

# Represents the Queries record for the operation: packages/get-all-package-versions-for-package-owned-by-authenticated-user

type PackagesGetAllPackageVersionsForPackageOwnedByAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # The state of the package, either active or deleted
        "active"|"deleted"  state = "active";
};

# Represents the Queries record for the operation: packages/get-all-package-versions-for-package-owned-by-org

type PackagesGetAllPackageVersionsForPackageOwnedByOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # The state of the package, either active or deleted
        "active"|"deleted"  state = "active";
};

# Represents the Queries record for the operation: packages/list-packages-for-authenticated-user

type PackagesListPackagesForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The selected visibility of the packages.  This parameter is optional and only filters an existing result set.

The internal visibility is only supported for GitHub Packages registries that allow for granular permissions. For other ecosystems internal is synonymous with private.
For the list of GitHub Packages registries that support granular permissions, see "[About permissions for GitHub Packages](https://docs.github.com/packages/learn-github-packages/about-permissions-for-github-packages#granular-permissions-for-userorganization-scoped-packages)."
        "public"|"private"|"internal"  visibility?;
    # Page number of the results to fetch
    int page = 1;
    # The type of supported package. Packages in GitHub's Gradle registry have the type maven. Docker images pushed to GitHub's Container registry (ghcr.io) have the type container. You can use the type docker to find images that were pushed to GitHub's Docker registry (docker.pkg.github.com), even if these have now been migrated to the Container registry
        "npm"|"maven"|"rubygems"|"docker"|"nuget"|"container"  packageType;
};

# Represents the Queries record for the operation: packages/list-packages-for-organization

type PackagesListPackagesForOrganizationQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The selected visibility of the packages.  This parameter is optional and only filters an existing result set.

The internal visibility is only supported for GitHub Packages registries that allow for granular permissions. For other ecosystems internal is synonymous with private.
For the list of GitHub Packages registries that support granular permissions, see "[About permissions for GitHub Packages](https://docs.github.com/packages/learn-github-packages/about-permissions-for-github-packages#granular-permissions-for-userorganization-scoped-packages)."
        "public"|"private"|"internal"  visibility?;
    # Page number of the results to fetch
    int page = 1;
    # The type of supported package. Packages in GitHub's Gradle registry have the type maven. Docker images pushed to GitHub's Container registry (ghcr.io) have the type container. You can use the type docker to find images that were pushed to GitHub's Docker registry (docker.pkg.github.com), even if these have now been migrated to the Container registry
        "npm"|"maven"|"rubygems"|"docker"|"nuget"|"container"  packageType;
};

# Represents the Queries record for the operation: packages/list-packages-for-user

type PackagesListPackagesForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The selected visibility of the packages.  This parameter is optional and only filters an existing result set.

The internal visibility is only supported for GitHub Packages registries that allow for granular permissions. For other ecosystems internal is synonymous with private.
For the list of GitHub Packages registries that support granular permissions, see "[About permissions for GitHub Packages](https://docs.github.com/packages/learn-github-packages/about-permissions-for-github-packages#granular-permissions-for-userorganization-scoped-packages)."
        "public"|"private"|"internal"  visibility?;
    # Page number of the results to fetch
    int page = 1;
    # The type of supported package. Packages in GitHub's Gradle registry have the type maven. Docker images pushed to GitHub's Container registry (ghcr.io) have the type container. You can use the type docker to find images that were pushed to GitHub's Docker registry (docker.pkg.github.com), even if these have now been migrated to the Container registry
        "npm"|"maven"|"rubygems"|"docker"|"nuget"|"container"  packageType;
};

# Represents the Queries record for the operation: packages/restore-package-for-authenticated-user

type PackagesRestorePackageForAuthenticatedUserQueries record {
    # package token
    string token?;
};

# Represents the Queries record for the operation: packages/restore-package-for-org

type PackagesRestorePackageForOrgQueries record {
    # package token
    string token?;
};

# Represents the Queries record for the operation: packages/restore-package-for-user

type PackagesRestorePackageForUserQueries record {
    # package token
    string token?;
};

# A version of a software package

type PackageVersion record {
    # The license associated with this package version.
    string license?;
    # Additional metadata specific to the package version's ecosystem.
    PackageVersionMetadata metadata?;
    # The date and time the package version was last updated.
    string updatedAt;
    # The HTML URL for viewing the package version on GitHub.
    string htmlUrl?;
    # The name of the package version
    string name;
    # A brief description of the package version.
    string description?;
    # The date and time the package version was created.
    string createdAt;
    # Unique identifier of the package version
    int id;
    # The date and time the package version was deleted.
    string deletedAt?;
    # The API URL for this package version.
    string url;
    # The HTML URL for the parent package on GitHub.
    string packageHtmlUrl;
};


type PackageVersionMetadata record {
    # Container-specific metadata for the package version.
    ContainerMetadata container?;
    # The type of package ecosystem for this version.
        "npm"|"maven"|"rubygems"|"docker"|"nuget"|"container"  packageType;
    # Docker-specific metadata for the package version.
    DockerMetadata docker?;
};

# The configuration for GitHub Pages for a repository

type Page record {
    # Whether the GitHub Pages site is publicly visible. If set to true, the site is accessible to anyone on the internet. If set to false, the site will only be accessible to users who have at least read access to the repository that published the site
    boolean 'public;
    # The HTTPS certificate associated with the GitHub Pages site.
    PagesHttpsCertificate httpsCertificate?;
    # Whether the Page has a custom 404 page
    boolean custom404 = false;
    # The web address the Page can be accessed from
    string htmlUrl?;
    # The Pages site's custom domain
    string? cname;
    # Whether https is enabled on the domain
    boolean httpsEnforced?;
    # The timestamp when a pending domain becomes unverified
    string? pendingDomainUnverifiedAt?;
    # The source branch and directory from which the Pages site is built.
    PagesSourceHash 'source?;
    # The state if the domain is verified
        "pending"|"verified"|"unverified"? protectedDomainState?;
    # The process in which the Page will be built
        "legacy"|"workflow"? buildType?;
    # The API address for accessing this Page resource
    string url;
    # The status of the most recent build of the Page
        "built"|"building"|"errored"? status;
};

# Page Build

type PageBuild record {
    # The duration of the build in milliseconds.
    int duration;
    # The user who triggered the GitHub Pages build.
    NullableSimpleUser? pusher;
    # The date and time the build was last updated.
    string updatedAt;
    # The SHA of the commit that triggered the build.
    string 'commit;
    # The date and time the build was created.
    string createdAt;
    # Error information if the build failed.
    PageBuildError 'error;
    # The API URL for this page build.
    string url;
    # The current status of the GitHub Pages build.
    string status;
};


type PageBuildError record {
    # The error message describing the GitHub Pages build failure.
    string? message;
};

# Page Build Status

type PageBuildStatus record {
    # The API URL for the GitHub Pages build.
    string url;
    # The current status of the GitHub Pages build.
    string status;
};

# The GitHub Pages deployment status

type PageDeployment record {
    # The URI to the deployed GitHub Pages
    string pageUrl;
    # The URI to the deployed GitHub Pages preview
    string previewUrl?;
    # The URI to monitor GitHub Pages deployment status
    string statusUrl;
};

# The object used to create GitHub Pages deployment

type PagesDeploymentBody record {
    # The URL of an artifact that contains the .zip or .tar of static assets to deploy. The artifact belongs to the repository
    string artifactUrl;
    # The target environment for this GitHub Pages deployment
    string environment = "github-pages";
    # The OIDC token issued by GitHub Actions certifying the origin of the deployment
    string oidcToken;
    # A unique string that represents the version of the build for this deployment
    string pagesBuildVersion = "GITHUB_SHA";
};

# Pages Health Check Status

type PagesHealthCheck record {
    # The health check details for the alternate domain of the Pages site.
    PagesHealthCheckAltDomain? altDomain?;
    # The health check details for the primary domain of the Pages site.
    PagesHealthCheckDomain domain?;
};


type PagesHealthCheckAltDomain record {
    # The reason for the domain's current health status.
    string? reason?;
    # Whether the domain resolves to a Cloudflare IP address.
    boolean? isCloudflareIp?;
    # Whether the domain enforces HTTPS.
    boolean enforcesHttps?;
    # Whether the domain is pointed to a GitHub Pages IP address.
    boolean? isPointedToGithubPagesIp?;
    # Whether the domain is actively served by GitHub Pages.
    boolean? isServedByPages?;
    # Whether the domain resolves to a Fastly IP address.
    boolean? isFastlyIp?;
    # Whether the domain is an apex (root) domain.
    boolean isApexDomain?;
    # The hostname of the alternate domain.
    string host?;
    # Whether the domain responds to HTTPS requests.
    boolean respondsToHttps?;
    # Whether the domain has a CNAME record pointing to pages.github.com.
    boolean? isCnameToPagesDotGithubDotCom?;
    # Whether the domain has an A record configured.
    boolean? isARecord?;
    # Whether the domain has a CNAME record pointing to a GitHub user domain.
    boolean? isCnameToGithubUserDomain?;
    # Whether the domain is a GitHub Pages domain.
    boolean isPagesDomain?;
    # Whether the domain points to a deprecated GitHub Pages IP address.
    boolean? isOldIpAddress?;
    # Whether the domain has a CNAME record configured.
    boolean? hasCnameRecord?;
    # Whether the domain has a CNAME record pointing to Fastly.
    boolean? isCnameToFastly?;
    # Whether the domain's DNS resolves successfully.
    boolean dnsResolves?;
    # Whether the domain is eligible for HTTPS enforcement.
    boolean? isHttpsEligible?;
    # The error message related to CAA DNS record validation, if any.
    string? caaError?;
    # Whether the domain is served through a proxy.
    boolean? isProxied?;
    # Whether the domain is a valid domain name.
    boolean isValidDomain?;
    # Whether a non-GitHub Pages IP address is present in DNS records.
    boolean? isNonGithubPagesIpPresent?;
    # The URI of the alternate domain.
    string uri?;
    # Whether the domain should be configured as an A record.
    boolean? shouldBeARecord?;
    # The nameservers associated with the domain.
    string nameservers?;
    # Whether the domain has MX records present.
    boolean? hasMxRecordsPresent?;
    # Whether the alternate domain configuration is valid.
    boolean isValid?;
    # The error message related to HTTPS configuration, if any.
    string? httpsError?;
};


type PagesHealthCheckDomain record {
    # Explanation of the domain health check result or failure reason.
    string? reason?;
    # Indicates whether the domain resolves to a Cloudflare IP address.
    boolean? isCloudflareIp?;
    # Indicates whether the domain enforces HTTPS connections.
    boolean enforcesHttps?;
    # Indicates whether the domain points to a GitHub Pages IP address.
    boolean? isPointedToGithubPagesIp?;
    # Indicates whether the domain is actively served by GitHub Pages.
    boolean? isServedByPages?;
    # Indicates whether the domain resolves to a Fastly CDN IP address.
    boolean? isFastlyIp?;
    # Indicates whether the domain is an apex (root) domain.
    boolean isApexDomain?;
    # The hostname of the domain being health-checked.
    string host?;
    # Indicates whether the domain responds to HTTPS requests.
    boolean respondsToHttps?;
    # Indicates whether the domain has a CNAME record pointing to pages.github.com.
    boolean? isCnameToPagesDotGithubDotCom?;
    # Indicates whether the domain has a DNS A record configured.
    boolean? isARecord?;
    # Indicates whether the domain CNAME points to a GitHub user domain.
    boolean? isCnameToGithubUserDomain?;
    # Indicates whether this is a recognized GitHub Pages domain.
    boolean isPagesDomain?;
    # Indicates whether the domain resolves to a deprecated GitHub Pages IP address.
    boolean? isOldIpAddress?;
    # Indicates whether the domain has a CNAME DNS record.
    boolean? hasCnameRecord?;
    # Indicates whether the domain has a CNAME record pointing to Fastly.
    boolean? isCnameToFastly?;
    # Indicates whether the domain successfully resolves via DNS.
    boolean dnsResolves?;
    # Indicates whether the domain is eligible for HTTPS enforcement.
    boolean? isHttpsEligible?;
    # Error message from the CAA DNS record check, if any.
    string? caaError?;
    # Indicates whether the domain traffic is routed through a proxy.
    boolean? isProxied?;
    # Indicates whether the domain name is a valid domain.
    boolean isValidDomain?;
    # Indicates whether a non-GitHub Pages IP address is present for the domain.
    boolean? isNonGithubPagesIpPresent?;
    # The full URI of the domain being health-checked.
    string uri?;
    # Indicates whether the domain should be configured as an A record.
    boolean? shouldBeARecord?;
    # The nameservers responsible for the domain's DNS resolution.
    string nameservers?;
    # Indicates whether MX records are present for the domain.
    boolean? hasMxRecordsPresent?;
    # Indicates whether the domain configuration is valid for GitHub Pages.
    boolean isValid?;
    # Error message describing any HTTPS configuration issue for the domain.
    string? httpsError?;
};


type PagesHttpsCertificate record {
    # The date and time when the HTTPS certificate expires.
    string expiresAt?;
    # A description of the current certificate state or error.
    string description;
    # Array of the domain set and its alternate name (if it is configured)
    string[] domains;
    # The current provisioning state of the HTTPS certificate.
        "new"|"authorization_created"|"authorization_pending"|"authorized"|"authorization_revoked"|"issued"|"uploaded"|"approved"|"errored"|"bad_authz"|"destroy_pending"|"dns_changed"  state;
};


type PagesSourceHash record {
    # The directory path used as the GitHub Pages source.
    string path;
    # The branch used as the GitHub Pages source.
    string branch;
};


type ParticipationStats record {
    # Weekly commit counts for all users over the past year.
    int[] all;
    # Weekly commit counts for the repository owner over the past year.
    int[] owner;
};

# Details of a deployment that is waiting for protection rules to pass

type PendingDeployment record {
    # The environment that is waiting for deployment approval.
    PendingDeploymentEnvironment environment;
    # The time that the wait timer began
    string? waitTimerStartedAt;
    # The set duration of the wait timer
    int waitTimer;
    # Whether the currently authenticated user can approve the deployment
    boolean currentUserCanApprove;
    # The people or teams that may approve jobs that reference the environment. You can list up to six users or teams as reviewers. The reviewers must have at least read access to the repository. Only one of the required reviewers needs to approve the job for it to proceed
    PendingDeploymentReviewers[] reviewers;
};


type PendingDeploymentEnvironment record {
    # The HTML URL of the environment on GitHub.
    string htmlUrl?;
    # The name of the environment
    string name?;
    # The id of the environment
    int id?;
    # The API URL of the environment.
    string url?;
    # The GraphQL node identifier of the environment.
    string nodeId?;
};


type PendingDeploymentReviewers record {
    # The user or team assigned to review the pending deployment.
    SimpleUser|Team reviewer?;
    # The type of reviewer, either a user or team.
    DeploymentReviewerType 'type?;
};


type PermissionsRepositoriesBody record {
    # List of repository IDs to enable for GitHub Actions
    int[] selectedRepositoryIds;
};


type PersonalAccessTokenRequestspatRequestIdBody record {
    # Reason for approving or denying the request. Max 1024 characters
    string? reason?;
    # Action to apply to the request
        "approve"|"deny"  action;
};


type PersonalAccessTokenspatIdBody record {
    # Action to apply to the fine-grained personal access token
        "revoke"  action;
};

# Porter Author

type PorterAuthor record {
    # The author's name as it appears in the remote source repository.
    string remoteName;
    # The author's identifier in the remote source repository.
    string remoteId;
    # The API URL for updating this author during import.
    string importUrl;
    # The mapped GitHub name for the author.
    string name;
    # The unique identifier of the porter author.
    int id;
    # The mapped GitHub email address for the author.
    string email;
    # The API URL for this porter author resource.
    string url;
};

# Porter Large File

type PorterLargeFile record {
    # The Git reference name associated with the large file.
    string refName;
    # The file path of the large file in the repository.
    string path;
    # The size of the large file in bytes.
    int size;
    # The Git object identifier (OID) of the large file.
    string oid;
};

# Private User

type PrivateUser record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # Whether the user has two-factor authentication enabled
    boolean twoFactorAuthentication;
    # API URL template to check who the user is following
    string followingUrl;
    # The Twitter username of the user
    string? twitterUsername?;
    # The biography of the user
    string? bio;
    # The date the user account was created
    string createdAt;
    # The username of the user
    string login;
    # The type of the account
    string 'type;
    # The URL of the user's blog or website
    string? blog;
    # The number of private gists the user owns
    int privateGists;
    # The total number of private repositories the user owns
    int totalPrivateRepos;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # The date the user account was last updated
    string updatedAt;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The total disk usage across all repositories in kilobytes
    int diskUsage;
    # The number of collaborators across the user's private repositories
    int collaborators;
    # The company the user belongs to
    string? company;
    # The number of private repositories owned by the user
    int ownedPrivateRepos;
    # The unique identifier of the user
    int id;
    # The number of public repositories the user owns
    int publicRepos;
    # The Gravatar ID of the user
    string? gravatarId;
    # The GitHub subscription plan associated with the user's account.
    PublicUserPlan plan?;
    # The publicly visible email address of the user
    string? email;
    # API URL to list the user's organizations
    string organizationsUrl;
    # Whether the user is available for hire
    boolean? hireable;
    # API URL template for repositories the user has starred
    string starredUrl;
    # API URL to list the user's followers
    string followersUrl;
    # The number of public gists the user owns
    int publicGists;
    # API URL for the user
    string url;
    # API URL for events received by the user
    string receivedEventsUrl;
    # The LDAP distinguished name for the user
    string ldapDn?;
    # The number of followers the user has
    int followers;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # Whether the user has a GitHub Business Plus plan
    boolean businessPlus?;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The number of users the user is following
    int following;
    # The display name of the user
    string? name;
    # The geographic location of the user
    string? location;
    # The GraphQL node identifier of the user
    string nodeId;
    # The date the user account was suspended
    string? suspendedAt?;
};


type PrivateVulnerabilityReportCreate record {
    # A short summary of the advisory
    string summary;
    # The severity of the advisory. You must choose between setting this field or cvss_vector_string
        "critical"|"high"|"medium"|"low"? severity?;
    # The CVSS vector that calculates the severity of the advisory. You must choose between setting this field or severity
    string? cvssVectorString?;
    # A detailed description of what the advisory impacts
    string description;
    # An array of products affected by the vulnerability detailed in a repository security advisory
    RepositoryAdvisoryCreateVulnerabilities[]? vulnerabilities?;
    # A list of Common Weakness Enumeration (CWE) IDs
    string[]? cweIds?;
};

# Projects are a way to organize columns and cards of work

type Project record {
    # The API URL to list columns in this project.
    string columnsUrl;
    # The GitHub user who created this project.
    NullableSimpleUser? creator;
    # Whether or not this project can be seen by everyone. Only present if owner is an organization
    boolean 'private?;
    # The baseline permission that all organization members have on this project. Only present if owner is an organization
        "read"|"write"|"admin"|"none"  organizationPermission?;
    # The timestamp when this project was created.
    string createdAt;
    # Body of the project
    string? body;
    # The API URL for this project resource.
    string url;
    # The API URL of the project's owner resource.
    string ownerUrl;
    # The sequential number identifying this project within its owner.
    int number;
    # The timestamp when this project was last updated.
    string updatedAt;
    # The URL to view this project on GitHub.
    string htmlUrl;
    # Name of the project
    string name;
    # The unique numeric identifier for this project.
    int id;
    # State of the project; either 'open' or 'closed'
    string state;
    # The unique GraphQL node identifier for this project.
    string nodeId;
};

# Project cards represent a scope of work

type ProjectCard record {
    # The API URL for the column this card belongs to.
    string columnUrl;
    # The text note content of the project card.
    string? note;
    # The user who created the project card.
    NullableSimpleUser? creator;
    # The name of the column this card belongs to.
    string columnName?;
    # The timestamp when the project card was created.
    string createdAt;
    # The API URL for the project containing this card.
    string projectUrl;
    # The API URL for this project card.
    string url;
    # Whether or not the card is archived
    boolean archived?;
    # The timestamp when the project card was last updated.
    string updatedAt;
    # The identifier of the project containing this card.
    string projectId?;
    # The API URL for the issue or pull request linked to this card.
    string contentUrl?;
    # The project card's ID
    int id;
    # The GraphQL node identifier of the project card.
    string nodeId;
};

# Project Collaborator Permission

type ProjectCollaboratorPermission record {
    # The permission level of the collaborator on the project.
    string permission;
    # The user associated with the collaborator permission.
    NullableSimpleUser? user;
};

# Project columns contain cards of work

type ProjectColumn record {
    # The date and time the project column was last updated.
    string updatedAt;
    # The API URL to list cards in this column.
    string cardsUrl;
    # Name of the project column
    string name;
    # The API URL of the project this column belongs to.
    string projectUrl;
    # The date and time the project column was created.
    string createdAt;
    # The unique identifier of the project column
    int id;
    # The API URL for this project column.
    string url;
    # The GraphQL node identifier of the project column.
    string nodeId;
};


type ProjectscolumnscolumnIdcardsOneOf1 record {
    # The project card's note
    string? note;
};


type ProjectscolumnscolumnIdcardsprojectscolumnscolumnIdcardsOneOf12 record {
    # The piece of content associated with the card
    string contentType;
    # The unique identifier of the content associated with the card
    int contentId;
};

# Represents the Queries record for the operation: projects/list-cards

type ProjectsListCardsQueries record {
    # Filters the project cards that are returned by the card's state
        "all"|"archived"|"not_archived"  archivedState = "not_archived";
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: projects/list-collaborators

type ProjectsListCollaboratorsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filters the collaborators by their affiliation. outside means outside collaborators of a project that are not a member of the project's organization. direct means collaborators with permissions to a project, regardless of organization membership status. all means all collaborators the authenticated user can see
        "outside"|"direct"|"all"  affiliation = "all";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: projects/list-columns

type ProjectsListColumnsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: projects/list-for-org

type ProjectsListForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the projects to return
        "open"|"closed"|"all"  state = "open";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: projects/list-for-repo

type ProjectsListForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the projects to return
        "open"|"closed"|"all"  state = "open";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: projects/list-for-user

type ProjectsListForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Indicates the state of the projects to return
        "open"|"closed"|"all"  state = "open";
    # Page number of the results to fetch
    int page = 1;
};


type ProjectsprojectIdBody record {
    # The permission to grant to the team for this project. Default: the team's permission attribute will be used to determine what permission to grant the team on this project. Note that, if you choose not to pass any parameters, you'll need to set Content-Length to zero when calling this endpoint. For more information, see "[HTTP verbs](https://docs.github.com/rest/overview/resources-in-the-rest-api#http-verbs)."
        "read"|"write"|"admin"  permission?;
};


type ProjectsprojectIdBody1 record {
    # Whether or not this project can be seen by everyone
    boolean 'private?;
    # The baseline permission that all organization members have on this project
        "read"|"write"|"admin"|"none"  organizationPermission?;
    # Name of the project
    string name?;
    # State of the project; either 'open' or 'closed'
    string state?;
    # Body of the project
    string? body?;
};


type ProjectsprojectIdBody2 record {
    # The permission to grant to the team for this project. Default: the team's permission attribute will be used to determine what permission to grant the team on this project. Note that, if you choose not to pass any parameters, you'll need to set Content-Length to zero when calling this endpoint. For more information, see "[HTTP verbs](https://docs.github.com/rest/overview/resources-in-the-rest-api#http-verbs)."
        "read"|"write"|"admin"  permission?;
};

# Branch protections protect branches

type ProtectedBranch record {
    # Settings for required pull request reviews before merging.
    ProtectedBranchRequiredPullRequestReviews requiredPullRequestReviews?;
    # Settings requiring signed commits on the protected branch.
    ProtectedBranchRequiredSignatures requiredSignatures?;
    # Status check policy required before merging into the branch.
    StatusCheckPolicy requiredStatusChecks?;
    # Settings controlling whether fork syncing is allowed on the branch.
    ProtectedBranchAllowForkSyncing allowForkSyncing?;
    # Settings requiring a linear commit history on the branch.
    ProtectedBranchRequiredLinearHistory requiredLinearHistory?;
    # Access restrictions controlling who can push to the branch.
    BranchRestrictionPolicy restrictions?;
    # Settings for enforcing branch protections on administrators.
    ProtectedBranchEnforceAdmins enforceAdmins?;
    # API URL for the protected branch.
    string url;
    # Settings controlling whether force pushes are allowed on the branch.
    ProtectedBranchRequiredLinearHistory allowForcePushes?;
    # Settings controlling whether the branch is locked from changes.
    ProtectedBranchLockBranch lockBranch?;
    # Settings requiring all comments to be resolved before merging.
    ProtectedBranchRequiredConversationResolution requiredConversationResolution?;
    # Settings blocking creation of matching branches or tags.
    ProtectedBranchRequiredLinearHistory blockCreations?;
    # Settings controlling whether the branch can be deleted.
    ProtectedBranchRequiredLinearHistory allowDeletions?;
};

# Protected Branch Admin Enforced

type ProtectedBranchAdminEnforced record {
    # The API URL for this admin enforcement resource.
    string url;
    # Whether admin enforcement is enabled for the protected branch.
    boolean enabled;
};

# Whether users can pull changes from upstream when the branch is locked. Set to true to allow fork syncing. Set to false to prevent fork syncing

type ProtectedBranchAllowForkSyncing record {
    # Indicates whether fork syncing is allowed for the locked branch.
    boolean enabled = false;
};


type ProtectedBranchEnforceAdmins record {
    # The API URL for the enforce admins branch protection setting.
    string url;
    # Indicates whether admin enforcement is enabled for the branch.
    boolean enabled;
};

# Whether to set the branch as read-only. If this is true, users will not be able to push to the branch

type ProtectedBranchLockBranch record {
    # Indicates whether the branch is locked as read-only.
    boolean enabled = false;
};

# Protected Branch Pull Request Review

type ProtectedBranchPullRequestReview record {
    # Restrictions on who can dismiss pull request reviews.
    ProtectedBranchPullRequestReviewDismissalRestrictions dismissalRestrictions?;
    # The number of approving reviews required before merging.
    int requiredApprovingReviewCount?;
    # Whether an approval from a code owner is required to merge.
    boolean requireCodeOwnerReviews;
    # Whether approvals are dismissed when new commits are pushed.
    boolean dismissStaleReviews;
    # Users, teams, or apps allowed to bypass required pull request reviews.
    ProtectedBranchPullRequestReviewBypassPullRequestAllowances bypassPullRequestAllowances?;
    # Whether the most recent push must be approved by someone other than the person who pushed it
    boolean requireLastPushApproval = false;
    # API URL for the pull request review protection settings.
    string url?;
};

# Allow specific users, teams, or apps to bypass pull request requirements

type ProtectedBranchPullRequestReviewBypassPullRequestAllowances record {
    # The list of teams allowed to bypass pull request requirements
    Team[] teams?;
    # The list of users allowed to bypass pull request requirements
    SimpleUser[] users?;
    # The list of apps allowed to bypass pull request requirements
    Integration[] apps?;
};


type ProtectedBranchPullRequestReviewDismissalRestrictions record {
    # API URL for listing teams with review dismissal access.
    string teamsUrl?;
    # The list of teams with review dismissal access
    Team[] teams?;
    # API URL for listing users with review dismissal access.
    string usersUrl?;
    # The list of users with review dismissal access
    SimpleUser[] users?;
    # API URL for the dismissal restrictions resource.
    string url?;
    # The list of apps with review dismissal access
    Integration[] apps?;
};


type ProtectedBranchRequiredConversationResolution record {
    # Whether all conversations must be resolved before merging.
    boolean enabled?;
};


type ProtectedBranchRequiredLinearHistory record {
    # Whether linear history is required for the protected branch.
    boolean enabled;
};


type ProtectedBranchRequiredPullRequestReviews record {
    # Users and teams allowed to dismiss pull request reviews.
    ProtectedBranchRequiredPullRequestReviewsDismissalRestrictions dismissalRestrictions?;
    # The number of approving reviews required before merging.
    int requiredApprovingReviewCount?;
    # Indicates whether code owner review is required before merging.
    boolean requireCodeOwnerReviews?;
    # Indicates whether approvals are dismissed when new commits are pushed.
    boolean dismissStaleReviews?;
    # Users and teams allowed to bypass required pull request reviews.
    ProtectedBranchRequiredPullRequestReviewsBypassPullRequestAllowances bypassPullRequestAllowances?;
    # Whether the most recent push must be approved by someone other than the person who pushed it
    boolean requireLastPushApproval = false;
    # The API URL for the required pull request reviews settings.
    string url;
};


type ProtectedBranchRequiredPullRequestReviewsBypassPullRequestAllowances record {
    # Teams allowed to bypass required pull request reviews.
    Team[] teams;
    # Users allowed to bypass required pull request reviews.
    SimpleUser[] users;
    # Apps allowed to bypass required pull request reviews.
    Integration[] apps?;
};


type ProtectedBranchRequiredPullRequestReviewsDismissalRestrictions record {
    # API URL for listing teams with pull request review dismissal rights.
    string teamsUrl;
    # The list of teams allowed to dismiss pull request reviews.
    Team[] teams;
    # API URL for listing users with pull request review dismissal rights.
    string usersUrl;
    # API URL for the dismissal restrictions resource.
    string url;
    # The list of users allowed to dismiss pull request reviews.
    SimpleUser[] users;
    # The list of apps allowed to dismiss pull request reviews.
    Integration[] apps?;
};


type ProtectedBranchRequiredSignatures record {
    # API URL for the required signatures protection rule.
    string url;
    # Indicates whether required commit signatures are enabled for the branch.
    boolean enabled;
};

# Protected Branch Required Status Check

type ProtectedBranchRequiredStatusCheck record {
    # The enforcement level for required status checks on the branch.
    string enforcementLevel?;
    # The list of status checks required before merging.
    ProtectedBranchRequiredStatusCheckChecks[] checks;
    # API URL to retrieve the required status check contexts.
    string contextsUrl?;
    # The list of status check context names that are required.
    string[] contexts;
    # Indicates whether branches must be up to date before merging.
    boolean strict?;
    # API URL for this required status check configuration.
    string url?;
};


type ProtectedBranchRequiredStatusCheckChecks record {
    # Name of the required status check context.
    string context;
    # GitHub App ID that must provide this required status check.
    int? appId;
};


type ProtectionRequiredPullRequestReviewsBody record {
    # Users and teams allowed to dismiss pull request reviews.
    ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsDismissalRestrictions dismissalRestrictions?;
    # Specifies the number of reviewers required to approve pull requests. Use a number between 1 and 6 or 0 to not require reviewers
    int requiredApprovingReviewCount?;
    # Blocks merging pull requests until [code owners](https://docs.github.com/articles/about-code-owners/) have reviewed
    boolean requireCodeOwnerReviews?;
    # Set to true if you want to automatically dismiss approving reviews when someone pushes a new commit
    boolean dismissStaleReviews?;
    # Users and teams allowed to bypass required pull request reviews.
    ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsBypassPullRequestAllowances bypassPullRequestAllowances?;
    # Whether the most recent push must be approved by someone other than the person who pushed it. Default: false
    boolean requireLastPushApproval = false;
};


type ProtectionRequiredStatusChecksBody record {
    # The list of status checks to require in order to merge into this branch
    ReposownerrepobranchesbranchprotectionRequiredStatusChecksChecks[] checks?;
    # **Deprecated**: The list of status checks to require in order to merge into this branch. If any of these checks have recently been set by a particular GitHub App, they will be required to come from that app in future for the branch to merge. Use checks instead of contexts for more fine-grained control
    string[] contexts?;
    # Require branches to be up to date before merging
    boolean strict?;
};


type ProtectionRulesAnyOf1 record {
    # The unique identifier of the protection rule.
    int id;
    # The wait timer duration before the environment can be deployed to.
    WaitTimer waitTimer?;
    # The type of the protection rule.
    string 'type;
    # The GraphQL node identifier of the protection rule.
    string nodeId;
};


type ProtectionRulesProtectionRulesAnyOf12 record {
    # Whether deployments to this environment can be approved by the user who created the deployment
    boolean preventSelfReview?;
    # Unique numeric identifier for this protection rule.
    int id;
    # The type of protection rule.
    string 'type;
    # The people or teams that may approve jobs that reference the environment. You can list up to six users or teams as reviewers. The reviewers must have at least read access to the repository. Only one of the required reviewers needs to approve the job for it to proceed
    PendingDeploymentReviewers[] reviewers?;
    # Global node identifier for this protection rule.
    string nodeId;
};


type ProtectionRulesProtectionRulesProtectionRulesAnyOf123 record {
    # Unique numeric identifier of the protection rule.
    int id;
    # Type of environment protection rule.
    string 'type;
    # Global GraphQL node identifier for this protection rule.
    string nodeId;
};

# Public User

type PublicUser record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # API URL template to check who the user is following
    string followingUrl;
    # The Twitter username of the user
    string? twitterUsername?;
    # The biography of the user
    string? bio;
    # The date the user account was created
    string createdAt;
    # The username of the user
    string login;
    # The type of the account
    string 'type;
    # The URL of the user's blog or website
    string? blog;
    # The number of private gists the user owns
    int privateGists?;
    # The total number of private repositories the user owns
    int totalPrivateRepos?;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # The date the user account was last updated
    string updatedAt;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The total disk usage across all repositories in kilobytes
    int diskUsage?;
    # The number of collaborators across the user's private repositories
    int collaborators?;
    # The company the user belongs to
    string? company;
    # The number of private repositories owned by the user
    int ownedPrivateRepos?;
    # The unique identifier of the user
    int id;
    # The number of public repositories the user owns
    int publicRepos;
    # The Gravatar ID of the user
    string? gravatarId;
    # The subscription plan associated with the user's account.
    PublicUserPlan plan?;
    # The publicly visible email address of the user
    string? email;
    # API URL to list the user's organizations
    string organizationsUrl;
    # Whether the user is available for hire
    boolean? hireable;
    # API URL template for repositories the user has starred
    string starredUrl;
    # API URL to list the user's followers
    string followersUrl;
    # The number of public gists the user owns
    int publicGists;
    # API URL for the user
    string url;
    # API URL for events received by the user
    string receivedEventsUrl;
    # The number of followers the user has
    int followers;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The number of users the user is following
    int following;
    # The display name of the user
    string? name;
    # The geographic location of the user
    string? location;
    # The GraphQL node identifier of the user
    string nodeId;
    # The date the user account was suspended
    string? suspendedAt?;
};

# The billing plan for the user

type PublicUserPlan record {
    # Number of private repositories allowed under this billing plan.
    int privateRepos;
    # The name of the billing plan.
    string name;
    # Number of collaborators allowed under this billing plan.
    int collaborators;
    # Amount of storage space allocated under this billing plan in bytes.
    int space;
};


type PullNumberCodespacesBody record {
    # The geographic area for this codespace. If not specified, the value is assigned by IP. This property replaces location, which is being deprecated
        "EuropeWest"|"SoutheastAsia"|"UsEast"|"UsWest"  geo?;
    # Path to devcontainer.json config to use for this codespace
    string devcontainerPath?;
    # Whether to authorize requested permissions from devcontainer.json
    boolean multiRepoPermissionsOptOut?;
    # Machine type to use for this codespace
    string machine?;
    # The requested location for a new codespace. Best efforts are made to respect this upon creation. Assigned by IP if not provided
    string location?;
    # IP for location auto-detection when proxying a request
    string clientIp?;
    # Working directory for this codespace
    string workingDirectory?;
    # Duration in minutes after codespace has gone idle in which it will be deleted. Must be integer minutes between 0 and 43200 (30 days)
    int retentionPeriodMinutes?;
    # Display name for this codespace
    string displayName?;
    # Time in minutes before codespace stops from inactivity
    int idleTimeoutMinutes?;
};


type PullNumberCommentsBody record {
    # The relative path to the file that necessitates a comment
    string path;
    # In a split diff view, the side of the diff that the pull request's changes appear on. Can be LEFT or RIGHT. Use LEFT for deletions that appear in red. Use RIGHT for additions that appear in green or unchanged lines that appear in white and are shown for context. For a multi-line comment, side represents whether the last line of the comment range is a deletion or addition. For more information, see "[Diff view options](https://docs.github.com/articles/about-comparing-branches-in-pull-requests#diff-view-options)" in the GitHub Help documentation
        "LEFT"|"RIGHT"  side?;
    # The level at which the comment is targeted
        "line"|"file"  subjectType?;
    # **Required unless using subject_type:file**. The line of the blob in the pull request diff that the comment applies to. For a multi-line comment, the last line of the range that your comment applies to
    int line?;
    # **Required when using multi-line comments unless using in_reply_to**. The start_line is the first line in the pull request diff that your multi-line comment applies to. To learn more about multi-line comments, see "[Commenting on a pull request](https://docs.github.com/articles/commenting-on-a-pull-request#adding-line-comments-to-a-pull-request)" in the GitHub Help documentation
    int startLine?;
    # **This parameter is deprecated. Use line instead**. The position in the diff where you want to add a review comment. Note this value is not the same as the line number in the file. For help finding the position value, read the note above
    int position?;
    # The text of the review comment
    string body;
    # The SHA of the commit needing a comment. Not using the latest commit SHA may render your comment outdated if a subsequent commit modifies the line you specify as the position
    string commitId;
    # **Required when using multi-line comments unless using in_reply_to**. The start_side is the starting side of the diff that the comment applies to. Can be LEFT or RIGHT. To learn more about multi-line comments, see "[Commenting on a pull request](https://docs.github.com/articles/commenting-on-a-pull-request#adding-line-comments-to-a-pull-request)" in the GitHub Help documentation. See side in this table for additional context
        "LEFT"|"RIGHT"|"side"  startSide?;
    # The ID of the review comment to reply to. To find the ID of a review comment with ["List review comments on a pull request"](#list-review-comments-on-a-pull-request). When specified, all parameters other than body in the request body are ignored
    int inReplyTo?;
};


type PullNumberMergeBody record {
    # Title for the automatic commit message
    string commitTitle?;
    # Extra detail to append to automatic commit message
    string commitMessage?;
    # SHA that pull request head must match to allow merge
    string sha?;
    # The merge method to use
        "merge"|"squash"|"rebase"  mergeMethod?;
};


type PullNumberRequestedReviewersBody1 record {
    # An array of team slugs that will be removed
    string[] teamReviewers?;
    # An array of user logins that will be removed
    string[] reviewers;
};


type PullNumberReviewsBody record {
    # Use the following table to specify the location, destination, and contents of the draft review comment
    ReposownerrepopullspullNumberreviewsComments[] comments?;
    # **Required** when using REQUEST_CHANGES or COMMENT for the event parameter. The body text of the pull request review
    string body?;
    # The review action you want to perform. The review actions include: APPROVE, REQUEST_CHANGES, or COMMENT. By leaving this blank, you set the review action state to PENDING, which means you will need to [submit the pull request review](https://docs.github.com/rest/pulls/reviews#submit-a-review-for-a-pull-request) when you are ready
        "APPROVE"|"REQUEST_CHANGES"|"COMMENT"  event?;
    # The SHA of the commit that needs a review. Not using the latest commit SHA may render your review comment outdated if a subsequent commit modifies the line you specify as the position. Defaults to the most recent commit in the pull request when you do not specify a value
    string commitId?;
};


type PullNumberUpdateBranchBody record {
    # The expected SHA of the pull request's HEAD ref. This is the most recent commit on the pull request's branch. If the expected SHA does not match the pull request's HEAD, you will receive a 422 Unprocessable Entity status. You can use the "[List commits](https://docs.github.com/rest/commits/commits#list-commits)" endpoint to find the most recent commit SHA. Default: SHA of the pull request's current HEAD ref
    string expectedHeadSha?;
};

# Pull requests let you tell others about changes you've pushed to a repository on GitHub. Once a pull request is sent, interested parties can review the set of changes, discuss potential modifications, and even push follow-up commits if necessary

type PullRequest record {
    # API URL for the associated issue
    string issueUrl;
    # Whether the pull request can be rebased
    boolean? rebaseable?;
    # Hypermedia links related to the pull request.
    PullRequestLinks links;
    # The number of lines deleted in the pull request
    int deletions;
    # URL of the pull request diff
    string diffUrl;
    # The date the pull request was created
    string createdAt;
    # The users assigned to the pull request
    SimpleUser[]? assignees?;
    # The users requested to review the pull request
    SimpleUser[]? requestedReviewers?;
    # The user who merged the pull request
    NullableSimpleUser? mergedBy;
    # The title of the pull request
    string title;
    # The body text of the pull request
    string? body;
    # The teams requested to review the pull request
    TeamSimple[]? requestedTeams?;
    # The head branch details of the pull request.
    PullRequestHead head;
    # The association of the author with the repository
    AuthorAssociation authorAssociation;
    # Number uniquely identifying the pull request within its repository
    int number;
    # Whether the pull request can be merged
    boolean? mergeable;
    # URL of the pull request patch
    string patchUrl;
    # The date the pull request was last updated
    string updatedAt;
    # Indicates whether or not the pull request is a draft
    boolean draft?;
    # API URL for the pull request comments
    string commentsUrl;
    # The SHA of the merge commit
    string? mergeCommitSha;
    # API URL template for a pull request review comment
    string reviewCommentUrl;
    # The reason the pull request conversation was locked
    string? activeLockReason?;
    # The unique identifier of the pull request
    int id;
    # State of this Pull Request. Either open or closed
        "open"|"closed"  state;
    # Whether the pull request is locked
    boolean locked;
    # The mergeability state of the pull request
    string mergeableState;
    # API URL for the pull request commits
    string commitsUrl;
    # The date the pull request was closed
    string? closedAt;
    # The number of comments on the pull request
    int comments;
    # The number of lines added in the pull request
    int additions;
    # API URL for the pull request commit statuses
    string statusesUrl;
    # The date the pull request was merged
    string? mergedAt;
    # Whether the pull request has been merged
    boolean merged;
    # The number of review comments on the pull request
    int reviewComments;
    # The auto-merge configuration for the pull request
    AutoMerge? autoMerge;
    # The number of files changed in the pull request
    int changedFiles;
    # API URL for the pull request
    string url;
    # The labels associated with the pull request
    PullRequestLabels[] labels;
    # The milestone associated with the pull request.
    NullableMilestone? milestone;
    # URL of the pull request page on GitHub
    string htmlUrl;
    # API URL for the pull request review comments
    string reviewCommentsUrl;
    # Indicates whether maintainers can modify the pull request
    boolean maintainerCanModify;
    # The number of commits in the pull request
    int commits;
    # The primary user assigned to the pull request.
    NullableSimpleUser? assignee;
    # The user who created the pull request.
    SimpleUser user;
    # The GraphQL node identifier of the pull request
    string nodeId;
    # The base branch details that the pull request targets.
    PullRequestBase base;
};

# The base branch of the pull request

type PullRequestBase record {
    # The branch name of the base branch.
    string ref;
    # The repository object for the base branch.
    PullRequestBaseRepo repo;
    # The label identifying the base branch with owner prefix.
    string label;
    # The SHA of the latest commit on the base branch.
    string sha;
    # The user associated with the base branch.
    PullRequestHeadRepoOwner user;
};


type PullRequestBaseRepo record {
    # Indicates whether forking of the repository is allowed.
    boolean allowForking?;
    # Indicates whether the repository is a template repository.
    boolean isTemplate?;
    # Number of users who have starred the repository.
    int stargazersCount;
    # Timestamp of the most recent push to the repository.
    string pushedAt;
    # API URL to manage subscription notifications for the repository.
    string subscriptionUrl;
    # The primary programming language of the repository.
    string? language;
    # API URL template for the repository's branches.
    string branchesUrl;
    # API URL template for issue comments in the repository.
    string issueCommentUrl;
    # Indicates whether rebase merging is allowed for pull requests.
    boolean allowRebaseMerge?;
    # API URL template for the repository's labels.
    string labelsUrl;
    # API URL listing subscribers watching the repository.
    string subscribersUrl;
    # The permissions the authenticated user has on this repository.
    RepoSearchResultItemPermissions permissions?;
    # Temporary token used for cloning the repository.
    string tempCloneToken?;
    # API URL template for the repository's releases.
    string releasesUrl;
    # The SVN URL for the repository.
    string svnUrl;
    # The unique numeric identifier for the repository.
    int id;
    # Indicates whether the repository has discussions enabled.
    boolean hasDiscussions;
    # The name of the repository's master branch.
    string masterBranch?;
    # Number of forks of the repository.
    int forks;
    # API URL template for accessing archived contents of the repository.
    string archiveUrl;
    # Indicates whether merge commits are allowed for pull requests.
    boolean allowMergeCommit?;
    # API URL template for the repository's Git refs.
    string gitRefsUrl;
    # API URL listing forks of the repository.
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility?;
    # API URL template for commit statuses in the repository.
    string statusesUrl;
    # The SSH URL used to clone the repository.
    string sshUrl;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # The full name of the repository including the owner, e.g., owner/repo.
    string fullName;
    # The size of the repository in kilobytes.
    int size;
    # API URL listing the languages used in the repository.
    string languagesUrl;
    # API URL template for the repository's collaborators.
    string collaboratorsUrl;
    # The URL to the repository's GitHub page.
    string htmlUrl;
    # The HTTPS URL used to clone the repository.
    string cloneUrl;
    # The name of the repository.
    string name;
    # API URL template for pull requests in the repository.
    string pullsUrl;
    # The default branch of the repository.
    string defaultBranch;
    # API URL for the repository's webhooks.
    string hooksUrl;
    # API URL template for Git trees in the repository.
    string treesUrl;
    # API URL for the repository's tags.
    string tagsUrl;
    # API URL listing contributors to the repository.
    string contributorsUrl;
    # Indicates whether the repository is private.
    boolean 'private;
    # Indicates whether the repository has downloads enabled.
    boolean hasDownloads;
    # API URL template for notifications related to the repository.
    string notificationsUrl;
    # Number of open issues in the repository.
    int openIssuesCount;
    # A short description of the repository.
    string? description;
    # Number of users watching the repository.
    int watchers;
    # Timestamp when the repository was created.
    string createdAt;
    # API URL for the repository's deployments.
    string deploymentsUrl;
    # API URL template for the repository's deploy keys.
    string keysUrl;
    # Indicates whether the repository has projects enabled.
    boolean hasProjects;
    # Indicates whether the repository is archived and read-only.
    boolean archived;
    # Indicates whether the repository has a wiki enabled.
    boolean hasWiki;
    # Timestamp when the repository was last updated.
    string updatedAt;
    # API URL template for comments in the repository.
    string commentsUrl;
    # API URL listing users who have starred the repository.
    string stargazersUrl;
    # Indicates whether the repository has been disabled.
    boolean disabled;
    # The Git URL used to clone the repository.
    string gitUrl;
    # Indicates whether the repository has GitHub Pages enabled.
    boolean hasPages;
    # The owner of the repository.
    PullRequestHeadRepoOwner owner;
    # Indicates whether squash merging is allowed for pull requests.
    boolean allowSquashMerge?;
    # API URL template for commits in the repository.
    string commitsUrl;
    # API URL template for comparing commits in the repository.
    string compareUrl;
    # API URL template for Git commits in the repository.
    string gitCommitsUrl;
    # List of topic tags associated with the repository.
    string[] topics?;
    # API URL template for Git blobs in the repository.
    string blobsUrl;
    # API URL template for Git tags in the repository.
    string gitTagsUrl;
    # API URL for merges in the repository.
    string mergesUrl;
    # API URL for the repository's downloads.
    string downloadsUrl;
    # Indicates whether the repository has issues enabled.
    boolean hasIssues;
    # Indicates whether web-based commits must include a sign-off.
    boolean webCommitSignoffRequired?;
    # The API URL for the repository resource.
    string url;
    # API URL template for the repository's file contents.
    string contentsUrl;
    # The URL of the source repository if this is a mirror.
    string? mirrorUrl;
    # API URL template for the repository's milestones.
    string milestonesUrl;
    # API URL listing teams with access to the repository.
    string teamsUrl;
    # Indicates whether the repository is a fork.
    boolean 'fork;
    # API URL template for issues in the repository.
    string issuesUrl;
    # API URL for events related to the repository.
    string eventsUrl;
    # API URL for events related to issues in the repository.
    string issueEventsUrl;
    # API URL template for assignees in the repository.
    string assigneesUrl;
    # Number of currently open issues and pull requests.
    int openIssues;
    # Number of users watching the repository for notifications.
    int watchersCount;
    # The GraphQL node ID for the repository.
    string nodeId;
    # Total number of forks of the repository.
    int forksCount;
    # The URL of the repository's associated website or homepage.
    string? homepage;
};

# The head branch of the pull request

type PullRequestHead record {
    # The branch name of the head ref.
    string ref;
    # The repository containing the head branch.
    PullRequestHeadRepo? repo;
    # The label identifying the head branch, including owner prefix.
    string label;
    # The SHA of the head commit.
    string sha;
    # The user associated with the head branch.
    PullRequestHeadRepoOwner user;
};


type PullRequestHeadRepo record {
    # Indicates whether forking this repository is allowed.
    boolean allowForking?;
    # The number of users who have starred the repository.
    int stargazersCount;
    # Indicates whether this repository is a template repository.
    boolean isTemplate?;
    # Timestamp of the most recent push to the repository.
    string pushedAt;
    # API URL for managing repository subscriptions.
    string subscriptionUrl;
    # The primary programming language used in the repository.
    string? language;
    # API URL template for accessing repository branches.
    string branchesUrl;
    # API URL template for accessing issue comments.
    string issueCommentUrl;
    # Indicates whether rebase merging is allowed for pull requests.
    boolean allowRebaseMerge?;
    # API URL template for accessing repository labels.
    string labelsUrl;
    # API URL for listing repository subscribers.
    string subscribersUrl;
    # The permissions the current user has on this repository.
    RepoSearchResultItemPermissions permissions?;
    # Temporary token used for cloning the repository.
    string tempCloneToken?;
    # API URL template for accessing repository releases.
    string releasesUrl;
    # The SVN-compatible URL for the repository.
    string svnUrl;
    # The unique numeric identifier of the repository.
    int id;
    # Indicates whether the repository has discussions enabled.
    boolean hasDiscussions;
    # The name of the master branch of the repository.
    string masterBranch?;
    # The number of forks of the repository.
    int forks;
    # API URL template for downloading repository archives.
    string archiveUrl;
    # Indicates whether merge commits are allowed for pull requests.
    boolean allowMergeCommit?;
    # API URL template for accessing git refs.
    string gitRefsUrl;
    # API URL for listing repository forks.
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility?;
    # API URL template for accessing commit statuses.
    string statusesUrl;
    # The SSH URL used to clone the repository.
    string sshUrl;
    # The license associated with the repository.
    PullRequestHeadRepoLicense? license;
    # The full name of the repository, including owner and repo name.
    string fullName;
    # The size of the repository in kilobytes.
    int size;
    # API URL for listing the programming languages used.
    string languagesUrl;
    # API URL template for accessing repository collaborators.
    string collaboratorsUrl;
    # The HTML URL for viewing the repository on GitHub.
    string htmlUrl;
    # The HTTPS URL used to clone the repository.
    string cloneUrl;
    # The name of the repository.
    string name;
    # API URL template for accessing pull requests.
    string pullsUrl;
    # The default branch of the repository.
    string defaultBranch;
    # API URL for managing repository webhooks.
    string hooksUrl;
    # API URL template for accessing git trees.
    string treesUrl;
    # API URL for listing repository tags.
    string tagsUrl;
    # API URL for listing repository contributors.
    string contributorsUrl;
    # Indicates whether the repository is private.
    boolean 'private;
    # Indicates whether the repository has downloads enabled.
    boolean hasDownloads;
    # API URL template for accessing repository notifications.
    string notificationsUrl;
    # The number of open issues in the repository.
    int openIssuesCount;
    # A short description of the repository.
    string? description;
    # The number of users watching the repository.
    int watchers;
    # Timestamp when the repository was created.
    string createdAt;
    # API URL for listing repository deployments.
    string deploymentsUrl;
    # API URL template for accessing deploy keys.
    string keysUrl;
    # Indicates whether the repository has projects enabled.
    boolean hasProjects;
    # Indicates whether the repository has been archived.
    boolean archived;
    # Indicates whether the repository has a wiki enabled.
    boolean hasWiki;
    # Timestamp when the repository was last updated.
    string updatedAt;
    # API URL template for accessing repository comments.
    string commentsUrl;
    # API URL for listing users who starred the repository.
    string stargazersUrl;
    # Indicates whether the repository has been disabled.
    boolean disabled;
    # The Git URL used to clone the repository.
    string gitUrl;
    # Indicates whether the repository has GitHub Pages enabled.
    boolean hasPages;
    # The user or organization that owns the repository.
    PullRequestHeadRepoOwner owner;
    # Indicates whether squash merging is allowed for pull requests.
    boolean allowSquashMerge?;
    # API URL template for accessing repository commits.
    string commitsUrl;
    # API URL template for comparing two refs in the repository.
    string compareUrl;
    # API URL template for accessing git commit objects.
    string gitCommitsUrl;
    # List of topics associated with the repository.
    string[] topics?;
    # API URL template for accessing git blob objects.
    string blobsUrl;
    # API URL template for accessing git tag objects.
    string gitTagsUrl;
    # API URL for performing repository merges.
    string mergesUrl;
    # API URL for listing repository downloads.
    string downloadsUrl;
    # Indicates whether the repository has issues enabled.
    boolean hasIssues;
    # Indicates whether web commits must be signed off.
    boolean webCommitSignoffRequired?;
    # The API URL for the repository.
    string url;
    # API URL template for accessing repository contents.
    string contentsUrl;
    # The URL this repository mirrors, if applicable.
    string? mirrorUrl;
    # API URL template for accessing repository milestones.
    string milestonesUrl;
    # API URL for listing teams with access to the repository.
    string teamsUrl;
    # Indicates whether this repository is a fork.
    boolean 'fork;
    # API URL template for accessing repository issues.
    string issuesUrl;
    # API URL for listing repository events.
    string eventsUrl;
    # API URL for listing issue events in the repository.
    string issueEventsUrl;
    # API URL template for accessing possible issue assignees.
    string assigneesUrl;
    # The number of open issues and pull requests.
    int openIssues;
    # The number of users watching the repository.
    int watchersCount;
    # The GraphQL node ID of the repository.
    string nodeId;
    # The number of forks of the repository.
    int forksCount;
    # The URL of the repository's homepage.
    string? homepage;
};


type PullRequestHeadRepoLicense record {
    # The full name of the license.
    string name;
    # The SPDX identifier for the license.
    string? spdxId;
    # The unique key identifying the license.
    string 'key;
    # API URL for the license details.
    string? url;
    # The GraphQL node identifier for the license.
    string nodeId;
};


type PullRequestHeadRepoOwner record {
    # API URL template for the owner's gists.
    string gistsUrl;
    # API URL for listing the owner's repositories.
    string reposUrl;
    # API URL template for users the owner is following.
    string followingUrl;
    # API URL template for repositories the owner has starred.
    string starredUrl;
    # API URL for listing the owner's followers.
    string followersUrl;
    # The username of the repository owner.
    string login;
    # The type of account, such as User or Organization.
    string 'type;
    # API URL for the owner's account.
    string url;
    # API URL for the owner's watched repositories.
    string subscriptionsUrl;
    # API URL for events received by the owner.
    string receivedEventsUrl;
    # URL of the owner's profile avatar image.
    string avatarUrl;
    # API URL template for events performed by the owner.
    string eventsUrl;
    # URL of the owner's profile page on GitHub.
    string htmlUrl;
    # Whether the owner is a GitHub site administrator.
    boolean siteAdmin;
    # The unique identifier of the owner.
    int id;
    # The Gravatar ID associated with the owner's email.
    string? gravatarId;
    # The GraphQL node identifier of the owner.
    string nodeId;
    # API URL for listing the owner's organizations.
    string organizationsUrl;
};


type PullRequestLabels record {
    # Indicates whether this is a default label for the repository.
    boolean default;
    # The hexadecimal color code for the label.
    string color;
    # The name of the label.
    string name;
    # A short description of the label.
    string? description;
    # The unique numeric identifier of the label.
    int id;
    # The URL of the label resource.
    string url;
    # The GraphQL node identifier for the label.
    string nodeId;
};

# Hypermedia links for the pull request

type PullRequestLinks record {
    # Hypermedia link to the pull request's comments.
    Link comments;
    # Hypermedia link to the issue associated with the pull request.
    Link issue;
    # Hypermedia link to the pull request's commits.
    Link commits;
    # Hypermedia link to the pull request's commit statuses.
    Link statuses;
    # Hypermedia link to the pull request's review comments.
    Link reviewComments;
    # Hypermedia link to the pull request itself.
    Link self;
    # Hypermedia link to the pull request's HTML page.
    Link html;
    # Hypermedia link to a single review comment resource.
    Link reviewComment;
};

# Pull Request Merge Result

type PullRequestMergeResult record {
    # Whether the pull request was successfully merged.
    boolean merged;
    # A message describing the result of the merge.
    string message;
    # The SHA of the merge commit.
    string sha;
};


type PullRequestMinimal record {
    # The head branch information for the pull request.
    PullRequestMinimalHead head;
    # The pull request number within the repository.
    int number;
    # Unique numeric identifier for the pull request.
    int id;
    # API URL for this pull request.
    string url;
    # The base branch information for the pull request.
    PullRequestMinimalHead base;
};


type PullRequestMinimalHead record {
    # The name of the head branch.
    string ref;
    # The repository containing the head branch.
    PullRequestMinimalHeadRepo repo;
    # The SHA of the head commit.
    string sha;
};


type PullRequestMinimalHeadRepo record {
    # The name of the repository.
    string name;
    # The unique identifier of the repository.
    int id;
    # API URL for the repository.
    string url;
};

# Pull Request Reviews are reviews on pull requests

type PullRequestReview record {
    # The HTML-rendered body of the review
    string bodyHtml?;
    # Hypermedia links related to the pull request review.
    PullRequestReviewLinks links;
    # The date the review was submitted
    string submittedAt?;
    # The plain text body of the review
    string bodyText?;
    # API URL for the reviewed pull request
    string pullRequestUrl;
    # The text of the review
    string body;
    # The association of the reviewer with the repository
    AuthorAssociation authorAssociation;
    # URL of the review on GitHub
    string htmlUrl;
    # Unique identifier of the review
    int id;
    # The state of the review
    string state;
    # The user who submitted the pull request review.
    NullableSimpleUser? user;
    # A commit SHA for the review. If the commit object was garbage collected or forcibly deleted, then it no longer exists in Git and this value will be null
    string? commitId;
    # The GraphQL node identifier of the review
    string nodeId;
};

# Pull Request Review Comments are comments on a portion of the Pull Request's diff

type PullRequestReviewComment record {
    # The comment body rendered as HTML.
    string bodyHtml?;
    # The SHA of the original commit to which the comment applies
    string originalCommitId;
    # Hypermedia links related to the pull request review comment.
    PullRequestReviewCommentLinks links;
    # The comment body rendered as plain text.
    string bodyText?;
    # The comment ID to reply to
    int inReplyToId?;
    # The line of the blob to which the comment applies. The last line of the range for a multi-line comment
    int line?;
    # The diff of the line that the comment refers to
    string diffHunk;
    # The date and time the comment was created.
    string createdAt;
    # The first line of the range for a multi-line comment
    int? startLine?;
    # The text of the comment
    string body;
    # The commenter's association with the repository.
    AuthorAssociation authorAssociation;
    # The relative path of the file to which the comment applies
    string path;
    # The index of the original line in the diff to which the comment applies. This field is deprecated; use original_line instead
    int originalPosition?;
    # The date and time the comment was last updated.
    string updatedAt;
    # The ID of the pull request review to which the comment belongs
    int? pullRequestReviewId;
    # The ID of the pull request review comment
    int id;
    # The side of the diff to which the comment applies. The side of the last line of the range for a multi-line comment
        "LEFT"|"RIGHT"  side = "RIGHT";
    # The level at which the comment is targeted, can be a diff line or a file
        "line"|"file"  subjectType?;
    # URL for the pull request that the review comment belongs to
    string pullRequestUrl;
    # URL for the pull request review comment
    string url;
    # The side of the first line of the range for a multi-line comment
        "LEFT"|"RIGHT"? startSide = "RIGHT";
    # The line of the blob to which the comment applies. The last line of the range for a multi-line comment
    int originalLine?;
    # The first line of the range for a multi-line comment
    int? originalStartLine?;
    # HTML URL for the pull request review comment
    string htmlUrl;
    # Reaction rollup totals for the pull request review comment.
    ReactionRollup reactions?;
    # The line index in the diff to which the comment applies. This field is deprecated; use line instead
    int position?;
    # The SHA of the commit to which the comment applies
    string commitId;
    # The user who created the pull request review comment.
    SimpleUser user;
    # The node ID of the pull request review comment
    string nodeId;
};


type PullRequestReviewCommentLinks record {
    # Hypermedia link to the associated pull request.
    PullRequestReviewCommentLinksPullRequest pullRequest;
    # Hypermedia link to the review comment itself.
    PullRequestReviewCommentLinksSelf self;
    # Hypermedia link to the HTML page of the review comment.
    PullRequestReviewCommentLinksHtml html;
};


type PullRequestReviewCommentLinksHtml record {
    # URL to the pull request review comment on GitHub.
    string href;
};


type PullRequestReviewCommentLinksPullRequest record {
    # The URL of the pull request associated with this review comment.
    string href;
};


type PullRequestReviewCommentLinksSelf record {
    # The URL of the pull request review comment itself.
    string href;
};

# Hypermedia links for the review

type PullRequestReviewLinks record {
    # Hypermedia link to the pull request the review belongs to.
    TimelineReviewedEventLinksHtml pullRequest;
    # Hypermedia link to the HTML page of the pull request review.
    TimelineReviewedEventLinksHtml html;
};

# Pull Request Review Request

type PullRequestReviewRequest record {
    # The list of teams requested to review the pull request.
    Team[] teams;
    # The list of users requested to review the pull request.
    SimpleUser[] users;
};

# Pull Request Simple

type PullRequestSimple record {
    # API URL for the associated issue
    string issueUrl;
    # Hypermedia links related to the pull request.
    PullRequestLinks links;
    # URL of the pull request diff
    string diffUrl;
    # The date the pull request was created
    string createdAt;
    # The users assigned to the pull request
    SimpleUser[]? assignees?;
    # The users requested to review the pull request
    SimpleUser[]? requestedReviewers?;
    # The title of the pull request
    string title;
    # The body text of the pull request
    string? body;
    # The teams requested to review the pull request
    Team[]? requestedTeams?;
    # The head branch details of the pull request.
    PullRequestSimpleHead head;
    # The association of the author with the repository
    AuthorAssociation authorAssociation;
    # The pull request number
    int number;
    # URL of the pull request patch
    string patchUrl;
    # The date the pull request was last updated
    string updatedAt;
    # Indicates whether or not the pull request is a draft
    boolean draft?;
    # API URL for the pull request comments
    string commentsUrl;
    # The SHA of the merge commit
    string? mergeCommitSha;
    # API URL template for a pull request review comment
    string reviewCommentUrl;
    # The reason the pull request conversation was locked
    string? activeLockReason?;
    # The unique identifier of the pull request
    int id;
    # The state of the pull request
    string state;
    # Whether the pull request is locked
    boolean locked;
    # API URL for the pull request commits
    string commitsUrl;
    # The date the pull request was closed
    string? closedAt;
    # API URL for the pull request commit statuses
    string statusesUrl;
    # The date the pull request was merged
    string? mergedAt;
    # The auto-merge configuration for the pull request
    AutoMerge? autoMerge;
    # API URL for the pull request
    string url;
    # The labels associated with the pull request
    PullRequestLabels[] labels;
    # The milestone associated with the pull request.
    NullableMilestone? milestone;
    # URL of the pull request page on GitHub
    string htmlUrl;
    # API URL for the pull request review comments
    string reviewCommentsUrl;
    # The primary user assigned to the pull request.
    NullableSimpleUser? assignee;
    # The user who created the pull request.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the pull request
    string nodeId;
    # The base branch details of the pull request.
    PullRequestSimpleBase base;
};

# The base branch of the pull request

type PullRequestSimpleBase record {
    # The branch name of the base branch.
    string ref;
    # The repository that the base branch belongs to.
    Repository repo;
    # The label identifying the base branch with its owner.
    string label;
    # The SHA of the latest commit on the base branch.
    string sha;
    # The user associated with the base branch.
    NullableSimpleUser? user;
};

# The head branch of the pull request

type PullRequestSimpleHead record {
    # The branch name of the head ref.
    string ref;
    # The repository containing the head branch.
    Repository repo;
    # The label identifying the head in user:branch format.
    string label;
    # The SHA of the head commit.
    string sha;
    # The user associated with the head branch.
    NullableSimpleUser? user;
};

# Represents the Queries record for the operation: pulls/list-comments-for-review

type PullsListCommentsForReviewQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: pulls/list-commits

type PullsListCommitsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: pulls/list-files

type PullsListFilesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: pulls/list

type PullsListQueries record {
    # Filter pulls by head user or head organization and branch name in the format of user:ref-name or organization:ref-name. For example: github:new-script-format or octocat:test-branch
    string head?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Either open, closed, or all to filter by state
        "open"|"closed"|"all"  state = "open";
    # What to sort results by. popularity will sort by the number of comments. long-running will sort by date created and will limit the results to pull requests that have been open for more than a month and have had activity within the past month
        "created"|"updated"|"popularity"|"long-running"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # Filter pulls by base branch name. Example: gh-pages
    string base?;
    # The direction of the sort. Default: desc when sort is created or sort is not specified, otherwise asc
        "asc"|"desc"  direction?;
};

# Represents the Queries record for the operation: pulls/list-review-comments-for-repo

type PullsListReviewCommentsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort results by.
        "created"|"updated"|"created_at"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort results. Ignored without sort parameter
        "asc"|"desc"  direction?;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: pulls/list-review-comments

type PullsListReviewCommentsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort results. Ignored without sort parameter
        "asc"|"desc"  direction?;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: pulls/list-reviews

type PullsListReviewsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type PullspullNumberBody record {
    # Indicates whether [maintainers can modify](https://docs.github.com/articles/allowing-changes-to-a-pull-request-branch-created-from-a-fork/) the pull request
    boolean maintainerCanModify?;
    # State of this Pull Request. Either open or closed
        "open"|"closed"  state?;
    # The title of the pull request
    string title?;
    # The contents of the pull request
    string body?;
    # The name of the branch you want your changes pulled into. This should be an existing branch on the current repository. You cannot update the base branch on a pull request to point to another repository
    string base?;
};


type RateLimit record {
    # The maximum number of requests allowed per rate limit window.
    int 'limit;
    # The Unix timestamp when the rate limit window resets.
    int reset;
    # The number of requests used in the current rate limit window.
    int used;
    # The number of requests remaining in the current rate limit window.
    int remaining;
};

# Rate Limit Overview

type RateLimitOverview record {
    # The core rate limit data for the authenticated user.
    RateLimit rate;
    # Rate limit data broken down by resource type.
    RateLimitOverviewResources resources;
};


type RateLimitOverviewResources record {
    # Rate limit information for core API requests.
    RateLimit core;
    # Rate limit information for SCIM API requests.
    RateLimit scim?;
    # Rate limit information for search API requests.
    RateLimit search;
    # Rate limit information for source import API requests.
    RateLimit sourceImport?;
    # Rate limit information for Actions runner registration requests.
    RateLimit actionsRunnerRegistration?;
    # Rate limit information for GraphQL API requests.
    RateLimit graphql?;
    # Rate limit information for code scanning upload requests.
    RateLimit codeScanningUpload?;
    # Rate limit information for integration manifest API requests.
    RateLimit integrationManifest?;
    # Rate limit information for code search API requests.
    RateLimit codeSearch?;
    # Rate limit information for dependency snapshot API requests.
    RateLimit dependencySnapshots?;
};

# Reactions to conversations provide a way to help people express their feelings more simply and effectively

type Reaction record {
    # The date and time the reaction was created.
    string createdAt;
    # The unique identifier of the reaction.
    int id;
    # The user who created the reaction.
    NullableSimpleUser? user;
    # The reaction to use
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content;
    # The GraphQL node identifier of the reaction.
    string nodeId;
};


type ReactionRollup record {
    # Count of confused reactions on the subject.
    int confused;
    # Count of thumbs-down reactions on the subject.
    int minus1;
    # Total number of reactions on the subject.
    int totalCount;
    # Count of thumbs-up reactions on the subject.
    int plus1;
    # Count of rocket reactions on the subject.
    int rocket;
    # Count of hooray reactions on the subject.
    int hooray;
    # Count of eyes reactions on the subject.
    int eyes;
    # The API URL for the reactions resource.
    string url;
    # Count of laugh reactions on the subject.
    int laugh;
    # Count of heart reactions on the subject.
    int heart;
};

# Represents the Queries record for the operation: reactions/list-for-commit-comment

type ReactionsListForCommitCommentQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a commit comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-issue-comment

type ReactionsListForIssueCommentQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to an issue comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-issue

type ReactionsListForIssueQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to an issue
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-pull-request-review-comment

type ReactionsListForPullRequestReviewCommentQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a pull request review comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-release

type ReactionsListForReleaseQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a release
        "+1"|"laugh"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-team-discussion-comment-in-org

type ReactionsListForTeamDiscussionCommentInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a team discussion comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-team-discussion-comment-legacy

type ReactionsListForTeamDiscussionCommentLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a team discussion comment
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-team-discussion-in-org

type ReactionsListForTeamDiscussionInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a team discussion
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# Represents the Queries record for the operation: reactions/list-for-team-discussion-legacy

type ReactionsListForTeamDiscussionLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Returns a single [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions). Omit this parameter to list all reactions to a team discussion
        "+1"|"-1"|"laugh"|"confused"|"heart"|"hooray"|"rocket"|"eyes"  content?;
};

# A workflow referenced/reused by the initial caller workflow

type ReferencedWorkflow record {
    # The file path of the referenced workflow within the repository.
    string path;
    # The ref (branch or tag) of the referenced workflow.
    string ref?;
    # The commit SHA of the referenced workflow.
    string sha;
};

# Referrer Traffic

type ReferrerTraffic record {
    # The URL or domain of the referring site.
    string referrer;
    # The total number of views from this referrer.
    int count;
    # The number of unique visitors from this referrer.
    int uniques;
};


type RefsrefBody record {
    # Indicates whether to force the update or to make sure the update is a fast-forward update. Leaving this out or setting it to false will make sure you're not overwriting work
    boolean force = false;
    # The SHA1 value to set this reference to
    string sha;
};

# A release

type Release record {
    # The HTML-rendered description of the release
    string bodyHtml?;
    # The plain text description of the release
    string bodyText?;
    # The name of the tag
    string tagName;
    # The user who authored the release.
    SimpleUser author;
    # The date the release was created
    string createdAt;
    # The number of mentions found in the release body
    int mentionsCount?;
    # The description body of the release
    string? body?;
    # API URL for the release
    string url;
    # API URL for the release assets
    string assetsUrl;
    # The list of assets attached to the release
    ReleaseAsset[] assets;
    # Whether to identify the release as a prerelease or a full release
    boolean prerelease;
    # URL of the release page on GitHub
    string htmlUrl;
    # URL to download the release source as a zip file
    string? zipballUrl;
    # Specifies the commitish value that determines where the Git tag is created from
    string targetCommitish;
    # true to create a draft (unpublished) release, false to create a published one
    boolean draft;
    # The name of the release
    string? name;
    # URL template for uploading release assets
    string uploadUrl;
    # Reaction counts for the release.
    ReactionRollup reactions?;
    # The unique identifier of the release
    int id;
    # URL to download the release source as a tarball
    string? tarballUrl;
    # The date the release was published
    string? publishedAt;
    # The GraphQL node identifier of the release
    string nodeId;
    # The URL of the release discussion
    string discussionUrl?;
};

# Data related to a release

type ReleaseAsset record {
    # The date and time the release asset was created.
    string createdAt;
    # The URL to download the release asset via a browser.
    string browserDownloadUrl;
    # An optional display label for the release asset.
    string? label;
    # API URL for the release asset.
    string url;
    # The number of times the release asset has been downloaded.
    int downloadCount;
    # The MIME type of the release asset.
    string contentType;
    # The size of the release asset in bytes.
    int size;
    # The date and time the release asset was last updated.
    string updatedAt;
    # The user who uploaded the release asset.
    NullableSimpleUser? uploader;
    # The file name of the asset
    string name;
    # The unique identifier of the release asset.
    int id;
    # State of the release asset
        "uploaded"|"open"  state;
    # The GraphQL node identifier of the release asset.
    string nodeId;
};


type ReleaseIdReactionsBody record {
    # The [reaction type](https://docs.github.com/rest/reactions/reactions#about-reactions) to add to the release
        "+1"|"laugh"|"heart"|"hooray"|"rocket"|"eyes"  content;
};

# Generated name and body describing a release

type ReleaseNotesContent record {
    # The generated name of the release
    string name;
    # The generated body describing the contents of the release supporting markdown formatting
    string body;
};


type ReleasesGenerateNotesBody record {
    # The tag name for the release. This can be an existing tag or a new one
    string tagName;
    # Specifies the commitish value that will be the target for the release's tag. Required if the supplied tag_name does not reference an existing tag. Ignored if the tag_name already exists
    string targetCommitish?;
    # The name of the previous tag to use as the starting point for the release notes. Use to manually specify the range for the set of changes considered as part this release
    string previousTagName?;
    # Specifies a path to a file in the repository containing configuration settings used for generating the release notes. If unspecified, the configuration file located in the repository at '.github/release.yml' or '.github/release.yaml' will be used. If that is not present, the default configuration will be used
    string configurationFilePath?;
};


type ReleasesreleaseIdBody record {
    # If specified, a discussion of the specified category is created and linked to the release. The value must be a category that already exists in the repository. If there is already a discussion linked to the release, this parameter is ignored. For more information, see "[Managing categories for discussions in your repository](https://docs.github.com/discussions/managing-discussions-for-your-community/managing-categories-for-discussions-in-your-repository)."
    string discussionCategoryName?;
    # Specifies whether this release should be set as the latest release for the repository. Drafts and prereleases cannot be set as latest. Defaults to true for newly published releases. legacy specifies that the latest release should be determined based on the release creation date and higher semantic version
        "true"|"false"|"legacy"  makeLatest = "true";
    # The name of the tag
    string tagName?;
    # true to identify the release as a prerelease, false to identify the release as a full release
    boolean prerelease?;
    # Specifies the commitish value that determines where the Git tag is created from. Can be any branch or commit SHA. Unused if the Git tag already exists. Default: the repository's default branch
    string targetCommitish?;
    # true makes the release a draft, and false publishes the release
    boolean draft?;
    # The name of the release
    string name?;
    # Text describing the contents of the tag
    string body?;
};

# Removed from Project Issue Event

type RemovedFromProjectIssueEvent record {
    # The user who triggered the removed-from-project event.
    SimpleUser actor;
    # The API URL of the commit associated with this event.
    string? commitUrl;
    # The GitHub App that performed this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The timestamp when this event was created.
    string createdAt;
    # The unique identifier of this issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL for this issue event.
    string url;
    # The project card from which the issue was removed.
    RemovedFromProjectIssueEventProjectCard projectCard?;
    # The global node ID of this issue event.
    string nodeId;
};


type RemovedFromProjectIssueEventProjectCard record {
    # The unique identifier of the project the card belonged to.
    int projectId;
    # The name of the column from which the card was removed.
    string columnName;
    # The URL of the project the card belonged to.
    string projectUrl;
    # The unique identifier of the project card.
    int id;
    # The name of the column the card was in before removal.
    string previousColumnName?;
    # The API URL for the project card.
    string url;
};

# Renamed Issue Event

type RenamedIssueEvent record {
    # The user who triggered the rename event.
    SimpleUser actor;
    # The API URL of the commit associated with this event.
    string? commitUrl;
    # The GitHub App that performed this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The old and new title values from the rename action.
    RenamedIssueEventRename rename;
    # The date and time when the rename event occurred.
    string createdAt;
    # The unique identifier of the issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL for this issue event.
    string url;
    # The GraphQL node identifier for this event.
    string nodeId;
};


type RenamedIssueEventRename record {
    # The previous title of the issue before renaming.
    string 'from;
    # The new title of the issue after renaming.
    string to;
};


type RepoAutolinksBody record {
    # This prefix appended by certain characters will generate a link any time it is found in an issue, pull request, or commit
    string keyPrefix;
    # The URL must contain <num> for the reference number. <num> matches different characters depending on the value of is_alphanumeric
    string urlTemplate;
    # Whether this autolink reference matches alphanumeric characters. If true, the <num> parameter of the url_template matches alphanumeric characters A-Z (case insensitive), 0-9, and -. If false, this autolink reference only matches numeric characters
    boolean isAlphanumeric = true;
};


type RepoCheckSuitesBody record {
    # The sha of the head commit
    string headSha;
};


type RepoCodespacesBody record {
    # The geographic area for this codespace. If not specified, the value is assigned by IP. This property replaces location, which is being deprecated
        "EuropeWest"|"SoutheastAsia"|"UsEast"|"UsWest"  geo?;
    # Path to devcontainer.json config to use for this codespace
    string devcontainerPath?;
    # Git ref (typically a branch name) for this codespace
    string ref?;
    # Whether to authorize requested permissions from devcontainer.json
    boolean multiRepoPermissionsOptOut?;
    # Machine type to use for this codespace
    string machine?;
    # The requested location for a new codespace. Best efforts are made to respect this upon creation. Assigned by IP if not provided
    string location?;
    # IP for location auto-detection when proxying a request
    string clientIp?;
    # Working directory for this codespace
    string workingDirectory?;
    # Duration in minutes after codespace has gone idle in which it will be deleted. Must be integer minutes between 0 and 43200 (30 days)
    int retentionPeriodMinutes?;
    # Display name for this codespace
    string displayName?;
    # Time in minutes before codespace stops from inactivity
    int idleTimeoutMinutes?;
};

# Set repository secrets for GitHub Codespaces

type RepoCodespacesSecret record {
    # The date and time the secret was last updated.
    string updatedAt;
    # The name of the secret
    string name;
    # The date and time the secret was created.
    string createdAt;
};

# Set repository secrets for GitHub Codespaces

type RepoCodespacesSecretResponse record {
    # The total number of Codespaces secrets for the repository.
    int totalCount;
    # The list of Codespaces secrets for the repository.
    RepoCodespacesSecret[] secrets;
};


type RepoDeploymentsBody record {
    # The ref to deploy. This can be a branch, tag, or SHA
    string ref;
    # Name for the target deployment environment (e.g., production, staging, qa)
    string environment = "production";
    # Specifies a task to execute (e.g., deploy or deploy:migrations)
    string task = "deploy";
    # The [status](https://docs.github.com/rest/commits/statuses) contexts to verify against commit status checks. If you omit this parameter, GitHub verifies all unique contexts before creating a deployment. To bypass checking entirely, pass an empty array. Defaults to all unique contexts
    string[] requiredContexts?;
    # Optional JSON payload with extra information about the deployment.
    record {}|string payload?;
    # Specifies if the given environment is specific to the deployment and will no longer exist at some point in the future. Default: false
    boolean transientEnvironment = false;
    # Short description of the deployment
    string? description = "";
    # Attempts to automatically merge the default branch into the requested ref, if it's behind the default branch
    boolean autoMerge = true;
    # Specifies if the given environment is one that end-users directly interact with. Default: true when environment is production and false otherwise
    boolean productionEnvironment?;
};


type RepoDispatchesBody record {
    # A custom webhook event name. Must be 100 characters or fewer
    string eventType;
    # JSON payload with extra information about the webhook event that your action or workflow may use. The maximum number of top-level properties is 10
    record {} clientPayload?;
};


type RepoForksBody record {
    # Optional parameter to specify the organization name if forking into an organization
    string organization?;
    # When forking from an existing repository, a new name for the fork
    string name?;
    # When forking from an existing repository, fork with only the default branch
    boolean defaultBranchOnly?;
};


type RepoHooksBody record {
    # Use web to create a webhook. Default: web. This parameter only accepts the value web
    string name?;
    # Determines if notifications are sent when the webhook is triggered. Set to true to send notifications
    boolean active = true;
    # Configuration object defining the webhook URL, content type, and secret.
    ReposownerrepohooksConfig config?;
    # Determines what [events](https://docs.github.com/webhooks/event-payloads) the hook is triggered for
    string[] events = ["push"];
};


type RepoImportBody record {
    # If authentication is required, the username to provide to vcs_url
    string vcsUsername?;
    # If authentication is required, the password to provide to vcs_url
    string vcsPassword?;
    # The URL of the originating repository
    string vcsUrl;
    # For a tfvc import, the name of the project that is being imported
    string tfvcProject?;
    # The originating VCS type. Without this parameter, the import job will take additional time to detect the VCS type before beginning the import. This detection step will be reflected in the response
        "subversion"|"git"|"mercurial"|"tfvc"  vcs?;
};


type RepoImportBody1 record {
    # The username to provide to the originating repository
    string vcsUsername?;
    # The password to provide to the originating repository
    string vcsPassword?;
    # For a tfvc import, the name of the project that is being imported
    string tfvcProject?;
    # The type of version control system you are migrating from
        "subversion"|"tfvc"|"git"|"mercurial"  vcs?;
};


type RepoIssuesBody record {
    # The milestone to associate with the issue.
    string|int? milestone?;
    # Logins for Users to assign to this issue. _NOTE: Only users with push access can set assignees for new issues. Assignees are silently dropped otherwise._
    string[] assignees?;
    # Login for the user that this issue should be assigned to. _NOTE: Only users with push access can set the assignee for new issues. The assignee is silently dropped otherwise. **This field is deprecated.**_
    string? assignee?;
    # The title of the issue
    string|int title;
    # The contents of the issue
    string body?;
    # Labels to associate with this issue. _NOTE: Only users with push access can set labels for new issues. Labels are silently dropped otherwise._
    ReposownerrepoissuesLabels[] labels?;
};


type RepoKeysBody record {
    # If true, the key will only be able to read repository contents. Otherwise, the key will be able to read and write.  

Deploy keys with write access can perform the same actions as an organization member with admin access, or a collaborator on a personal repository. For more information, see "[Repository permission levels for an organization](https://docs.github.com/articles/repository-permission-levels-for-an-organization/)" and "[Permission levels for a user account repository](https://docs.github.com/articles/permission-levels-for-a-user-account-repository/)."
    boolean readOnly?;
    # A name for the key
    string title?;
    # The contents of the key
    string 'key;
};


type RepoLabelsBody record {
    # The [hexadecimal color code](http://www.color-hex.com/) for the label, without the leading #
    string color?;
    # The name of the label. Emoji can be added to label names, using either native emoji or colon-style markup. For example, typing :strawberry: will render the emoji ![:strawberry:](https://github.githubassets.com/images/icons/emoji/unicode/1f353.png ":strawberry:"). For a full list of available emoji and codes, see "[Emoji cheat sheet](https://github.com/ikatyang/emoji-cheat-sheet)."
    string name;
    # A short description of the label. Must be 100 characters or fewer
    string description?;
};


type RepoMergesBody record {
    # The head to merge. This can be a branch name or a commit SHA1
    string head;
    # Commit message to use for the merge commit. If omitted, a default message will be used
    string commitMessage?;
    # The name of the base branch that the head will be merged into
    string base;
};


type RepoMergeUpstreamBody record {
    # The name of the branch which should be updated to match upstream
    string branch;
};


type RepoMilestonesBody record {
    # A description of the milestone
    string description?;
    # The state of the milestone. Either open or closed
        "open"|"closed"  state = "open";
    # The title of the milestone
    string title;
    # The milestone due date. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string dueOn?;
};


type RepoNotificationsBody record {
    # Describes the last point that notifications were checked. Anything updated since this time will not be marked as read. If you omit this parameter, all notifications are marked as read. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ. Default: The current timestamp
    string lastReadAt?;
};


type RepoPullsBody record {
    # The name of the branch where your changes are implemented. For cross-repository pull requests in the same network, namespace head with a user like this: username:branch
    string head;
    # An issue in the repository to convert to a pull request. The issue title, body, and comments will become the title, body, and comments on the new pull request. Required unless title is specified
    int issue?;
    # The name of the repository where the changes in the pull request were made. This field is required for cross-repository pull requests if both repositories are owned by the same organization
    string headRepo?;
    # Indicates whether the pull request is a draft. See "[Draft Pull Requests](https://docs.github.com/articles/about-pull-requests#draft-pull-requests)" in the GitHub Help documentation to learn more
    boolean draft?;
    # Indicates whether [maintainers can modify](https://docs.github.com/articles/allowing-changes-to-a-pull-request-branch-created-from-a-fork/) the pull request
    boolean maintainerCanModify?;
    # The title of the new pull request. Required unless issue is specified
    string title?;
    # The contents of the pull request
    string body?;
    # The name of the branch you want the changes pulled into. This should be an existing branch on the current repository. You cannot submit a pull request to one repository that requests a merge to a base of another repository
    string base;
};


type RepoReleasesBody record {
    # If specified, a discussion of the specified category is created and linked to the release. The value must be a category that already exists in the repository. For more information, see "[Managing categories for discussions in your repository](https://docs.github.com/discussions/managing-discussions-for-your-community/managing-categories-for-discussions-in-your-repository)."
    string discussionCategoryName?;
    # Specifies whether this release should be set as the latest release for the repository. Drafts and prereleases cannot be set as latest. Defaults to true for newly published releases. legacy specifies that the latest release should be determined based on the release creation date and higher semantic version
        "true"|"false"|"legacy"  makeLatest = "true";
    # The name of the tag
    string tagName;
    # true to identify the release as a prerelease. false to identify the release as a full release
    boolean prerelease = false;
    # Specifies the commitish value that determines where the Git tag is created from. Can be any branch or commit SHA. Unused if the Git tag already exists. Default: the repository's default branch
    string targetCommitish?;
    # true to create a draft (unpublished) release, false to create a published one
    boolean draft = false;
    # The name of the release
    string name?;
    # Text describing the contents of the tag
    string body?;
    # Whether to automatically generate the name and body for this release. If name is specified, the specified name will be used; otherwise, a name will be automatically generated. If body is specified, the body will be pre-pended to the automatically generated notes
    boolean generateReleaseNotes = false;
};


type RepoRulesetsBody record {
    # The actors that can bypass the rules in this ruleset
    RepositoryRulesetBypassActor[] bypassActors?;
    # The name of the ruleset
    string name;
    # The enforcement level for this ruleset.
    RepositoryRuleEnforcement enforcement;
    # An array of rules within the ruleset
    RepositoryRule[] rules?;
    # The conditions that determine which refs the ruleset applies to.
    RepositoryRulesetConditions conditions?;
    # The target of the ruleset
        "branch"|"tag"  target?;
};

# Represents the Queries record for the operation: repos/codeowners-errors

type ReposCodeownersErrorsQueries record {
    # A branch, tag or commit name used to determine which version of the CODEOWNERS file to use. Default: the repository's default branch (e.g. main)
    string ref?;
};

# Represents the Queries record for the operation: repos/compare-commits

type ReposCompareCommitsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Repo Search Result Item

type RepoSearchResultItem record {
    # Indicates whether the repository can be forked.
    boolean allowForking?;
    # The number of users who have starred the repository.
    int stargazersCount;
    # Indicates whether the repository is a template repository.
    boolean isTemplate?;
    # The date and time of the most recent push to the repository.
    string pushedAt;
    # The primary programming language used in the repository.
    string? language;
    # API URL to manage the authenticated user's subscription to this repository.
    string subscriptionUrl;
    # API URL template for accessing the repository's branches.
    string branchesUrl;
    # API URL template for accessing comments on issues in the repository.
    string issueCommentUrl;
    # Indicates whether rebase merging is allowed for pull requests.
    boolean allowRebaseMerge?;
    # API URL template for accessing the repository's labels.
    string labelsUrl;
    # The search relevance score of this repository result.
    decimal score;
    # API URL for listing subscribers watching the repository.
    string subscribersUrl;
    # The permissions the authenticated user has on this repository.
    RepoSearchResultItemPermissions permissions?;
    # A temporary token used for cloning the repository.
    string tempCloneToken?;
    # API URL template for accessing the repository's releases.
    string releasesUrl;
    # The SVN URL for the repository.
    string svnUrl;
    # The unique numeric identifier of the repository.
    int id;
    # The name of the master branch of the repository.
    string masterBranch?;
    # Indicates whether the repository has discussions enabled.
    boolean hasDiscussions?;
    # The number of forks of the repository.
    int forks;
    # API URL template for downloading an archive of the repository.
    string archiveUrl;
    # Indicates whether merge commits are allowed for pull requests.
    boolean allowMergeCommit?;
    # API URL template for accessing the repository's Git refs.
    string gitRefsUrl;
    # API URL for listing forks of the repository.
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility?;
    # API URL template for accessing commit statuses in the repository.
    string statusesUrl;
    # The SSH URL used to clone the repository.
    string sshUrl;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # The full name of the repository, including the owner's login.
    string fullName;
    # The size of the repository in kilobytes.
    int size;
    # Indicates whether auto-merge is allowed for pull requests.
    boolean allowAutoMerge?;
    # API URL for listing the programming languages used in the repository.
    string languagesUrl;
    # The HTML URL for viewing the repository on GitHub.
    string htmlUrl;
    # API URL template for accessing the repository's collaborators.
    string collaboratorsUrl;
    # The HTTPS URL used to clone the repository.
    string cloneUrl;
    # The name of the repository.
    string name;
    # API URL template for accessing the repository's pull requests.
    string pullsUrl;
    # The name of the repository's default branch.
    string defaultBranch;
    # API URL for accessing the repository's webhooks.
    string hooksUrl;
    # API URL template for accessing the repository's Git trees.
    string treesUrl;
    # API URL for listing the repository's tags.
    string tagsUrl;
    # Indicates whether the repository is private.
    boolean 'private;
    # API URL for listing the repository's contributors.
    string contributorsUrl;
    # Indicates whether the repository has downloads enabled.
    boolean hasDownloads;
    # The number of open issues in the repository.
    int openIssuesCount;
    # API URL template for accessing notifications for the repository.
    string notificationsUrl;
    # A short description of the repository.
    string? description;
    # The date and time the repository was created.
    string createdAt;
    # The number of users watching the repository.
    int watchers;
    # API URL template for accessing the repository's deploy keys.
    string keysUrl;
    # API URL for listing the repository's deployments.
    string deploymentsUrl;
    # Indicates whether the repository has projects enabled.
    boolean hasProjects;
    # Indicates whether the repository has been archived.
    boolean archived;
    # Indicates whether the repository has a wiki enabled.
    boolean hasWiki;
    # The date and time the repository was last updated.
    string updatedAt;
    # API URL template for accessing comments in the repository.
    string commentsUrl;
    # API URL for listing users who have starred the repository.
    string stargazersUrl;
    # Returns whether or not this repository disabled
    boolean disabled;
    # Indicates whether branches are automatically deleted after merging.
    boolean deleteBranchOnMerge?;
    # The Git URL used to clone the repository.
    string gitUrl;
    # Indicates whether the repository has GitHub Pages enabled.
    boolean hasPages;
    # The user or organization that owns the repository.
    NullableSimpleUser? owner;
    # Indicates whether squash merging is allowed for pull requests.
    boolean allowSquashMerge?;
    # API URL template for accessing the repository's commits.
    string commitsUrl;
    # API URL template for comparing two commits or branches.
    string compareUrl;
    # API URL template for accessing the repository's Git commits.
    string gitCommitsUrl;
    # A list of topic tags associated with the repository.
    string[] topics?;
    # API URL template for accessing Git blobs in the repository.
    string blobsUrl;
    # API URL template for accessing Git tags in the repository.
    string gitTagsUrl;
    # API URL for performing branch merges in the repository.
    string mergesUrl;
    # API URL for accessing the repository's downloads.
    string downloadsUrl;
    # Indicates whether the repository has issues enabled.
    boolean hasIssues;
    # Indicates whether web-based commits must include a sign-off.
    boolean webCommitSignoffRequired?;
    # The API URL for the repository.
    string url;
    # API URL template for accessing files and directories in the repository.
    string contentsUrl;
    # The URL of the repository this is mirrored from, if applicable.
    string? mirrorUrl;
    # API URL template for accessing the repository's milestones.
    string milestonesUrl;
    # API URL for listing teams with access to the repository.
    string teamsUrl;
    # Indicates whether this repository is a fork of another repository.
    boolean 'fork;
    # API URL template for accessing the repository's issues.
    string issuesUrl;
    # API URL for listing events associated with the repository.
    string eventsUrl;
    # API URL for listing events on issues in the repository.
    string issueEventsUrl;
    # Text fragments that matched the search query within the repository.
    SearchResultTextMatches textMatches?;
    # API URL template for accessing potential assignees for the repository.
    string assigneesUrl;
    # The number of open issues and pull requests in the repository.
    int openIssues;
    # The number of users watching the repository for notifications.
    int watchersCount;
    # The GraphQL node ID of the repository.
    string nodeId;
    # The URL of the repository's homepage or project website.
    string? homepage;
    # The total number of forks of the repository.
    int forksCount;
};


type RepoSearchResultItemPermissions record {
    # Indicates whether the user has pull permission on the repository.
    boolean pull;
    # Indicates whether the user has maintain permission on the repository.
    boolean maintain?;
    # Indicates whether the user has admin permission on the repository.
    boolean admin;
    # Indicates whether the user has triage permission on the repository.
    boolean triage?;
    # Indicates whether the user has push permission on the repository.
    boolean push;
};

# Repo Search Result Item

type RepoSearchResultItemResponse record {
    # The total number of repositories matching the search query.
    int totalCount;
    # Whether the search results are incomplete due to a timeout.
    boolean incompleteResults;
    # The list of repositories matching the search query.
    RepoSearchResultItem[] items;
};

# Represents the Queries record for the operation: repos/get-all-environments

type ReposGetAllEnvironmentsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-all-topics

type ReposGetAllTopicsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-branch-rules

type ReposGetBranchRulesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-clones

type ReposGetClonesQueries record {
    # The time frame to display results for
        "day"|"week"  per = "day";
};

# Represents the Queries record for the operation: repos/get-combined-status-for-ref

type ReposGetCombinedStatusForRefQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-commit

type ReposGetCommitQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-content

type ReposGetContentQueries record {
    # The name of the commit/branch/tag. Default: the repository’s default branch
    string ref?;
};

# Represents the Queries record for the operation: repos/get-org-rulesets

type ReposGetOrgRulesetsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/get-org-rule-suites

type ReposGetOrgRuleSuitesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The handle for the GitHub user account to filter on. When specified, only rule evaluations triggered by this actor will be returned
    string actorName?;
    # Page number of the results to fetch
    int page = 1;
    # The name of the repository to filter on. When specified, only rule evaluations from this repository will be returned
    int repositoryName?;
    # The time period to filter by.

For example, day will filter for rule suites that occurred in the past 24 hours, and week will filter for insights that occurred in the past 7 days (168 hours)
        "hour"|"day"|"week"|"month"  timePeriod = "day";
    # The rule results to filter on. When specified, only suites with this result will be returned
        "pass"|"fail"|"bypass"|"all"  ruleSuiteResult = "all";
};

# Represents the Queries record for the operation: repos/get-readme-in-directory

type ReposGetReadmeInDirectoryQueries record {
    # The name of the commit/branch/tag. Default: the repository’s default branch
    string ref?;
};

# Represents the Queries record for the operation: repos/get-readme

type ReposGetReadmeQueries record {
    # The name of the commit/branch/tag. Default: the repository’s default branch
    string ref?;
};

# Represents the Queries record for the operation: repos/get-repo-ruleset

type ReposGetRepoRulesetQueries record {
    # Include rulesets configured at higher levels that apply to this repository
    boolean includesParents = true;
};

# Represents the Queries record for the operation: repos/get-repo-rulesets

type ReposGetRepoRulesetsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # Include rulesets configured at higher levels that apply to this repository
    boolean includesParents = true;
};

# Represents the Queries record for the operation: repos/get-repo-rule-suites

type ReposGetRepoRuleSuitesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name of the ref. Cannot contain wildcard characters. When specified, only rule evaluations triggered for this ref will be returned
    string ref?;
    # The handle for the GitHub user account to filter on. When specified, only rule evaluations triggered by this actor will be returned
    string actorName?;
    # Page number of the results to fetch
    int page = 1;
    # The time period to filter by.

For example, day will filter for rule suites that occurred in the past 24 hours, and week will filter for insights that occurred in the past 7 days (168 hours)
        "hour"|"day"|"week"|"month"  timePeriod = "day";
    # The rule results to filter on. When specified, only suites with this result will be returned
        "pass"|"fail"|"bypass"|"all"  ruleSuiteResult = "all";
};

# Represents the Queries record for the operation: repos/get-views

type ReposGetViewsQueries record {
    # The time frame to display results for
        "day"|"week"  per = "day";
};

# A repository on GitHub

type Repository record {
    # Whether to allow forking this repo
    boolean allowForking?;
    # Whether anonymous git access is enabled for this repository
    boolean anonymousAccessEnabled?;
    # API URL for the authenticated user's subscription to the repository
    string subscriptionUrl;
    # API URL template for listing repository branches
    string branchesUrl;
    # API URL template for accessing issue comments
    string issueCommentUrl;
    # Whether to allow rebase merges for pull requests
    boolean allowRebaseMerge = true;
    # The permissions the authenticated user has on this repository.
    RepositoryPermissions permissions?;
    # API URL for listing repository watchers
    string subscribersUrl;
    # A temporary token for cloning the repository
    string tempCloneToken?;
    # API URL template for listing repository releases
    string releasesUrl;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # The number of users watching the repository
    int subscribersCount?;
    # Unique identifier of the repository
    int id;
    # Whether discussions are enabled
    boolean hasDiscussions = false;
    # The number of forks of the repository
    int forks;
    # API URL template for accessing Git references
    string gitRefsUrl;
    # The SSH URL for cloning the repository
    string sshUrl;
    # The full name of the repository in owner/name format
    string fullName;
    # The size of the repository. Size is calculated hourly. When a repository is initially created, the size is 0
    int size;
    # The template repository used to create this repository, if any.
    RepositoryTemplateRepository? templateRepository?;
    # API URL for listing programming languages used
    string languagesUrl;
    # URL of the repository page on GitHub
    string htmlUrl;
    # API URL template for listing repository collaborators
    string collaboratorsUrl;
    # The HTTPS URL for cloning the repository
    string cloneUrl;
    # The default branch of the repository
    string defaultBranch;
    # API URL for listing repository webhooks
    string hooksUrl;
    # API URL template for accessing Git trees
    string treesUrl;
    # Whether downloads are enabled
    boolean hasDownloads = true;
    # The date the repository was created
    string? createdAt;
    # The number of watchers on the repository
    int watchers;
    # API URL for listing repository deployments
    string deploymentsUrl;
    # API URL template for listing repository deploy keys
    string keysUrl;
    # Whether the repository is archived
    boolean archived = false;
    # Whether the wiki is enabled
    boolean hasWiki = true;
    # The date the repository was last updated
    string? updatedAt;
    # Returns whether or not this repository disabled
    boolean disabled;
    # API URL template for comparing two commits
    string compareUrl;
    # API URL template for accessing Git commits
    string gitCommitsUrl;
    # The list of topics associated with the repository
    string[] topics?;
    # Whether or not a pull request head branch that is behind its base branch can always be updated even if it is not required to be up to date before merging
    boolean allowUpdateBranch = false;
    # API URL template for accessing Git tags
    string gitTagsUrl;
    # API URL for performing merge operations
    string mergesUrl;
    # The time the authenticated user starred the repository
    string starredAt?;
    # API URL for the repository
    string url;
    # API URL template for accessing repository contents
    string contentsUrl;
    # API URL template for listing repository issues
    string issuesUrl;
    # Whether a squash merge commit can use the pull request title as default. **This property has been deprecated. Please use squash_merge_commit_title instead
    boolean useSquashPrTitleAsDefault = false;
    # The organization that owns the repository, if applicable.
    NullableSimpleUser? organization?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # API URL template for listing repository assignees
    string assigneesUrl;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # The number of open issues in the repository
    int openIssues;
    # The GraphQL node identifier of the repository
    string nodeId;
    # The number of stars on the repository
    int stargazersCount;
    # Whether this repository acts as a template that can be used to generate new repositories
    boolean isTemplate = false;
    # The date of the most recent push to the repository
    string? pushedAt;
    # The primary programming language of the repository
    string? language;
    # API URL template for listing repository labels
    string labelsUrl;
    # The Subversion URL for the repository
    string svnUrl;
    # The name of the master branch
    string masterBranch?;
    # API URL template for downloading repository archives
    string archiveUrl;
    # Whether to allow merge commits for pull requests
    boolean allowMergeCommit = true;
    # API URL for listing repository forks
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility = "public";
    # API URL template for listing commit statuses
    string statusesUrl;
    # The number of repositories in the fork network
    int networkCount?;
    # The license applied to the repository.
    NullableLicenseSimple? license;
    # Whether to allow Auto-merge to be used on pull requests
    boolean allowAutoMerge = false;
    # The name of the repository
    string name;
    # API URL template for listing pull requests
    string pullsUrl;
    # API URL for listing repository tags
    string tagsUrl;
    # Whether the repository is private or public
    boolean 'private = false;
    # API URL for listing repository contributors
    string contributorsUrl;
    # API URL template for listing repository notifications
    string notificationsUrl;
    # The number of open issues in the repository
    int openIssuesCount;
    # A short description of the repository
    string? description;
    # Whether projects are enabled
    boolean hasProjects = true;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # API URL template for listing commit comments
    string commentsUrl;
    # API URL for listing users who starred the repository
    string stargazersUrl;
    # Whether to delete head branches when pull requests are merged
    boolean deleteBranchOnMerge = false;
    # The Git protocol URL for the repository
    string gitUrl;
    # Whether the repository has GitHub Pages enabled
    boolean hasPages;
    # The account that owns the repository.
    SimpleUser owner;
    # Whether to allow squash merges for pull requests
    boolean allowSquashMerge = true;
    # API URL template for listing repository commits
    string commitsUrl;
    # API URL template for accessing repository blobs
    string blobsUrl;
    # API URL for listing repository downloads
    string downloadsUrl;
    # Whether issues are enabled
    boolean hasIssues = true;
    # Whether to require contributors to sign off on web-based commits
    boolean webCommitSignoffRequired = false;
    # The URL of the mirror for the repository
    string? mirrorUrl;
    # API URL template for listing repository milestones
    string milestonesUrl;
    # API URL for listing teams with access to the repository
    string teamsUrl;
    # Whether the repository is a fork
    boolean 'fork;
    # API URL for listing repository events
    string eventsUrl;
    # API URL template for listing issue events
    string issueEventsUrl;
    # The number of watchers on the repository
    int watchersCount;
    # The URL of the repository's homepage
    string? homepage;
    # The number of forks of the repository
    int forksCount;
};

# A repository security advisory

type RepositoryAdvisory record {
    # Detailed credit information including credit type for each credited user.
    RepositoryAdvisoryCredit[]? creditsDetailed;
    # A detailed description of what the advisory entails
    string? description;
    # The date and time of when the advisory was created, in ISO 8601 format
    string? createdAt;
    # The CWE (Common Weakness Enumeration) entries associated with the advisory.
    GlobalAdvisoryCwes[]? cwes;
    # The date and time of when the advisory was last updated, in ISO 8601 format
    string? updatedAt;
    # Users credited for discovering or fixing the advisory.
    RepositoryAdvisoryCredits[]? credits;
    # The date and time of when the advisory was withdrawn, in ISO 8601 format
    string? withdrawnAt;
    # The state of the advisory
        "published"|"closed"|"withdrawn"|"draft"|"triage"  state;
    # The date and time of when the advisory was published, in ISO 8601 format
    string? publishedAt;
    # A list of users that collaborate on the advisory
    SimpleUser[]? collaboratingUsers;
    # A temporary private fork of the advisory's repository for collaborating on a fix
    SimpleRepository? privateFork;
    # A short summary of the advisory
    string summary;
    # The severity of the advisory
        "critical"|"high"|"medium"|"low"? severity;
    # The date and time of when the advisory was closed, in ISO 8601 format
    string? closedAt;
    # The author of the advisory
    SimpleUser? author;
    # A list of identifiers such as CVE or GHSA IDs for the advisory.
    GlobalAdvisoryIdentifiers[] identifiers;
    # The API URL for the advisory
    string url;
    # The GitHub Security Advisory ID
    string ghsaId;
    # The Common Vulnerabilities and Exposures (CVE) ID
    string? cveId;
    # A list of teams that collaborate on the advisory
    Team[]? collaboratingTeams;
    # The URL for the advisory
    string htmlUrl;
    # The publisher of the advisory
    SimpleUser? publisher;
    # The packages and version ranges affected by this advisory.
    RepositoryAdvisoryVulnerability[]? vulnerabilities;
    # Submission details if the advisory was submitted by an external reporter.
    RepositoryAdvisorySubmission? submission;
    # The CVSS score and vector string for the advisory.
    GlobalAdvisoryCvss? cvss;
    # A list of only the CWE IDs
    string[]? cweIds;
};


type RepositoryAdvisoryCreate record {
    # A short summary of the advisory
    string summary;
    # The severity of the advisory. You must choose between setting this field or cvss_vector_string
        "critical"|"high"|"medium"|"low"? severity?;
    # The Common Vulnerabilities and Exposures (CVE) ID
    string? cveId?;
    # A list of users receiving credit for their participation in the security advisory
    RepositoryAdvisoryCreateCredits[]? credits?;
    # The CVSS vector that calculates the severity of the advisory. You must choose between setting this field or severity
    string? cvssVectorString?;
    # A detailed description of what the advisory impacts
    string description;
    # A product affected by the vulnerability detailed in a repository security advisory
    RepositoryAdvisoryCreateVulnerabilities[] vulnerabilities;
    # A list of Common Weakness Enumeration (CWE) IDs
    string[]? cweIds?;
};


type RepositoryAdvisoryCreateCredits record {
    # The username of the user credited
    string login;
    # The type of contribution credited to the user for the advisory.
    SecurityAdvisoryCreditTypes 'type;
};

# The name of the package affected by the vulnerability

type RepositoryAdvisoryCreatePackage record {
    # The package ecosystem where the vulnerability exists.
    SecurityAdvisoryEcosystems ecosystem;
    # The unique package name within its ecosystem
    string? name?;
};


type RepositoryAdvisoryCreateVulnerabilities record {
    # The package affected by this vulnerability.
    RepositoryAdvisoryCreatePackage package;
    # The functions in the package that are affected
    string[]? vulnerableFunctions?;
    # The range of the package versions affected by the vulnerability
    string? vulnerableVersionRange?;
    # The package version(s) that resolve the vulnerability
    string? patchedVersions?;
};

# A credit given to a user for a repository security advisory

type RepositoryAdvisoryCredit record {
    # The state of the user's acceptance of the credit
        "accepted"|"declined"|"pending"  state;
    # The type of credit assigned to the user for the advisory.
    SecurityAdvisoryCreditTypes 'type;
    # The user receiving credit for the security advisory.
    SimpleUser user;
};


type RepositoryAdvisoryCredits record {
    # The username of the user credited
    string login?;
    # The type of credit assigned to the credited user.
    SecurityAdvisoryCreditTypes 'type?;
};


type RepositoryAdvisorySubmission record {
    # Whether a private vulnerability report was accepted by the repository's administrators
    boolean accepted;
};


type RepositoryAdvisoryUpdate record {
    # A short summary of the advisory
    string summary?;
    # The severity of the advisory. You must choose between setting this field or cvss_vector_string
        "critical"|"high"|"medium"|"low"? severity?;
    # The Common Vulnerabilities and Exposures (CVE) ID
    string? cveId?;
    # A list of users receiving credit for their participation in the security advisory
    RepositoryAdvisoryCreateCredits[]? credits?;
    # The CVSS vector that calculates the severity of the advisory. You must choose between setting this field or severity
    string? cvssVectorString?;
    # A list of team slugs which have been granted write access to the advisory
    string[]? collaboratingTeams?;
    # A detailed description of what the advisory impacts
    string description?;
    # A product affected by the vulnerability detailed in a repository security advisory
    RepositoryAdvisoryCreateVulnerabilities[] vulnerabilities?;
    # The state of the advisory
        "published"|"closed"|"draft"  state?;
    # A list of usernames who have been granted write access to the advisory
    string[]? collaboratingUsers?;
    # A list of Common Weakness Enumeration (CWE) IDs
    string[]? cweIds?;
};

# A product affected by the vulnerability detailed in a repository security advisory

type RepositoryAdvisoryVulnerability record {
    # The package affected by the vulnerability.
    RepositoryAdvisoryVulnerabilityPackage? package;
    # The functions in the package that are affected
    string[]? vulnerableFunctions;
    # The range of the package versions affected by the vulnerability
    string? vulnerableVersionRange;
    # The package version(s) that resolve the vulnerability
    string? patchedVersions;
};

# The name of the package affected by the vulnerability

type RepositoryAdvisoryVulnerabilityPackage record {
    # The package ecosystem where the vulnerability exists.
    SecurityAdvisoryEcosystems ecosystem;
    # The unique package name within its ecosystem
    string? name;
};

# Repository Collaborator Permission

type RepositoryCollaboratorPermission record {
    # The name of the role assigned to the collaborator.
    string roleName;
    # The permission level granted to the collaborator.
    string permission;
    # The collaborator user associated with this permission.
    NullableCollaborator? user;
};

# Conditions to target repositories by id and refs by name

type RepositoryIdAndRefName record {
    RepositoryRulesetConditionsRefName refName;
    RepositoryRulesetConditionsRepositoryIdTargetRepositoryId repositoryId;
};

# Repository invitations let you manage who you collaborate with

type RepositoryInvitation record {
    # Whether or not the invitation has expired
    boolean expired?;
    # The permission associated with the invitation
        "read"|"write"|"admin"|"triage"|"maintain"  permissions;
    # URL of the repository invitation page on GitHub.
    string htmlUrl;
    # The date and time the invitation was created.
    string createdAt;
    # The user who sent the repository invitation.
    NullableSimpleUser? inviter;
    # Unique identifier of the repository invitation
    int id;
    # The repository to which the user is invited.
    MinimalRepository repository;
    # URL for the repository invitation
    string url;
    # The user who received the repository invitation.
    NullableSimpleUser? invitee;
    # The GraphQL node identifier of the invitation.
    string nodeId;
};

# Conditions to target repositories by name and refs by name

type RepositoryNameAndRefName record {
    RepositoryRulesetConditionsRefName refName;
    RepositoryRulesetConditionsRepositoryNameTargetRepositoryName repositoryName;
};

# The permissions the authenticated user has on the repository

type RepositoryPermissions record {
    # Whether the authenticated user can pull from the repository.
    boolean pull;
    # Whether the authenticated user has maintain permissions on the repository.
    boolean maintain?;
    # Whether the authenticated user has admin permissions on the repository.
    boolean admin;
    # Whether the authenticated user has triage permissions on the repository.
    boolean triage?;
    # Whether the authenticated user can push to the repository.
    boolean push;
};

# A repository on GitHub

type RepositoryResponse record {
    # Indicates whether all or selected repositories are included.
    string repositorySelection?;
    # The list of repositories returned in this response.
    Repository[] repositories;
    # The total number of repositories matching the request.
    int totalCount;
};

# Parameters to be used for the branch_name_pattern rule

type RepositoryRuleBranchNamePattern record {
    # The rule type, identifying this as a branch name pattern rule.
        "branch_name_pattern"  'type;
    # The configuration parameters for the branch name pattern rule.
    RepositoryRuleBranchNamePatternParameters parameters?;
};


type RepositoryRuleBranchNamePatternParameters record {
    # If true, the rule will fail if the pattern matches
    boolean negate?;
    # How this rule will appear to users
    string name?;
    # The pattern to match with
    string pattern;
    # The operator to use for matching
        "starts_with"|"ends_with"|"contains"|"regex"  operator;
};

# Parameters to be used for the commit_author_email_pattern rule

type RepositoryRuleCommitAuthorEmailPattern record {
    # The type identifier for the commit author email pattern rule.
        "commit_author_email_pattern"  'type;
    # The parameters defining the pattern to match against commit author emails.
    RepositoryRuleBranchNamePatternParameters parameters?;
};

# Parameters to be used for the commit_message_pattern rule

type RepositoryRuleCommitMessagePattern record {
    # The type identifier for the commit message pattern rule.
        "commit_message_pattern"  'type;
    # Parameters defining the commit message pattern matching criteria.
    RepositoryRuleBranchNamePatternParameters parameters?;
};

# Parameters to be used for the committer_email_pattern rule

type RepositoryRuleCommitterEmailPattern record {
    # The rule type identifier, always 'committer_email_pattern'.
        "committer_email_pattern"  'type;
    # The parameters defining the committer email pattern rule.
    RepositoryRuleBranchNamePatternParameters parameters?;
};

# Only allow users with bypass permission to create matching refs

type RepositoryRuleCreation record {
    # The rule type, always 'creation' for this rule.
        "creation"  'type;
};

# Only allow users with bypass permissions to delete matching refs

type RepositoryRuleDeletion record {
    # The rule type, always 'deletion' for this rule.
        "deletion"  'type;
};


type RepositoryRuleDetailedOneOf1 record {
    "creation" type;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf10 record {
    "commit_message_pattern" type;
    RepositoryRuleBranchNamePatternParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf11 record {
    "commit_author_email_pattern" type;
    RepositoryRuleBranchNamePatternParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf12 record {
    "committer_email_pattern" type;
    RepositoryRuleBranchNamePatternParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf13 record {
    "branch_name_pattern" type;
    RepositoryRuleBranchNamePatternParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf14 record {
    "tag_name_pattern" type;
    RepositoryRuleBranchNamePatternParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf2 record {
    "update" type;
    RepositoryRuleUpdateParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf3 record {
    "deletion" type;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf4 record {
    "required_linear_history" type;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf5 record {
    "required_deployments" type;
    RepositoryRuleRequiredDeploymentsParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf6 record {
    "required_signatures" type;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf7 record {
    "pull_request" type;
    RepositoryRulePullRequestParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf8 record {
    "required_status_checks" type;
    RepositoryRuleRequiredStatusChecksParameters parameters;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};


type RepositoryRuleDetailedOneOf9 record {
    "non_fast_forward" type;
    "Repository"|"Organization" ruleset_source_type;
    string ruleset_source;
    int ruleset_id;
};

# Prevent users with push access from force pushing to refs

type RepositoryRuleNonFastForward record {
    # The rule type identifier for non-fast-forward prevention.
        "non_fast_forward"  'type;
};

# Required status check

type RepositoryRuleParamsStatusCheckConfiguration record {
    # The optional integration ID that this status check must originate from
    int integrationId?;
    # The status check context name that must be present on the commit
    string context;
};

# Require all commits be made to a non-target branch and submitted via a pull request before they can be merged

type RepositoryRulePullRequest record {
    # The rule type, always 'pull_request' for this rule.
        "pull_request"  'type;
    # The parameters configuring the pull request rule requirements.
    RepositoryRulePullRequestParameters parameters?;
};


type RepositoryRulePullRequestParameters record {
    # All conversations on code must be resolved before a pull request can be merged
    boolean requiredReviewThreadResolution;
    # The number of approving reviews that are required before a pull request can be merged
    int requiredApprovingReviewCount;
    # Require an approving review in pull requests that modify files that have a designated code owner
    boolean requireCodeOwnerReview;
    # New, reviewable commits pushed will dismiss previous pull request review approvals
    boolean dismissStaleReviewsOnPush;
    # Whether the most recent reviewable push must be approved by someone other than the person who pushed it
    boolean requireLastPushApproval;
};

# Choose which environments must be successfully deployed to before refs can be merged into a branch that matches this rule

type RepositoryRuleRequiredDeployments record {
    # The rule type identifier, always 'required_deployments'.
        "required_deployments"  'type;
    # Configuration parameters for the required deployments rule.
    RepositoryRuleRequiredDeploymentsParameters parameters?;
};


type RepositoryRuleRequiredDeploymentsParameters record {
    # The environments that must be successfully deployed to before branches can be merged
    string[] requiredDeploymentEnvironments;
};

# Prevent merge commits from being pushed to matching refs

type RepositoryRuleRequiredLinearHistory record {
    # The rule type, must be 'required_linear_history'.
        "required_linear_history"  'type;
};

# Commits pushed to matching refs must have verified signatures

type RepositoryRuleRequiredSignatures record {
    # The rule type identifier, always 'required_signatures'.
        "required_signatures"  'type;
};

# Choose which status checks must pass before branches can be merged into a branch that matches this rule. When enabled, commits must first be pushed to another branch, then merged or pushed directly to a ref that matches this rule after status checks have passed

type RepositoryRuleRequiredStatusChecks record {
    # The rule type identifier for required status checks.
        "required_status_checks"  'type;
    # Configuration parameters for the required status checks rule.
    RepositoryRuleRequiredStatusChecksParameters parameters?;
};


type RepositoryRuleRequiredStatusChecksParameters record {
    # Whether pull requests targeting a matching branch must be tested with the latest code. This setting will not take effect unless at least one status check is enabled
    boolean strictRequiredStatusChecksPolicy;
    # Status checks that are required
    RepositoryRuleParamsStatusCheckConfiguration[] requiredStatusChecks;
};

# User-defined metadata to store domain-specific information limited to 8 keys with scalar values

type RepositoryRuleRulesetInfo record {
    # The type of source for the ruleset that includes this rule.
        "Repository"|"Organization"  ruleset_source_type?;
    # The name of the source of the ruleset that includes this rule.
    string ruleset_source?;
    # The ID of the ruleset that includes this rule.
    int ruleset_id?;
};

# A set of rules to apply when specified conditions are met

type RepositoryRuleset record {
    # Hypermedia links for navigating related ruleset resources.
    RepositoryRulesetLinks links?;
    # The enforcement level of the ruleset.
    RepositoryRuleEnforcement enforcement;
    # Timestamp when the ruleset was created.
    string createdAt?;
    # The type of the source of the ruleset
        "Repository"|"Organization"  sourceType?;
    # The list of rules included in this ruleset.
    RepositoryRule[] rules?;
    # The name of the source
    string 'source;
    # The target of the ruleset
        "branch"|"tag"  target?;
    # The actors that can bypass the rules in this ruleset
    RepositoryRulesetBypassActor[] bypassActors?;
    # Timestamp when the ruleset was last updated.
    string updatedAt?;
    # The bypass type of the user making the API request for this ruleset. This field is only returned when
querying the repository-level endpoint
        "always"|"pull_requests_only"|"never"  currentUserCanBypass?;
    # The name of the ruleset
    string name;
    # The ID of the ruleset
    int id;
    # The conditions that determine when this ruleset applies.
    RepositoryRulesetConditions|OrgRulesetConditions conditions?;
    # The GraphQL node ID of the ruleset.
    string nodeId?;
};

# An actor that can bypass rules in a ruleset

type RepositoryRulesetBypassActor record {
    # The type of actor that can bypass a ruleset
        "RepositoryRole"|"Team"|"Integration"|"OrganizationAdmin"  actorType;
    # When the specified actor can bypass the ruleset. pull_request means that an actor can only bypass rules on pull requests
        "always"|"pull_request"  bypassMode;
    # The ID of the actor that can bypass a ruleset. If actor_type is OrganizationAdmin, this should be 1
    int actorId;
};

# Parameters for a repository ruleset ref name condition

type RepositoryRulesetConditions record {
    # Conditions specifying which ref names this ruleset applies to.
    RepositoryRulesetConditionsRefName refName?;
};


type RepositoryRulesetConditionsRefName record {
    # Array of ref names or patterns to include. One of these patterns must match for the condition to pass. Also accepts ~DEFAULT_BRANCH to include the default branch or ~ALL to include all branches
    string[] include?;
    # Array of ref names or patterns to exclude. The condition will not pass if any of these patterns match
    string[] exclude?;
};

# Parameters for a repository ID condition

type RepositoryRulesetConditionsRepositoryIdTarget record {
    # The repository ID condition parameters for the ruleset.
    RepositoryRulesetConditionsRepositoryIdTargetRepositoryId repositoryId;
};


type RepositoryRulesetConditionsRepositoryIdTargetRepositoryId record {
    # The repository IDs that the ruleset applies to. One of these IDs must match for the condition to pass
    int[] repositoryIds?;
};

# Parameters for a repository name condition

type RepositoryRulesetConditionsRepositoryNameTarget record {
    # Conditions targeting repositories by their name pattern.
    RepositoryRulesetConditionsRepositoryNameTargetRepositoryName repositoryName;
};


type RepositoryRulesetConditionsRepositoryNameTargetRepositoryName record {
    # Array of repository names or patterns to include. One of these patterns must match for the condition to pass. Also accepts ~ALL to include all repositories
    string[] include?;
    # Whether renaming of target repositories is prevented
    boolean protected?;
    # Array of repository names or patterns to exclude. The condition will not pass if any of these patterns match
    string[] exclude?;
};


type RepositoryRulesetLinks record {
    # The API link to this ruleset resource.
    RepositoryRulesetLinksSelf self?;
    # The HTML link to view this ruleset on GitHub.
    RepositoryRulesetLinksHtml html?;
};


type RepositoryRulesetLinksHtml record {
    # The html URL of the ruleset
    string href?;
};


type RepositoryRulesetLinksSelf record {
    # The URL of the ruleset
    string href?;
};

# Parameters to be used for the tag_name_pattern rule

type RepositoryRuleTagNamePattern record {
    # Identifies this rule as a tag name pattern rule.
        "tag_name_pattern"  'type;
    # The pattern parameters applied to tag name matching.
    RepositoryRuleBranchNamePatternParameters parameters?;
};

# Only allow users with bypass permission to update matching refs

type RepositoryRuleUpdate record {
    # The rule type, always 'update' for this rule.
        "update"  'type;
    # Parameters for the update rule configuration.
    RepositoryRuleUpdateParameters parameters?;
};


type RepositoryRuleUpdateParameters record {
    # Branch can pull changes from its upstream repository
    boolean updateAllowsFetchAndMerge;
};

# Repository invitations let you manage who you collaborate with

type RepositorySubscription record {
    # Determines if notifications should be received from this repository
    boolean subscribed;
    # Determines if all notifications should be blocked from this repository
    boolean ignored;
    # The reason for the subscription to this repository.
    string? reason;
    # The date and time when the subscription was created.
    string createdAt;
    # API URL for the subscribed repository.
    string repositoryUrl;
    # API URL for this subscription.
    string url;
};

# The template repository from which this repository was created

type RepositoryTemplateRepository record {
    # The number of users who have starred the repository.
    int stargazersCount?;
    # Whether the repository is a template repository.
    boolean isTemplate?;
    # The timestamp of the most recent push to the repository.
    string pushedAt?;
    # The API URL for managing repository subscriptions.
    string subscriptionUrl?;
    # The primary programming language used in the repository.
    string language?;
    # The API URL template for the repository's branches.
    string branchesUrl?;
    # The API URL template for the repository's issue comments.
    string issueCommentUrl?;
    # Whether rebase-merging pull requests is allowed.
    boolean allowRebaseMerge?;
    # The API URL template for the repository's labels.
    string labelsUrl?;
    # The API URL for the repository's subscribers.
    string subscribersUrl?;
    # The permissions the authenticated user has on this repository.
    RepositoryTemplateRepositoryPermissions permissions?;
    # A temporary token used for cloning the repository.
    string tempCloneToken?;
    # The API URL template for the repository's releases.
    string releasesUrl?;
    # The Subversion URL for accessing the repository.
    string svnUrl?;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # The number of users watching the repository.
    int subscribersCount?;
    # The unique identifier of the repository.
    int id?;
    # The API URL template for downloading repository archives.
    string archiveUrl?;
    # Whether merging pull requests with a merge commit is allowed.
    boolean allowMergeCommit?;
    # The API URL template for the repository's Git refs.
    string gitRefsUrl?;
    # The API URL for the repository's forks.
    string forksUrl?;
    # The visibility level of the repository.
    string visibility?;
    # The API URL template for the repository's commit statuses.
    string statusesUrl?;
    # The number of repositories in the network.
    int networkCount?;
    # The SSH URL used to clone the repository.
    string sshUrl?;
    # The full repository name including owner and repository name.
    string fullName?;
    # The size of the repository in kilobytes.
    int size?;
    # Whether auto-merge is allowed on pull requests.
    boolean allowAutoMerge?;
    # The API URL for the repository's language breakdown.
    string languagesUrl?;
    # The URL of the repository page on GitHub.
    string htmlUrl?;
    # The API URL template for the repository's collaborators.
    string collaboratorsUrl?;
    # The HTTPS URL used to clone the repository.
    string cloneUrl?;
    # The name of the repository.
    string name?;
    # The API URL template for the repository's pull requests.
    string pullsUrl?;
    # The name of the repository's default branch.
    string defaultBranch?;
    # The API URL for the repository's webhooks.
    string hooksUrl?;
    # The API URL template for the repository's Git trees.
    string treesUrl?;
    # The API URL for the repository's tags.
    string tagsUrl?;
    # Whether the repository is private.
    boolean 'private?;
    # The API URL for the repository's contributors.
    string contributorsUrl?;
    # Whether the repository has downloads enabled.
    boolean hasDownloads?;
    # The API URL template for the repository's notifications.
    string notificationsUrl?;
    # The number of open issues in the repository.
    int openIssuesCount?;
    # The description of the repository.
    string description?;
    # The timestamp when the repository was created.
    string createdAt?;
    # The API URL for the repository's deployments.
    string deploymentsUrl?;
    # The API URL template for the repository's deploy keys.
    string keysUrl?;
    # Whether projects are enabled for the repository.
    boolean hasProjects?;
    # Whether the repository is archived.
    boolean archived?;
    # Whether the wiki is enabled for the repository.
    boolean hasWiki?;
    # The timestamp when the repository was last updated.
    string updatedAt?;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # The API URL template for the repository's comments.
    string commentsUrl?;
    # The API URL for the repository's stargazers.
    string stargazersUrl?;
    # Whether the repository is disabled.
    boolean disabled?;
    # Whether branches are automatically deleted after pull request merges.
    boolean deleteBranchOnMerge?;
    # The Git URL used to access the repository.
    string gitUrl?;
    # Whether GitHub Pages is enabled for the repository.
    boolean hasPages?;
    # The owner of the template repository.
    RepositoryTemplateRepositoryOwner owner?;
    # Whether squash-merging pull requests is allowed.
    boolean allowSquashMerge?;
    # The API URL template for the repository's commits.
    string commitsUrl?;
    # The API URL template for comparing branches or commits.
    string compareUrl?;
    # The API URL template for the repository's Git commits.
    string gitCommitsUrl?;
    # The list of topics associated with the repository.
    string[] topics?;
    # The API URL template for the repository's Git blobs.
    string blobsUrl?;
    # Whether pull request branches can be updated even if not required.
    boolean allowUpdateBranch?;
    # The API URL template for the repository's Git tags.
    string gitTagsUrl?;
    # The API URL for merging branches in the repository.
    string mergesUrl?;
    # The API URL for the repository's downloads.
    string downloadsUrl?;
    # Whether issues are enabled for the repository.
    boolean hasIssues?;
    # The API URL for the repository.
    string url?;
    # The API URL template for the repository's contents.
    string contentsUrl?;
    # The URL of the source repository if this is a mirror.
    string mirrorUrl?;
    # The API URL template for the repository's milestones.
    string milestonesUrl?;
    # The API URL for the repository's teams.
    string teamsUrl?;
    # Whether the repository is a fork of another repository.
    boolean 'fork?;
    # The API URL template for the repository's issues.
    string issuesUrl?;
    # The API URL for the repository's events.
    string eventsUrl?;
    # Whether squash merge commits use the pull request title by default.
    boolean useSquashPrTitleAsDefault?;
    # The API URL template for the repository's issue events.
    string issueEventsUrl?;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # The API URL template for the repository's assignees.
    string assigneesUrl?;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # The number of users watching the repository.
    int watchersCount?;
    # The GraphQL node identifier of the repository.
    string nodeId?;
    # The URL of the repository's homepage or website.
    string homepage?;
    # The number of forks of the repository.
    int forksCount?;
};


type RepositoryTemplateRepositoryOwner record {
    # The API URL template for the owner's gists.
    string gistsUrl?;
    # The API URL for the owner's repositories.
    string reposUrl?;
    # The API URL template for users this owner follows.
    string followingUrl?;
    # The API URL template for the owner's starred repositories.
    string starredUrl?;
    # The username of the repository owner.
    string login?;
    # The API URL for the owner's followers.
    string followersUrl?;
    # The type of account, either User or Organization.
    string 'type?;
    # The API URL for the owner's account.
    string url?;
    # The API URL for the owner's subscriptions.
    string subscriptionsUrl?;
    # The API URL for events received by the owner.
    string receivedEventsUrl?;
    # The URL of the owner's avatar image.
    string avatarUrl?;
    # The API URL template for the owner's events.
    string eventsUrl?;
    # The HTML URL for the owner's GitHub profile.
    string htmlUrl?;
    # Indicates whether the owner is a GitHub site administrator.
    boolean siteAdmin?;
    # The unique numeric identifier for the owner.
    int id?;
    # The Gravatar ID associated with the owner's email.
    string gravatarId?;
    # The GraphQL node ID for the owner.
    string nodeId?;
    # The API URL for the owner's organizations.
    string organizationsUrl?;
};


type RepositoryTemplateRepositoryPermissions record {
    # Whether the user has pull (read) permission on the repository.
    boolean pull?;
    # Whether the user has maintain permission on the repository.
    boolean maintain?;
    # Whether the user has admin permission on the repository.
    boolean admin?;
    # Whether the user has triage permission on the repository.
    boolean triage?;
    # Whether the user has push (write) permission on the repository.
    boolean push?;
};

# Represents the Queries record for the operation: repos/list-activities

type ReposListActivitiesQueries record {
    # The GitHub username to use to filter by the actor who performed the activity
    string actor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # The Git reference for the activities you want to list.

The ref for a branch can be formatted either as refs/heads/BRANCH_NAME or BRANCH_NAME, where BRANCH_NAME is the name of your branch
    string ref?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # The activity type to filter by.

For example, you can choose to filter by "force_push", to see all force pushes to the repository
        "push"|"force_push"|"branch_creation"|"branch_deletion"|"pr_merge"|"merge_queue_merge"  activityType?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # The time period to filter by.

For example, day will filter for activity that occurred in the past 24 hours, and week will filter for activity that occurred in the past 7 days (168 hours)
        "day"|"week"|"month"|"quarter"|"year"  timePeriod?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: repos/list-autolinks

type ReposListAutolinksQueries record {
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-branches

type ReposListBranchesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Setting to true returns only protected branches. When set to false, only unprotected branches are returned. Omitting this parameter returns all branches
    boolean protected?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-collaborators

type ReposListCollaboratorsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter collaborators returned by their affiliation. outside means all outside collaborators of an organization-owned repository. direct means all collaborators with permissions to an organization-owned repository, regardless of organization membership status. all means all collaborators the authenticated user can see
        "outside"|"direct"|"all"  affiliation = "all";
    # Filter collaborators by the permissions they have on the repository. If not specified, all collaborators will be returned
        "pull"|"triage"|"push"|"maintain"|"admin"  permission?;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-comments-for-commit

type ReposListCommentsForCommitQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-commit-comments-for-repo

type ReposListCommitCommentsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-commits

type ReposListCommitsQueries record {
    # Only commits containing this file path will be returned
    string path?;
    # The number of results per page (max 100)
    int perPage = 30;
    # GitHub username or email address to use to filter by commit committer
    string committer?;
    # GitHub username or email address to use to filter by commit author
    string author?;
    # Only commits before this date will be returned. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string until?;
    # Page number of the results to fetch
    int page = 1;
    # SHA or branch to start listing commits from. Default: the repository’s default branch (usually main)
    string sha?;
    # Only show results that were last updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: repos/list-commit-statuses-for-ref

type ReposListCommitStatusesForRefQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-contributors

type ReposListContributorsQueries record {
    # Set to 1 or true to include anonymous contributors in results
    string anon?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-custom-deployment-rule-integrations

type ReposListCustomDeploymentRuleIntegrationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-deploy-keys

type ReposListDeployKeysQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-deployment-branch-policies

type ReposListDeploymentBranchPoliciesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-deployments

type ReposListDeploymentsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The name of the ref. This can be a branch, tag, or SHA
    string ref = "none";
    # The name of the environment that was deployed to (e.g., staging or production)
    string? environment = "none";
    # The name of the task for the deployment (e.g., deploy or deploy:migrations)
    string task = "none";
    # Page number of the results to fetch
    int page = 1;
    # The SHA recorded at creation time
    string sha = "none";
};

# Represents the Queries record for the operation: repos/list-deployment-statuses

type ReposListDeploymentStatusesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-for-authenticated-user

type ReposListForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Limit results to repositories with the specified visibility
        "all"|"public"|"private"  visibility = "all";
    # Comma-separated list of values. Can include:  
* owner: Repositories that are owned by the authenticated user.  
* collaborator: Repositories that the user has been added to as a collaborator.  
* organization_member: Repositories that the user has access to through being a member of an organization. This includes every repository on every team that the user is on
    string affiliation = "owner,collaborator,organization_member";
    # Only show repositories updated before the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string before?;
    # The property to sort the results by
        "created"|"updated"|"pushed"|"full_name"  sort = "full_name";
    # Page number of the results to fetch
    int page = 1;
    # Limit results to repositories of the specified type. Will cause a 422 error if used in the same request as **visibility** or **affiliation**
        "all"|"owner"|"public"|"private"|"member"  'type?;
    # The order to sort by. Default: asc when using full_name, otherwise desc
        "asc"|"desc"  direction?;
    # Only show repositories updated after the given time. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string since?;
};

# Represents the Queries record for the operation: repos/list-forks

type ReposListForksQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The sort order. stargazers will sort by star count
        "newest"|"oldest"|"stargazers"|"watchers"  sort = "newest";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-for-org

type ReposListForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"|"pushed"|"full_name"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # Specifies the types of repositories you want returned
        "all"|"public"|"private"|"forks"|"sources"|"member"  'type = "all";
    # The order to sort by. Default: asc when using full_name, otherwise desc
        "asc"|"desc"  direction?;
};

# Represents the Queries record for the operation: repos/list-for-user

type ReposListForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # The property to sort the results by
        "created"|"updated"|"pushed"|"full_name"  sort = "full_name";
    # Page number of the results to fetch
    int page = 1;
    # Limit results to repositories of the specified type
        "all"|"owner"|"member"  'type = "owner";
    # The order to sort by. Default: asc when using full_name, otherwise desc
        "asc"|"desc"  direction?;
};

# Represents the Queries record for the operation: repos/list-invitations-for-authenticated-user

type ReposListInvitationsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-invitations

type ReposListInvitationsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-pages-builds

type ReposListPagesBuildsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-public

type ReposListPublicQueries record {
    # A repository ID. Only return repositories with an ID greater than this ID
    int since?;
};

# Represents the Queries record for the operation: repos/list-pull-requests-associated-with-commit

type ReposListPullRequestsAssociatedWithCommitQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-release-assets

type ReposListReleaseAssetsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-releases

type ReposListReleasesQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-tags

type ReposListTagsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-teams

type ReposListTeamsQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: repos/list-webhook-deliveries

type ReposListWebhookDeliveriesQueries record {
    # Used for pagination: the starting delivery from which the page of deliveries is fetched. Refer to the link header for the next and previous page cursors
    string cursor?;
    # The number of results per page (max 100)
    int perPage = 30;
    # Filter to only return redeliveries.
    boolean redelivery?;
};

# Represents the Queries record for the operation: repos/list-webhooks

type ReposListWebhooksQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Require at least one approving review on a pull request, before merging. Set to null to disable

type ReposownerrepobranchesbranchprotectionRequiredPullRequestReviews record {
    # Specifies users, teams, or apps allowed to dismiss pull request reviews.
    ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsDismissalRestrictions dismissalRestrictions?;
    # Specify the number of reviewers required to approve pull requests. Use a number between 1 and 6 or 0 to not require reviewers
    int requiredApprovingReviewCount?;
    # Blocks merging pull requests until [code owners](https://docs.github.com/articles/about-code-owners/) review them
    boolean requireCodeOwnerReviews?;
    # Set to true if you want to automatically dismiss approving reviews when someone pushes a new commit
    boolean dismissStaleReviews?;
    # Specifies users, teams, or apps allowed to bypass required pull request reviews.
    ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsBypassPullRequestAllowances bypassPullRequestAllowances?;
    # Whether the most recent push must be approved by someone other than the person who pushed it. Default: false
    boolean requireLastPushApproval = false;
};

# Allow specific users, teams, or apps to bypass pull request requirements

type ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsBypassPullRequestAllowances record {
    # The list of team slugs allowed to bypass pull request requirements
    string[] teams?;
    # The list of user logins allowed to bypass pull request requirements
    string[] users?;
    # The list of app slugs allowed to bypass pull request requirements
    string[] apps?;
};

# Specify which users, teams, and apps can dismiss pull request reviews. Pass an empty dismissal_restrictions object to disable. User and team dismissal_restrictions are only available for organization-owned repositories. Omit this parameter for personal repositories

type ReposownerrepobranchesbranchprotectionRequiredPullRequestReviewsDismissalRestrictions record {
    # The list of team slugs with dismissal access
    string[] teams?;
    # The list of user logins with dismissal access
    string[] users?;
    # The list of app slugs with dismissal access
    string[] apps?;
};

# Require status checks to pass before merging. Set to null to disable

type ReposownerrepobranchesbranchprotectionRequiredStatusChecks record {
    # The list of status checks to require in order to merge into this branch
    ReposownerrepobranchesbranchprotectionRequiredStatusChecksChecks[] checks?;
    # **Deprecated**: The list of status checks to require in order to merge into this branch. If any of these checks have recently been set by a particular GitHub App, they will be required to come from that app in future for the branch to merge. Use checks instead of contexts for more fine-grained control
    string[] contexts;
    # Require branches to be up to date before merging
    boolean strict;
};


type ReposownerrepobranchesbranchprotectionRequiredStatusChecksChecks record {
    # The name of the required check
    string context;
    # The ID of the GitHub App that must provide this check. Omit this field to automatically select the GitHub App that has recently provided this check, or any app if it was not set by a GitHub App. Pass -1 to explicitly allow any app to set the status
    int appId?;
};


type ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf1 record {
    # The name of the status checks
    string[] contexts;
};


type ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf11 record {
    # The name of the status checks
    string[] contexts;
};


type ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf12 record {
    # The name of the status checks
    string[] contexts;
};

# Restrict who can push to the protected branch. User, app, and team restrictions are only available for organization-owned repositories. Set to null to disable

type ReposownerrepobranchesbranchprotectionRestrictions record {
    # The list of team slugs with push access
    string[] teams;
    # The list of user logins with push access
    string[] users;
    # The list of app slugs with push access
    string[] apps?;
};


type ReposownerrepobranchesbranchprotectionrestrictionsappsOneOf1 record {
    # The GitHub Apps that have push access to this branch. Use the slugified version of the app name. **Note**: The list of users, apps, and teams in total is limited to 100 items
    string[] apps;
};


type ReposownerrepobranchesbranchprotectionrestrictionsteamsOneOf1 record {
    # The slug values for teams
    string[] teams;
};


type ReposownerrepobranchesbranchprotectionrestrictionsusersOneOf1 record {
    # The username for users
    string[] users;
};


type ReposownerrepocheckSuitespreferencesAutoTriggerChecks record {
    # The id of the GitHub App
    int appId;
    # Set to true to enable automatic creation of CheckSuite events upon pushes to the repository, or false to disable them
    boolean setting = true;
};

# The author of the file. Default: The committer or the authenticated user if you omit committer

type ReposownerrepocontentspathAuthor record {
    # The date and time the commit was authored, in ISO 8601 format.
    string date?;
    # The name of the author or committer of the commit. You'll receive a 422 status code if name is omitted
    string name;
    # The email of the author or committer of the commit. You'll receive a 422 status code if email is omitted
    string email;
};

# object containing information about the author

type ReposownerrepocontentspathAuthor1 record {
    # The name of the author (or committer) of the commit
    string name?;
    # The email of the author (or committer) of the commit
    string email?;
};

# The person that committed the file. Default: the authenticated user

type ReposownerrepocontentspathCommitter record {
    # The date and time of the commit in ISO 8601 format.
    string date?;
    # The name of the author or committer of the commit. You'll receive a 422 status code if name is omitted
    string name;
    # The email of the author or committer of the commit. You'll receive a 422 status code if email is omitted
    string email;
};

# object containing information about the committer

type ReposownerrepocontentspathCommitter1 record {
    # The name of the author (or committer) of the commit
    string name?;
    # The email of the author (or committer) of the commit
    string email?;
};


type ReposownerrepoenvironmentsenvironmentNameReviewers record {
    # The id of the user or team who can review the deployment
    int id?;
    # The type of reviewer, either a user or a team.
    DeploymentReviewerType 'type?;
};

# Information about the author of the commit. By default, the author will be the authenticated user and the current date. See the author and committer object below for details

type ReposownerrepogitcommitsAuthor record {
    # Indicates when this commit was authored (or committed). This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string date?;
    # The name of the author (or committer) of the commit
    string name;
    # The email of the author (or committer) of the commit
    string email;
};

# Information about the person who is making the commit. By default, committer will use the information set in author. See the author and committer object below for details

type ReposownerrepogitcommitsCommitter record {
    # Indicates when this commit was authored (or committed). This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string date?;
    # The name of the author (or committer) of the commit
    string name?;
    # The email of the author (or committer) of the commit
    string email?;
};

# An object with information about the individual creating the tag

type ReposownerrepogittagsTagger record {
    # When this object was tagged. This is a timestamp in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format: YYYY-MM-DDTHH:MM:SSZ
    string date?;
    # The name of the author of the tag
    string name;
    # The email of the author of the tag
    string email;
};


type ReposownerrepogittreesTree record {
    # The file mode; one of 100644 for file (blob), 100755 for executable (blob), 040000 for subdirectory (tree), 160000 for submodule (commit), or 120000 for a blob that specifies the path of a symlink
        "100644"|"100755"|"040000"|"160000"|"120000"  mode?;
    # The file referenced in the tree
    string path?;
    # Either blob, tree, or commit
        "blob"|"tree"|"commit"  'type?;
    # The SHA1 checksum ID of the object in the tree. Also called tree.sha. If the value is null then the file will be deleted.  

**Note:** Use either tree.sha or content to specify the contents of the entry. Using both tree.sha and content will return an error
    string? sha?;
    # The content you want this file to have. GitHub will write this blob out and use that SHA for this entry. Use either this, or tree.sha.  

**Note:** Use either tree.sha or content to specify the contents of the entry. Using both tree.sha and content will return an error
    string content?;
};

# Key/value pairs to provide settings for this webhook

type ReposownerrepohooksConfig record {
    # The media type used to serialize payloads sent to this webhook.
    WebhookConfigContentType contentType?;
    # Determines whether SSL verification is performed for webhook deliveries.
    WebhookConfigInsecureSsl insecureSsl?;
    # The digest algorithm used to sign webhook payloads.
    string digest?;
    # The secret used to sign webhook payloads for verification.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads will be delivered.
    WebhookConfigUrl url?;
    # The token used for authentication with the webhook endpoint.
    string token?;
};

# Key/value pairs to provide settings for this webhook

type ReposownerrepohookshookIdConfig record {
    # The address used for certain webhook service integrations.
    string address?;
    # The media type used to serialize payloads sent to the webhook.
    WebhookConfigContentType contentType?;
    # Determines whether SSL verification is skipped for webhook delivery.
    WebhookConfigInsecureSsl insecureSsl?;
    # The shared secret used to sign webhook payloads.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads are delivered.
    WebhookConfigUrl url;
    # The chat room identifier for certain webhook service integrations.
    string room?;
};


type ReposownerrepoissuesissueNumberlabelsLabels record {
    # The name of the label to add to the issue.
    string name;
};


type ReposownerrepoissuesissueNumberlabelsOneOf1 record {
    # The names of the labels to set for the issue. The labels you set replace any existing labels. You can pass an empty array to remove all labels. Alternatively, you can pass a single label as a string or an array of labels directly, but GitHub recommends passing an object with the labels key. You can also add labels to the existing labels for an issue. For more information, see "[Add labels to an issue](https://docs.github.com/rest/issues/labels#add-labels-to-an-issue)."
    string[] labels?;
};


type ReposownerrepoissuesissueNumberlabelsOneOf11 record {
    # The names of the labels to add to the issue's existing labels. You can pass an empty array to remove all labels. Alternatively, you can pass a single label as a string or an array of labels directly, but GitHub recommends passing an object with the labels key. You can also replace all of the labels for an issue. For more information, see "[Set labels for an issue](https://docs.github.com/rest/issues/labels#set-labels-for-an-issue)."
    string[] labels?;
};


type ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf1123 record {
    # List of label names to apply to the issue.
    ReposownerrepoissuesissueNumberlabelsLabels[] labels?;
};


type ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf123 record {
    # The list of labels to apply to the issue.
    ReposownerrepoissuesissueNumberlabelsLabels[] labels?;
};


type ReposownerrepopullspullNumberreviewsComments record {
    # The relative path to the file that necessitates a review comment
    string path;
    # The side of the diff to place the review comment on.
    string side?;
    # The line number in the file where the comment applies.
    int line?;
    # The first line of the range for a multi-line comment.
    int startLine?;
    # The position in the diff where you want to add a review comment. Note this value is not the same as the line number in the file. For help finding the position value, read the note below
    int position?;
    # Text of the review comment
    string body;
    # The side of the diff for the start of a multi-line comment.
    string startSide?;
};

# Specify which security and analysis features to enable or disable for the repository.
# 
# To use this parameter, you must have admin permissions for the repository or be an owner or security manager for the organization that owns the repository. For more information, see "[Managing security managers in your organization](https://docs.github.com/organizations/managing-peoples-access-to-your-organization-with-roles/managing-security-managers-in-your-organization)."
# 
# For example, to enable GitHub Advanced Security, use this data in the body of the PATCH request:
# { "security_and_analysis": {"advanced_security": { "status": "enabled" } } }.
# 
# You can check which security and analysis features are currently enabled by using a GET /repos/{owner}/{repo} request

type ReposownerrepoSecurityAndAnalysis record {
    # Settings for secret scanning push protection on the repository.
    ReposownerrepoSecurityAndAnalysisSecretScanningPushProtection secretScanningPushProtection?;
    # Settings for secret scanning on the repository.
    ReposownerrepoSecurityAndAnalysisSecretScanning secretScanning?;
    # Settings for GitHub Advanced Security on the repository.
    ReposownerrepoSecurityAndAnalysisAdvancedSecurity advancedSecurity?;
};

# Use the status property to enable or disable GitHub Advanced Security for this repository. For more information, see "[About GitHub Advanced Security](/github/getting-started-with-github/learning-about-github/about-github-advanced-security)."

type ReposownerrepoSecurityAndAnalysisAdvancedSecurity record {
    # Can be enabled or disabled
    string status?;
};

# Use the status property to enable or disable secret scanning for this repository. For more information, see "[About secret scanning](/code-security/secret-security/about-secret-scanning)."

type ReposownerrepoSecurityAndAnalysisSecretScanning record {
    # Can be enabled or disabled
    string status?;
};

# Use the status property to enable or disable secret scanning push protection for this repository. For more information, see "[Protecting pushes with secret scanning](/code-security/secret-scanning/protecting-pushes-with-secret-scanning)."

type ReposownerrepoSecurityAndAnalysisSecretScanningPushProtection record {
    # Can be enabled or disabled
    string status?;
};


type RepoSubscriptionBody record {
    # Determines if notifications should be received from this repository
    boolean subscribed?;
    # Determines if all notifications should be blocked from this repository
    boolean ignored?;
};

# Represents the Queries record for the operation: repos/upload-release-asset

type ReposUploadReleaseAssetQueries record {
    # The file name of the asset.
    string name;
    # An optional label to display instead of the asset file name.
    string label?;
};


type RepoTopicsBody record {
    # An array of topics to add to the repository. Pass one or more topics to _replace_ the set of existing topics. Send an empty array ([]) to clear all topics from the repository. **Note:** Topic names cannot contain uppercase letters
    string[] names;
};


type RepoTransferBody record {
    # The username or organization name the repository will be transferred to
    string newOwner;
    # ID of the team or teams to add to the repository. Teams can only be added to organization-owned repositories
    int[] teamIds?;
    # The new name to be given to the repository
    string newName?;
};

# Legacy Review Comment

type ReviewComment record {
    # The HTML-rendered body of the review comment
    string bodyHtml?;
    # The SHA of the original commit to which the comment applied
    string originalCommitId;
    # Hypermedia links related to the review comment.
    ReviewCommentLinks links;
    # The plain text body of the review comment
    string bodyText?;
    # The ID of the comment this comment replies to
    int inReplyToId?;
    # The line of the blob to which the comment applies. The last line of the range for a multi-line comment
    int line?;
    # The diff hunk to which the comment applies
    string diffHunk;
    # The date the review comment was created
    string createdAt;
    # The first line of the range for a multi-line comment
    int? startLine?;
    # The body text of the review comment
    string body;
    # The association of the comment author with the repository
    AuthorAssociation authorAssociation;
    # The relative path of the file to which the comment applies
    string path;
    # The original line index in the diff to which the comment applied
    int originalPosition;
    # The date the review comment was last updated
    string updatedAt;
    # The ID of the pull request review this comment belongs to
    int? pullRequestReviewId;
    # The unique identifier of the review comment
    int id;
    # The side of the first line of the range for a multi-line comment
        "LEFT"|"RIGHT"  side = "RIGHT";
    # API URL for the pull request
    string pullRequestUrl;
    # API URL for the review comment
    string url;
    # The side of the first line of the range for a multi-line comment
        "LEFT"|"RIGHT"? startSide = "RIGHT";
    # The original line of the blob to which the comment applies. The last line of the range for a multi-line comment
    int originalLine?;
    # The original first line of the range for a multi-line comment
    int? originalStartLine?;
    # URL of the review comment on GitHub
    string htmlUrl;
    # Reaction rollup summarizing emoji reactions on the review comment.
    ReactionRollup reactions?;
    # The line index in the diff to which the comment applies
    int? position;
    # The SHA of the commit to which the comment applies
    string commitId;
    # The user who created the review comment.
    NullableSimpleUser? user;
    # The GraphQL node identifier of the review comment
    string nodeId;
};

# Hypermedia links for the review comment

type ReviewCommentLinks record {
    # Hypermedia link to the pull request the comment belongs to.
    Link pullRequest;
    # Hypermedia link to the review comment itself.
    Link self;
    # Hypermedia link to the HTML page of the review comment.
    Link html;
};


type ReviewCustomGatesCommentRequired record {
    # Comment associated with the pending deployment protection rule. **Required when state is not provided.**
    string comment;
    # The name of the environment to approve or reject
    string environmentName;
};


type ReviewCustomGatesStateRequired record {
    # Optional comment to include with the review
    string comment?;
    # Whether to approve or reject deployment to the specified environments
        "approved"|"rejected"  state;
    # The name of the environment to approve or reject
    string environmentName;
};

# Review Dismissed Issue Event

type ReviewDismissedIssueEvent record {
    # The user who dismissed the pull request review.
    SimpleUser actor;
    # The URL of the commit associated with this event.
    string? commitUrl;
    # Details about the review that was dismissed.
    ReviewDismissedIssueEventDismissedReview dismissedReview;
    # The GitHub App that performed this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The timestamp when the review dismissal event occurred.
    string createdAt;
    # The unique identifier of the issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL of the issue event.
    string url;
    # The GraphQL node ID of the issue event.
    string nodeId;
};


type ReviewDismissedIssueEventDismissedReview record {
    # The unique identifier of the dismissed review.
    int reviewId;
    # The state of the review when it was dismissed.
    string state;
    # The SHA of the commit at which the review was dismissed.
    string dismissalCommitId?;
    # The message provided when the review was dismissed.
    string? dismissalMessage;
};


type ReviewIdDismissalsBody record {
    # The message for the pull request review dismissal
    string message;
    # The event action for dismissing the pull request review.
        "DISMISS"  event?;
};


type ReviewIdEventsBody record {
    # The body text of the pull request review
    string body?;
    # The review action you want to perform. The review actions include: APPROVE, REQUEST_CHANGES, or COMMENT. When you leave this blank, the API returns _HTTP 422 (Unrecognizable entity)_ and sets the review action state to PENDING, which means you will need to re-submit the pull request review using a review action
        "APPROVE"|"REQUEST_CHANGES"|"COMMENT"  event;
};

# Review Requested Issue Event

type ReviewRequestedIssueEvent record {
    # The user who requested the review.
    SimpleUser actor;
    # The API URL of the commit associated with this event.
    string? commitUrl;
    # The team requested to review the pull request.
    Team requestedTeam?;
    # The GitHub App that performed this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The user who made the review request.
    SimpleUser reviewRequester;
    # The timestamp when this event was created.
    string createdAt;
    # The user requested to review the pull request.
    SimpleUser requestedReviewer?;
    # The unique identifier of this issue event.
    int id;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL for this issue event.
    string url;
    # The global node ID of this issue event.
    string nodeId;
};

# Review Request Removed Issue Event

type ReviewRequestRemovedIssueEvent record {
    # The user who triggered the review request removal event.
    SimpleUser actor;
    # The URL of the commit associated with the event.
    string? commitUrl;
    # The team whose review request was removed.
    Team requestedTeam?;
    # The GitHub App that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp;
    # The user who originally requested the review.
    SimpleUser reviewRequester;
    # The date and time the event was created.
    string createdAt;
    # The user whose review request was removed.
    SimpleUser requestedReviewer?;
    # The unique numeric identifier for the event.
    int id;
    # The type of issue event that occurred.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL for this issue event.
    string url;
    # The GraphQL node ID for the event.
    string nodeId;
};


type ReviewsreviewIdBody record {
    # The body text of the pull request review
    string body;
};


type Root record {
    # URL template for searching labels across repositories.
    string labelSearchUrl;
    # URL template for listing repositories belonging to an organization.
    string organizationRepositoriesUrl;
    # URL template for listing repositories belonging to a user.
    string userRepositoriesUrl;
    # URL template for accessing GitHub Gists.
    string gistsUrl;
    # URL for accessing the authenticated user's notifications.
    string notificationsUrl;
    # URL template for listing users the authenticated user follows.
    string followingUrl;
    # URL for accessing the authenticated user's SSH keys.
    string keysUrl;
    # URL template for searching GitHub users.
    string userSearchUrl;
    # URL for accessing available Atom RSS feeds.
    string feedsUrl;
    # URL template for searching repository topics.
    string topicSearchUrl?;
    # URL for accessing gists starred by the authenticated user.
    string starredGistsUrl;
    # URL template for accessing a specific user's profile.
    string userUrl;
    # URL template for accessing a specific repository.
    string repositoryUrl;
    # URL for listing organizations the authenticated user belongs to.
    string userOrganizationsUrl;
    # HTML URL for managing the current user's OAuth authorizations.
    string currentUserAuthorizationsHtmlUrl;
    # URL for retrieving available GitHub emojis.
    string emojisUrl;
    # URL template for accessing a specific organization's profile.
    string organizationUrl;
    # URL for GitHub's PubSubHubbub hub endpoint.
    string hubUrl;
    # URL template for accessing repositories starred by the authenticated user.
    string starredUrl;
    # URL for listing the authenticated user's followers.
    string followersUrl;
    # URL for managing the authenticated user's email addresses.
    string emailsUrl;
    # URL for checking the current API rate limit status.
    string rateLimitUrl;
    # URL template for searching commits across repositories.
    string commitSearchUrl;
    # URL template for accessing GitHub issues.
    string issuesUrl;
    # URL template for listing teams within an organization.
    string organizationTeamsUrl;
    # URL for listing all public GitHub Gists.
    string publicGistsUrl;
    # URL for managing OAuth authorizations.
    string authorizationsUrl;
    # URL for listing public GitHub events.
    string eventsUrl;
    # URL for accessing the authenticated user's profile.
    string currentUserUrl;
    # URL for listing the authenticated user's repositories.
    string currentUserRepositoriesUrl;
    # URL template for searching issues across repositories.
    string issueSearchUrl;
    # URL template for searching code across repositories.
    string codeSearchUrl;
    # URL template for searching GitHub repositories.
    string repositorySearchUrl;
};


type RulesetsrulesetIdBody record {
    # The actors that can bypass the rules in this ruleset
    RepositoryRulesetBypassActor[] bypassActors?;
    # The name of the ruleset
    string name?;
    # The enforcement level of the ruleset.
    RepositoryRuleEnforcement enforcement?;
    # An array of rules within the ruleset
    RepositoryRule[] rules?;
    # The conditions that determine which repositories the ruleset applies to.
    OrgRulesetConditions conditions?;
    # The target of the ruleset
        "branch"|"tag"  target?;
};


type RulesetsrulesetIdBody1 record {
    # The actors that can bypass the rules in this ruleset
    RepositoryRulesetBypassActor[] bypassActors?;
    # The name of the ruleset
    string name?;
    # The enforcement level of the ruleset.
    RepositoryRuleEnforcement enforcement?;
    # An array of rules within the ruleset
    RepositoryRule[] rules?;
    # The conditions that determine which refs the ruleset applies to.
    RepositoryRulesetConditions conditions?;
    # The target of the ruleset
        "branch"|"tag"  target?;
};

# Response

type RuleSuite record {
    # The result of the rule evaluations for rules with the active enforcement status
        "pass"|"fail"|"bypass"  result?;
    # The first commit sha before the push evaluation
    string beforeSha?;
    # The ref name that the evaluation ran on
    string ref?;
    # The result of the rule evaluations for rules with the active and evaluate enforcement statuses, demonstrating whether rules would pass or fail if all rules in the rule suite were active
        "pass"|"fail"  evaluationResult?;
    # Details on the evaluated rules
    RuleSuiteRuleEvaluations[] ruleEvaluations?;
    # The timestamp of when the push event occurred.
    string pushedAt?;
    # The handle for the GitHub user account
    string actorName?;
    # The ID of the repository associated with the rule evaluation
    int repositoryId?;
    # The unique identifier of the rule insight
    int id?;
    # The number that identifies the user
    int actorId?;
    # The last commit sha in the push evaluation
    string afterSha?;
    # The name of the repository without the .git extension
    string repositoryName?;
};


type RuleSuiteRuleEvaluations record {
    # The source ruleset or rule that was evaluated.
    RuleSuiteRuleSource ruleSource?;
    # The result of the evaluation of the individual rule
        "pass"|"fail"  result?;
    # The type of rule
    string ruleType?;
    # The enforcement level of this rule source
        "active"|"evaluate"|"deleted ruleset"  enforcement?;
    # Any associated details with the rule evaluation
    string details?;
};


type RuleSuiteRuleSource record {
    # The name of the rule source
    string? name?;
    # The ID of the rule source
    int? id?;
    # The type of rule source
    string 'type?;
};


type RuleSuitesInner record {
    # The result of the rule evaluations for rules with the active enforcement status
        "pass"|"fail"|"bypass"  result?;
    # The first commit sha before the push evaluation
    string beforeSha?;
    # The ref name that the evaluation ran on
    string ref?;
    # The result of the rule evaluations for rules with the active and evaluate enforcement statuses, demonstrating whether rules would pass or fail if all rules in the rule suite were active
        "pass"|"fail"  evaluationResult?;
    # The timestamp indicating when the push event occurred.
    string pushedAt?;
    # The handle for the GitHub user account
    string actorName?;
    # The ID of the repository associated with the rule evaluation
    int repositoryId?;
    # The unique identifier of the rule insight
    int id?;
    # The number that identifies the user
    int actorId?;
    # The last commit sha in the push evaluation
    string afterSha?;
    # The name of the repository without the .git extension
    string repositoryName?;
};


type RunIdPendingDeploymentsBody record {
    # A comment to accompany the deployment review
    string comment;
    # Whether to approve or reject deployment to the specified environments
        "approved"|"rejected"  state;
    # The list of environment ids to approve or reject
    int[] environmentIds;
};

# A self hosted runner

type Runner record {
    # The Operating System of the runner
    string os;
    # Whether the runner is currently executing a job.
    boolean busy;
    # The name of the runner
    string name;
    # The id of the runner
    int id;
    # The id of the runner group
    int runnerGroupId?;
    # The status of the runner
    string status;
    # The labels assigned to the runner for job routing.
    RunnerLabel[] labels;
};

# Runner Application

type RunnerApplication record {
    # The SHA-256 checksum of the runner application download.
    string sha256Checksum?;
    # The filename of the runner application package.
    string filename;
    # The operating system the runner application targets.
    string os;
    # A short lived bearer token used to download the runner, if needed
    string tempDownloadToken?;
    # The URL to download the runner application.
    string downloadUrl;
    # The CPU architecture the runner application targets.
    string architecture;
};


type RunnerIdLabelsBody record {
    # The names of the custom labels to set for the runner. You can pass an empty array to remove all custom labels
    string[] labels;
};


type RunnerIdLabelsBody1 record {
    # The names of the custom labels to add to the runner
    string[] labels;
};

# A label for a self hosted runner

type RunnerLabel record {
    # Name of the label
    string name;
    # Unique identifier of the label
    int id?;
    # The type of label. Read-only labels are applied automatically when the runner is configured
        "read-only"|"custom"  'type?;
};

# A label for a self hosted runner

type RunnerLabelResponse record {
    # The total number of labels associated with the runner.
    int totalCount;
    # The list of labels assigned to the self-hosted runner.
    RunnerLabel[] labels;
};

# A self hosted runner

type RunnerResponse record {
    # The total number of self-hosted runners available.
    int totalCount;
    # The list of self-hosted runners.
    Runner[] runners;
};


type RunnersGenerateJitconfigBody record {
    # The working directory to be used for job execution, relative to the runner install directory
    string workFolder = "_work";
    # The name of the new runner
    string name;
    # The ID of the runner group to register the runner to
    int runnerGroupId;
    # The names of the custom labels to add to the runner. **Minimum items**: 1. **Maximum items**: 100
    string[] labels;
};

# Represents the Queries record for the operation: search/code

type SearchCodeQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query). See "[Searching code](https://docs.github.com/search-github/searching-on-github/searching-code)" for a detailed list of qualifiers
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # **This field is deprecated.** Sorts the results of your query. Can only be indexed, which indicates how recently a file has been indexed by the GitHub search infrastructure. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "indexed"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # **This field is deprecated.** Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort.
        "desc"|"asc"  'order = "desc";
};

# Represents the Queries record for the operation: search/commits

type SearchCommitsQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query). See "[Searching commits](https://docs.github.com/search-github/searching-on-github/searching-commits)" for a detailed list of qualifiers
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # Sorts the results of your query by author-date or committer-date. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "author-date"|"committer-date"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort
        "desc"|"asc"  'order = "desc";
};

# Represents the Queries record for the operation: search/issues-and-pull-requests

type SearchIssuesAndPullRequestsQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query). See "[Searching issues and pull requests](https://docs.github.com/search-github/searching-on-github/searching-issues-and-pull-requests)" for a detailed list of qualifiers
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # Sorts the results of your query by the number of comments, reactions, reactions-+1, reactions--1, reactions-smile, reactions-thinking_face, reactions-heart, reactions-tada, or interactions. You can also sort results by how recently the items were created or updated, Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "comments"|"reactions"|"reactions-+1"|"reactions--1"|"reactions-smile"|"reactions-thinking_face"|"reactions-heart"|"reactions-tada"|"interactions"|"created"|"updated"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort
        "desc"|"asc"  'order = "desc";
};

# Represents the Queries record for the operation: search/labels

type SearchLabelsQueries record {
    # The search keywords. This endpoint does not accept qualifiers in the query. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query)
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # The id of the repository
    int repositoryId;
    # Sorts the results of your query by when the label was created or updated. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "created"|"updated"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort
        "desc"|"asc"  'order = "desc";
};

# Represents the Queries record for the operation: search/repos

type SearchReposQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query). See "[Searching for repositories](https://docs.github.com/articles/searching-for-repositories/)" for a detailed list of qualifiers
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # Sorts the results of your query by number of stars, forks, or help-wanted-issues or how recently the items were updated. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "stars"|"forks"|"help-wanted-issues"|"updated"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort
        "desc"|"asc"  'order = "desc";
};


type SearchResultTextMatchesInner record {
    # A text fragment from the object that contains the match.
    string fragment?;
    # The URL of the object where the match was found.
    string objectUrl?;
    # The type of object that contains the search match.
    string? objectType?;
    # The property name of the object where the match occurred.
    string property?;
    # A list of match details including indices and text.
    SearchResultTextMatchesInnerMatches[] matches?;
};


type SearchResultTextMatchesInnerMatches record {
    # Start and end character positions of the match within the text.
    int[] indices?;
    # The matched text fragment found in the search result.
    string text?;
};

# Represents the Queries record for the operation: search/topics

type SearchTopicsQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query)
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: search/users

type SearchUsersQueries record {
    # The query contains one or more search keywords and qualifiers. Qualifiers allow you to limit your search to specific areas of GitHub. The REST API supports the same qualifiers as the web interface for GitHub. To learn more about the format of the query, see [Constructing a search query](https://docs.github.com/rest/search/search#constructing-a-search-query). See "[Searching users](https://docs.github.com/search-github/searching-on-github/searching-users)" for a detailed list of qualifiers
    string q;
    # The number of results per page (max 100)
    int perPage = 30;
    # Sorts the results of your query by number of followers or repositories, or when the person joined GitHub. Default: [best match](https://docs.github.com/rest/search/search#ranking-search-results)
        "followers"|"repositories"|"joined"  sort?;
    # Page number of the results to fetch
    int page = 1;
    # Determines whether the first search result returned is the highest number of matches (desc) or lowest number of matches (asc). This parameter is ignored unless you provide sort
        "desc"|"asc"  'order = "desc";
};


type SecretNameRepositoriesBody record {
    # An array of repository ids that can access the organization secret. You can only provide a list of repository ids when the visibility is set to selected. You can add and remove individual repositories using the [Add selected repository to an organization secret](https://docs.github.com/rest/actions/secrets#add-selected-repository-to-an-organization-secret) and [Remove selected repository from an organization secret](https://docs.github.com/rest/actions/secrets#remove-selected-repository-from-an-organization-secret) endpoints
    int[] selectedRepositoryIds;
};


type SecretNameRepositoriesBody1 record {
    # An array of repository ids that can access the organization secret. You can only provide a list of repository ids when the visibility is set to selected. You can add and remove individual repositories using the [Set selected repositories for an organization secret](https://docs.github.com/rest/codespaces/organization-secrets#set-selected-repositories-for-an-organization-secret) and [Remove selected repository from an organization secret](https://docs.github.com/rest/codespaces/organization-secrets#remove-selected-repository-from-an-organization-secret) endpoints
    int[] selectedRepositoryIds;
};


type SecretNameRepositoriesBody2 record {
    # An array of repository ids that can access the organization secret. You can only provide a list of repository ids when the visibility is set to selected. You can add and remove individual repositories using the [Set selected repositories for an organization secret](https://docs.github.com/rest/dependabot/secrets#set-selected-repositories-for-an-organization-secret) and [Remove selected repository from an organization secret](https://docs.github.com/rest/dependabot/secrets#remove-selected-repository-from-an-organization-secret) endpoints
    int[] selectedRepositoryIds;
};


type SecretNameRepositoriesBody3 record {
    # An array of repository ids for which a codespace can access the secret. You can manage the list of selected repositories using the [List selected repositories for a user secret](https://docs.github.com/rest/codespaces/secrets#list-selected-repositories-for-a-user-secret), [Add a selected repository to a user secret](https://docs.github.com/rest/codespaces/secrets#add-a-selected-repository-to-a-user-secret), and [Remove a selected repository from a user secret](https://docs.github.com/rest/codespaces/secrets#remove-a-selected-repository-from-a-user-secret) endpoints
    int[] selectedRepositoryIds;
};


type SecretScanningAlert record {
    # The type of secret that secret scanning detected
    string secretType?;
    # The user who bypassed push protection for this alert.
    NullableSimpleUser? pushProtectionBypassedBy?;
    # The time that push protection was bypassed in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string? pushProtectionBypassedAt?;
    # Timestamp when the secret scanning alert was created.
    AlertCreatedAt createdAt?;
    # User-friendly name for the detected secret, matching the secret_type.
For a list of built-in patterns, see "[Secret scanning patterns](https://docs.github.com/code-security/secret-scanning/secret-scanning-patterns#supported-secrets-for-advanced-security)."
    string secretTypeDisplayName?;
    # The secret that was detected
    string secret?;
    # The reason for resolving the secret scanning alert.
    SecretScanningAlertResolution? resolution?;
    # REST API URL of the secret scanning alert.
    AlertUrl url?;
    # The unique number identifying this secret scanning alert.
    AlertNumber number?;
    # The user who resolved the secret scanning alert.
    NullableSimpleUser? resolvedBy?;
    # Timestamp when the secret scanning alert was last updated.
    NullableAlertUpdatedAt? updatedAt?;
    # The REST API URL of the code locations for this alert
    string locationsUrl?;
    # An optional comment to resolve an alert
    string? resolutionComment?;
    # The time that the alert was resolved in ISO 8601 format: YYYY-MM-DDTHH:MM:SSZ
    string? resolvedAt?;
    # URL to the secret scanning alert on GitHub.
    AlertHtmlUrl htmlUrl?;
    # Whether push protection was bypassed for the detected secret
    boolean? pushProtectionBypassed?;
    # The state of the secret scanning alert, open or resolved.
    SecretScanningAlertState state?;
};

# Represents the Queries record for the operation: secret-scanning/list-alerts-for-enterprise

type SecretScanningListAlertsForEnterpriseQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of secret types to return. By default all secret types are returned.
See "[Secret scanning patterns](https://docs.github.com/code-security/secret-scanning/secret-scanning-patterns#supported-secrets-for-advanced-security)"
for a complete list of secret types
    string secretType?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # Set to open or resolved to only list secret scanning alerts in a specific state
        "open"|"resolved"  state?;
    # The property to sort the results by. created means when the alert was created. updated means when the alert was updated or resolved
        "created"|"updated"  sort = "created";
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # A comma-separated list of resolutions. Only secret scanning alerts with one of these resolutions are listed. Valid resolutions are false_positive, wont_fix, revoked, pattern_edited, pattern_deleted or used_in_tests
    string resolution?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: secret-scanning/list-alerts-for-org

type SecretScanningListAlertsForOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of secret types to return. By default all secret types are returned.
See "[Secret scanning patterns](https://docs.github.com/code-security/secret-scanning/secret-scanning-patterns#supported-secrets-for-advanced-security)"
for a complete list of secret types
    string secretType?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for events before this cursor. To receive an initial cursor on your first request, include an empty "before" query string
    string before?;
    # Set to open or resolved to only list secret scanning alerts in a specific state
        "open"|"resolved"  state?;
    # The property to sort the results by. created means when the alert was created. updated means when the alert was updated or resolved
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for events after this cursor.  To receive an initial cursor on your first request, include an empty "after" query string
    string after?;
    # A comma-separated list of resolutions. Only secret scanning alerts with one of these resolutions are listed. Valid resolutions are false_positive, wont_fix, revoked, pattern_edited, pattern_deleted or used_in_tests
    string resolution?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: secret-scanning/list-alerts-for-repo

type SecretScanningListAlertsForRepoQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # A comma-separated list of secret types to return. By default all secret types are returned.
See "[Secret scanning patterns](https://docs.github.com/code-security/secret-scanning/secret-scanning-patterns#supported-secrets-for-advanced-security)"
for a complete list of secret types
    string secretType?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for events before this cursor. To receive an initial cursor on your first request, include an empty "before" query string
    string before?;
    # Set to open or resolved to only list secret scanning alerts in a specific state
        "open"|"resolved"  state?;
    # The property to sort the results by. created means when the alert was created. updated means when the alert was updated or resolved
        "created"|"updated"  sort = "created";
    # Page number of the results to fetch
    int page = 1;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for events after this cursor.  To receive an initial cursor on your first request, include an empty "after" query string
    string after?;
    # A comma-separated list of resolutions. Only secret scanning alerts with one of these resolutions are listed. Valid resolutions are false_positive, wont_fix, revoked, pattern_edited, pattern_deleted or used_in_tests
    string resolution?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: secret-scanning/list-locations-for-alert

type SecretScanningListLocationsForAlertQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type SecretScanningLocation record {
    # Location details specific to the type where the secret was found.
    SecretScanningLocationCommit|SecretScanningLocationIssueTitle|SecretScanningLocationIssueBody|SecretScanningLocationIssueComment details;
    # The location type. Because secrets may be found in different types of resources (ie. code, comments, issues), this field identifies the type of resource where the secret was found
        "commit"|"issue_title"|"issue_body"|"issue_comment"  'type;
};

# Represents a 'commit' secret scanning location type. This location type shows that a secret was detected inside a commit to a repository

type SecretScanningLocationCommit record {
    # The file path in the repository
    string path;
    # The column at which the secret starts within the start line when the file is interpreted as 8BIT ASCII
    decimal startColumn;
    # The API URL to get the associated commit resource
    string commitUrl;
    # SHA-1 hash ID of the associated commit
    string commitSha;
    # Line number at which the secret starts in the file
    decimal startLine;
    # SHA-1 hash ID of the associated blob
    string blobSha;
    # The API URL to get the associated blob resource
    string blobUrl;
    # Line number at which the secret ends in the file
    decimal endLine;
    # The column at which the secret ends within the end line when the file is interpreted as 8BIT ASCII
    decimal endColumn;
};

# Represents an 'issue_body' secret scanning location type. This location type shows that a secret was detected in the body of an issue

type SecretScanningLocationIssueBody record {
    # The API URL to get the issue where the secret was detected
    string issueBodyUrl;
};

# Represents an 'issue_comment' secret scanning location type. This location type shows that a secret was detected in a comment on an issue

type SecretScanningLocationIssueComment record {
    # The API URL to get the issue comment where the secret was detected
    string issueCommentUrl;
};

# Represents an 'issue_title' secret scanning location type. This location type shows that a secret was detected in the title of an issue

type SecretScanningLocationIssueTitle record {
    # The API URL to get the issue where the secret was detected
    string issueTitleUrl;
};


type SecretssecretNameBody record {
    # An array of repository ids that can access the organization secret. You can only provide a list of repository ids when the visibility is set to selected. You can manage the list of selected repositories using the [List selected repositories for an organization secret](https://docs.github.com/rest/actions/secrets#list-selected-repositories-for-an-organization-secret), [Set selected repositories for an organization secret](https://docs.github.com/rest/actions/secrets#set-selected-repositories-for-an-organization-secret), and [Remove selected repository from an organization secret](https://docs.github.com/rest/actions/secrets#remove-selected-repository-from-an-organization-secret) endpoints
    int[] selectedRepositoryIds?;
    # Which type of organization repositories have access to the organization secret. selected means only the repositories specified by selected_repository_ids can access the secret
        "all"|"private"|"selected"  visibility;
    # ID of the key you used to encrypt the secret
    string keyId?;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get an organization public key](https://docs.github.com/rest/actions/secrets#get-an-organization-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody1 record {
    # An array of repository IDs that can access the organization secret. You can only provide a list of repository IDs when the visibility is set to selected. You can manage the list of selected repositories using the [List selected repositories for an organization secret](https://docs.github.com/rest/codespaces/organization-secrets#list-selected-repositories-for-an-organization-secret), [Set selected repositories for an organization secret](https://docs.github.com/rest/codespaces/organization-secrets#set-selected-repositories-for-an-organization-secret), and [Remove selected repository from an organization secret](https://docs.github.com/rest/codespaces/organization-secrets#remove-selected-repository-from-an-organization-secret) endpoints
    int[] selectedRepositoryIds?;
    # Which type of organization repositories have access to the organization secret. selected means only the repositories specified by selected_repository_ids can access the secret
        "all"|"private"|"selected"  visibility;
    # The ID of the key you used to encrypt the secret
    string keyId?;
    # The value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get an organization public key](https://docs.github.com/rest/codespaces/organization-secrets#get-an-organization-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody2 record {
    # An array of repository ids that can access the organization secret. You can only provide a list of repository ids when the visibility is set to selected. You can manage the list of selected repositories using the [List selected repositories for an organization secret](https://docs.github.com/rest/dependabot/secrets#list-selected-repositories-for-an-organization-secret), [Set selected repositories for an organization secret](https://docs.github.com/rest/dependabot/secrets#set-selected-repositories-for-an-organization-secret), and [Remove selected repository from an organization secret](https://docs.github.com/rest/dependabot/secrets#remove-selected-repository-from-an-organization-secret) endpoints
    string[] selectedRepositoryIds?;
    # Which type of organization repositories have access to the organization secret. selected means only the repositories specified by selected_repository_ids can access the secret
        "all"|"private"|"selected"  visibility;
    # ID of the key you used to encrypt the secret
    string keyId?;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get an organization public key](https://docs.github.com/rest/dependabot/secrets#get-an-organization-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody3 record {
    # ID of the key you used to encrypt the secret
    string keyId?;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get a repository public key](https://docs.github.com/rest/actions/secrets#get-a-repository-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody4 record {
    # ID of the key you used to encrypt the secret
    string keyId?;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get a repository public key](https://docs.github.com/rest/codespaces/repository-secrets#get-a-repository-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody5 record {
    # ID of the key you used to encrypt the secret
    string keyId?;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get a repository public key](https://docs.github.com/rest/dependabot/secrets#get-a-repository-public-key) endpoint
    string encryptedValue?;
};


type SecretssecretNameBody6 record {
    # ID of the key you used to encrypt the secret
    string keyId;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get an environment public key](https://docs.github.com/rest/actions/secrets#get-an-environment-public-key) endpoint
    string encryptedValue;
};


type SecretssecretNameBody7 record {
    # An array of repository ids that can access the user secret. You can manage the list of selected repositories using the [List selected repositories for a user secret](https://docs.github.com/rest/codespaces/secrets#list-selected-repositories-for-a-user-secret), [Set selected repositories for a user secret](https://docs.github.com/rest/codespaces/secrets#set-selected-repositories-for-a-user-secret), and [Remove a selected repository from a user secret](https://docs.github.com/rest/codespaces/secrets#remove-a-selected-repository-from-a-user-secret) endpoints
    UsercodespacessecretssecretNameSelectedRepositoryIds[] selectedRepositoryIds?;
    # ID of the key you used to encrypt the secret
    string keyId;
    # Value for your secret, encrypted with [LibSodium](https://libsodium.gitbook.io/doc/bindings_for_other_languages) using the public key retrieved from the [Get the public key for the authenticated user](https://docs.github.com/rest/codespaces/secrets#get-public-key-for-the-authenticated-user) endpoint
    string encryptedValue?;
};

# Represents the Queries record for the operation: security-advisories/list-global-advisories

type SecurityAdvisoriesListGlobalAdvisoriesQueries record {
    # If specified, only advisories with these severities will be returned
        "unknown"|"low"|"medium"|"high"|"critical"  severity?;
    # The number of results per page (max 100)
    int perPage = 30;
    # If specified, only advisories for these ecosystems will be returned
        "actions"|"composer"|"erlang"|"go"|"maven"|"npm"|"nuget"|"other"|"pip"|"pub"|"rubygems"|"rust"  ecosystem?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # Whether to only return advisories that have been withdrawn
    boolean isWithdrawn?;
    # If specified, only return advisories that affect any of package or package@version. A maximum of 1000 packages can be specified.
If the query parameter causes the URL to exceed the maximum URL length supported by your client, you must specify fewer packages.

Example: affects=package1,package2@1.0.0,package3@^2.0.0 or affects[]=package1&affects[]=package2@1.0.0
    Affects affects?;
    # If specified, only return advisories that were published on a date or date range.

For more information on the syntax of the date range, see "[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates)."
    string published?;
    # The property to sort the results by
        "updated"|"published"  sort = "published";
    # If specified, only advisories of this type will be returned. By default, a request with no other parameters defined will only return reviewed advisories that are not malware
        "reviewed"|"malware"|"unreviewed"  'type = "reviewed";
    # If specified, only advisories with these Common Weakness Enumerations (CWEs) will be returned.

Example: cwes=79,284,22 or cwes[]=79&cwes[]=284&cwes[]=22
    Cwes cwes?;
    # If specified, only advisories with this GHSA (GitHub Security Advisory) identifier will be returned
    string ghsaId?;
    # If specified, only advisories with this CVE (Common Vulnerabilities and Exposures) identifier will be returned
    string cveId?;
    # If specified, only show advisories that were updated or published on a date or date range.

For more information on the syntax of the date range, see "[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates)."
    string modified?;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # If specified, only return advisories that were updated on a date or date range.

For more information on the syntax of the date range, see "[Understanding the search syntax](https://docs.github.com/search-github/getting-started-with-searching-on-github/understanding-the-search-syntax#query-for-dates)."
    string updated?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: security-advisories/list-org-repository-advisories

type SecurityAdvisoriesListOrgRepositoryAdvisoriesQueries record {
    # The number of advisories to return per page
    int perPage = 30;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # The property to sort the results by
        "created"|"updated"|"published"  sort = "created";
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # Filter by the state of the repository advisories. Only advisories of this state will be returned
        "triage"|"draft"|"published"|"closed"  state?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: security-advisories/list-repository-advisories

type SecurityAdvisoriesListRepositoryAdvisoriesQueries record {
    # Number of advisories to return per page
    int perPage = 30;
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results before this cursor
    string before?;
    # The property to sort the results by
        "created"|"updated"|"published"  sort = "created";
    # A cursor, as given in the [Link header](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api#using-link-headers). If specified, the query only searches for results after this cursor
    string after?;
    # Filter by state of the repository advisories. Only advisories of this state will be returned
        "triage"|"draft"|"published"|"closed"  state?;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};


type SecurityAndAnalysis record {
    # Settings for secret scanning push protection on the repository.
    SecurityAndAnalysisSecretScanningPushProtection secretScanningPushProtection?;
    # Settings for Dependabot security updates on the repository.
    SecurityAndAnalysisDependabotSecurityUpdates dependabotSecurityUpdates?;
    # Settings for secret scanning on the repository.
    SecurityAndAnalysisSecretScanningPushProtection secretScanning?;
    # Settings for GitHub Advanced Security on the repository.
    SecurityAndAnalysisAdvancedSecurity advancedSecurity?;
};

# Enable or disable GitHub Advanced Security for the repository.
# 
# For standalone Code Scanning or Secret Protection products, this parameter cannot be used

type SecurityAndAnalysisAdvancedSecurity record {
    # The enablement status of GitHub Advanced Security for the repository.
        "enabled"|"disabled"  status?;
};

# Enable or disable Dependabot security updates for the repository

type SecurityAndAnalysisDependabotSecurityUpdates record {
    # The enablement status of Dependabot security updates for the repository
        "enabled"|"disabled"  status?;
};


type SecurityAndAnalysisSecretScanningPushProtection record {
    # Whether secret scanning push protection is enabled or disabled.
        "enabled"|"disabled"  status?;
};


type SecurityProductenablementBody record {
    # CodeQL query suite to be used. If you specify the query_suite parameter, the default setup will be configured with this query suite only on all repositories that didn't have default setup already configured. It will not change the query suite on repositories that already have default setup configured.
If you don't specify any query_suite in your request, the preferred query suite of the organization will be applied.
        "default"|"extended"  querySuite?;
};


type SelectedActions record {
    # Specifies a list of string-matching patterns to allow specific action(s) and reusable workflow(s). Wildcards, tags, and SHAs are allowed. For example, monalisa/octocat@*, monalisa/octocat@v2, monalisa/*.

**Note**: The patterns_allowed setting only applies to public repositories
    string[] patternsAllowed?;
    # Whether actions from GitHub Marketplace verified creators are allowed. Set to true to allow all actions by GitHub Marketplace verified creators
    boolean verifiedAllowed?;
    # Whether GitHub-owned actions are allowed. For example, this includes the actions in the actions organization
    boolean githubOwnedAllowed?;
};

# Short Blob

type ShortBlob record {
    # The SHA hash of the blob.
    string sha;
    # API URL for the blob.
    string url;
};

# Short Branch

type ShortBranch record {
    # Whether the branch has branch protection enabled.
    boolean protected;
    # The name of the branch.
    string name;
    # The latest commit on the branch.
    CommitSearchResultItemCommitTree 'commit;
    # Branch protection settings applied to this branch.
    BranchProtection protection?;
    # The API URL for the branch's protection settings.
    string protectionUrl?;
};

# A GitHub Classroom classroom

type SimpleClassroom record {
    # Returns whether classroom is archived or not
    boolean archived;
    # The name of the classroom
    string name;
    # Unique identifier of the classroom
    int id;
    # The url of the classroom on GitHub Classroom
    string url;
};

# A GitHub Classroom assignment

type SimpleClassroomAssignment record {
    # The selected editor for the assignment
    string editor;
    # Whether feedback pull request will be created on assignment acceptance
    boolean feedbackPullRequestsEnabled;
    # Whether the invitation link is enabled. Visiting an enabled invitation link will accept the assignment
    boolean invitationsEnabled;
    # Whether students are admins on created repository on accepted assignment
    boolean studentsAreRepoAdmins;
    # The number of students that have accepted the assignment
    int accepted;
    # The programming language used in the assignment
    string language;
    # The GitHub Classroom this assignment belongs to.
    SimpleClassroom classroom;
    # Assignment title
    string title;
    # Whether it's a Group Assignment or Individual Assignment
        "individual"|"group"  'type;
    # The link that a student can use to accept the assignment
    string inviteLink;
    # The number of students that have submitted the assignment
    int submitted;
    # The maximum allowable teams for the assignment
    int? maxTeams?;
    # Whether an accepted assignment creates a public repository
    boolean publicRepo;
    # The maximum allowable members per team
    int? maxMembers?;
    # Unique identifier of the repository
    int id;
    # The number of students that have passed the assignment
    int passing;
    # The time at which the assignment is due
    string? deadline;
    # Sluggified name of the assignment
    string slug;
};

# A GitHub organization

type SimpleClassroomOrganization record {
    # URL of the organization's avatar image.
    string avatarUrl;
    # URL of the organization's GitHub profile page.
    string htmlUrl;
    # The display name of the organization.
    string? name;
    # The unique identifier of the organization.
    int id;
    # The login name of the organization.
    string login;
    # The GraphQL node identifier of the organization.
    string nodeId;
};

# A GitHub repository view for Classroom

type SimpleClassroomRepository record {
    # Whether the repository is private
    boolean 'private;
    # The full, globally unique name of the repository
    string fullName;
    # The URL to view the repository on GitHub.com
    string htmlUrl;
    # The default branch for the repository
    string defaultBranch;
    # A unique identifier of the repository
    int id;
    # The GraphQL identifier of the repository
    string nodeId;
};

# A GitHub user simplified for Classroom

type SimpleClassroomUser record {
    # The URL of the user's avatar image.
    string avatarUrl;
    # The HTML URL of the user's GitHub profile.
    string htmlUrl;
    # The unique numeric identifier of the GitHub user.
    int id;
    # The GitHub username (login handle) of the user.
    string login;
};

# A commit

type SimpleCommit record {
    # The person who committed the changes to the repository.
    SimpleCommitCommitter? committer;
    # SHA for the commit's tree
    string treeId;
    # The person who authored the commit.
    SimpleCommitAuthor? author;
    # SHA for the commit
    string id;
    # Message describing the purpose of the commit
    string message;
    # Timestamp of the commit
    string timestamp;
};

# Information about the Git author

type SimpleCommitAuthor record {
    # Name of the commit's author
    string name;
    # Git email address of the commit's author
    string email;
};

# Information about the Git committer

type SimpleCommitCommitter record {
    # Name of the commit's committer
    string name;
    # Git email address of the commit's committer
    string email;
};


type SimpleCommitStatus record {
    # The avatar URL of the user or app that created the status.
    string? avatarUrl;
    # The date and time the status was last updated.
    string updatedAt;
    # The URL associated with the status for additional details.
    string? targetUrl;
    # The label identifying the status check provider.
    string context;
    # A short description of the commit status.
    string? description;
    # The date and time the status was created.
    string createdAt;
    # The unique identifier of the commit status.
    int id;
    # The state of the commit status.
    string state;
    # Whether this status check is required to pass.
    boolean? required?;
    # The API URL for the commit status.
    string url;
    # The GraphQL node identifier of the commit status.
    string nodeId;
};

# A GitHub repository

type SimpleRepository record {
    # The API URL to get information about tags on the repository
    string tagsUrl;
    # Whether the repository is private
    boolean 'private;
    # A template for the API URL to list the contributors to the repository
    string contributorsUrl;
    # A template for the API URL to get information about notifications on the repository
    string notificationsUrl;
    # The repository description
    string? description;
    # The API URL to subscribe to notifications for this repository
    string subscriptionUrl;
    # A template for the API URL to get information about branches in the repository
    string branchesUrl;
    # The API URL to list the deployments of the repository
    string deploymentsUrl;
    # A template for the API URL to get information about deploy keys on the repository
    string keysUrl;
    # A template for the API URL to get information about issue comments on the repository
    string issueCommentUrl;
    # A template for the API URL to get information about labels of the repository
    string labelsUrl;
    # The API URL to list the subscribers on the repository
    string subscribersUrl;
    # A template for the API URL to get information about releases on the repository
    string releasesUrl;
    # A template for the API URL to get information about comments on the repository
    string commentsUrl;
    # The API URL to list the stargazers on the repository
    string stargazersUrl;
    # A unique identifier of the repository
    int id;
    # The user or organization that owns the repository.
    SimpleUser owner;
    # A template for the API URL to download the repository as an archive
    string archiveUrl;
    # A template for the API URL to get information about commits on the repository
    string commitsUrl;
    # A template for the API URL to get information about Git refs of the repository
    string gitRefsUrl;
    # A template for the API URL to compare two commits or refs
    string compareUrl;
    # The API URL to list the forks of the repository
    string forksUrl;
    # A template for the API URL to get information about statuses of a commit
    string statusesUrl;
    # A template for the API URL to get information about Git commits of the repository
    string gitCommitsUrl;
    # A template for the API URL to create or retrieve a raw Git blob in the repository
    string blobsUrl;
    # A template for the API URL to get information about Git tags of the repository
    string gitTagsUrl;
    # The API URL to merge branches in the repository
    string mergesUrl;
    # The API URL to list the downloads on the repository
    string downloadsUrl;
    # The URL to get more information about the repository from the GitHub API
    string url;
    # A template for the API URL to get the contents of the repository
    string contentsUrl;
    # A template for the API URL to get information about milestones of the repository
    string milestonesUrl;
    # The API URL to list the teams on the repository
    string teamsUrl;
    # Whether the repository is a fork
    boolean 'fork;
    # A template for the API URL to get information about issues on the repository
    string issuesUrl;
    # The full, globally unique, name of the repository
    string fullName;
    # The API URL to list the events of the repository
    string eventsUrl;
    # A template for the API URL to get information about issue events on the repository
    string issueEventsUrl;
    # The API URL to get information about the languages of the repository
    string languagesUrl;
    # The URL to view the repository on GitHub.com
    string htmlUrl;
    # A template for the API URL to get information about collaborators of the repository
    string collaboratorsUrl;
    # The name of the repository
    string name;
    # A template for the API URL to get information about pull requests on the repository
    string pullsUrl;
    # The API URL to list the hooks on the repository
    string hooksUrl;
    # A template for the API URL to list the available assignees for issues in the repository
    string assigneesUrl;
    # A template for the API URL to create or retrieve a raw Git tree of the repository
    string treesUrl;
    # The GraphQL identifier of the repository
    string nodeId;
};

# A GitHub user

type SimpleUser record {
    # API URL template for the user's gists
    string gistsUrl;
    # API URL to list the user's repositories
    string reposUrl;
    # API URL template to check who the user is following
    string followingUrl;
    # API URL template for repositories the user has starred
    string starredUrl;
    # The username of the user
    string login;
    # API URL to list the user's followers
    string followersUrl;
    # The type of the account
    string 'type;
    # The time the user starred the resource
    string starredAt?;
    # API URL for the user
    string url;
    # API URL to list repositories the user is watching
    string subscriptionsUrl;
    # API URL for events received by the user
    string receivedEventsUrl;
    # URL of the user's avatar image
    string avatarUrl;
    # API URL template for the user's events
    string eventsUrl;
    # URL of the user's GitHub profile page
    string htmlUrl;
    # The display name of the user
    string? name?;
    # Whether the user is a GitHub site administrator
    boolean siteAdmin;
    # The unique identifier of the user
    int id;
    # The Gravatar ID of the user
    string? gravatarId;
    # The publicly visible email address of the user
    string? email?;
    # The GraphQL node identifier of the user
    string nodeId;
    # API URL to list the user's organizations
    string organizationsUrl;
};

# Create a new snapshot of a repository's dependencies

type Snapshot record {
    # The repository branch that triggered this snapshot
    string ref;
    # Additional metadata associated with the dependency snapshot.
    Metadata metadata?;
    # The time at which the snapshot was scanned
    string scanned;
    # A collection of package manifests, which are a collection of related dependencies declared in a file or representing a logical group of dependencies
    record {} manifests?;
    # Details about the CI job that generated this snapshot.
    SnapshotJob job;
    # The version of the repository snapshot submission
    int version;
    # The commit SHA associated with this dependency snapshot. Maximum length: 40 characters
    string sha;
    # Information about the tool that detected the dependencies.
    SnapshotDetector detector;
};

# A description of the detector used

type SnapshotDetector record {
    # The name of the detector used
    string name;
    # The version of the detector used
    string version;
    # The url of the detector used
    string url;
};


type SnapshotJob record {
    # Correlator provides a key that is used to group snapshots submitted over time. Only the "latest" submitted snapshot for a given combination of job.correlator and detector.name will be considered when calculating a repository's current dependencies. Correlator should be as unique as it takes to distinguish all detection runs for a given "wave" of CI workflow you run. If you're using GitHub Actions, a good default value for this could be the environment variables GITHUB_WORKFLOW and GITHUB_JOB concatenated together. If you're using a build matrix, then you'll also need to add additional key(s) to distinguish between each submission inside a matrix variation
    string correlator;
    # The url for the job
    string htmlUrl?;
    # The external ID of the job
    string id;
};

# Snapshot of a newly created repository's dependencies

type SnapshotResponse record {
    # Either "SUCCESS", "ACCEPTED", or "INVALID". "SUCCESS" indicates that the snapshot was successfully created and the repository's dependencies were updated. "ACCEPTED" indicates that the snapshot was successfully created, but the repository's dependencies were not updated. "INVALID" indicates that the snapshot was malformed
    string result;
    # The time at which the snapshot was created
    string createdAt;
    # ID of the created snapshot
    int id;
    # A message providing further details about the result, such as why the dependencies were not updated
    string message;
};

# Social media account

type SocialAccount record {
    # The social media platform provider name.
    string provider;
    # The URL of the user's social media profile.
    string url;
};

# A public SSH key used to sign Git commits

type SshSigningKey record {
    # The date and time the SSH signing key was created.
    string createdAt;
    # The unique identifier of the SSH signing key.
    int id;
    # The descriptive name given to the SSH signing key.
    string title;
    # The public SSH key value used for signing Git commits.
    string 'key;
};

# Stargazer

type Stargazer record {
    # The date and time when the user starred the repository.
    string starredAt;
    # The user who starred the repository.
    NullableSimpleUser? user;
};

# Starred Repository

type StarredRepository record {
    # The repository that was starred.
    Repository repo;
    # The date and time when the repository was starred.
    string starredAt;
};

# State Change Issue Event

type StateChangeIssueEvent record {
    # The user who triggered the state change event.
    SimpleUser actor;
    # The URL of the commit associated with the state change.
    string? commitUrl;
    # The reason for the issue state change.
    string? stateReason?;
    # The GitHub App that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp;
    # The date and time the event was created.
    string createdAt;
    # The unique identifier of the event.
    int id;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with the state change.
    string? commitId;
    # The API URL for the event.
    string url;
    # The GraphQL node identifier of the event.
    string nodeId;
};

# The status of a commit

type Status record {
    # The user who created this commit status.
    NullableSimpleUser? creator;
    # The avatar URL of the status creator.
    string? avatarUrl;
    # The timestamp when this status was last updated.
    string updatedAt;
    # The URL associated with this status for details.
    string? targetUrl;
    # A label identifying the service that set this status.
    string context;
    # A short description of the status.
    string? description;
    # The timestamp when this status was created.
    string createdAt;
    # The unique identifier of this commit status.
    int id;
    # The state of the commit status (e.g., success, failure).
    string state;
    # The API URL for this commit status.
    string url;
    # The global node ID of this commit status.
    string nodeId;
};

# Status Check Policy

type StatusCheckPolicy record {
    # The list of required status checks that must pass before merging.
    StatusCheckPolicyChecks[] checks;
    # API URL for listing required status check contexts.
    string contextsUrl;
    # The list of required status check context names.
    string[] contexts;
    # Whether the branch must be up to date before merging.
    boolean strict;
    # API URL for the status check policy.
    string url;
};


type StatusCheckPolicyChecks record {
    # The name of the required status check context.
    string context;
    # The ID of the GitHub App that must provide this status check.
    int? appId;
};


type StatusesshaBody record {
    # The target URL to associate with this status. This URL will be linked from the GitHub UI to allow users to easily see the source of the status.  
For example, if your continuous integration system is posting build status, you would want to provide the deep link for the build output for this specific SHA:  
http://ci.example.com/user/repo/build/sha
    string? targetUrl?;
    # A string label to differentiate this status from the status of other systems. This field is case-insensitive
    string context = "default";
    # A short description of the status
    string? description?;
    # The state of the status
        "error"|"failure"|"pending"|"success"  state;
};

# Tag

type Tag record {
    # The URL to download the repository as a zip archive at this tag.
    string zipballUrl;
    # The name of the tag.
    string name;
    # The commit object that the tag points to.
    CommitSearchResultItemCommitTree 'commit;
    # The URL to download the repository as a tarball at this tag.
    string tarballUrl;
    # The GraphQL node identifier for the tag.
    string nodeId;
};

# Tag protection

type TagProtection record {
    # The timestamp when this tag protection rule was last updated.
    string updatedAt?;
    # The pattern that tags must match to be protected.
    string pattern;
    # The timestamp when this tag protection rule was created.
    string createdAt?;
    # Unique numeric identifier for this tag protection rule.
    int id?;
    # Whether this tag protection rule is currently enabled.
    boolean enabled?;
};


type TagsProtectionBody record {
    # An optional glob pattern to match against when enforcing tag protection
    string pattern;
};

# Groups of organization members that gives permissions on specified repositories

type Team record {
    # The parent team of this team, if nested.
    NullableTeamSimple? parent;
    # The API URL for listing repositories this team has access to.
    string repositoriesUrl;
    # The API URL template for listing team members.
    string membersUrl;
    # A short description of the team.
    string? description;
    # The privacy level of the team, either secret or visible.
    string privacy?;
    # The default permission level for the team on its repositories.
    string permission;
    # The API URL for the team.
    string url;
    # The notification setting the team has chosen for its members.
    string notificationSetting?;
    # The permissions the team has on its repositories.
    TeamPermissions permissions?;
    # The URL of the team's page on GitHub.
    string htmlUrl;
    # The name of the team.
    string name;
    # The unique identifier of the team.
    int id;
    # The URL-friendly identifier of the team.
    string slug;
    # The GraphQL node identifier of the team.
    string nodeId;
};

# A team discussion is a persistent record of a free-form conversation within a team

type TeamDiscussion record {
    # The HTML-rendered version of the discussion body.
    string bodyHtml;
    # Whether or not this discussion should be pinned for easy retrieval
    boolean pinned;
    # Whether or not this discussion should be restricted to team members and organization administrators
    boolean 'private;
    # The current version of the body content. If provided, this update operation will be rejected if the given version does not match the latest version on the server
    string bodyVersion;
    # The user who authored the discussion.
    NullableSimpleUser? author;
    # The timestamp indicating when the discussion was created.
    string createdAt;
    # The main text of the discussion
    string body;
    # The title of the discussion
    string title;
    # The API URL of the discussion resource.
    string url;
    # The unique sequence number of a team discussion
    int number;
    # The timestamp indicating when the discussion was last updated.
    string updatedAt;
    # The total number of comments on the discussion.
    int commentsCount;
    # The URL to view the discussion on GitHub.
    string htmlUrl;
    # The API URL to list comments on the discussion.
    string commentsUrl;
    # The timestamp indicating when the discussion was last edited.
    string? lastEditedAt;
    # The reaction counts for the discussion.
    ReactionRollup reactions?;
    # The API URL of the team that owns the discussion.
    string teamUrl;
    # The GraphQL node identifier of the discussion.
    string nodeId;
};

# A reply to a discussion within a team

type TeamDiscussionComment record {
    # The HTML-rendered body of the comment.
    string bodyHtml;
    # The current version of the body content. If provided, this update operation will be rejected if the given version does not match the latest version on the server
    string bodyVersion;
    # The user who authored the comment.
    NullableSimpleUser? author;
    # The date and time when the comment was created.
    string createdAt;
    # The main text of the comment
    string body;
    # API URL for this discussion comment.
    string url;
    # The unique sequence number of a team discussion comment
    int number;
    # The date and time when the comment was last updated.
    string updatedAt;
    # URL of the comment on GitHub.
    string htmlUrl;
    # The date and time when the comment was last edited.
    string? lastEditedAt;
    # Reaction counts aggregated for this comment.
    ReactionRollup reactions?;
    # API URL for the discussion this comment belongs to.
    string discussionUrl;
    # The GraphQL node identifier of the comment.
    string nodeId;
};

# Groups of organization members that gives permissions on specified repositories

type TeamFull record {
    # The parent team of this team, if applicable.
    NullableTeamSimple? parent?;
    # The API URL for the team's repositories.
    string repositoriesUrl;
    # The number of repositories accessible to the team.
    int reposCount;
    # The API URL template for the team's members.
    string membersUrl;
    # The description of the team.
    string? description;
    # The level of privacy this team should have
        "closed"|"secret"  privacy?;
    # The timestamp when the team was created.
    string createdAt;
    # Permission that the team will have for its repositories
    string permission;
    # URL for the team
    string url;
    # The notification setting the team has set
        "notifications_enabled"|"notifications_disabled"  notificationSetting?;
    # Distinguished Name (DN) that team maps to within LDAP environment
    string ldapDn?;
    # The timestamp when the team was last updated.
    string updatedAt;
    # The URL of the team page on GitHub.
    string htmlUrl;
    # The organization to which the team belongs.
    TeamOrganization organization;
    # Name of the team
    string name;
    # The number of members in the team.
    int membersCount;
    # Unique identifier of the team
    int id;
    # The URL-friendly name of the team.
    string slug;
    # The GraphQL node identifier of the team.
    string nodeId;
};

# Team Membership

type TeamMembership record {
    # The role of the user in the team
        "member"|"maintainer"  role = "member";
    # The state of the user's membership in the team
        "active"|"pending"  state;
    # The API URL for the team membership resource.
    string url;
};

# Team Organization

type TeamOrganization record {
    # API URL for listing the organization's repositories
    string reposUrl;
    # Whether members can create internal repositories
    boolean membersCanCreateInternalRepositories?;
    # Whether members can create public GitHub Pages sites
    boolean membersCanCreatePublicPages?;
    # The URL of the organization's blog or website
    string blog?;
    # The type of the account
    string 'type;
    # API URL template for listing public members
    string publicMembersUrl;
    # The number of private gists
    int? privateGists?;
    # The default permission level for organization repositories
    string? defaultRepositoryPermission?;
    # The billing email address for the organization
    string? billingEmail?;
    # The total disk usage in kilobytes
    int? diskUsage?;
    # The number of collaborators across all repositories
    int? collaborators?;
    # The unique identifier of the organization
    int id;
    # The billing plan associated with the organization.
    TeamOrganizationPlan plan?;
    # Whether members can create private GitHub Pages sites
    boolean membersCanCreatePrivatePages?;
    # Whether members can create repositories
    boolean? membersCanCreateRepositories?;
    # Whether members can create private repositories
    boolean membersCanCreatePrivateRepositories?;
    # The number of public gists in the organization
    int publicGists;
    # The number of followers of the organization
    int followers;
    # Whether the organization has projects enabled
    boolean hasOrganizationProjects;
    # The number of accounts the organization is following
    int following;
    # URL of the organization's GitHub profile page
    string htmlUrl;
    # The display name of the organization
    string name?;
    # API URL for listing the organization's webhooks
    string hooksUrl;
    # Whether repositories can have projects enabled
    boolean hasRepositoryProjects;
    # API URL template for listing the organization's members
    string membersUrl;
    # The Twitter username of the organization
    string? twitterUsername?;
    # A short description of the organization
    string? description;
    # The date the organization was created
    string createdAt;
    # The login name of the organization
    string login;
    # The total number of private repositories
    int totalPrivateRepos?;
    # The date the organization was last updated
    string updatedAt;
    # The types of repositories members can create
    string membersAllowedRepositoryCreationType?;
    # Whether members can fork private repositories
    boolean? membersCanForkPrivateRepositories?;
    # The company name of the organization
    string company?;
    # The number of owned private repositories
    int ownedPrivateRepos?;
    # The number of public repositories in the organization
    int publicRepos;
    # The publicly visible email of the organization
    string email?;
    # Whether two-factor authentication is required for members
    boolean? twoFactorRequirementEnabled?;
    # The date the organization was archived
    string? archivedAt;
    # Whether the organization's domain is verified
    boolean isVerified?;
    # Whether commit sign-off is required for web-based commits
    boolean webCommitSignoffRequired?;
    # API URL for the organization
    string url;
    # Whether members can create public repositories
    boolean membersCanCreatePublicRepositories?;
    # API URL for listing the organization's issues
    string issuesUrl;
    # URL of the organization's avatar image
    string avatarUrl;
    # API URL for listing the organization's events
    string eventsUrl;
    # Whether members can create GitHub Pages sites
    boolean membersCanCreatePages?;
    # The geographic location of the organization
    string location?;
    # The GraphQL node identifier of the organization
    string nodeId;
};

# The billing plan for the organization

type TeamOrganizationPlan record {
    # The number of private repositories allowed under this plan.
    int privateRepos;
    # The number of seats currently occupied in the organization.
    int filledSeats?;
    # The name of the billing plan.
    string name;
    # The total number of seats available in this billing plan.
    int seats?;
    # The total storage space in bytes allocated by this plan.
    int space;
};


type TeamPermissions record {
    # Whether the team has pull (read) access to the repository.
    boolean pull;
    # Whether the team has maintain access to the repository.
    boolean maintain;
    # Whether the team has admin access to the repository.
    boolean admin;
    # Whether the team has triage access to the repository.
    boolean triage;
    # Whether the team has push (write) access to the repository.
    boolean push;
};

# A team's access to a project

type TeamProject record {
    # The API URL to list the project's columns.
    string columnsUrl;
    # The user who created this project.
    SimpleUser creator;
    # Whether the project is private or not. Only present when owner is an organization
    boolean 'private?;
    # The organization permission for this project. Only present when owner is an organization
    string organizationPermission?;
    # The timestamp when this project was created.
    string createdAt;
    # The description or body text of the project.
    string? body;
    # The API URL for this project.
    string url;
    # The API URL of the project's owner.
    string ownerUrl;
    # The project number within the owner's namespace.
    int number;
    # The timestamp when this project was last updated.
    string updatedAt;
    # The team's permissions for this project.
    TeamProjectPermissions permissions;
    # The HTML URL to view this project on GitHub.
    string htmlUrl;
    # The name of the project.
    string name;
    # The unique identifier of this project.
    int id;
    # The current state of the project (open or closed).
    string state;
    # The global node ID of this project.
    string nodeId;
};


type TeamProjectPermissions record {
    # Whether the team has read permission on the project.
    boolean read;
    # Whether the team has admin permission on the project.
    boolean admin;
    # Whether the team has write permission on the project.
    boolean write;
};

# A team's access to a repository

type TeamRepository record {
    # Whether to allow forking this repo
    boolean allowForking = false;
    # The number of users who have starred the repository.
    int stargazersCount;
    # Whether this repository acts as a template that can be used to generate new repositories
    boolean isTemplate = false;
    # The date and time of the most recent push to the repository.
    string? pushedAt;
    # The API URL to manage the authenticated user's subscription to the repository.
    string subscriptionUrl;
    # The primary programming language used in the repository.
    string? language;
    # A template for the API URL to get information about branches in the repository.
    string branchesUrl;
    # A template for the API URL to get information about issue comments on the repository.
    string issueCommentUrl;
    # Whether to allow rebase merges for pull requests
    boolean allowRebaseMerge = true;
    # A template for the API URL to get information about labels of the repository.
    string labelsUrl;
    # The team's permissions for the repository.
    TeamRepositoryPermissions permissions?;
    # The API URL to list the subscribers on the repository.
    string subscribersUrl;
    # A temporary token used to clone the repository.
    string tempCloneToken?;
    # A template for the API URL to get information about releases on the repository.
    string releasesUrl;
    # The Subversion URL to check out the repository.
    string svnUrl;
    # The number of users watching the repository.
    int subscribersCount?;
    # Unique identifier of the repository
    int id;
    # The name of the master branch of the repository.
    string masterBranch?;
    # The number of forks of the repository.
    int forks;
    # A template for the API URL to download the repository as an archive.
    string archiveUrl;
    # Whether to allow merge commits for pull requests
    boolean allowMergeCommit = true;
    # A template for the API URL to get information about Git refs of the repository.
    string gitRefsUrl;
    # The API URL to list the forks of the repository.
    string forksUrl;
    # The repository visibility: public, private, or internal
    string visibility = "public";
    # A template for the API URL to get information about statuses of a commit.
    string statusesUrl;
    # The number of repositories in the repository's network.
    int networkCount?;
    # The SSH URL to clone the repository.
    string sshUrl;
    # The role name assigned to the team for this repository.
    string roleName?;
    # The license associated with the repository.
    NullableLicenseSimple? license;
    # The full, globally unique name of the repository.
    string fullName;
    # The size of the repository in kilobytes.
    int size;
    # The repository used as a template to create this repository.
    NullableRepository? templateRepository?;
    # Whether to allow Auto-merge to be used on pull requests
    boolean allowAutoMerge = false;
    # The API URL to get information about the languages used in the repository.
    string languagesUrl;
    # The URL to view the repository on GitHub.com.
    string htmlUrl;
    # A template for the API URL to get information about collaborators of the repository.
    string collaboratorsUrl;
    # The HTTPS URL to clone the repository.
    string cloneUrl;
    # The name of the repository
    string name;
    # A template for the API URL to get information about pull requests on the repository.
    string pullsUrl;
    # The default branch of the repository
    string defaultBranch;
    # The API URL to list the hooks on the repository.
    string hooksUrl;
    # A template for the API URL to create or retrieve a raw Git tree of the repository.
    string treesUrl;
    # The API URL to get information about tags on the repository.
    string tagsUrl;
    # Whether the repository is private or public
    boolean 'private = false;
    # The API URL to list the contributors to the repository.
    string contributorsUrl;
    # Whether downloads are enabled
    boolean hasDownloads = true;
    # A template for the API URL to get information about notifications on the repository.
    string notificationsUrl;
    # The number of open issues in the repository.
    int openIssuesCount;
    # The description of the repository.
    string? description;
    # The date and time the repository was created.
    string? createdAt;
    # The number of users watching the repository.
    int watchers;
    # The API URL to list the deployments of the repository.
    string deploymentsUrl;
    # A template for the API URL to get information about deploy keys on the repository.
    string keysUrl;
    # Whether projects are enabled
    boolean hasProjects = true;
    # Whether the repository is archived
    boolean archived = false;
    # Whether the wiki is enabled
    boolean hasWiki = true;
    # The date and time the repository was last updated.
    string? updatedAt;
    # A template for the API URL to get information about comments on the repository.
    string commentsUrl;
    # The API URL to list the stargazers on the repository.
    string stargazersUrl;
    # Returns whether or not this repository disabled
    boolean disabled;
    # Whether to delete head branches when pull requests are merged
    boolean deleteBranchOnMerge = false;
    # The Git URL to clone the repository.
    string gitUrl;
    # Whether the repository has GitHub Pages enabled.
    boolean hasPages;
    # The user or organization that owns the repository.
    NullableSimpleUser? owner;
    # Whether to allow squash merges for pull requests
    boolean allowSquashMerge = true;
    # A template for the API URL to get information about commits on the repository.
    string commitsUrl;
    # A template for the API URL to compare two commits or refs.
    string compareUrl;
    # A template for the API URL to get information about Git commits of the repository.
    string gitCommitsUrl;
    # The list of topics associated with the repository.
    string[] topics?;
    # A template for the API URL to create or retrieve a raw Git blob in the repository.
    string blobsUrl;
    # A template for the API URL to get information about Git tags of the repository.
    string gitTagsUrl;
    # The API URL to merge branches in the repository.
    string mergesUrl;
    # The API URL to list the downloads on the repository.
    string downloadsUrl;
    # Whether issues are enabled
    boolean hasIssues = true;
    # Whether to require contributors to sign off on web-based commits
    boolean webCommitSignoffRequired = false;
    # The API URL for the repository.
    string url;
    # A template for the API URL to get the contents of the repository.
    string contentsUrl;
    # The URL of the repository's mirror, if it is a mirror.
    string? mirrorUrl;
    # A template for the API URL to get information about milestones of the repository.
    string milestonesUrl;
    # The API URL to list the teams on the repository.
    string teamsUrl;
    # Whether the repository is a fork of another repository.
    boolean 'fork;
    # A template for the API URL to get information about issues on the repository.
    string issuesUrl;
    # The API URL to list the events of the repository.
    string eventsUrl;
    # A template for the API URL to get information about issue events on the repository.
    string issueEventsUrl;
    # A template for the API URL to list the available assignees for issues in the repository.
    string assigneesUrl;
    # The number of open issues and pull requests in the repository.
    int openIssues;
    # The number of users watching the repository.
    int watchersCount;
    # The GraphQL node identifier of the repository.
    string nodeId;
    # The URL of the repository's homepage.
    string? homepage;
    # The number of forks of the repository.
    int forksCount;
};


type TeamRepositoryPermissions record {
    # Indicates whether the team has pull (read) access to the repository.
    boolean pull;
    # Indicates whether the team has maintain access to the repository.
    boolean maintain?;
    # Indicates whether the team has admin access to the repository.
    boolean admin;
    # Indicates whether the team has triage access to the repository.
    boolean triage?;
    # Indicates whether the team has push (write) access to the repository.
    boolean push;
};

# Groups of organization members that gives permissions on specified repositories

type TeamSimple record {
    # The API URL listing repositories accessible to the team.
    string repositoriesUrl;
    # The API URL template listing members of the team.
    string membersUrl;
    # Description of the team
    string? description;
    # The level of privacy this team should have
    string privacy?;
    # Permission that the team will have for its repositories
    string permission;
    # URL for the team
    string url;
    # The notification setting the team has set
    string notificationSetting?;
    # Distinguished Name (DN) that team maps to within LDAP environment
    string ldapDn?;
    # The URL to view the team on GitHub.
    string htmlUrl;
    # Name of the team
    string name;
    # Unique identifier of the team
    int id;
    # The URL-friendly identifier for the team.
    string slug;
    # The GraphQL node identifier for the team.
    string nodeId;
};

# Represents the Queries record for the operation: teams/list-child-in-org

type TeamsListChildInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-child-legacy

type TeamsListChildLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-discussion-comments-in-org

type TeamsListDiscussionCommentsInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: teams/list-discussion-comments-legacy

type TeamsListDiscussionCommentsLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: teams/list-discussions-in-org

type TeamsListDiscussionsInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Pinned discussions only filter
    string pinned?;
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: teams/list-discussions-legacy

type TeamsListDiscussionsLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
    # The direction to sort the results by
        "asc"|"desc"  direction = "desc";
};

# Represents the Queries record for the operation: teams/list-for-authenticated-user

type TeamsListForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-members-in-org

type TeamsListMembersInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filters members returned by their role in the team
        "member"|"maintainer"|"all"  role = "all";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-members-legacy

type TeamsListMembersLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Filters members returned by their role in the team
        "member"|"maintainer"|"all"  role = "all";
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-pending-invitations-in-org

type TeamsListPendingInvitationsInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-pending-invitations-legacy

type TeamsListPendingInvitationsLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-projects-in-org

type TeamsListProjectsInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-projects-legacy

type TeamsListProjectsLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list

type TeamsListQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-repos-in-org

type TeamsListReposInOrgQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: teams/list-repos-legacy

type TeamsListReposLegacyQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type TeamSlugDiscussionsBody record {
    # Private posts are only visible to team members, organization owners, and team maintainers. Public posts are visible to all members of the organization. Set to true to create a private post
    boolean 'private = false;
    # The discussion post's title
    string title;
    # The discussion post's body text
    string body;
};


type TeamsteamIdBody record {
    # The ID of a team to set as the parent team
    int? parentTeamId?;
    # The name of the team
    string name;
    # The description of the team
    string description?;
    # The level of privacy this team should have. Editing teams without specifying this parameter leaves privacy intact. The options are:  
**For a non-nested team:**  
* secret - only visible to organization owners and members of this team.  
* closed - visible to all members of this organization.  
**For a parent or child team:**  
* closed - visible to all members of this organization
        "secret"|"closed"  privacy?;
    # **Deprecated**. The permission that new repositories will be added to the team with when none is specified
        "pull"|"push"|"admin"  permission = "pull";
    # The notification setting the team has chosen. Editing teams without specifying this parameter leaves notification_setting intact. The options are: 
* notifications_enabled - team members receive notifications when the team is @mentioned.  
* notifications_disabled - no one receives notifications
        "notifications_enabled"|"notifications_disabled"  notificationSetting?;
};


type TeamsteamSlugBody record {
    # The ID of a team to set as the parent team
    int? parentTeamId?;
    # The name of the team
    string name?;
    # The description of the team
    string description?;
    # The level of privacy this team should have. Editing teams without specifying this parameter leaves privacy intact. When a team is nested, the privacy for parent teams cannot be secret. The options are:  
**For a non-nested team:**  
* secret - only visible to organization owners and members of this team.  
* closed - visible to all members of this organization.  
**For a parent or child team:**  
* closed - visible to all members of this organization
        "secret"|"closed"  privacy?;
    # **Deprecated**. The permission that new repositories will be added to the team with when none is specified
        "pull"|"push"|"admin"  permission = "pull";
    # The notification setting the team has chosen. Editing teams without specifying this parameter leaves notification_setting intact. The options are: 
* notifications_enabled - team members receive notifications when the team is @mentioned.  
* notifications_disabled - no one receives notifications
        "notifications_enabled"|"notifications_disabled"  notificationSetting?;
};


type TemplateRepoGenerateBody record {
    # The organization or person who will own the new repository. To create a new repository in an organization, the authenticated user must be a member of the specified organization
    string owner?;
    # Either true to create a new private repository or false to create a new public one
    boolean 'private = false;
    # Set to true to include the directory structure and files from all branches in the template repository, and not just the default branch. Default: false
    boolean includeAllBranches = false;
    # The name of the new repository
    string name;
    # A short description of the new repository
    string description?;
};


type ThreadIdSubscriptionBody record {
    # Whether to block all notifications from a thread
    boolean ignored = false;
};

# Thread Subscription

type ThreadSubscription record {
    # Whether the authenticated user is subscribed to the thread.
    boolean subscribed;
    # Whether the authenticated user has chosen to ignore the thread.
    boolean ignored;
    # The reason the authenticated user is subscribed to the thread.
    string? reason;
    # The date and time the subscription was created.
    string? createdAt;
    # API URL for the subscribed thread.
    string threadUrl?;
    # API URL for the repository containing the thread.
    string repositoryUrl?;
    # API URL for the thread subscription.
    string url;
};

# Timeline Assigned Issue Event

type TimelineAssignedIssueEvent record {
    # The user who performed the assignment action.
    SimpleUser actor;
    # API URL of the commit associated with this event.
    string? commitUrl;
    # The GitHub App that triggered this event, if any.
    NullableIntegration? performedViaGithubApp;
    # The date and time the event was created.
    string createdAt;
    # The unique identifier of the event.
    int id;
    # The user who was assigned to the issue.
    SimpleUser assignee;
    # The type of event that occurred.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # API URL for this event.
    string url;
    # The GraphQL node identifier of the event.
    string nodeId;
};

# Timeline Comment Event

type TimelineCommentEvent record {
    # The API URL of the issue this comment belongs to.
    string issueUrl;
    # The HTML-rendered body of the comment.
    string bodyHtml?;
    # The plain text body of the comment.
    string bodyText?;
    # The date and time the comment was created.
    string createdAt;
    # Contents of the issue comment
    string body?;
    # URL for the issue comment
    string url;
    # The user who performed the comment action.
    SimpleUser actor;
    # The role of the commenter in relation to the repository.
    AuthorAssociation authorAssociation;
    # The date and time the comment was last updated.
    string updatedAt;
    # The GitHub app that created the comment, if applicable.
    NullableIntegration? performedViaGithubApp?;
    # The URL of the comment on GitHub.
    string htmlUrl;
    # The reaction counts for this comment.
    ReactionRollup reactions?;
    # Unique identifier of the issue comment
    int id;
    # The type of timeline event.
    string event;
    # The user who created the comment.
    SimpleUser user;
    # The GraphQL node identifier of this comment.
    string nodeId;
};

# Timeline Commit Commented Event

type TimelineCommitCommentedEvent record {
    # The list of comments made on the commit.
    CommitComment[] comments?;
    # The type of timeline event.
    string event?;
    # The SHA of the commit that was commented on.
    string commitId?;
    # The GraphQL node identifier of this event.
    string nodeId?;
};

# Timeline Committed Event

type TimelineCommittedEvent record {
    # The person who committed the code to the repository.
    GitCommitCommitter committer;
    # The person who authored the commit.
    GitCommitCommitter author;
    # The URL to view the commit on GitHub.
    string htmlUrl;
    # The Git tree object associated with this commit.
    TimelineCommittedEventTree tree;
    # The type of timeline event, identifying this as a commit.
    string event?;
    # Message describing the purpose of the commit
    string message;
    # SHA for the commit
    string sha;
    # The API URL of the commit resource.
    string url;
    # The verification details for the commit signature.
    TimelineCommittedEventVerification verification;
    # The GraphQL node identifier of the commit.
    string nodeId;
    # The list of parent commits for this commit.
    GitCommitParents[] parents;
};


type TimelineCommittedEventTree record {
    # SHA for the commit
    string sha;
    # API URL for the tree object.
    string url;
};


type TimelineCommittedEventVerification record {
    # The reason for the verification status of the commit signature.
    string reason;
    # The cryptographic signature used to verify the commit.
    string? signature;
    # The signed data payload used for commit signature verification.
    string? payload;
    # Indicates whether the commit signature has been verified.
    boolean verified;
};

# Timeline Cross Referenced Event

type TimelineCrossReferencedEvent record {
    # The user who created the cross-reference.
    SimpleUser actor?;
    # The date and time the cross-reference event was last updated.
    string updatedAt;
    # The date and time the cross-reference event was created.
    string createdAt;
    # The source issue or pull request that contains the cross-reference.
    TimelineCrossReferencedEventSource 'source;
    # The type of timeline event, indicating a cross-reference.
    string event;
};


type TimelineCrossReferencedEventSource record {
    # The issue that is the source of the cross-reference.
    Issue issue?;
    # The type of source that created the cross-reference.
    string 'type?;
};

# Timeline Line Commented Event

type TimelineLineCommentedEvent record {
    # The list of pull request review comments on this line.
    PullRequestReviewComment[] comments?;
    # The type of timeline event.
    string event?;
    # The GraphQL node identifier of this event.
    string nodeId?;
};

# Timeline Reviewed Event

type TimelineReviewedEvent record {
    # The HTML-rendered body of the review.
    string bodyHtml?;
    # Hypermedia links related to the review.
    TimelineReviewedEventLinks links;
    # The date and time when the review was submitted.
    string submittedAt?;
    # The plain text body of the review.
    string bodyText?;
    # API URL for the pull request this review belongs to.
    string pullRequestUrl;
    # The text of the review
    string? body;
    # The association of the review author with the repository.
    AuthorAssociation authorAssociation;
    # URL of the review on GitHub.
    string htmlUrl;
    # Unique identifier of the review
    int id;
    # The state of the review, such as approved or changes requested.
    string state;
    # The event type that triggered this timeline entry.
    string event;
    # The user who submitted the review.
    SimpleUser user;
    # A commit SHA for the review
    string commitId;
    # The GraphQL node identifier of the review.
    string nodeId;
};


type TimelineReviewedEventLinks record {
    # Hypermedia link to the pull request associated with the review event.
    TimelineReviewedEventLinksHtml pullRequest;
    # Hypermedia link to the HTML page of the review event.
    TimelineReviewedEventLinksHtml html;
};


type TimelineReviewedEventLinksHtml record {
    # The URL linking to the HTML view of the reviewed event.
    string href;
};

# Timeline Unassigned Issue Event

type TimelineUnassignedIssueEvent record {
    # The user who performed the unassign action.
    SimpleUser actor;
    # The URL of the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed this event, if applicable.
    NullableIntegration? performedViaGithubApp;
    # The date and time the event was created.
    string createdAt;
    # The unique numeric identifier for the event.
    int id;
    # The user who was unassigned from the issue.
    SimpleUser assignee;
    # The type of timeline event that occurred.
    string event;
    # The SHA of the commit associated with this event.
    string? commitId;
    # The API URL for this timeline event.
    string url;
    # The GraphQL node ID for the event.
    string nodeId;
};


type TokenScopedBody record {
    # The access token used to authenticate to the GitHub API
    string accessToken;
    # The list of repository IDs to scope the user access token to. repository_ids may not be specified if repositories is specified
    int[] repositoryIds?;
    # The list of repository names to scope the user access token to. repositories may not be specified if repository_ids is specified
    string[] repositories?;
    # The permissions granted to the scoped user access token.
    AppPermissions permissions?;
    # The ID of the user or organization to scope the user access token to. **Required** unless target is specified
    int targetId?;
    # The name of the user or organization to scope the user access token to. **Required** unless target_id is specified
    string target?;
};

# A topic aggregates entities that are related to a subject

type Topic record {
    # The list of topic names associated with the subject.
    string[] names;
};

# Topic Search Result Item

type TopicSearchResultItem record {
    # A brief summary description of the topic.
    string? shortDescription;
    # Indicates whether this topic is featured by GitHub.
    boolean featured;
    # List of alternative names or aliases for this topic.
    TopicSearchResultItemAliases[]? aliases?;
    # The URL of the logo image associated with this topic.
    string? logoUrl?;
    # The full description of the topic.
    string? description;
    # The timestamp when this topic was created.
    string createdAt;
    # The human-readable display name for this topic.
    string? displayName;
    # The user or organization that created this topic.
    string? createdBy;
    # The search relevance score for this topic result.
    decimal score;
    # The timestamp when this topic was last updated.
    string updatedAt;
    # List of topics related to this topic.
    TopicSearchResultItemAliases[]? related?;
    # Indicates whether this topic has been curated by GitHub.
    boolean curated;
    # Text fragments matching the search query within this topic.
    SearchResultTextMatches textMatches?;
    # The unique name identifier for this topic.
    string name;
    # The number of repositories tagged with this topic.
    int? repositoryCount?;
    # The release date or version associated with this topic.
    string? released;
};


type TopicSearchResultItemAliases record {
    # The relationship details between this topic and its alias topic.
    TopicSearchResultItemTopicRelation topicRelation?;
};

# Topic Search Result Item

type TopicSearchResultItemResponse record {
    # Total number of topics matching the search query.
    int totalCount;
    # Indicates whether the search results are incomplete.
    boolean incompleteResults;
    # List of topic search result items matching the query.
    TopicSearchResultItem[] items;
};


type TopicSearchResultItemTopicRelation record {
    # The name of the related topic.
    string name?;
    # The unique identifier of the topic relation.
    int id?;
    # The unique identifier of the related topic.
    int topicId?;
    # The type of relationship between topics.
    string relationType?;
};


type Traffic record {
    # The number of unique visitors for this traffic entry.
    int uniques;
    # The total number of views or clones for this traffic entry.
    int count;
    # The timestamp representing this traffic data point.
    string timestamp;
};

# Unassigned Issue Event

type UnassignedIssueEvent record {
    # The user who triggered the unassigned event.
    SimpleUser actor;
    # The URL of the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed the unassigned event.
    NullableIntegration? performedViaGithubApp;
    # The user who removed the assignee from the issue.
    SimpleUser assigner;
    # The date and time the event was created.
    string createdAt;
    # Unique identifier of the issue event.
    int id;
    # The user who was unassigned from the issue.
    SimpleUser assignee;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # The API URL for the issue event.
    string url;
    # The GraphQL node ID of the issue event.
    string nodeId;
};

# Unlabeled Issue Event

type UnlabeledIssueEvent record {
    # The user who triggered the unlabeled event.
    SimpleUser actor;
    # The URL of the commit associated with the event.
    string? commitUrl;
    # The GitHub App that performed the unlabeled event.
    NullableIntegration? performedViaGithubApp;
    # The date and time the event was created.
    string createdAt;
    # Unique identifier of the issue event.
    int id;
    # The label that was removed from the issue.
    UnlabeledIssueEventLabel label;
    # The type of event that occurred on the issue.
    string event;
    # The SHA of the commit associated with the event.
    string? commitId;
    # The API URL for the issue event.
    string url;
    # The GraphQL node ID of the issue event.
    string nodeId;
};


type UnlabeledIssueEventLabel record {
    # The hex color code of the label.
    string color;
    # The name of the label that was removed.
    string name;
};


type UserBody record {
    # The new hiring availability of the user
    boolean hireable?;
    # The new name of the user
    string name?;
    # The new Twitter username of the user
    string? twitterUsername?;
    # The new short biography of the user
    string bio?;
    # The new company of the user
    string company?;
    # The new location of the user
    string location?;
    # The new blog URL of the user
    string blog?;
    # The publicly visible email address of the user
    string email?;
};


type UsercodespacesOneOf1 record {
    # The geographic area for this codespace. If not specified, the value is assigned by IP. This property replaces location, which is being deprecated
        "EuropeWest"|"SoutheastAsia"|"UsEast"|"UsWest"  geo?;
    # Path to devcontainer.json config to use for this codespace
    string devcontainerPath?;
    # Git ref (typically a branch name) for this codespace
    string ref?;
    # Whether to authorize requested permissions from devcontainer.json
    boolean multiRepoPermissionsOptOut?;
    # Machine type to use for this codespace
    string machine?;
    # Repository id for this codespace
    int repositoryId;
    # The requested location for a new codespace. Best efforts are made to respect this upon creation. Assigned by IP if not provided
    string location?;
    # IP for location auto-detection when proxying a request
    string clientIp?;
    # Working directory for this codespace
    string workingDirectory?;
    # Duration in minutes after codespace has gone idle in which it will be deleted. Must be integer minutes between 0 and 43200 (30 days)
    int retentionPeriodMinutes?;
    # Display name for this codespace
    string displayName?;
    # Time in minutes before codespace stops from inactivity
    int idleTimeoutMinutes?;
};

# Pull request number for this codespace

type UsercodespacesPullRequest record {
    # Pull request number
    int pullRequestNumber;
    # Repository id for this codespace
    int repositoryId;
};


type UsercodespacesusercodespacesOneOf12 record {
    # The geographic area for this codespace. If not specified, the value is assigned by IP. This property replaces location, which is being deprecated
        "EuropeWest"|"SoutheastAsia"|"UsEast"|"UsWest"  geo?;
    # Path to devcontainer.json config to use for this codespace
    string devcontainerPath?;
    # The pull request to create a codespace from.
    UsercodespacesPullRequest pullRequest;
    # Machine type to use for this codespace
    string machine?;
    # The requested location for a new codespace. Best efforts are made to respect this upon creation. Assigned by IP if not provided
    string location?;
    # Working directory for this codespace
    string workingDirectory?;
    # Time in minutes before codespace stops from inactivity
    int idleTimeoutMinutes?;
};


type UseremailsOneOf1 record {
    # Adds one or more email addresses to your GitHub account. Must contain at least one email address. **Note:** Alternatively, you can pass a single email address or an array of emails addresses directly, but we recommend that you pass an object using the emails key
    string[] emails;
};

# Deletes one or more email addresses from your GitHub account. Must contain at least one email address. **Note:** Alternatively, you can pass a single email address or an array of emails addresses directly, but we recommend that you pass an object using the emails key

type UseremailsOneOf11 record {
    # Email addresses associated with the GitHub user account
    string[] emails;
};


type UserGpgKeysBody record {
    # A GPG key in ASCII-armored format
    string armoredPublicKey;
    # A descriptive name for the new key
    string name?;
};


type UserKeysBody record {
    # A descriptive name for the new key
    string title?;
    # The public SSH key to add to your GitHub account
    string 'key;
};

# User Marketplace Purchase

type UserMarketplacePurchase record {
    # The date and time when the free trial period ends.
    string? freeTrialEndsOn;
    # Whether the purchase is currently on a free trial.
    boolean onFreeTrial;
    # The timestamp when the marketplace purchase was last updated.
    string? updatedAt;
    # The billing cycle frequency, such as monthly or yearly.
    string billingCycle;
    # The Marketplace listing plan associated with this purchase.
    MarketplaceListingPlan plan;
    # The number of units purchased in the plan.
    int? unitCount;
    # The marketplace account associated with this purchase.
    MarketplaceAccount account;
    # The date of the next scheduled billing.
    string? nextBillingDate;
};


type UserMigrationsBody record {
    # Indicates whether metadata should be excluded and only git source should be included for the migration
    boolean excludeMetadata?;
    # The list of repository names to include in the migration.
    string[] repositories;
    # Indicates whether the repository git data should be excluded from the migration
    boolean excludeGitData?;
    # Do not include attachments in the migration
    boolean excludeAttachments?;
    # Do not include releases in the migration
    boolean excludeReleases?;
    # Indicates whether projects owned by the organization or users should be excluded
    boolean excludeOwnerProjects?;
    # Indicates whether this should only include organization metadata (repositories array should be empty and will ignore other flags)
    boolean orgMetadataOnly = false;
    # Exclude attributes from the API response to improve performance
    ("repositories")[] exclude?;
    # Lock the repositories being migrated at the start of the migration
    boolean lockRepositories?;
};


type UserProjectsBody record {
    # Name of the project
    string name;
    # Body of the project
    string? body?;
};


type UserReposBody record {
    # Whether the repository is initialized with a minimal README
    boolean autoInit = false;
    # The desired language or platform to apply to the .gitignore
    string gitignoreTemplate?;
    # Whether to allow squash merges for pull requests
    boolean allowSquashMerge = true;
    # Whether to allow merge commits for pull requests
    boolean allowMergeCommit = true;
    # Whether the repository is private
    boolean 'private = false;
    # Whether downloads are enabled
    boolean hasDownloads = true;
    # Whether this repository acts as a template that can be used to generate new repositories
    boolean isTemplate = false;
    # A short description of the repository
    string description?;
    # The id of the team that will be granted access to this repository. This is only valid when creating a repository in an organization
    int teamId?;
    # Whether to allow rebase merges for pull requests
    boolean allowRebaseMerge = true;
    # Whether issues are enabled
    boolean hasIssues = true;
    # Whether projects are enabled
    boolean hasProjects = true;
    # The license keyword of the open source license for this repository
    string licenseTemplate?;
    # Whether the wiki is enabled
    boolean hasWiki = true;
    # Whether to allow Auto-merge to be used on pull requests
    boolean allowAutoMerge = false;
    # The default value for a merge commit title.

- PR_TITLE - default to the pull request's title.
- MERGE_MESSAGE - default to the classic title for a merge message (e.g., Merge pull request #123 from branch-name)
        "PR_TITLE"|"MERGE_MESSAGE"  mergeCommitTitle?;
    # The name of the repository
    string name;
    # The default value for a squash merge commit message:

- PR_BODY - default to the pull request's body.
- COMMIT_MESSAGES - default to the branch's commit messages.
- BLANK - default to a blank commit message
        "PR_BODY"|"COMMIT_MESSAGES"|"BLANK"  squashMergeCommitMessage?;
    # Whether to delete head branches when pull requests are merged
    boolean deleteBranchOnMerge = false;
    # The default value for a merge commit message.

- PR_TITLE - default to the pull request's title.
- PR_BODY - default to the pull request's body.
- BLANK - default to a blank commit message
        "PR_BODY"|"PR_TITLE"|"BLANK"  mergeCommitMessage?;
    # The default value for a squash merge commit title:

- PR_TITLE - default to the pull request's title.
- COMMIT_OR_PR_TITLE - default to the commit's title (if only one commit) or the pull request's title (when more than one commit)
        "PR_TITLE"|"COMMIT_OR_PR_TITLE"  squashMergeCommitTitle?;
    # Whether discussions are enabled
    boolean hasDiscussions = false;
    # A URL with more information about the repository
    string homepage?;
};

# User Search Result Item

type UserSearchResultItem record {
    # API URL listing the user's repositories.
    string reposUrl;
    # API URL template for the user's gists.
    string gistsUrl;
    # API URL template for users this user follows.
    string followingUrl;
    # The date and time when the user account was created.
    string createdAt?;
    # The user's profile biography.
    string? bio?;
    # The user's GitHub login username.
    string login;
    # The type of GitHub account.
    string 'type;
    # The URL of the user's blog or website.
    string? blog?;
    # API URL for the user's repository subscriptions.
    string subscriptionsUrl;
    # The search relevance score for this user result.
    decimal score;
    # The date and time when the user profile was last updated.
    string updatedAt?;
    # Whether the user is a GitHub site administrator.
    boolean siteAdmin;
    # The company the user is affiliated with.
    string? company?;
    # The unique numeric identifier of the user.
    int id;
    # The number of public repositories owned by the user.
    int publicRepos?;
    # The Gravatar identifier for the user's avatar.
    string? gravatarId;
    # The publicly visible email address of the user.
    string? email?;
    # API URL for the user's organization memberships.
    string organizationsUrl;
    # Whether the user is available for hire.
    boolean? hireable?;
    # API URL template for repositories starred by the user.
    string starredUrl;
    # API URL for the user's followers.
    string followersUrl;
    # The number of public gists owned by the user.
    int publicGists?;
    # API URL for the user.
    string url;
    # API URL for events received by the user.
    string receivedEventsUrl;
    # The number of users following this user.
    int followers?;
    # URL of the user's avatar image.
    string avatarUrl;
    # API URL template for events performed by the user.
    string eventsUrl;
    # URL of the user's profile page on GitHub.
    string htmlUrl;
    # The number of users this user is following.
    int following?;
    # Text matches for search highlighting in the result.
    SearchResultTextMatches textMatches?;
    # The display name of the user.
    string? name?;
    # The geographic location of the user.
    string? location?;
    # The GraphQL node identifier of the user.
    string nodeId;
    # The date and time when the user account was suspended.
    string? suspendedAt?;
};

# User Search Result Item

type UserSearchResultItemResponse record {
    # The total number of users matching the search query.
    int totalCount;
    # Indicates whether the search results may be incomplete.
    boolean incompleteResults;
    # The list of user search result items.
    UserSearchResultItem[] items;
};

# Represents the Queries record for the operation: users/get-context-for-user

type UsersGetContextForUserQueries record {
    # Uses the ID for the subject_type you specified. **Required** when using subject_type
    string subjectId?;
    # Identifies which additional information you'd like to receive about the person's hovercard. Can be organization, repository, issue, pull_request. **Required** when using subject_id
        "organization"|"repository"|"issue"|"pull_request"  subjectType?;
};

# Represents the Queries record for the operation: users/list-blocked-by-authenticated-user

type UsersListBlockedByAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-emails-for-authenticated-user

type UsersListEmailsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-followed-by-authenticated-user

type UsersListFollowedByAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-followers-for-authenticated-user

type UsersListFollowersForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-followers-for-user

type UsersListFollowersForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-following-for-user

type UsersListFollowingForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-gpg-keys-for-authenticated-user

type UsersListGpgKeysForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-gpg-keys-for-user

type UsersListGpgKeysForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-public-emails-for-authenticated-user

type UsersListPublicEmailsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-public-keys-for-user

type UsersListPublicKeysForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-public-ssh-keys-for-authenticated-user

type UsersListPublicSshKeysForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list

type UsersListQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # A user ID. Only return users with an ID greater than this ID
    int since?;
};

# Represents the Queries record for the operation: users/list-social-accounts-for-authenticated-user

type UsersListSocialAccountsForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-social-accounts-for-user

type UsersListSocialAccountsForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-ssh-signing-keys-for-authenticated-user

type UsersListSshSigningKeysForAuthenticatedUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};

# Represents the Queries record for the operation: users/list-ssh-signing-keys-for-user

type UsersListSshSigningKeysForUserQueries record {
    # The number of results per page (max 100)
    int perPage = 30;
    # Page number of the results to fetch
    int page = 1;
};


type UserSocialAccountsBody record {
    # Full URLs for the social media profiles to add
    string[] accountUrls;
};


type UserSocialAccountsBody1 record {
    # Full URLs for the social media profiles to delete
    string[] accountUrls;
};


type UserSshSigningKeysBody record {
    # A descriptive name for the new key
    string title?;
    # The public SSH key to add to your GitHub account. For more information, see "[Checking for existing SSH keys](https://docs.github.com/authentication/connecting-to-github-with-ssh/checking-for-existing-ssh-keys)."
    string 'key;
};


type VariablesnameBody record {
    # An array of repository ids that can access the organization variable. You can only provide a list of repository ids when the visibility is set to selected
    int[] selectedRepositoryIds?;
    # The type of repositories in the organization that can access the variable. selected means only the repositories specified by selected_repository_ids can access the variable
        "all"|"private"|"selected"  visibility?;
    # The name of the variable
    string name?;
    # The value of the variable
    string value?;
};


type VariablesnameBody1 record {
    # The name of the variable
    string name?;
    # The value of the variable
    string value?;
};


type Verification record {
    # The reason for the verification status of the commit.
    string reason;
    # The signed payload used to verify the commit signature.
    string? payload;
    # The cryptographic signature used to verify the commit.
    string? signature;
    # Indicates whether the commit signature was verified by GitHub.
    boolean verified;
};

# View Traffic

type ViewTraffic record {
    # The total number of views for the repository.
    int count;
    # The number of unique visitors who viewed the repository.
    int uniques;
    # A list of daily view traffic breakdown objects.
    Traffic[] views;
};

# Configuration object of the webhook

type WebhookConfig record {
    # The media type used to serialize the payload of the webhook.
    WebhookConfigContentType contentType?;
    # Whether SSL verification is skipped when delivering payloads.
    WebhookConfigInsecureSsl insecureSsl?;
    # The shared secret used to generate the HMAC signature of webhook payloads.
    WebhookConfigSecret secret?;
    # The URL to which webhook payloads are delivered.
    WebhookConfigUrl url?;
};

# A GitHub Actions workflow

type Workflow record {
    # URL for the workflow's status badge image.
    string badgeUrl;
    # The file path of the workflow within the repository.
    string path;
    # The date and time the workflow was last updated.
    string updatedAt;
    # URL of the workflow's page on GitHub.
    string htmlUrl;
    # The name of the workflow.
    string name;
    # The date and time the workflow was created.
    string createdAt;
    # The unique identifier of the workflow.
    int id;
    # The current state of the workflow.
        "active"|"deleted"|"disabled_fork"|"disabled_inactivity"|"disabled_manually"  state;
    # The date and time the workflow was deleted.
    string deletedAt?;
    # The API URL for the workflow.
    string url;
    # The GraphQL node identifier of the workflow.
    string nodeId;
};


type WorkflowIdDispatchesBody record {
    # The git reference for the workflow. The reference can be a branch or tag name
    string ref;
    # Input keys and values configured in the workflow file. The maximum number of properties is 10. Any default properties configured in the workflow file will be used when inputs are omitted
    record {} inputs?;
};

# A GitHub Actions workflow

type WorkflowResponse record {
    # The total number of workflows in the repository.
    int totalCount;
    # The list of GitHub Actions workflows.
    Workflow[] workflows;
};

# An invocation of a workflow

type WorkflowRun record {
    # The ID of the parent workflow
    int workflowId;
    # The head commit associated with this workflow run.
    NullableSimpleCommit? headCommit;
    # The start time of the latest run. Resets on re-run
    string runStartedAt?;
    # The branch from which the workflow run was triggered.
    string? headBranch;
    # The URL to the associated check suite
    string checkSuiteUrl;
    # The URL to the artifacts for the workflow run
    string artifactsUrl;
    # The event-specific title associated with the run or the run-name if set, or the value of run-name if it is set in the workflow
    string displayTitle;
    # The URL to download the logs for the workflow run
    string logsUrl;
    # The date and time when the workflow run was created.
    string createdAt;
    # The URL to the jobs for the workflow run
    string jobsUrl;
    # The repository where the workflow run occurred.
    MinimalRepository repository;
    # The SHA of the head commit that points to the version of the workflow being run
    string headSha;
    # The final result of the completed workflow run.
    string? conclusion;
    # The URL to rerun the workflow run
    string rerunUrl;
    # The full path of the workflow
    string path;
    # Pull requests that are open with a head_sha or head_branch that matches the workflow run. The returned pull requests do not necessarily indicate pull requests that triggered the run
    PullRequestMinimal[]? pullRequests;
    # The date and time when the workflow run was last updated.
    string updatedAt;
    # The URL to the previous attempted run of this workflow, if one exists
    string? previousAttemptUrl?;
    # The ID of the workflow run
    int id;
    # The repository containing the head branch for this run.
    MinimalRepository headRepository;
    # The event that triggered the workflow run.
    string event;
    # The user who triggered the workflow run.
    SimpleUser triggeringActor?;
    # The ID of the associated check suite
    int checkSuiteId?;
    # The URL to the workflow
    string workflowUrl;
    # The node ID of the associated check suite
    string checkSuiteNodeId?;
    # The unique identifier of the head repository.
    int headRepositoryId?;
    # The URL to the workflow run
    string url;
    # The user who initiated the workflow run.
    SimpleUser actor?;
    # List of reusable workflows referenced by this workflow run.
    ReferencedWorkflow[]? referencedWorkflows?;
    # The URL to view the workflow run on GitHub.
    string htmlUrl;
    # The name of the workflow run
    string? name?;
    # The auto incrementing run number for the workflow run
    int runNumber;
    # Attempt number of the run, 1 for first attempt and higher if the workflow was re-run
    int runAttempt?;
    # The URL to cancel the workflow run
    string cancelUrl;
    # The GraphQL node identifier for this workflow run.
    string nodeId;
    # The current status of the workflow run.
    string? status;
};

# An invocation of a workflow

type WorkflowRunResponse record {
    # The total number of workflow runs matching the request.
    int totalCount;
    # The list of workflow run objects.
    WorkflowRun[] workflowRuns;
};

# Workflow Run Usage

type WorkflowRunUsage record {
    # The total duration of the workflow run in milliseconds.
    int runDurationMs?;
    # Billable usage breakdown by operating system for the workflow run.
    WorkflowRunUsageBillable billable;
};


type WorkflowRunUsageBillable record {
    # Billing usage data for jobs run on Ubuntu runners.
    WorkflowRunUsageBillableUBUNTU uBUNTU?;
    # Billing usage data for jobs run on macOS runners.
    WorkflowRunUsageBillableUBUNTU mACOS?;
    # Billing usage data for jobs run on Windows runners.
    WorkflowRunUsageBillableUBUNTU wINDOWS?;
};


type WorkflowRunUsageBillableUBUNTU record {
    # The list of individual job run billing details for Ubuntu runners.
    WorkflowRunUsageBillableUBUNTUJobRuns[] jobRuns?;
    # The total billable time in milliseconds for Ubuntu runners.
    int totalMs;
    # The total number of jobs run on Ubuntu runners.
    int jobs;
};


type WorkflowRunUsageBillableUBUNTUJobRuns record {
    # The duration of the job run in milliseconds.
    int durationMs;
    # The unique identifier of the workflow job.
    int jobId;
};

# Workflow Usage

type WorkflowUsage record {
    # Billable minutes used by each operating system for this workflow.
    WorkflowUsageBillable billable;
};


type WorkflowUsageBillable record {
    # Billable usage minutes for Ubuntu-based workflow runs.
    WorkflowUsageBillableUBUNTU uBUNTU?;
    # Billable usage minutes for macOS-based workflow runs.
    WorkflowUsageBillableUBUNTU mACOS?;
    # Billable usage minutes for Windows-based workflow runs.
    WorkflowUsageBillableUBUNTU wINDOWS?;
};


type WorkflowUsageBillableUBUNTU record {
    # The total billable milliseconds used on Ubuntu runners.
    int totalMs?;
};

// Unknown type: LabelsOneOf1

// Unknown type: UseremailsuseremailsuseremailsOneOf1123

// Unknown type: AlertDismissedAt

// Unknown type: CwesOneOf1

// Unknown type: CodeScanningAnalysisToolName

// Unknown type: WebhookConfigUrl

// Unknown type: LabelsOneOf13

// Unknown type: LabelsOneOf12

// Unknown type: LabelsOneOf11

// Unknown type: CodeScanningRef

// Unknown type: AlertFixedAt

// Unknown type: CodeScanningAnalysisCreatedAt

// Unknown type: SecretScanningAlertResolutionComment

// Unknown type: AffectsOneOf1

// Unknown type: SelectedActionsUrl

// Unknown type: CodeScanningAlertEnvironment

// Unknown type: CodeScanningAnalysisToolVersion

// Unknown type: CodeScanningAnalysisSarifFile

// Unknown type: WebhookConfigInsecureSslOneOf1

// Unknown type: AlertUpdatedAt

// Unknown type: AlertCreatedAt

// Unknown type: WebhookConfigSecret

// Unknown type: CodeScanningAnalysisToolGuid

// Unknown type: AlertAutoDismissedAt

// Unknown type: AlertHtmlUrl

// Unknown type: CodeScanningAnalysisCommitSha

// Unknown type: NullableAlertUpdatedAt

// Unknown type: AlertUrl

// Unknown type: CodeScanningAnalysisSarifId

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf112345

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf12345

// Unknown type: CodeScanningAnalysisEnvironment

// Unknown type: CodeScanningAlertDismissedComment

// Unknown type: WebhookConfigContentType

// Unknown type: AlertInstancesUrl

// Unknown type: UseremailsuseremailsuseremailsOneOf123

// Unknown type: CodeScanningAnalysisCategory

// Unknown type: CodeScanningAnalysisAnalysisKey

// Unknown type: CodeScanningAnalysisUrl

// Unknown type: SelectedRepositoryIdsSelectedRepositoryIdsAnyOf12

// Unknown type: SelectedRepositoryIdsAnyOf1

// Unknown type: AlertNumber

// Unknown type: WaitTimer

// Unknown type: WebhookConfigInsecureSslWebhookConfigInsecureSslOneOf12

// Unknown type: ReposownerrepobranchesbranchprotectionrestrictionsappsreposownerrepobranchesbranchprotectionrestrictionsappsOneOf12

// Unknown type: StarredRepositoryResponseStarredRepositoryResponseAnyOf12

// Unknown type: UseremailsuseremailsOneOf112

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf11234

// Unknown type: ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf112

// Unknown type: DependencyGraphDiff

// Unknown type: CwesCwesOneOf12

// Unknown type: StargazerResponseStargazerResponseAnyOf12

// Unknown type: RuleSuites

// Unknown type: StarredRepositoryResponseAnyOf1

// Unknown type: UseremailsuseremailsOneOf12

// Unknown type: ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf122

// Unknown type: ContentDirectory

// Unknown type: ReposownerrepobranchesbranchprotectionrestrictionsteamsreposownerrepobranchesbranchprotectionrestrictionsteamsOneOf12

// Unknown type: ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf12

// Unknown type: ReposownerrepobranchesbranchprotectionrestrictionsusersreposownerrepobranchesbranchprotectionrestrictionsusersOneOf12

// Unknown type: CodeFrequencyStat

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf12

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf112

// Unknown type: SearchResultTextMatches

// Unknown type: StargazerResponseAnyOf1

// Unknown type: ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf1234

// Unknown type: AffectsAffectsOneOf12

type UsercodespacessecretssecretNameSelectedRepositoryIds SelectedRepositoryIdsAnyOf1|SelectedRepositoryIdsSelectedRepositoryIdsAnyOf12;

# The duration of the interaction restriction. Default: one_day
type InteractionExpiry "one_day"|"three_days"|"one_week"|"one_month"|"six_months";

type Cwes CwesOneOf1|CwesCwesOneOf12;

# Severity of a code scanning alert
type CodeScanningAlertSeverity "critical"|"high"|"medium"|"low"|"warning"|"note"|"error";

type RestrictionsTeamsBody ReposownerrepobranchesbranchprotectionrestrictionsteamsOneOf1|ReposownerrepobranchesbranchprotectionrestrictionsteamsreposownerrepobranchesbranchprotectionrestrictionsteamsOneOf12;

# A classification of the file. For example to identify it as generated
type CodeScanningAlertClassification "source"|"generated"|"test"|"library"?;

type RestrictionsAppsBody ReposownerrepobranchesbranchprotectionrestrictionsappsOneOf1|ReposownerrepobranchesbranchprotectionrestrictionsappsreposownerrepobranchesbranchprotectionrestrictionsappsOneOf12;

# The policy that controls the repositories in the organization that are allowed to run GitHub Actions
type EnabledRepositories "all"|"none"|"selected";

# State of a code scanning alert
type CodeScanningAlertState "open"|"dismissed"|"fixed";

type Affects AffectsOneOf1|AffectsAffectsOneOf12;

type RequiredStatusChecksContextsBody ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf1|ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf12;

# The package's language or package management ecosystem
type SecurityAdvisoryEcosystems "rubygems"|"npm"|"pip"|"maven"|"nuget"|"composer"|"go"|"rust"|"erlang"|"actions"|"pub"|"other"|"swift";

# **Required when the state is resolved.** The reason for resolving the alert
type SecretScanningAlertResolution "false_positive"|"wont_fix"|"revoked"|"used_in_tests"?;

type IssueNumberLabelsBody1 ReposownerrepoissuesissueNumberlabelsOneOf11|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf112|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf1123|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf11234|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf112345;

# A repository rule
type RepositoryRule RepositoryRuleCreation|RepositoryRuleUpdate|RepositoryRuleDeletion|RepositoryRuleRequiredLinearHistory|RepositoryRuleRequiredDeployments|RepositoryRuleRequiredSignatures|RepositoryRulePullRequest|RepositoryRuleRequiredStatusChecks|RepositoryRuleNonFastForward|RepositoryRuleCommitMessagePattern|RepositoryRuleCommitAuthorEmailPattern|RepositoryRuleCommitterEmailPattern|RepositoryRuleBranchNamePattern|RepositoryRuleTagNamePattern;

# Sets the state of the secret scanning alert. You must provide resolution when you set the state to resolved
type SecretScanningAlertState "open"|"resolved";

type RepoCheckRunsBody record {"completed"  status; }|record {"queued"|"in_progress"  status; };

# Interaction limit settings
type InteractionLimitResponseAny InteractionLimitResponse|InteractionLimitResponseAnyAnyOf2;

type ReposownerrepoissuesissueNumberLabels LabelsOneOf11|LabelsLabelsOneOf112;

type RequiredStatusChecksContextsBody2 ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf12|ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf122;

type RequiredStatusChecksContextsBody1 ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf11|ReposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsreposownerrepobranchesbranchprotectionrequiredStatusCheckscontextsOneOf112;

type UserResponse PrivateUser|PublicUser;

type ColumnIdCardsBody ProjectscolumnscolumnIdcardsOneOf1|ProjectscolumnscolumnIdcardsprojectscolumnscolumnIdcardsOneOf12;

type IssueLabels LabelsOneOf12|LabelsLabelsOneOf122;

# Sets the state of the code scanning alert. You must provide dismissed_reason when you set the state to dismissed
type CodeScanningAlertSetState "open"|"dismissed";

# The permissions policy that controls the actions and reusable workflows that are allowed to run
type AllowedActions "all"|"local_only"|"selected";

# The enforcement level of the ruleset. evaluate allows admins to test rules before enforcing them. Admins can view insights on the Rule Insights page (evaluate is only available with GitHub Enterprise)
type RepositoryRuleEnforcement "disabled"|"active"|"evaluate";

type UserEmailsBody UseremailsOneOf1|UseremailsuseremailsOneOf12|UseremailsuseremailsuseremailsOneOf123;

# The type of reviewer
type DeploymentReviewerType "User"|"Team";

type ReposownerrepoissuesLabels LabelsOneOf1|LabelsLabelsOneOf12;

type UserCodespacesBody UsercodespacesOneOf1|UsercodespacesusercodespacesOneOf12;

# Starred Repository
type StarredRepositoryResponse StarredRepositoryResponseAnyOf1|StarredRepositoryResponseStarredRepositoryResponseAnyOf12;

# The type of GitHub user that can comment, open issues, or create pull requests while the interaction limit is in effect
type InteractionGroup "existing_users"|"contributors_only"|"collaborators_only";

type RunIdDeploymentProtectionRuleBody ReviewCustomGatesCommentRequired|ReviewCustomGatesStateRequired;

type IssueNumberLabelsBody ReposownerrepoissuesissueNumberlabelsOneOf1|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf12|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf123|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf1234|ReposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsreposownerrepoissuesissueNumberlabelsOneOf12345;

type RestrictionsUsersBody ReposownerrepobranchesbranchprotectionrestrictionsusersOneOf1|ReposownerrepobranchesbranchprotectionrestrictionsusersreposownerrepobranchesbranchprotectionrestrictionsusersOneOf12;

# The type of credit the user is receiving
type SecurityAdvisoryCreditTypes "analyst"|"finder"|"reporter"|"coordinator"|"remediation_developer"|"remediation_reviewer"|"remediation_verifier"|"tool"|"sponsor"|"other";

type UserEmailsBody1 UseremailsOneOf11|UseremailsuseremailsOneOf112|UseremailsuseremailsuseremailsOneOf1123;

# **Required when the state is dismissed.** The reason for dismissing or closing the alert
type CodeScanningAlertDismissedReason "false positive"|"won't fix"|"used in tests"?;

type InlineResponse200 ContentDirectory|ContentFile|ContentSymlink|ContentSubmodule;

type WebhookConfigInsecureSsl WebhookConfigInsecureSslOneOf1|WebhookConfigInsecureSslWebhookConfigInsecureSslOneOf12;

type NullableIssueLabels LabelsOneOf13|LabelsLabelsOneOf132;

# Issue Event for Issue
type IssueEventForIssue LabeledIssueEvent|UnlabeledIssueEvent|AssignedIssueEvent|UnassignedIssueEvent|MilestonedIssueEvent|DemilestonedIssueEvent|RenamedIssueEvent|ReviewRequestedIssueEvent|ReviewRequestRemovedIssueEvent|ReviewDismissedIssueEvent|LockedIssueEvent|AddedToProjectIssueEvent|MovedColumnInProjectIssueEvent|RemovedFromProjectIssueEvent|ConvertedNoteToIssueIssueEvent;

# State of a code scanning alert
type CodeScanningAlertStateQuery "open"|"closed"|"dismissed"|"fixed";

# Timeline Event
type TimelineIssueEvents LabeledIssueEvent|UnlabeledIssueEvent|MilestonedIssueEvent|DemilestonedIssueEvent|RenamedIssueEvent|ReviewRequestedIssueEvent|ReviewRequestRemovedIssueEvent|ReviewDismissedIssueEvent|LockedIssueEvent|AddedToProjectIssueEvent|MovedColumnInProjectIssueEvent|RemovedFromProjectIssueEvent|ConvertedNoteToIssueIssueEvent|TimelineCommentEvent|TimelineCrossReferencedEvent|TimelineCommittedEvent|TimelineReviewedEvent|TimelineLineCommentedEvent|TimelineCommitCommentedEvent|TimelineAssignedIssueEvent|TimelineUnassignedIssueEvent|StateChangeIssueEvent;

# Stargazer
type StargazerResponse StargazerResponseAnyOf1|StargazerResponseStargazerResponseAnyOf12;

type CheckRunscheckRunIdBody record {"completed"  status; }|record {"queued"|"in_progress"  status; };

# A repository rule with ruleset details
type RepositoryRuleDetailed RepositoryRuleDetailedOneOf1|RepositoryRuleDetailedOneOf2|RepositoryRuleDetailedOneOf3|RepositoryRuleDetailedOneOf4|RepositoryRuleDetailedOneOf5|RepositoryRuleDetailedOneOf6|RepositoryRuleDetailedOneOf7|RepositoryRuleDetailedOneOf8|RepositoryRuleDetailedOneOf9|RepositoryRuleDetailedOneOf10|RepositoryRuleDetailedOneOf11|RepositoryRuleDetailedOneOf12|RepositoryRuleDetailedOneOf13|RepositoryRuleDetailedOneOf14;

# The default workflow permissions granted to the GITHUB_TOKEN when running workflows
type ActionsDefaultWorkflowPermissions "read"|"write";

# Conditions for an organization ruleset. The conditions object should contain both repository_name and ref_name properties or both repository_id and ref_name properties
type OrgRulesetConditions RepositoryNameAndRefName|RepositoryIdAndRefName;

# How the author is associated with the repository
type AuthorAssociation "COLLABORATOR"|"CONTRIBUTOR"|"FIRST_TIMER"|"FIRST_TIME_CONTRIBUTOR"|"MANNEQUIN"|"MEMBER"|"NONE"|"OWNER";

type workflowId int|string;

type EnvironmentProtectionRules ProtectionRulesAnyOf1|ProtectionRulesProtectionRulesAnyOf12|ProtectionRulesProtectionRulesProtectionRulesAnyOf123;

// Unknown type: ActionsCanApprovePullRequestReviews

// Unknown type: ActionsEnabled

// Unknown type: PreventSelfReview

// --- Client ---

# GitHub's v3 REST API.
client class Client {
    function init(ConnectionConfig config, string serviceUrl = "https://api.github.com") returns error?;

    # GitHub API Root
    resource function get .(map<string|string[]> headers = {}) returns Root|error;

    # List global security advisories
    resource function get advisories(map<string|string[]> headers = {}, SecurityAdvisoriesListGlobalAdvisoriesQueries queries) returns GlobalAdvisory[]|error;

    # Get a global security advisory
    resource function get advisories/[string ghsaId](map<string|string[]> headers = {}) returns GlobalAdvisory|error;

    # Get the authenticated app
    resource function get app(map<string|string[]> headers = {}) returns Integration|error;

    # Create a GitHub App from a manifest
    resource function post app\-manifests/[string code]/conversions(map<string|string[]> headers = {}) returns ManifestConversions|error;

    # Get a webhook configuration for an app
    resource function get app/hook/config(map<string|string[]> headers = {}) returns WebhookConfig|error;

    # Update a webhook configuration for an app
    resource function patch app/hook/config(HookConfigBody payload, map<string|string[]> headers = {}) returns WebhookConfig|error;

    # List deliveries for an app webhook
    resource function get app/hook/deliveries(map<string|string[]> headers = {}, AppsListWebhookDeliveriesQueries queries) returns HookDeliveryItem[]|error;

    # Get a delivery for an app webhook
    resource function get app/hook/deliveries/[int deliveryId](map<string|string[]> headers = {}) returns HookDelivery|error;

    # Redeliver a delivery for an app webhook
    resource function post app/hook/deliveries/[int deliveryId]/attempts(map<string|string[]> headers = {}) returns record {}|error;

    # List installation requests for the authenticated app
    resource function get app/installation\-requests(map<string|string[]> headers = {}, AppsListInstallationRequestsForAuthenticatedAppQueries queries) returns IntegrationInstallationRequest[]|error?;

    # List installations for the authenticated app
    resource function get app/installations(map<string|string[]> headers = {}, AppsListInstallationsQueries queries) returns Installation[]|error;

    # Get an installation for the authenticated app
    resource function get app/installations/[int installationId](map<string|string[]> headers = {}) returns Installation|error;

    # Delete an installation for the authenticated app
    resource function delete app/installations/[int installationId](map<string|string[]> headers = {}) returns error?;

    # Create an installation access token for an app
    resource function post app/installations/[int installationId]/access_tokens(InstallationIdAccessTokensBody payload, map<string|string[]> headers = {}) returns InstallationToken|error;

    # Suspend an app installation
    resource function put app/installations/[int installationId]/suspended(map<string|string[]> headers = {}) returns error?;

    # Unsuspend an app installation
    resource function delete app/installations/[int installationId]/suspended(map<string|string[]> headers = {}) returns error?;

    # Delete an app authorization
    resource function delete applications/[string clientId]/grant(ClientIdGrantBody payload, map<string|string[]> headers = {}) returns error?;

    # Check a token
    resource function post applications/[string clientId]/token(ClientIdTokenBody payload, map<string|string[]> headers = {}) returns Authorization|error;

    # Delete an app token
    resource function delete applications/[string clientId]/token(ClientIdGrantBody payload, map<string|string[]> headers = {}) returns error?;

    # Reset a token
    resource function patch applications/[string clientId]/token(ClientIdTokenBody payload, map<string|string[]> headers = {}) returns Authorization|error;

    # Create a scoped access token
    resource function post applications/[string clientId]/token/scoped(TokenScopedBody payload, map<string|string[]> headers = {}) returns Authorization|error;

    # Get an app
    resource function get apps/[string appSlug](map<string|string[]> headers = {}) returns Integration|error;

    # Get an assignment
    resource function get assignments/[int assignmentId](map<string|string[]> headers = {}) returns ClassroomAssignment|error;

    # List accepted assignments for an assignment
    resource function get assignments/[int assignmentId]/accepted_assignments(map<string|string[]> headers = {}, ClassroomListAcceptedAssigmentsForAnAssignmentQueries queries) returns ClassroomAcceptedAssignment[]|error;

    # Get assignment grades
    resource function get assignments/[int assignmentId]/grades(map<string|string[]> headers = {}) returns ClassroomAssignmentGrade[]|error;

    # List classrooms
    resource function get classrooms(map<string|string[]> headers = {}, ClassroomListClassroomsQueries queries) returns SimpleClassroom[]|error;

    # Get a classroom
    resource function get classrooms/[int classroomId](map<string|string[]> headers = {}) returns Classroom|error;

    # List assignments for a classroom
    resource function get classrooms/[int classroomId]/assignments(map<string|string[]> headers = {}, ClassroomListAssignmentsForAClassroomQueries queries) returns SimpleClassroomAssignment[]|error;

    # Get all codes of conduct
    resource function get codes_of_conduct(map<string|string[]> headers = {}) returns CodeOfConduct[]|error?;

    # Get a code of conduct
    resource function get codes_of_conduct/[string 'key](map<string|string[]> headers = {}) returns CodeOfConduct|error?;

    # Get emojis
    resource function get emojis(map<string|string[]> headers = {}) returns record {}|error?;

    # List Dependabot alerts for an enterprise
    resource function get enterprises/[string enterprise]/dependabot/alerts(map<string|string[]> headers = {}, DependabotListAlertsForEnterpriseQueries queries) returns DependabotAlertWithRepository[]|error?;

    # List secret scanning alerts for an enterprise
    resource function get enterprises/[string enterprise]/secret\-scanning/alerts(map<string|string[]> headers = {}, SecretScanningListAlertsForEnterpriseQueries queries) returns OrganizationSecretScanningAlert[]|error;

    # List public events
    resource function get events(map<string|string[]> headers = {}, ActivityListPublicEventsQueries queries) returns Event[]|error?;

    # Get feeds
    resource function get feeds(map<string|string[]> headers = {}) returns Feed|error;

    # List gists for the authenticated user
    resource function get gists(map<string|string[]> headers = {}, GistsListQueries queries) returns BaseGist[]|error?;

    # Create a gist
    resource function post gists(GistsBody payload, map<string|string[]> headers = {}) returns GistSimple|error?;

    # List public gists
    resource function get gists/'public(map<string|string[]> headers = {}, GistsListPublicQueries queries) returns BaseGist[]|error?;

    # List starred gists
    resource function get gists/starred(map<string|string[]> headers = {}, GistsListStarredQueries queries) returns BaseGist[]|error?;

    # Get a gist
    resource function get gists/[string gistId](map<string|string[]> headers = {}) returns GistSimple|error?;

    # Delete a gist
    resource function delete gists/[string gistId](map<string|string[]> headers = {}) returns error?;

    # Update a gist
    resource function patch gists/[string gistId](GistsgistIdBody payload, map<string|string[]> headers = {}) returns GistSimple|error;

    # List gist comments
    resource function get gists/[string gistId]/comments(map<string|string[]> headers = {}, GistsListCommentsQueries queries) returns GistComment[]|error?;

    # Create a gist comment
    resource function post gists/[string gistId]/comments(GistIdCommentsBody payload, map<string|string[]> headers = {}) returns GistComment|error?;

    # Get a gist comment
    resource function get gists/[string gistId]/comments/[int commentId](map<string|string[]> headers = {}) returns GistComment|error?;

    # Delete a gist comment
    resource function delete gists/[string gistId]/comments/[int commentId](map<string|string[]> headers = {}) returns error?;

    # Update a gist comment
    resource function patch gists/[string gistId]/comments/[int commentId](GistIdCommentsBody payload, map<string|string[]> headers = {}) returns GistComment|error;

    # List gist commits
    resource function get gists/[string gistId]/commits(map<string|string[]> headers = {}, GistsListCommitsQueries queries) returns GistCommit[]|error?;

    # List gist forks
    resource function get gists/[string gistId]/forks(map<string|string[]> headers = {}, GistsListForksQueries queries) returns GistSimple[]|error?;

    # Fork a gist
    resource function post gists/[string gistId]/forks(map<string|string[]> headers = {}) returns BaseGist|error?;

    # Check if a gist is starred
    resource function get gists/[string gistId]/star(map<string|string[]> headers = {}) returns error?;

    # Star a gist
    resource function put gists/[string gistId]/star(map<string|string[]> headers = {}) returns error?;

    # Unstar a gist
    resource function delete gists/[string gistId]/star(map<string|string[]> headers = {}) returns error?;

    # Get a gist revision
    resource function get gists/[string gistId]/[string sha](map<string|string[]> headers = {}) returns GistSimple|error;

    # Get all gitignore templates
    resource function get gitignore/templates(map<string|string[]> headers = {}) returns string[]|error?;

    # Get a gitignore template
    resource function get gitignore/templates/[string name](map<string|string[]> headers = {}) returns GitignoreTemplate|error?;

    # List repositories accessible to the app installation
    resource function get installation/repositories(map<string|string[]> headers = {}, AppsListReposAccessibleToInstallationQueries queries) returns RepositoryResponse|error?;

    # Revoke an installation access token
    resource function delete installation/token(map<string|string[]> headers = {}) returns error?;

    # List issues assigned to the authenticated user
    resource function get issues(map<string|string[]> headers = {}, IssuesListQueries queries) returns Issue[]|error?;

    # Get all commonly used licenses
    resource function get licenses(map<string|string[]> headers = {}, LicensesGetAllCommonlyUsedQueries queries) returns LicenseSimple[]|error?;

    # Get a license
    resource function get licenses/[string license](map<string|string[]> headers = {}) returns License|error?;

    # Render a Markdown document
    resource function post markdown(MarkdownBody payload, map<string|string[]> headers = {}) returns string|error?;

    # Render a Markdown document in raw mode
    resource function post markdown/raw(string payload, map<string|string[]> headers = {}) returns string|error?;

    # Get a subscription plan for an account
    resource function get marketplace_listing/accounts/[int accountId](map<string|string[]> headers = {}) returns MarketplacePurchase|error;

    # List plans
    resource function get marketplace_listing/plans(map<string|string[]> headers = {}, AppsListPlansQueries queries) returns MarketplaceListingPlan[]|error;

    # List accounts for a plan
    resource function get marketplace_listing/plans/[int planId]/accounts(map<string|string[]> headers = {}, AppsListAccountsForPlanQueries queries) returns MarketplacePurchase[]|error;

    # Get a subscription plan for an account (stubbed)
    resource function get marketplace_listing/stubbed/accounts/[int accountId](map<string|string[]> headers = {}) returns MarketplacePurchase|error;

    # List plans (stubbed)
    resource function get marketplace_listing/stubbed/plans(map<string|string[]> headers = {}, AppsListPlansStubbedQueries queries) returns MarketplaceListingPlan[]|error;

    # List accounts for a plan (stubbed)
    resource function get marketplace_listing/stubbed/plans/[int planId]/accounts(map<string|string[]> headers = {}, AppsListAccountsForPlanStubbedQueries queries) returns MarketplacePurchase[]|error;

    # Get GitHub meta information
    resource function get meta(map<string|string[]> headers = {}) returns ApiOverview|error?;

    # List public events for a network of repositories
    resource function get networks/[string owner]/[string repo]/events(map<string|string[]> headers = {}, ActivityListPublicEventsForRepoNetworkQueries queries) returns Event[]|error?;

    # List notifications for the authenticated user
    resource function get notifications(map<string|string[]> headers = {}, ActivityListNotificationsForAuthenticatedUserQueries queries) returns NotificationThread[]|error?;

    # Mark notifications as read
    resource function put notifications(NotificationsBody payload, map<string|string[]> headers = {}) returns NotificationRead|error?;

    # Get a thread
    resource function get notifications/threads/[int threadId](map<string|string[]> headers = {}) returns NotificationThread|error?;

    # Mark a thread as read
    resource function patch notifications/threads/[int threadId](map<string|string[]> headers = {}) returns error?;

    # Get a thread subscription for the authenticated user
    resource function get notifications/threads/[int threadId]/subscription(map<string|string[]> headers = {}) returns ThreadSubscription|error?;

    # Set a thread subscription
    resource function put notifications/threads/[int threadId]/subscription(ThreadIdSubscriptionBody payload, map<string|string[]> headers = {}) returns ThreadSubscription|error?;

    # Delete a thread subscription
    resource function delete notifications/threads/[int threadId]/subscription(map<string|string[]> headers = {}) returns error?;

    # Get Octocat
    resource function get octocat(map<string|string[]> headers = {}, MetaGetOctocatQueries queries) returns http:Response|error; // Special Agent Note: Response FROM ballerina/http package

    # List organizations
    resource function get organizations(map<string|string[]> headers = {}, OrgsListQueries queries) returns OrganizationSimple[]|error?;

    # Get an organization
    resource function get orgs/[string org](map<string|string[]> headers = {}) returns OrganizationFull|error;

    # Delete an organization
    resource function delete orgs/[string org](map<string|string[]> headers = {}) returns record {}|error;

    # Update an organization
    resource function patch orgs/[string org](OrgsorgBody payload, map<string|string[]> headers = {}) returns OrganizationFull|error;

    # Get GitHub Actions cache usage for an organization
    resource function get orgs/[string org]/actions/cache/usage(map<string|string[]> headers = {}) returns ActionsCacheUsageOrgEnterprise|error;

    # List repositories with GitHub Actions cache usage for an organization
    resource function get orgs/[string org]/actions/cache/usage\-by\-repository(map<string|string[]> headers = {}, ActionsGetActionsCacheUsageByRepoForOrgQueries queries) returns ActionsCacheUsageByRepositoryResponse|error;

    # Get the customization template for an OIDC subject claim for an organization
    resource function get orgs/[string org]/actions/oidc/customization/sub(map<string|string[]> headers = {}) returns OidcCustomSub|error;

    # Set the customization template for an OIDC subject claim for an organization
    resource function put orgs/[string org]/actions/oidc/customization/sub(OidcCustomSub payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get GitHub Actions permissions for an organization
    resource function get orgs/[string org]/actions/permissions(map<string|string[]> headers = {}) returns ActionsOrganizationPermissions|error;

    # Set GitHub Actions permissions for an organization
    resource function put orgs/[string org]/actions/permissions(ActionsPermissionsBody payload, map<string|string[]> headers = {}) returns error?;

    # List selected repositories enabled for GitHub Actions in an organization
    resource function get orgs/[string org]/actions/permissions/repositories(map<string|string[]> headers = {}, ActionsListSelectedRepositoriesEnabledGithubActionsOrganizationQueries queries) returns RepositoryResponse|error;

    # Set selected repositories enabled for GitHub Actions in an organization
    resource function put orgs/[string org]/actions/permissions/repositories(PermissionsRepositoriesBody payload, map<string|string[]> headers = {}) returns error?;

    # Enable a selected repository for GitHub Actions in an organization
    resource function put orgs/[string org]/actions/permissions/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Disable a selected repository for GitHub Actions in an organization
    resource function delete orgs/[string org]/actions/permissions/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Get allowed actions and reusable workflows for an organization
    resource function get orgs/[string org]/actions/permissions/selected\-actions(map<string|string[]> headers = {}) returns SelectedActions|error;

    # Set allowed actions and reusable workflows for an organization
    resource function put orgs/[string org]/actions/permissions/selected\-actions(SelectedActions payload, map<string|string[]> headers = {}) returns error?;

    # Get default workflow permissions for an organization
    resource function get orgs/[string org]/actions/permissions/workflow(map<string|string[]> headers = {}) returns ActionsGetDefaultWorkflowPermissions|error;

    # Set default workflow permissions for an organization
    resource function put orgs/[string org]/actions/permissions/workflow(ActionsSetDefaultWorkflowPermissions payload, map<string|string[]> headers = {}) returns error?;

    # List self-hosted runners for an organization
    resource function get orgs/[string org]/actions/runners(map<string|string[]> headers = {}, ActionsListSelfHostedRunnersForOrgQueries queries) returns RunnerResponse|error;

    # List runner applications for an organization
    resource function get orgs/[string org]/actions/runners/downloads(map<string|string[]> headers = {}) returns RunnerApplication[]|error;

    # Create configuration for a just-in-time runner for an organization
    resource function post orgs/[string org]/actions/runners/generate\-jitconfig(RunnersGenerateJitconfigBody payload, map<string|string[]> headers = {}) returns JitConfig|error;

    # Create a registration token for an organization
    resource function post orgs/[string org]/actions/runners/registration\-token(map<string|string[]> headers = {}) returns AuthenticationToken|error;

    # Create a remove token for an organization
    resource function post orgs/[string org]/actions/runners/remove\-token(map<string|string[]> headers = {}) returns AuthenticationToken|error;

    # Get a self-hosted runner for an organization
    resource function get orgs/[string org]/actions/runners/[int runnerId](map<string|string[]> headers = {}) returns Runner|error;

    # Delete a self-hosted runner from an organization
    resource function delete orgs/[string org]/actions/runners/[int runnerId](map<string|string[]> headers = {}) returns error?;

    # List labels for a self-hosted runner for an organization
    resource function get orgs/[string org]/actions/runners/[int runnerId]/labels(map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Set custom labels for a self-hosted runner for an organization
    resource function put orgs/[string org]/actions/runners/[int runnerId]/labels(RunnerIdLabelsBody payload, map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Add custom labels to a self-hosted runner for an organization
    resource function post orgs/[string org]/actions/runners/[int runnerId]/labels(RunnerIdLabelsBody1 payload, map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Remove all custom labels from a self-hosted runner for an organization
    resource function delete orgs/[string org]/actions/runners/[int runnerId]/labels(map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Remove a custom label from a self-hosted runner for an organization
    resource function delete orgs/[string org]/actions/runners/[int runnerId]/labels/[string name](map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # List organization secrets
    resource function get orgs/[string org]/actions/secrets(map<string|string[]> headers = {}, ActionsListOrgSecretsQueries queries) returns OrganizationActionsSecretResponse|error;

    # Get an organization public key
    resource function get orgs/[string org]/actions/secrets/public\-key(map<string|string[]> headers = {}) returns ActionsPublicKey|error;

    # Get an organization secret
    resource function get orgs/[string org]/actions/secrets/[string secretName](map<string|string[]> headers = {}) returns OrganizationActionsSecret|error;

    # Create or update an organization secret
    resource function put orgs/[string org]/actions/secrets/[string secretName](SecretssecretNameBody payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete an organization secret
    resource function delete orgs/[string org]/actions/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List selected repositories for an organization secret
    resource function get orgs/[string org]/actions/secrets/[string secretName]/repositories(map<string|string[]> headers = {}, ActionsListSelectedReposForOrgSecretQueries queries) returns MinimalRepositoryResponse|error;

    # Set selected repositories for an organization secret
    resource function put orgs/[string org]/actions/secrets/[string secretName]/repositories(SecretNameRepositoriesBody payload, map<string|string[]> headers = {}) returns error?;

    # Add selected repository to an organization secret
    resource function put orgs/[string org]/actions/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove selected repository from an organization secret
    resource function delete orgs/[string org]/actions/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # List organization variables
    resource function get orgs/[string org]/actions/variables(map<string|string[]> headers = {}, ActionsListOrgVariablesQueries queries) returns OrganizationActionsVariableResponse|error;

    # Create an organization variable
    resource function post orgs/[string org]/actions/variables(ActionsVariablesBody payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get an organization variable
    resource function get orgs/[string org]/actions/variables/[string name](map<string|string[]> headers = {}) returns OrganizationActionsVariable|error;

    # Delete an organization variable
    resource function delete orgs/[string org]/actions/variables/[string name](map<string|string[]> headers = {}) returns error?;

    # Update an organization variable
    resource function patch orgs/[string org]/actions/variables/[string name](VariablesnameBody payload, map<string|string[]> headers = {}) returns error?;

    # List selected repositories for an organization variable
    resource function get orgs/[string org]/actions/variables/[string name]/repositories(map<string|string[]> headers = {}, ActionsListSelectedReposForOrgVariableQueries queries) returns MinimalRepositoryResponse|error;

    # Set selected repositories for an organization variable
    resource function put orgs/[string org]/actions/variables/[string name]/repositories(NameRepositoriesBody payload, map<string|string[]> headers = {}) returns error?;

    # Add selected repository to an organization variable
    resource function put orgs/[string org]/actions/variables/[string name]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove selected repository from an organization variable
    resource function delete orgs/[string org]/actions/variables/[string name]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # List users blocked by an organization
    resource function get orgs/[string org]/blocks(map<string|string[]> headers = {}, OrgsListBlockedUsersQueries queries) returns SimpleUser[]|error;

    # Check if a user is blocked by an organization
    resource function get orgs/[string org]/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # Block a user from an organization
    resource function put orgs/[string org]/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # Unblock a user from an organization
    resource function delete orgs/[string org]/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # List secret scanning alerts for an organization
    resource function get orgs/[string org]/code\-scanning/alerts(map<string|string[]> headers = {}, CodeScanningListAlertsForOrgQueries queries) returns CodeScanningOrganizationAlertItems[]|error;

    # List codespaces for the organization
    resource function get orgs/[string org]/codespaces(map<string|string[]> headers = {}, CodespacesListInOrganizationQueries queries) returns CodespaceResponse|error?;

    # Manage access control for organization codespaces
    resource function put orgs/[string org]/codespaces/access(CodespacesAccessBody payload, map<string|string[]> headers = {}) returns error?;

    # Add users to Codespaces access for an organization
    resource function post orgs/[string org]/codespaces/access/selected_users(AccessSelectedUsersBody payload, map<string|string[]> headers = {}) returns error?;

    # Remove users from Codespaces access for an organization
    resource function delete orgs/[string org]/codespaces/access/selected_users(AccessSelectedUsersBody1 payload, map<string|string[]> headers = {}) returns error?;

    # List organization secrets
    resource function get orgs/[string org]/codespaces/secrets(map<string|string[]> headers = {}, CodespacesListOrgSecretsQueries queries) returns CodespacesOrgSecretResponse|error;

    # Get an organization public key
    resource function get orgs/[string org]/codespaces/secrets/public\-key(map<string|string[]> headers = {}) returns CodespacesPublicKey|error;

    # Get an organization secret
    resource function get orgs/[string org]/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns CodespacesOrgSecret|error;

    # Create or update an organization secret
    resource function put orgs/[string org]/codespaces/secrets/[string secretName](SecretssecretNameBody1 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete an organization secret
    resource function delete orgs/[string org]/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List selected repositories for an organization secret
    resource function get orgs/[string org]/codespaces/secrets/[string secretName]/repositories(map<string|string[]> headers = {}, CodespacesListSelectedReposForOrgSecretQueries queries) returns MinimalRepositoryResponse|error;

    # Set selected repositories for an organization secret
    resource function put orgs/[string org]/codespaces/secrets/[string secretName]/repositories(SecretNameRepositoriesBody1 payload, map<string|string[]> headers = {}) returns error?;

    # Add selected repository to an organization secret
    resource function put orgs/[string org]/codespaces/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove selected repository from an organization secret
    resource function delete orgs/[string org]/codespaces/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Get Copilot for Business seat information and settings for an organization
    resource function get orgs/[string org]/copilot/billing(map<string|string[]> headers = {}) returns CopilotOrganizationDetails|error;

    # List all Copilot for Business seat assignments for an organization
    resource function get orgs/[string org]/copilot/billing/seats(map<string|string[]> headers = {}, CopilotListCopilotSeatsQueries queries) returns CopilotSeatDetailsResponse|error;

    # Add teams to the Copilot for Business subscription for an organization
    resource function post orgs/[string org]/copilot/billing/selected_teams(BillingSelectedTeamsBody payload, map<string|string[]> headers = {}) returns CopilotSeatCreated|error;

    # Remove teams from the Copilot for Business subscription for an organization
    resource function delete orgs/[string org]/copilot/billing/selected_teams(BillingSelectedTeamsBody1 payload, map<string|string[]> headers = {}) returns CopilotSeatCancelled|error;

    # Add users to the Copilot for Business subscription for an organization
    resource function post orgs/[string org]/copilot/billing/selected_users(BillingSelectedUsersBody payload, map<string|string[]> headers = {}) returns CopilotSeatCreated|error;

    # Remove users from the Copilot for Business subscription for an organization
    resource function delete orgs/[string org]/copilot/billing/selected_users(BillingSelectedUsersBody1 payload, map<string|string[]> headers = {}) returns CopilotSeatCancelled|error;

    # List Dependabot alerts for an organization
    resource function get orgs/[string org]/dependabot/alerts(map<string|string[]> headers = {}, DependabotListAlertsForOrgQueries queries) returns DependabotAlertWithRepository[]|error?;

    # List organization secrets
    resource function get orgs/[string org]/dependabot/secrets(map<string|string[]> headers = {}, DependabotListOrgSecretsQueries queries) returns OrganizationDependabotSecretResponse|error;

    # Get an organization public key
    resource function get orgs/[string org]/dependabot/secrets/public\-key(map<string|string[]> headers = {}) returns DependabotPublicKey|error;

    # Get an organization secret
    resource function get orgs/[string org]/dependabot/secrets/[string secretName](map<string|string[]> headers = {}) returns OrganizationDependabotSecret|error;

    # Create or update an organization secret
    resource function put orgs/[string org]/dependabot/secrets/[string secretName](SecretssecretNameBody2 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete an organization secret
    resource function delete orgs/[string org]/dependabot/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List selected repositories for an organization secret
    resource function get orgs/[string org]/dependabot/secrets/[string secretName]/repositories(map<string|string[]> headers = {}, DependabotListSelectedReposForOrgSecretQueries queries) returns MinimalRepositoryResponse|error;

    # Set selected repositories for an organization secret
    resource function put orgs/[string org]/dependabot/secrets/[string secretName]/repositories(SecretNameRepositoriesBody2 payload, map<string|string[]> headers = {}) returns error?;

    # Add selected repository to an organization secret
    resource function put orgs/[string org]/dependabot/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove selected repository from an organization secret
    resource function delete orgs/[string org]/dependabot/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Get list of conflicting packages during Docker migration for organization
    resource function get orgs/[string org]/docker/conflicts(map<string|string[]> headers = {}) returns Package[]|error;

    # List public organization events
    resource function get orgs/[string org]/events(map<string|string[]> headers = {}, ActivityListPublicOrgEventsQueries queries) returns Event[]|error;

    # List failed organization invitations
    resource function get orgs/[string org]/failed_invitations(map<string|string[]> headers = {}, OrgsListFailedInvitationsQueries queries) returns OrganizationInvitation[]|error;

    # List organization webhooks
    resource function get orgs/[string org]/hooks(map<string|string[]> headers = {}, OrgsListWebhooksQueries queries) returns OrgHook[]|error;

    # Create an organization webhook
    resource function post orgs/[string org]/hooks(OrgHooksBody payload, map<string|string[]> headers = {}) returns OrgHook|error;

    # Get an organization webhook
    resource function get orgs/[string org]/hooks/[int hookId](map<string|string[]> headers = {}) returns OrgHook|error;

    # Delete an organization webhook
    resource function delete orgs/[string org]/hooks/[int hookId](map<string|string[]> headers = {}) returns error?;

    # Update an organization webhook
    resource function patch orgs/[string org]/hooks/[int hookId](HookshookIdBody payload, map<string|string[]> headers = {}) returns OrgHook|error;

    # Get a webhook configuration for an organization
    resource function get orgs/[string org]/hooks/[int hookId]/config(map<string|string[]> headers = {}) returns WebhookConfig|error;

    # Update a webhook configuration for an organization
    resource function patch orgs/[string org]/hooks/[int hookId]/config(HookConfigBody payload, map<string|string[]> headers = {}) returns WebhookConfig|error;

    # List deliveries for an organization webhook
    resource function get orgs/[string org]/hooks/[int hookId]/deliveries(map<string|string[]> headers = {}, OrgsListWebhookDeliveriesQueries queries) returns HookDeliveryItem[]|error;

    # Get a webhook delivery for an organization webhook
    resource function get orgs/[string org]/hooks/[int hookId]/deliveries/[int deliveryId](map<string|string[]> headers = {}) returns HookDelivery|error;

    # Redeliver a delivery for an organization webhook
    resource function post orgs/[string org]/hooks/[int hookId]/deliveries/[int deliveryId]/attempts(map<string|string[]> headers = {}) returns record {}|error;

    # Ping an organization webhook
    resource function post orgs/[string org]/hooks/[int hookId]/pings(map<string|string[]> headers = {}) returns error?;

    # Get an organization installation for the authenticated app
    resource function get orgs/[string org]/installation(map<string|string[]> headers = {}) returns Installation|error;

    # List app installations for an organization
    resource function get orgs/[string org]/installations(map<string|string[]> headers = {}, OrgsListAppInstallationsQueries queries) returns InstallationResponse|error;

    # Get interaction restrictions for an organization
    resource function get orgs/[string org]/interaction\-limits(map<string|string[]> headers = {}) returns InteractionLimitResponseAny|error;

    # Set interaction restrictions for an organization
    resource function put orgs/[string org]/interaction\-limits(InteractionLimit payload, map<string|string[]> headers = {}) returns InteractionLimitResponse|error;

    # Remove interaction restrictions for an organization
    resource function delete orgs/[string org]/interaction\-limits(map<string|string[]> headers = {}) returns error?;

    # List pending organization invitations
    resource function get orgs/[string org]/invitations(map<string|string[]> headers = {}, OrgsListPendingInvitationsQueries queries) returns OrganizationInvitation[]|error;

    # Create an organization invitation
    resource function post orgs/[string org]/invitations(OrgInvitationsBody payload, map<string|string[]> headers = {}) returns OrganizationInvitation|error;

    # Cancel an organization invitation
    resource function delete orgs/[string org]/invitations/[int invitationId](map<string|string[]> headers = {}) returns error?;

    # List organization invitation teams
    resource function get orgs/[string org]/invitations/[int invitationId]/teams(map<string|string[]> headers = {}, OrgsListInvitationTeamsQueries queries) returns Team[]|error;

    # List organization issues assigned to the authenticated user
    resource function get orgs/[string org]/issues(map<string|string[]> headers = {}, IssuesListForOrgQueries queries) returns Issue[]|error;

    # List organization members
    resource function get orgs/[string org]/members(map<string|string[]> headers = {}, OrgsListMembersQueries queries) returns SimpleUser[]|error;

    # Check organization membership for a user
    resource function get orgs/[string org]/members/[string username](map<string|string[]> headers = {}) returns error?;

    # Remove an organization member
    resource function delete orgs/[string org]/members/[string username](map<string|string[]> headers = {}) returns error?;

    # List codespaces for a user in organization
    resource function get orgs/[string org]/members/[string username]/codespaces(map<string|string[]> headers = {}, CodespacesGetCodespacesForUserInOrgQueries queries) returns CodespaceResponse|error?;

    # Delete a codespace from the organization
    resource function delete orgs/[string org]/members/[string username]/codespaces/[string codespaceName](map<string|string[]> headers = {}) returns record {}|error?;

    # Stop a codespace for an organization user
    resource function post orgs/[string org]/members/[string username]/codespaces/[string codespaceName]/stop(map<string|string[]> headers = {}) returns Codespace|error?;

    # Get Copilot for Business seat assignment details for a user
    resource function get orgs/[string org]/members/[string username]/copilot(map<string|string[]> headers = {}) returns CopilotSeatDetails|error;

    # Get organization membership for a user
    resource function get orgs/[string org]/memberships/[string username](map<string|string[]> headers = {}) returns OrgMembership|error;

    # Set organization membership for a user
    resource function put orgs/[string org]/memberships/[string username](MembershipsusernameBody payload, map<string|string[]> headers = {}) returns OrgMembership|error;

    # Remove organization membership for a user
    resource function delete orgs/[string org]/memberships/[string username](map<string|string[]> headers = {}) returns error?;

    # List organization migrations
    resource function get orgs/[string org]/migrations(map<string|string[]> headers = {}, MigrationsListForOrgQueries queries) returns Migration[]|error;

    # Start an organization migration
    resource function post orgs/[string org]/migrations(OrgMigrationsBody payload, map<string|string[]> headers = {}) returns Migration|error;

    # Get an organization migration status
    resource function get orgs/[string org]/migrations/[int migrationId](map<string|string[]> headers = {}, MigrationsGetStatusForOrgQueries queries) returns Migration|error;

    # Download an organization migration archive
    resource function get orgs/[string org]/migrations/[int migrationId]/archive(map<string|string[]> headers = {}) returns error?;

    # Delete an organization migration archive
    resource function delete orgs/[string org]/migrations/[int migrationId]/archive(map<string|string[]> headers = {}) returns error?;

    # Unlock an organization repository
    resource function delete orgs/[string org]/migrations/[int migrationId]/repos/[string repoName]/'lock(map<string|string[]> headers = {}) returns error?;

    # List repositories in an organization migration
    resource function get orgs/[string org]/migrations/[int migrationId]/repositories(map<string|string[]> headers = {}, MigrationsListReposForOrgQueries queries) returns MinimalRepository[]|error;

    # List outside collaborators for an organization
    resource function get orgs/[string org]/outside_collaborators(map<string|string[]> headers = {}, OrgsListOutsideCollaboratorsQueries queries) returns SimpleUser[]|error;

    # Convert an organization member to outside collaborator
    resource function put orgs/[string org]/outside_collaborators/[string username](OutsideCollaboratorsusernameBody payload, map<string|string[]> headers = {}) returns record {}|error?;

    # Remove outside collaborator from an organization
    resource function delete orgs/[string org]/outside_collaborators/[string username](map<string|string[]> headers = {}) returns error?;

    # List packages for an organization
    resource function get orgs/[string org]/packages(map<string|string[]> headers = {}, PackagesListPackagesForOrganizationQueries queries) returns Package[]|error;

    # Get a package for an organization
    resource function get orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns Package|error;

    # Delete a package for an organization
    resource function delete orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns error?;

    # Restore a package for an organization
    resource function post orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/restore(map<string|string[]> headers = {}, PackagesRestorePackageForOrgQueries queries) returns error?;

    # List package versions for a package owned by an organization
    resource function get orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions(map<string|string[]> headers = {}, PackagesGetAllPackageVersionsForPackageOwnedByOrgQueries queries) returns PackageVersion[]|error;

    # Get a package version for an organization
    resource function get orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns PackageVersion|error;

    # Delete package version for an organization
    resource function delete orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns error?;

    # Restore package version for an organization
    resource function post orgs/[string org]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId]/restore(map<string|string[]> headers = {}) returns error?;

    # List requests to access organization resources with fine-grained personal access tokens
    resource function get orgs/[string org]/personal\-access\-token\-requests(map<string|string[]> headers = {}, OrgsListPatGrantRequestsQueries queries) returns OrganizationProgrammaticAccessGrantRequest[]|error;

    # Review requests to access organization resources with fine-grained personal access tokens
    resource function post orgs/[string org]/personal\-access\-token\-requests(OrgPersonalAccessTokenRequestsBody payload, map<string|string[]> headers = {}) returns record {}|error;

    # Update the access a fine-grained personal access token has to organization resources
    resource function post orgs/[string org]/personal\-access\-token\-requests/[int patRequestId](PersonalAccessTokenRequestspatRequestIdBody payload, map<string|string[]> headers = {}) returns error?;

    # List repositories a fine-grained personal access token has access to
    resource function get orgs/[string org]/personal\-access\-token\-requests/[int patRequestId]/repositories(map<string|string[]> headers = {}, OrgsListPatGrantRequestRepositoriesQueries queries) returns MinimalRepository[]|error;

    # List fine-grained personal access tokens with access to organization resources
    resource function get orgs/[string org]/personal\-access\-tokens(map<string|string[]> headers = {}, OrgsListPatGrantsQueries queries) returns OrganizationProgrammaticAccessGrant[]|error;

    # Update the access to organization resources via fine-grained personal access tokens
    resource function post orgs/[string org]/personal\-access\-tokens(OrgPersonalAccessTokensBody payload, map<string|string[]> headers = {}) returns record {}|error;

    # Update the access a fine-grained personal access token has to organization resources
    resource function post orgs/[string org]/personal\-access\-tokens/[int patId](PersonalAccessTokenspatIdBody payload, map<string|string[]> headers = {}) returns error?;

    # List repositories a fine-grained personal access token has access to
    resource function get orgs/[string org]/personal\-access\-tokens/[int patId]/repositories(map<string|string[]> headers = {}, OrgsListPatGrantRepositoriesQueries queries) returns MinimalRepository[]|error;

    # List organization projects
    resource function get orgs/[string org]/projects(map<string|string[]> headers = {}, ProjectsListForOrgQueries queries) returns Project[]|error;

    # Create an organization project
    resource function post orgs/[string org]/projects(OrgProjectsBody payload, map<string|string[]> headers = {}) returns Project|error;

    # List public organization members
    resource function get orgs/[string org]/public_members(map<string|string[]> headers = {}, OrgsListPublicMembersQueries queries) returns SimpleUser[]|error;

    # Check public organization membership for a user
    resource function get orgs/[string org]/public_members/[string username](map<string|string[]> headers = {}) returns error?;

    # Set public organization membership for the authenticated user
    resource function put orgs/[string org]/public_members/[string username](map<string|string[]> headers = {}) returns error?;

    # Remove public organization membership for the authenticated user
    resource function delete orgs/[string org]/public_members/[string username](map<string|string[]> headers = {}) returns error?;

    # List organization repositories
    resource function get orgs/[string org]/repos(map<string|string[]> headers = {}, ReposListForOrgQueries queries) returns MinimalRepository[]|error;

    # Create an organization repository
    resource function post orgs/[string org]/repos(OrgReposBody payload, map<string|string[]> headers = {}) returns Repository|error;

    # Get all organization repository rulesets
    resource function get orgs/[string org]/rulesets(map<string|string[]> headers = {}, ReposGetOrgRulesetsQueries queries) returns RepositoryRuleset[]|error;

    # Create an organization repository ruleset
    resource function post orgs/[string org]/rulesets(OrgRulesetsBody payload, map<string|string[]> headers = {}) returns RepositoryRuleset|error;

    # List organization rule suites
    resource function get orgs/[string org]/rulesets/rule\-suites(map<string|string[]> headers = {}, ReposGetOrgRuleSuitesQueries queries) returns RuleSuites|error;

    # Get an organization rule suite
    resource function get orgs/[string org]/rulesets/rule\-suites/[int ruleSuiteId](map<string|string[]> headers = {}) returns RuleSuite|error;

    # Get an organization repository ruleset
    resource function get orgs/[string org]/rulesets/[int rulesetId](map<string|string[]> headers = {}) returns RepositoryRuleset|error;

    # Update an organization repository ruleset
    resource function put orgs/[string org]/rulesets/[int rulesetId](RulesetsrulesetIdBody payload, map<string|string[]> headers = {}) returns RepositoryRuleset|error;

    # Delete an organization repository ruleset
    resource function delete orgs/[string org]/rulesets/[int rulesetId](map<string|string[]> headers = {}) returns error?;

    # List secret scanning alerts for an organization
    resource function get orgs/[string org]/secret\-scanning/alerts(map<string|string[]> headers = {}, SecretScanningListAlertsForOrgQueries queries) returns OrganizationSecretScanningAlert[]|error;

    # List repository security advisories for an organization
    resource function get orgs/[string org]/security\-advisories(map<string|string[]> headers = {}, SecurityAdvisoriesListOrgRepositoryAdvisoriesQueries queries) returns RepositoryAdvisory[]|error;

    # List security manager teams
    resource function get orgs/[string org]/security\-managers(map<string|string[]> headers = {}) returns TeamSimple[]|error;

    # Add a security manager team
    resource function put orgs/[string org]/security\-managers/teams/[string teamSlug](map<string|string[]> headers = {}) returns error?;

    # Remove a security manager team
    resource function delete orgs/[string org]/security\-managers/teams/[string teamSlug](map<string|string[]> headers = {}) returns error?;

    # Get GitHub Actions billing for an organization
    resource function get orgs/[string org]/settings/billing/actions(map<string|string[]> headers = {}) returns ActionsBillingUsage|error;

    # Get GitHub Packages billing for an organization
    resource function get orgs/[string org]/settings/billing/packages(map<string|string[]> headers = {}) returns PackagesBillingUsage|error;

    # Get shared storage billing for an organization
    resource function get orgs/[string org]/settings/billing/shared\-storage(map<string|string[]> headers = {}) returns CombinedBillingUsage|error;

    # List teams
    resource function get orgs/[string org]/teams(map<string|string[]> headers = {}, TeamsListQueries queries) returns Team[]|error;

    # Create a team
    resource function post orgs/[string org]/teams(OrgTeamsBody payload, map<string|string[]> headers = {}) returns TeamFull|error;

    # Get a team by name
    resource function get orgs/[string org]/teams/[string teamSlug](map<string|string[]> headers = {}) returns TeamFull|error;

    # Delete a team
    resource function delete orgs/[string org]/teams/[string teamSlug](map<string|string[]> headers = {}) returns error?;

    # Update a team
    resource function patch orgs/[string org]/teams/[string teamSlug](TeamsteamSlugBody payload, map<string|string[]> headers = {}) returns TeamFull|error;

    # List discussions
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions(map<string|string[]> headers = {}, TeamsListDiscussionsInOrgQueries queries) returns TeamDiscussion[]|error;

    # Create a discussion
    resource function post orgs/[string org]/teams/[string teamSlug]/discussions(TeamSlugDiscussionsBody payload, map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # Get a discussion
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber](map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # Delete a discussion
    resource function delete orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber](map<string|string[]> headers = {}) returns error?;

    # Update a discussion
    resource function patch orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber](DiscussionsdiscussionNumberBody payload, map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # List discussion comments
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments(map<string|string[]> headers = {}, TeamsListDiscussionCommentsInOrgQueries queries) returns TeamDiscussionComment[]|error;

    # Create a discussion comment
    resource function post orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments(DiscussionNumberCommentsBody payload, map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # Get a discussion comment
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber](map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # Delete a discussion comment
    resource function delete orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber](map<string|string[]> headers = {}) returns error?;

    # Update a discussion comment
    resource function patch orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber](DiscussionNumberCommentsBody payload, map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # List reactions for a team discussion comment
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber]/reactions(map<string|string[]> headers = {}, ReactionsListForTeamDiscussionCommentInOrgQueries queries) returns Reaction[]|error;

    # Create reaction for a team discussion comment
    resource function post orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber]/reactions(CommentNumberReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete team discussion comment reaction
    resource function delete orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/comments/[int commentNumber]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # List reactions for a team discussion
    resource function get orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/reactions(map<string|string[]> headers = {}, ReactionsListForTeamDiscussionInOrgQueries queries) returns Reaction[]|error;

    # Create reaction for a team discussion
    resource function post orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/reactions(DiscussionNumberReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete team discussion reaction
    resource function delete orgs/[string org]/teams/[string teamSlug]/discussions/[int discussionNumber]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # List pending team invitations
    resource function get orgs/[string org]/teams/[string teamSlug]/invitations(map<string|string[]> headers = {}, TeamsListPendingInvitationsInOrgQueries queries) returns OrganizationInvitation[]|error;

    # List team members
    resource function get orgs/[string org]/teams/[string teamSlug]/members(map<string|string[]> headers = {}, TeamsListMembersInOrgQueries queries) returns SimpleUser[]|error;

    # Get team membership for a user
    resource function get orgs/[string org]/teams/[string teamSlug]/memberships/[string username](map<string|string[]> headers = {}) returns TeamMembership|error;

    # Add or update team membership for a user
    resource function put orgs/[string org]/teams/[string teamSlug]/memberships/[string username](MembershipsusernameBody1 payload, map<string|string[]> headers = {}) returns TeamMembership|error;

    # Remove team membership for a user
    resource function delete orgs/[string org]/teams/[string teamSlug]/memberships/[string username](map<string|string[]> headers = {}) returns error?;

    # List team projects
    resource function get orgs/[string org]/teams/[string teamSlug]/projects(map<string|string[]> headers = {}, TeamsListProjectsInOrgQueries queries) returns TeamProject[]|error;

    # Check team permissions for a project
    resource function get orgs/[string org]/teams/[string teamSlug]/projects/[int projectId](map<string|string[]> headers = {}) returns TeamProject|error;

    # Add or update team project permissions
    resource function put orgs/[string org]/teams/[string teamSlug]/projects/[int projectId](ProjectsprojectIdBody payload, map<string|string[]> headers = {}) returns error?;

    # Remove a project from a team
    resource function delete orgs/[string org]/teams/[string teamSlug]/projects/[int projectId](map<string|string[]> headers = {}) returns error?;

    # List team repositories
    resource function get orgs/[string org]/teams/[string teamSlug]/repos(map<string|string[]> headers = {}, TeamsListReposInOrgQueries queries) returns MinimalRepository[]|error;

    # Check team permissions for a repository
    resource function get orgs/[string org]/teams/[string teamSlug]/repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns TeamRepository|error?;

    # Add or update team repository permissions
    resource function put orgs/[string org]/teams/[string teamSlug]/repos/[string owner]/[string repo](OwnerrepoBody payload, map<string|string[]> headers = {}) returns error?;

    # Remove a repository from a team
    resource function delete orgs/[string org]/teams/[string teamSlug]/repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # List child teams
    resource function get orgs/[string org]/teams/[string teamSlug]/teams(map<string|string[]> headers = {}, TeamsListChildInOrgQueries queries) returns Team[]|error;

    # Enable or disable a security feature for an organization
    resource function post orgs/[string org]/["dependency_graph"|"dependabot_alerts"|"dependabot_security_updates"|"advanced_security"|"code_scanning_default_setup"|"secret_scanning"|"secret_scanning_push_protection" securityProduct]/["enable_all"|"disable_all" enablement](SecurityProductenablementBody payload, map<string|string[]> headers = {}) returns error?;

    # Get a project card
    resource function get projects/columns/cards/[int cardId](map<string|string[]> headers = {}) returns ProjectCard|error?;

    # Delete a project card
    resource function delete projects/columns/cards/[int cardId](map<string|string[]> headers = {}) returns error?;

    # Update an existing project card
    resource function patch projects/columns/cards/[int cardId](CardscardIdBody payload, map<string|string[]> headers = {}) returns ProjectCard|error?;

    # Move a project card
    resource function post projects/columns/cards/[int cardId]/moves(CardIdMovesBody payload, map<string|string[]> headers = {}) returns record {}|error?;

    # Get a project column
    resource function get projects/columns/[int columnId](map<string|string[]> headers = {}) returns ProjectColumn|error?;

    # Delete a project column
    resource function delete projects/columns/[int columnId](map<string|string[]> headers = {}) returns error?;

    # Update an existing project column
    resource function patch projects/columns/[int columnId](ColumnscolumnIdBody payload, map<string|string[]> headers = {}) returns ProjectColumn|error?;

    # List project cards
    resource function get projects/columns/[int columnId]/cards(map<string|string[]> headers = {}, ProjectsListCardsQueries queries) returns ProjectCard[]|error?;

    # Create a project card
    resource function post projects/columns/[int columnId]/cards(ColumnIdCardsBody payload, map<string|string[]> headers = {}) returns ProjectCard|error?;

    # Move a project column
    resource function post projects/columns/[int columnId]/moves(ColumnIdMovesBody payload, map<string|string[]> headers = {}) returns record {}|error?;

    # Get a project
    resource function get projects/[int projectId](map<string|string[]> headers = {}) returns Project|error?;

    # Delete a project
    resource function delete projects/[int projectId](map<string|string[]> headers = {}) returns error?;

    # Update a project
    resource function patch projects/[int projectId](ProjectsprojectIdBody1 payload, map<string|string[]> headers = {}) returns Project|error?;

    # List project collaborators
    resource function get projects/[int projectId]/collaborators(map<string|string[]> headers = {}, ProjectsListCollaboratorsQueries queries) returns SimpleUser[]|error?;

    # Add project collaborator
    resource function put projects/[int projectId]/collaborators/[string username](CollaboratorsusernameBody payload, map<string|string[]> headers = {}) returns error?;

    # Remove user as a collaborator
    resource function delete projects/[int projectId]/collaborators/[string username](map<string|string[]> headers = {}) returns error?;

    # Get project permission for a user
    resource function get projects/[int projectId]/collaborators/[string username]/permission(map<string|string[]> headers = {}) returns ProjectCollaboratorPermission|error?;

    # List project columns
    resource function get projects/[int projectId]/columns(map<string|string[]> headers = {}, ProjectsListColumnsQueries queries) returns ProjectColumn[]|error?;

    # Create a project column
    resource function post projects/[int projectId]/columns(ColumnscolumnIdBody payload, map<string|string[]> headers = {}) returns ProjectColumn|error?;

    # Get rate limit status for the authenticated user
    resource function get rate_limit(map<string|string[]> headers = {}) returns RateLimitOverview|error?;

    # Get a repository
    resource function get repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns FullRepository|error;

    # Delete a repository
    resource function delete repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # Update a repository
    resource function patch repos/[string owner]/[string repo](OwnerrepoBody1 payload, map<string|string[]> headers = {}) returns FullRepository|error;

    # List artifacts for a repository
    resource function get repos/[string owner]/[string repo]/actions/artifacts(map<string|string[]> headers = {}, ActionsListArtifactsForRepoQueries queries) returns ArtifactResponse|error;

    # Get an artifact
    resource function get repos/[string owner]/[string repo]/actions/artifacts/[int artifactId](map<string|string[]> headers = {}) returns Artifact|error;

    # Delete an artifact
    resource function delete repos/[string owner]/[string repo]/actions/artifacts/[int artifactId](map<string|string[]> headers = {}) returns error?;

    # Download an artifact
    resource function get repos/[string owner]/[string repo]/actions/artifacts/[int artifactId]/[string archiveFormat](map<string|string[]> headers = {}) returns error?;

    # Get GitHub Actions cache usage for a repository
    resource function get repos/[string owner]/[string repo]/actions/cache/usage(map<string|string[]> headers = {}) returns ActionsCacheUsageByRepository|error;

    # List GitHub Actions caches for a repository
    resource function get repos/[string owner]/[string repo]/actions/caches(map<string|string[]> headers = {}, ActionsGetActionsCacheListQueries queries) returns ActionsCacheList|error;

    # Delete GitHub Actions caches for a repository (using a cache key)
    resource function delete repos/[string owner]/[string repo]/actions/caches(map<string|string[]> headers = {}, ActionsDeleteActionsCacheByKeyQueries queries) returns ActionsCacheList|error;

    # Delete a GitHub Actions cache for a repository (using a cache ID)
    resource function delete repos/[string owner]/[string repo]/actions/caches/[int cacheId](map<string|string[]> headers = {}) returns error?;

    # Get a job for a workflow run
    resource function get repos/[string owner]/[string repo]/actions/jobs/[int jobId](map<string|string[]> headers = {}) returns Job|error;

    # Download job logs for a workflow run
    resource function get repos/[string owner]/[string repo]/actions/jobs/[int jobId]/logs(map<string|string[]> headers = {}) returns error?;

    # Re-run a job from a workflow run
    resource function post repos/[string owner]/[string repo]/actions/jobs/[int jobId]/rerun(JobIdRerunBody payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get the customization template for an OIDC subject claim for a repository
    resource function get repos/[string owner]/[string repo]/actions/oidc/customization/sub(map<string|string[]> headers = {}) returns OidcCustomSubRepo|error;

    # Set the customization template for an OIDC subject claim for a repository
    resource function put repos/[string owner]/[string repo]/actions/oidc/customization/sub(ActionsOIDCSubjectCustomizationForARepository payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # List repository organization secrets
    resource function get repos/[string owner]/[string repo]/actions/organization\-secrets(map<string|string[]> headers = {}, ActionsListRepoOrganizationSecretsQueries queries) returns ActionsSecretResponse|error;

    # List repository organization variables
    resource function get repos/[string owner]/[string repo]/actions/organization\-variables(map<string|string[]> headers = {}, ActionsListRepoOrganizationVariablesQueries queries) returns ActionsVariableResponse|error;

    # Get GitHub Actions permissions for a repository
    resource function get repos/[string owner]/[string repo]/actions/permissions(map<string|string[]> headers = {}) returns ActionsRepositoryPermissions|error;

    # Set GitHub Actions permissions for a repository
    resource function put repos/[string owner]/[string repo]/actions/permissions(ActionsPermissionsBody1 payload, map<string|string[]> headers = {}) returns error?;

    # Get the level of access for workflows outside of the repository
    resource function get repos/[string owner]/[string repo]/actions/permissions/access(map<string|string[]> headers = {}) returns ActionsWorkflowAccessToRepository|error;

    # Set the level of access for workflows outside of the repository
    resource function put repos/[string owner]/[string repo]/actions/permissions/access(ActionsWorkflowAccessToRepository payload, map<string|string[]> headers = {}) returns error?;

    # Get allowed actions and reusable workflows for a repository
    resource function get repos/[string owner]/[string repo]/actions/permissions/selected\-actions(map<string|string[]> headers = {}) returns SelectedActions|error;

    # Set allowed actions and reusable workflows for a repository
    resource function put repos/[string owner]/[string repo]/actions/permissions/selected\-actions(SelectedActions payload, map<string|string[]> headers = {}) returns error?;

    # Get default workflow permissions for a repository
    resource function get repos/[string owner]/[string repo]/actions/permissions/workflow(map<string|string[]> headers = {}) returns ActionsGetDefaultWorkflowPermissions|error;

    # Set default workflow permissions for a repository
    resource function put repos/[string owner]/[string repo]/actions/permissions/workflow(ActionsSetDefaultWorkflowPermissions payload, map<string|string[]> headers = {}) returns error?;

    # List self-hosted runners for a repository
    resource function get repos/[string owner]/[string repo]/actions/runners(map<string|string[]> headers = {}, ActionsListSelfHostedRunnersForRepoQueries queries) returns RunnerResponse|error;

    # List runner applications for a repository
    resource function get repos/[string owner]/[string repo]/actions/runners/downloads(map<string|string[]> headers = {}) returns RunnerApplication[]|error;

    # Create configuration for a just-in-time runner for a repository
    resource function post repos/[string owner]/[string repo]/actions/runners/generate\-jitconfig(RunnersGenerateJitconfigBody payload, map<string|string[]> headers = {}) returns JitConfig|error;

    # Create a registration token for a repository
    resource function post repos/[string owner]/[string repo]/actions/runners/registration\-token(map<string|string[]> headers = {}) returns AuthenticationToken|error;

    # Create a remove token for a repository
    resource function post repos/[string owner]/[string repo]/actions/runners/remove\-token(map<string|string[]> headers = {}) returns AuthenticationToken|error;

    # Get a self-hosted runner for a repository
    resource function get repos/[string owner]/[string repo]/actions/runners/[int runnerId](map<string|string[]> headers = {}) returns Runner|error;

    # Delete a self-hosted runner from a repository
    resource function delete repos/[string owner]/[string repo]/actions/runners/[int runnerId](map<string|string[]> headers = {}) returns error?;

    # List labels for a self-hosted runner for a repository
    resource function get repos/[string owner]/[string repo]/actions/runners/[int runnerId]/labels(map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Set custom labels for a self-hosted runner for a repository
    resource function put repos/[string owner]/[string repo]/actions/runners/[int runnerId]/labels(RunnerIdLabelsBody payload, map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Add custom labels to a self-hosted runner for a repository
    resource function post repos/[string owner]/[string repo]/actions/runners/[int runnerId]/labels(RunnerIdLabelsBody1 payload, map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Remove all custom labels from a self-hosted runner for a repository
    resource function delete repos/[string owner]/[string repo]/actions/runners/[int runnerId]/labels(map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # Remove a custom label from a self-hosted runner for a repository
    resource function delete repos/[string owner]/[string repo]/actions/runners/[int runnerId]/labels/[string name](map<string|string[]> headers = {}) returns RunnerLabelResponse|error;

    # List workflow runs for a repository
    resource function get repos/[string owner]/[string repo]/actions/runs(map<string|string[]> headers = {}, ActionsListWorkflowRunsForRepoQueries queries) returns WorkflowRunResponse|error;

    # Get a workflow run
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId](map<string|string[]> headers = {}, ActionsGetWorkflowRunQueries queries) returns WorkflowRun|error;

    # Delete a workflow run
    resource function delete repos/[string owner]/[string repo]/actions/runs/[int runId](map<string|string[]> headers = {}) returns error?;

    # Get the review history for a workflow run
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/approvals(map<string|string[]> headers = {}) returns EnvironmentApprovals[]|error;

    # Approve a workflow run for a fork pull request
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/approve(map<string|string[]> headers = {}) returns EmptyObject|error;

    # List workflow run artifacts
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/artifacts(map<string|string[]> headers = {}, ActionsListWorkflowRunArtifactsQueries queries) returns ArtifactResponse|error;

    # Get a workflow run attempt
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/attempts/[int attemptNumber](map<string|string[]> headers = {}, ActionsGetWorkflowRunAttemptQueries queries) returns WorkflowRun|error;

    # List jobs for a workflow run attempt
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/attempts/[int attemptNumber]/jobs(map<string|string[]> headers = {}, ActionsListJobsForWorkflowRunAttemptQueries queries) returns JobResponse|error;

    # Download workflow run attempt logs
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/attempts/[int attemptNumber]/logs(map<string|string[]> headers = {}) returns error?;

    # Cancel a workflow run
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/cancel(map<string|string[]> headers = {}) returns EmptyObject|error;

    # Review custom deployment protection rules for a workflow run
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/deployment_protection_rule(RunIdDeploymentProtectionRuleBody payload, map<string|string[]> headers = {}) returns error?;

    # Force cancel a workflow run
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/force\-cancel(map<string|string[]> headers = {}) returns EmptyObject|error;

    # List jobs for a workflow run
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/jobs(map<string|string[]> headers = {}, ActionsListJobsForWorkflowRunQueries queries) returns JobResponse|error;

    # Download workflow run logs
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/logs(map<string|string[]> headers = {}) returns error?;

    # Delete workflow run logs
    resource function delete repos/[string owner]/[string repo]/actions/runs/[int runId]/logs(map<string|string[]> headers = {}) returns error?;

    # Get pending deployments for a workflow run
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/pending_deployments(map<string|string[]> headers = {}) returns PendingDeployment[]|error;

    # Review pending deployments for a workflow run
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/pending_deployments(RunIdPendingDeploymentsBody payload, map<string|string[]> headers = {}) returns Deployment[]|error;

    # Re-run a workflow
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/rerun(JobIdRerunBody payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Re-run failed jobs from a workflow run
    resource function post repos/[string owner]/[string repo]/actions/runs/[int runId]/rerun\-failed\-jobs(JobIdRerunBody payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get workflow run usage
    resource function get repos/[string owner]/[string repo]/actions/runs/[int runId]/timing(map<string|string[]> headers = {}) returns WorkflowRunUsage|error;

    # List repository secrets
    resource function get repos/[string owner]/[string repo]/actions/secrets(map<string|string[]> headers = {}, ActionsListRepoSecretsQueries queries) returns ActionsSecretResponse|error;

    # Get a repository public key
    resource function get repos/[string owner]/[string repo]/actions/secrets/public\-key(map<string|string[]> headers = {}) returns ActionsPublicKey|error;

    # Get a repository secret
    resource function get repos/[string owner]/[string repo]/actions/secrets/[string secretName](map<string|string[]> headers = {}) returns ActionsSecret|error;

    # Create or update a repository secret
    resource function put repos/[string owner]/[string repo]/actions/secrets/[string secretName](SecretssecretNameBody3 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete a repository secret
    resource function delete repos/[string owner]/[string repo]/actions/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List repository variables
    resource function get repos/[string owner]/[string repo]/actions/variables(map<string|string[]> headers = {}, ActionsListRepoVariablesQueries queries) returns ActionsVariableResponse|error;

    # Create a repository variable
    resource function post repos/[string owner]/[string repo]/actions/variables(ActionsVariablesBody1 payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get a repository variable
    resource function get repos/[string owner]/[string repo]/actions/variables/[string name](map<string|string[]> headers = {}) returns ActionsVariable|error;

    # Delete a repository variable
    resource function delete repos/[string owner]/[string repo]/actions/variables/[string name](map<string|string[]> headers = {}) returns error?;

    # Update a repository variable
    resource function patch repos/[string owner]/[string repo]/actions/variables/[string name](VariablesnameBody1 payload, map<string|string[]> headers = {}) returns error?;

    # List repository workflows
    resource function get repos/[string owner]/[string repo]/actions/workflows(map<string|string[]> headers = {}, ActionsListRepoWorkflowsQueries queries) returns WorkflowResponse|error;

    # Get a workflow
    resource function get repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId](map<string|string[]> headers = {}) returns Workflow|error;

    # Disable a workflow
    resource function put repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId]/disable(map<string|string[]> headers = {}) returns error?;

    # Create a workflow dispatch event
    resource function post repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId]/dispatches(WorkflowIdDispatchesBody payload, map<string|string[]> headers = {}) returns error?;

    # Enable a workflow
    resource function put repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId]/enable(map<string|string[]> headers = {}) returns error?;

    # List workflow runs for a workflow
    resource function get repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId]/runs(map<string|string[]> headers = {}, ActionsListWorkflowRunsQueries queries) returns WorkflowRunResponse|error;

    # Get workflow usage
    resource function get repos/[string owner]/[string repo]/actions/workflows/[workflowId workflowId]/timing(map<string|string[]> headers = {}) returns WorkflowUsage|error;

    # List repository activities
    resource function get repos/[string owner]/[string repo]/activity(map<string|string[]> headers = {}, ReposListActivitiesQueries queries) returns Activity[]|error;

    # List assignees
    resource function get repos/[string owner]/[string repo]/assignees(map<string|string[]> headers = {}, IssuesListAssigneesQueries queries) returns SimpleUser[]|error;

    # Check if a user can be assigned
    resource function get repos/[string owner]/[string repo]/assignees/[string assignee](map<string|string[]> headers = {}) returns error?;

    # List all autolinks of a repository
    resource function get repos/[string owner]/[string repo]/autolinks(map<string|string[]> headers = {}, ReposListAutolinksQueries queries) returns Autolink[]|error;

    # Create an autolink reference for a repository
    resource function post repos/[string owner]/[string repo]/autolinks(RepoAutolinksBody payload, map<string|string[]> headers = {}) returns Autolink|error;

    # Get an autolink reference of a repository
    resource function get repos/[string owner]/[string repo]/autolinks/[int autolinkId](map<string|string[]> headers = {}) returns Autolink|error;

    # Delete an autolink reference from a repository
    resource function delete repos/[string owner]/[string repo]/autolinks/[int autolinkId](map<string|string[]> headers = {}) returns error?;

    # Check if automated security fixes are enabled for a repository
    resource function get repos/[string owner]/[string repo]/automated\-security\-fixes(map<string|string[]> headers = {}) returns CheckAutomatedSecurityFixes|error;

    # Enable automated security fixes
    resource function put repos/[string owner]/[string repo]/automated\-security\-fixes(map<string|string[]> headers = {}) returns error?;

    # Disable automated security fixes
    resource function delete repos/[string owner]/[string repo]/automated\-security\-fixes(map<string|string[]> headers = {}) returns error?;

    # List branches
    resource function get repos/[string owner]/[string repo]/branches(map<string|string[]> headers = {}, ReposListBranchesQueries queries) returns ShortBranch[]|error;

    # Get a branch
    resource function get repos/[string owner]/[string repo]/branches/[string branch](map<string|string[]> headers = {}) returns BranchWithProtection|error;

    # Get branch protection
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection(map<string|string[]> headers = {}) returns BranchProtection|error;

    # Update branch protection
    resource function put repos/[string owner]/[string repo]/branches/[string branch]/protection(BranchProtectionBody payload, map<string|string[]> headers = {}) returns ProtectedBranch|error;

    # Delete branch protection
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection(map<string|string[]> headers = {}) returns error?;

    # Get admin branch protection
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/enforce_admins(map<string|string[]> headers = {}) returns ProtectedBranchAdminEnforced|error;

    # Set admin branch protection
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/enforce_admins(map<string|string[]> headers = {}) returns ProtectedBranchAdminEnforced|error;

    # Delete admin branch protection
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/enforce_admins(map<string|string[]> headers = {}) returns error?;

    # Get pull request review protection
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/required_pull_request_reviews(map<string|string[]> headers = {}) returns ProtectedBranchPullRequestReview|error;

    # Delete pull request review protection
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/required_pull_request_reviews(map<string|string[]> headers = {}) returns error?;

    # Update pull request review protection
    resource function patch repos/[string owner]/[string repo]/branches/[string branch]/protection/required_pull_request_reviews(ProtectionRequiredPullRequestReviewsBody payload, map<string|string[]> headers = {}) returns ProtectedBranchPullRequestReview|error;

    # Get commit signature protection
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/required_signatures(map<string|string[]> headers = {}) returns ProtectedBranchAdminEnforced|error;

    # Create commit signature protection
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/required_signatures(map<string|string[]> headers = {}) returns ProtectedBranchAdminEnforced|error;

    # Delete commit signature protection
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/required_signatures(map<string|string[]> headers = {}) returns error?;

    # Get status checks protection
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks(map<string|string[]> headers = {}) returns StatusCheckPolicy|error;

    # Remove status check protection
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks(map<string|string[]> headers = {}) returns error?;

    # Update status check protection
    resource function patch repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks(ProtectionRequiredStatusChecksBody payload, map<string|string[]> headers = {}) returns StatusCheckPolicy|error;

    # Get all status check contexts
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks/contexts(map<string|string[]> headers = {}) returns string[]|error;

    # Set status check contexts
    resource function put repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks/contexts(RequiredStatusChecksContextsBody payload, map<string|string[]> headers = {}) returns string[]|error;

    # Add status check contexts
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks/contexts(RequiredStatusChecksContextsBody1 payload, map<string|string[]> headers = {}) returns string[]|error;

    # Remove status check contexts
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/required_status_checks/contexts(RequiredStatusChecksContextsBody2 payload, map<string|string[]> headers = {}) returns string[]|error;

    # Get access restrictions
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions(map<string|string[]> headers = {}) returns BranchRestrictionPolicy|error;

    # Delete access restrictions
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions(map<string|string[]> headers = {}) returns error?;

    # Get apps with access to the protected branch
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/apps(map<string|string[]> headers = {}) returns Integration[]|error;

    # Set app access restrictions
    resource function put repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/apps(RestrictionsAppsBody payload, map<string|string[]> headers = {}) returns Integration[]|error;

    # Add app access restrictions
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/apps(RestrictionsAppsBody payload, map<string|string[]> headers = {}) returns Integration[]|error;

    # Remove app access restrictions
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/apps(RestrictionsAppsBody payload, map<string|string[]> headers = {}) returns Integration[]|error;

    # Get teams with access to the protected branch
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/teams(map<string|string[]> headers = {}) returns Team[]|error;

    # Set team access restrictions
    resource function put repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/teams(RestrictionsTeamsBody payload, map<string|string[]> headers = {}) returns Team[]|error;

    # Add team access restrictions
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/teams(RestrictionsTeamsBody payload, map<string|string[]> headers = {}) returns Team[]|error;

    # Remove team access restrictions
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/teams(RestrictionsTeamsBody payload, map<string|string[]> headers = {}) returns Team[]|error;

    # Get users with access to the protected branch
    resource function get repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/users(map<string|string[]> headers = {}) returns SimpleUser[]|error;

    # Set user access restrictions
    resource function put repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/users(RestrictionsUsersBody payload, map<string|string[]> headers = {}) returns SimpleUser[]|error;

    # Add user access restrictions
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/users(RestrictionsUsersBody payload, map<string|string[]> headers = {}) returns SimpleUser[]|error;

    # Remove user access restrictions
    resource function delete repos/[string owner]/[string repo]/branches/[string branch]/protection/restrictions/users(RestrictionsUsersBody payload, map<string|string[]> headers = {}) returns SimpleUser[]|error;

    # Rename a branch
    resource function post repos/[string owner]/[string repo]/branches/[string branch]/rename(BranchRenameBody payload, map<string|string[]> headers = {}) returns BranchWithProtection|error;

    # Create a check run
    resource function post repos/[string owner]/[string repo]/check\-runs(RepoCheckRunsBody payload, map<string|string[]> headers = {}) returns CheckRun|error;

    # Get a repository security advisory
    resource function get repos/[string owner]/[string repo]/check\-runs/[int checkRunId](map<string|string[]> headers = {}) returns CheckRun|error;

    # Update a repository security advisory
    resource function patch repos/[string owner]/[string repo]/check\-runs/[int checkRunId](CheckRunscheckRunIdBody payload, map<string|string[]> headers = {}) returns CheckRun|error;

    # List check run annotations
    resource function get repos/[string owner]/[string repo]/check\-runs/[int checkRunId]/annotations(map<string|string[]> headers = {}, ChecksListAnnotationsQueries queries) returns CheckAnnotation[]|error;

    # Rerequest a check suite
    resource function post repos/[string owner]/[string repo]/check\-runs/[int checkRunId]/rerequest(map<string|string[]> headers = {}) returns EmptyObject|error;

    # Create a check suite
    resource function post repos/[string owner]/[string repo]/check\-suites(RepoCheckSuitesBody payload, map<string|string[]> headers = {}) returns CheckSuite|error;

    # Update repository preferences for check suites
    resource function patch repos/[string owner]/[string repo]/check\-suites/preferences(CheckSuitesPreferencesBody payload, map<string|string[]> headers = {}) returns CheckSuitePreference|error;

    # Get a repository security advisory
    resource function get repos/[string owner]/[string repo]/check\-suites/[int checkSuiteId](map<string|string[]> headers = {}) returns CheckSuite|error;

    # List check runs in a check suite
    resource function get repos/[string owner]/[string repo]/check\-suites/[int checkSuiteId]/check\-runs(map<string|string[]> headers = {}, ChecksListForSuiteQueries queries) returns CheckRunResponse|error;

    # Rerequest a check suite
    resource function post repos/[string owner]/[string repo]/check\-suites/[int checkSuiteId]/rerequest(map<string|string[]> headers = {}) returns EmptyObject|error;

    # List secret scanning alerts for a repository
    resource function get repos/[string owner]/[string repo]/code\-scanning/alerts(map<string|string[]> headers = {}, CodeScanningListAlertsForRepoQueries queries) returns CodeScanningAlertItems[]|error?;

    # Get a secret scanning alert
    resource function get repos/[string owner]/[string repo]/code\-scanning/alerts/[AlertNumber alertNumber](map<string|string[]> headers = {}) returns CodeScanningAlert|error?;

    # Update a secret scanning alert
    resource function patch repos/[string owner]/[string repo]/code\-scanning/alerts/[AlertNumber alertNumber](AlertsalertNumberBody payload, map<string|string[]> headers = {}) returns CodeScanningAlert|error;

    # List instances of a code scanning alert
    resource function get repos/[string owner]/[string repo]/code\-scanning/alerts/[AlertNumber alertNumber]/instances(map<string|string[]> headers = {}, CodeScanningListAlertInstancesQueries queries) returns CodeScanningAlertInstance[]|error;

    # List code scanning analyses for a repository
    resource function get repos/[string owner]/[string repo]/code\-scanning/analyses(map<string|string[]> headers = {}, CodeScanningListRecentAnalysesQueries queries) returns CodeScanningAnalysis[]|error;

    # Get a code scanning analysis for a repository
    resource function get repos/[string owner]/[string repo]/code\-scanning/analyses/[int analysisId](map<string|string[]> headers = {}) returns CodeScanningAnalysis|error;

    # Delete a code scanning analysis from a repository
    resource function delete repos/[string owner]/[string repo]/code\-scanning/analyses/[int analysisId](map<string|string[]> headers = {}, CodeScanningDeleteAnalysisQueries queries) returns CodeScanningAnalysisDeletion|error;

    # List CodeQL databases for a repository
    resource function get repos/[string owner]/[string repo]/code\-scanning/codeql/databases(map<string|string[]> headers = {}) returns CodeScanningCodeqlDatabase[]|error;

    # Get a CodeQL database for a repository
    resource function get repos/[string owner]/[string repo]/code\-scanning/codeql/databases/[string language](map<string|string[]> headers = {}) returns CodeScanningCodeqlDatabase|error?;

    # Get a code scanning default setup configuration
    resource function get repos/[string owner]/[string repo]/code\-scanning/default\-setup(map<string|string[]> headers = {}) returns CodeScanningDefaultSetup|error;

    # Update a code scanning default setup configuration
    resource function patch repos/[string owner]/[string repo]/code\-scanning/default\-setup(CodeScanningDefaultSetupUpdate payload, map<string|string[]> headers = {}) returns EmptyObject|CodeScanningDefaultSetupUpdateResponse|error;

    # Upload an analysis as SARIF data
    resource function post repos/[string owner]/[string repo]/code\-scanning/sarifs(CodeScanningSarifsBody payload, map<string|string[]> headers = {}) returns CodeScanningSarifsReceipt|error;

    # Get information about a SARIF upload
    resource function get repos/[string owner]/[string repo]/code\-scanning/sarifs/[string sarifId](map<string|string[]> headers = {}) returns CodeScanningSarifsStatus|error;

    # List CODEOWNERS errors
    resource function get repos/[string owner]/[string repo]/codeowners/errors(map<string|string[]> headers = {}, ReposCodeownersErrorsQueries queries) returns CodeownersErrors|error;

    # List codespaces in a repository for the authenticated user
    resource function get repos/[string owner]/[string repo]/codespaces(map<string|string[]> headers = {}, CodespacesListInRepositoryForAuthenticatedUserQueries queries) returns CodespaceResponse|error;

    # Create a codespace in a repository
    resource function post repos/[string owner]/[string repo]/codespaces(RepoCodespacesBody payload, map<string|string[]> headers = {}) returns Codespace|error;

    # List devcontainer configurations in a repository for the authenticated user
    resource function get repos/[string owner]/[string repo]/codespaces/devcontainers(map<string|string[]> headers = {}, CodespacesListDevcontainersInRepositoryForAuthenticatedUserQueries queries) returns DevcontainersResponse|error;

    # List available machine types for a repository
    resource function get repos/[string owner]/[string repo]/codespaces/machines(map<string|string[]> headers = {}, CodespacesRepoMachinesForAuthenticatedUserQueries queries) returns CodespaceMachineResponse|error?;

    # Get default attributes for a codespace
    resource function get repos/[string owner]/[string repo]/codespaces/'new(map<string|string[]> headers = {}, CodespacesPreFlightWithRepoForAuthenticatedUserQueries queries) returns CodespaceDefaultResponse|error;

    # Check if permissions defined by a devcontainer have been accepted by the authenticated user
    resource function get repos/[string owner]/[string repo]/codespaces/permissions_check(map<string|string[]> headers = {}, CodespacesCheckPermissionsForDevcontainerQueries queries) returns CodespacesPermissionsCheckForDevcontainer|error;

    # List repository secrets
    resource function get repos/[string owner]/[string repo]/codespaces/secrets(map<string|string[]> headers = {}, CodespacesListRepoSecretsQueries queries) returns RepoCodespacesSecretResponse|error;

    # Get a repository public key
    resource function get repos/[string owner]/[string repo]/codespaces/secrets/public\-key(map<string|string[]> headers = {}) returns CodespacesPublicKey|error;

    # Get a repository secret
    resource function get repos/[string owner]/[string repo]/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns RepoCodespacesSecret|error;

    # Create or update a repository secret
    resource function put repos/[string owner]/[string repo]/codespaces/secrets/[string secretName](SecretssecretNameBody4 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete a repository secret
    resource function delete repos/[string owner]/[string repo]/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List repository collaborators
    resource function get repos/[string owner]/[string repo]/collaborators(map<string|string[]> headers = {}, ReposListCollaboratorsQueries queries) returns Collaborator[]|error;

    # Check if a user is a repository collaborator
    resource function get repos/[string owner]/[string repo]/collaborators/[string username](map<string|string[]> headers = {}) returns error?;

    # Add a repository collaborator
    resource function put repos/[string owner]/[string repo]/collaborators/[string username](CollaboratorsusernameBody1 payload, map<string|string[]> headers = {}) returns RepositoryInvitation|error?;

    # Remove a repository collaborator
    resource function delete repos/[string owner]/[string repo]/collaborators/[string username](map<string|string[]> headers = {}) returns error?;

    # Get repository permissions for a user
    resource function get repos/[string owner]/[string repo]/collaborators/[string username]/permission(map<string|string[]> headers = {}) returns RepositoryCollaboratorPermission|error;

    # List commit comments for a repository
    resource function get repos/[string owner]/[string repo]/comments(map<string|string[]> headers = {}, ReposListCommitCommentsForRepoQueries queries) returns CommitComment[]|error;

    # Get a commit comment
    resource function get repos/[string owner]/[string repo]/comments/[int commentId](map<string|string[]> headers = {}) returns CommitComment|error;

    # Delete a commit comment
    resource function delete repos/[string owner]/[string repo]/comments/[int commentId](map<string|string[]> headers = {}) returns error?;

    # Update a commit comment
    resource function patch repos/[string owner]/[string repo]/comments/[int commentId](CommentscommentIdBody payload, map<string|string[]> headers = {}) returns CommitComment|error;

    # List reactions for a commit comment
    resource function get repos/[string owner]/[string repo]/comments/[int commentId]/reactions(map<string|string[]> headers = {}, ReactionsListForCommitCommentQueries queries) returns Reaction[]|error;

    # Create reaction for a commit comment
    resource function post repos/[string owner]/[string repo]/comments/[int commentId]/reactions(CommentIdReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete a commit comment reaction
    resource function delete repos/[string owner]/[string repo]/comments/[int commentId]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # List commits
    resource function get repos/[string owner]/[string repo]/commits(map<string|string[]> headers = {}, ReposListCommitsQueries queries) returns Commit[]|error;

    # List branches for HEAD commit
    resource function get repos/[string owner]/[string repo]/commits/[string commitSha]/branches\-where\-head(map<string|string[]> headers = {}) returns BranchShort[]|error;

    # List commit comments
    resource function get repos/[string owner]/[string repo]/commits/[string commitSha]/comments(map<string|string[]> headers = {}, ReposListCommentsForCommitQueries queries) returns CommitComment[]|error;

    # Create a commit comment
    resource function post repos/[string owner]/[string repo]/commits/[string commitSha]/comments(CommitShaCommentsBody payload, map<string|string[]> headers = {}) returns CommitComment|error;

    # List pull requests associated with a commit
    resource function get repos/[string owner]/[string repo]/commits/[string commitSha]/pulls(map<string|string[]> headers = {}, ReposListPullRequestsAssociatedWithCommitQueries queries) returns PullRequestSimple[]|error;

    # Get a commit
    resource function get repos/[string owner]/[string repo]/commits/[string ref](map<string|string[]> headers = {}, ReposGetCommitQueries queries) returns Commit|error;

    # List check runs for a Git reference
    resource function get repos/[string owner]/[string repo]/commits/[string ref]/check\-runs(map<string|string[]> headers = {}, ChecksListForRefQueries queries) returns CheckRunResponse|error;

    # List check suites for a Git reference
    resource function get repos/[string owner]/[string repo]/commits/[string ref]/check\-suites(map<string|string[]> headers = {}, ChecksListSuitesForRefQueries queries) returns CheckSuiteResponse|error;

    # Get the combined status for a specific reference
    resource function get repos/[string owner]/[string repo]/commits/[string ref]/status(map<string|string[]> headers = {}, ReposGetCombinedStatusForRefQueries queries) returns CombinedCommitStatus|error;

    # List commit statuses for a reference
    resource function get repos/[string owner]/[string repo]/commits/[string ref]/statuses(map<string|string[]> headers = {}, ReposListCommitStatusesForRefQueries queries) returns Status[]|error;

    # Get community profile metrics
    resource function get repos/[string owner]/[string repo]/community/profile(map<string|string[]> headers = {}) returns CommunityProfile|error;

    # Compare two commits
    resource function get repos/[string owner]/[string repo]/compare/[string basehead](map<string|string[]> headers = {}, ReposCompareCommitsQueries queries) returns CommitComparison|error;

    # Get repository content
    resource function get repos/[string owner]/[string repo]/contents/[string path](map<string|string[]> headers = {}, ReposGetContentQueries queries) returns InlineResponse200|error?;

    # Create or update file contents
    resource function put repos/[string owner]/[string repo]/contents/[string path](ContentspathBody payload, map<string|string[]> headers = {}) returns FileCommit|error;

    # Delete a file
    resource function delete repos/[string owner]/[string repo]/contents/[string path](ContentspathBody1 payload, map<string|string[]> headers = {}) returns FileCommit|error;

    # List repository contributors
    resource function get repos/[string owner]/[string repo]/contributors(map<string|string[]> headers = {}, ReposListContributorsQueries queries) returns Contributor[]|error?;

    # List Dependabot alerts for a repository
    resource function get repos/[string owner]/[string repo]/dependabot/alerts(map<string|string[]> headers = {}, DependabotListAlertsForRepoQueries queries) returns DependabotAlert[]|error?;

    # Get a Dependabot alert
    resource function get repos/[string owner]/[string repo]/dependabot/alerts/[AlertNumber alertNumber](map<string|string[]> headers = {}) returns DependabotAlert|error?;

    # Update a Dependabot alert
    resource function patch repos/[string owner]/[string repo]/dependabot/alerts/[AlertNumber alertNumber](AlertsalertNumberBody1 payload, map<string|string[]> headers = {}) returns DependabotAlert|error;

    # List repository secrets
    resource function get repos/[string owner]/[string repo]/dependabot/secrets(map<string|string[]> headers = {}, DependabotListRepoSecretsQueries queries) returns DependabotSecretResponse|error;

    # Get a repository public key
    resource function get repos/[string owner]/[string repo]/dependabot/secrets/public\-key(map<string|string[]> headers = {}) returns DependabotPublicKey|error;

    # Get a repository secret
    resource function get repos/[string owner]/[string repo]/dependabot/secrets/[string secretName](map<string|string[]> headers = {}) returns DependabotSecret|error;

    # Create or update a repository secret
    resource function put repos/[string owner]/[string repo]/dependabot/secrets/[string secretName](SecretssecretNameBody5 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete a repository secret
    resource function delete repos/[string owner]/[string repo]/dependabot/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # Get a diff of the dependencies between commits
    resource function get repos/[string owner]/[string repo]/dependency\-graph/compare/[string basehead](map<string|string[]> headers = {}, DependencyGraphDiffRangeQueries queries) returns DependencyGraphDiff|error;

    # Export a software bill of materials (SBOM) for a repository.
    resource function get repos/[string owner]/[string repo]/dependency\-graph/sbom(map<string|string[]> headers = {}) returns DependencyGraphSpdxSbom|error;

    # Create a snapshot of dependencies for a repository
    resource function post repos/[string owner]/[string repo]/dependency\-graph/snapshots(Snapshot payload, map<string|string[]> headers = {}) returns SnapshotResponse|error;

    # List deployments
    resource function get repos/[string owner]/[string repo]/deployments(map<string|string[]> headers = {}, ReposListDeploymentsQueries queries) returns Deployment[]|error;

    # Create a deployment
    resource function post repos/[string owner]/[string repo]/deployments(RepoDeploymentsBody payload, map<string|string[]> headers = {}) returns Deployment|MergedBranchResponse|error;

    # Get a deployment
    resource function get repos/[string owner]/[string repo]/deployments/[int deploymentId](map<string|string[]> headers = {}) returns Deployment|error;

    # Delete a deployment
    resource function delete repos/[string owner]/[string repo]/deployments/[int deploymentId](map<string|string[]> headers = {}) returns error?;

    # List deployment statuses
    resource function get repos/[string owner]/[string repo]/deployments/[int deploymentId]/statuses(map<string|string[]> headers = {}, ReposListDeploymentStatusesQueries queries) returns DeploymentStatus[]|error;

    # Create a deployment status
    resource function post repos/[string owner]/[string repo]/deployments/[int deploymentId]/statuses(DeploymentIdStatusesBody payload, map<string|string[]> headers = {}) returns DeploymentStatus|error;

    # Get a deployment status
    resource function get repos/[string owner]/[string repo]/deployments/[int deploymentId]/statuses/[int statusId](map<string|string[]> headers = {}) returns DeploymentStatus|error;

    # Create a repository dispatch event
    resource function post repos/[string owner]/[string repo]/dispatches(RepoDispatchesBody payload, map<string|string[]> headers = {}) returns error?;

    # List environments
    resource function get repos/[string owner]/[string repo]/environments(map<string|string[]> headers = {}, ReposGetAllEnvironmentsQueries queries) returns EnvironmentResponse|error;

    # Get an environment
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName](map<string|string[]> headers = {}) returns Environment|error;

    # Create or update an environment
    resource function put repos/[string owner]/[string repo]/environments/[string environmentName](EnvironmentsenvironmentNameBody payload, map<string|string[]> headers = {}) returns Environment|error;

    # Delete an environment
    resource function delete repos/[string owner]/[string repo]/environments/[string environmentName](map<string|string[]> headers = {}) returns error?;

    # List deployment branch policies
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName]/deployment\-branch\-policies(map<string|string[]> headers = {}, ReposListDeploymentBranchPoliciesQueries queries) returns DeploymentBranchPolicyResponse|error;

    # Create a deployment branch policy
    resource function post repos/[string owner]/[string repo]/environments/[string environmentName]/deployment\-branch\-policies(DeploymentBranchPolicyNamePatternWithType payload, map<string|string[]> headers = {}) returns DeploymentBranchPolicy|error?;

    # Get a deployment branch policy
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName]/deployment\-branch\-policies/[int branchPolicyId](map<string|string[]> headers = {}) returns DeploymentBranchPolicy|error;

    # Update a deployment branch policy
    resource function put repos/[string owner]/[string repo]/environments/[string environmentName]/deployment\-branch\-policies/[int branchPolicyId](DeploymentBranchPolicyNamePattern payload, map<string|string[]> headers = {}) returns DeploymentBranchPolicy|error;

    # Delete a deployment branch policy
    resource function delete repos/[string owner]/[string repo]/environments/[string environmentName]/deployment\-branch\-policies/[int branchPolicyId](map<string|string[]> headers = {}) returns error?;

    # Get all deployment protection rules for an environment
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName]/deployment_protection_rules(map<string|string[]> headers = {}) returns DeploymentProtectionRuleResponse|error;

    # Create a custom deployment protection rule on an environment
    resource function post repos/[string owner]/[string repo]/environments/[string environmentName]/deployment_protection_rules(EnvironmentNameDeploymentProtectionRulesBody payload, map<string|string[]> headers = {}) returns DeploymentProtectionRule|error;

    # List custom deployment rule integrations available for an environment
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName]/deployment_protection_rules/apps(map<string|string[]> headers = {}, ReposListCustomDeploymentRuleIntegrationsQueries queries) returns CustomDeploymentRuleAppResponse|error;

    # Get a custom deployment protection rule
    resource function get repos/[string owner]/[string repo]/environments/[string environmentName]/deployment_protection_rules/[int protectionRuleId](map<string|string[]> headers = {}) returns DeploymentProtectionRule|error;

    # Disable a custom protection rule for an environment
    resource function delete repos/[string owner]/[string repo]/environments/[string environmentName]/deployment_protection_rules/[int protectionRuleId](map<string|string[]> headers = {}) returns error?;

    # List repository events
    resource function get repos/[string owner]/[string repo]/events(map<string|string[]> headers = {}, ActivityListRepoEventsQueries queries) returns Event[]|error;

    # List forks
    resource function get repos/[string owner]/[string repo]/forks(map<string|string[]> headers = {}, ReposListForksQueries queries) returns MinimalRepository[]|error;

    # Create a fork
    resource function post repos/[string owner]/[string repo]/forks(RepoForksBody payload, map<string|string[]> headers = {}) returns FullRepository|error;

    # Create a blob
    resource function post repos/[string owner]/[string repo]/git/blobs(GitBlobsBody payload, map<string|string[]> headers = {}) returns ShortBlob|error;

    # Get a blob
    resource function get repos/[string owner]/[string repo]/git/blobs/[string fileSha](map<string|string[]> headers = {}) returns Blob|error;

    # Create a commit
    resource function post repos/[string owner]/[string repo]/git/commits(GitCommitsBody payload, map<string|string[]> headers = {}) returns GitCommit|error;

    # Get a commit object
    resource function get repos/[string owner]/[string repo]/git/commits/[string commitSha](map<string|string[]> headers = {}) returns GitCommit|error;

    # List matching references
    resource function get repos/[string owner]/[string repo]/git/matching\-refs/[string ref](map<string|string[]> headers = {}) returns GitRef[]|error;

    # Get a reference
    resource function get repos/[string owner]/[string repo]/git/ref/[string ref](map<string|string[]> headers = {}) returns GitRef|error;

    # Create a reference
    resource function post repos/[string owner]/[string repo]/git/refs(GitRefsBody payload, map<string|string[]> headers = {}) returns GitRef|error;

    # Delete a reference
    resource function delete repos/[string owner]/[string repo]/git/refs/[string ref](map<string|string[]> headers = {}) returns error?;

    # Update a reference
    resource function patch repos/[string owner]/[string repo]/git/refs/[string ref](RefsrefBody payload, map<string|string[]> headers = {}) returns GitRef|error;

    # Create a tag object
    resource function post repos/[string owner]/[string repo]/git/tags(GitTagsBody payload, map<string|string[]> headers = {}) returns GitTag|error;

    # Get a tag
    resource function get repos/[string owner]/[string repo]/git/tags/[string tagSha](map<string|string[]> headers = {}) returns GitTag|error;

    # Create a tree
    resource function post repos/[string owner]/[string repo]/git/trees(GitTreesBody payload, map<string|string[]> headers = {}) returns GitTree|error;

    # Get a tree
    resource function get repos/[string owner]/[string repo]/git/trees/[string treeSha](map<string|string[]> headers = {}, GitGetTreeQueries queries) returns GitTree|error;

    # List repository webhooks
    resource function get repos/[string owner]/[string repo]/hooks(map<string|string[]> headers = {}, ReposListWebhooksQueries queries) returns Hook[]|error;

    # Create a repository webhook
    resource function post repos/[string owner]/[string repo]/hooks(RepoHooksBody payload, map<string|string[]> headers = {}) returns Hook|error;

    # Get a repository webhook
    resource function get repos/[string owner]/[string repo]/hooks/[int hookId](map<string|string[]> headers = {}) returns Hook|error;

    # Delete a repository webhook
    resource function delete repos/[string owner]/[string repo]/hooks/[int hookId](map<string|string[]> headers = {}) returns error?;

    # Update a repository webhook
    resource function patch repos/[string owner]/[string repo]/hooks/[int hookId](HookshookIdBody1 payload, map<string|string[]> headers = {}) returns Hook|error;

    # Get a webhook configuration for a repository
    resource function get repos/[string owner]/[string repo]/hooks/[int hookId]/config(map<string|string[]> headers = {}) returns WebhookConfig|error;

    # Update a webhook configuration for a repository
    resource function patch repos/[string owner]/[string repo]/hooks/[int hookId]/config(HookIdConfigBody payload, map<string|string[]> headers = {}) returns WebhookConfig|error;

    # List deliveries for a repository webhook
    resource function get repos/[string owner]/[string repo]/hooks/[int hookId]/deliveries(map<string|string[]> headers = {}, ReposListWebhookDeliveriesQueries queries) returns HookDeliveryItem[]|error;

    # Get a delivery for a repository webhook
    resource function get repos/[string owner]/[string repo]/hooks/[int hookId]/deliveries/[int deliveryId](map<string|string[]> headers = {}) returns HookDelivery|error;

    # Redeliver a delivery for a repository webhook
    resource function post repos/[string owner]/[string repo]/hooks/[int hookId]/deliveries/[int deliveryId]/attempts(map<string|string[]> headers = {}) returns record {}|error;

    # Ping a repository webhook
    resource function post repos/[string owner]/[string repo]/hooks/[int hookId]/pings(map<string|string[]> headers = {}) returns error?;

    # Test the push repository webhook
    resource function post repos/[string owner]/[string repo]/hooks/[int hookId]/tests(map<string|string[]> headers = {}) returns error?;

    # Get an import status
    resource function get repos/[string owner]/[string repo]/'import(map<string|string[]> headers = {}) returns Import|error;

    # Start an import
    resource function put repos/[string owner]/[string repo]/'import(RepoImportBody payload, map<string|string[]> headers = {}) returns Import|error;

    # Cancel an import
    resource function delete repos/[string owner]/[string repo]/'import(map<string|string[]> headers = {}) returns error?;

    # Update an import
    resource function patch repos/[string owner]/[string repo]/'import(RepoImportBody1 payload, map<string|string[]> headers = {}) returns Import|error;

    # Get commit authors
    resource function get repos/[string owner]/[string repo]/'import/authors(map<string|string[]> headers = {}, MigrationsGetCommitAuthorsQueries queries) returns PorterAuthor[]|error;

    # Map a commit author
    resource function patch repos/[string owner]/[string repo]/'import/authors/[int authorId](AuthorsauthorIdBody payload, map<string|string[]> headers = {}) returns PorterAuthor|error;

    # Get large files
    resource function get repos/[string owner]/[string repo]/'import/large_files(map<string|string[]> headers = {}) returns PorterLargeFile[]|error;

    # Update Git LFS preference
    resource function patch repos/[string owner]/[string repo]/'import/lfs(ImportLfsBody payload, map<string|string[]> headers = {}) returns Import|error;

    # Get a repository installation for the authenticated app
    resource function get repos/[string owner]/[string repo]/installation(map<string|string[]> headers = {}) returns Installation|error;

    # Get interaction restrictions for a repository
    resource function get repos/[string owner]/[string repo]/interaction\-limits(map<string|string[]> headers = {}) returns InteractionLimitResponseAny|error;

    # Set interaction restrictions for a repository
    resource function put repos/[string owner]/[string repo]/interaction\-limits(InteractionLimit payload, map<string|string[]> headers = {}) returns InteractionLimitResponse|error;

    # Remove interaction restrictions for a repository
    resource function delete repos/[string owner]/[string repo]/interaction\-limits(map<string|string[]> headers = {}) returns error?;

    # List repository invitations
    resource function get repos/[string owner]/[string repo]/invitations(map<string|string[]> headers = {}, ReposListInvitationsQueries queries) returns RepositoryInvitation[]|error;

    # Delete a repository invitation
    resource function delete repos/[string owner]/[string repo]/invitations/[int invitationId](map<string|string[]> headers = {}) returns error?;

    # Update a repository invitation
    resource function patch repos/[string owner]/[string repo]/invitations/[int invitationId](InvitationsinvitationIdBody payload, map<string|string[]> headers = {}) returns RepositoryInvitation|error;

    # List repository issues
    resource function get repos/[string owner]/[string repo]/issues(map<string|string[]> headers = {}, IssuesListForRepoQueries queries) returns Issue[]|error;

    # Create an issue
    resource function post repos/[string owner]/[string repo]/issues(RepoIssuesBody payload, map<string|string[]> headers = {}) returns Issue|error;

    # List issue comments for a repository
    resource function get repos/[string owner]/[string repo]/issues/comments(map<string|string[]> headers = {}, IssuesListCommentsForRepoQueries queries) returns IssueComment[]|error;

    # Get an issue comment
    resource function get repos/[string owner]/[string repo]/issues/comments/[int commentId](map<string|string[]> headers = {}) returns IssueComment|error;

    # Delete an issue comment
    resource function delete repos/[string owner]/[string repo]/issues/comments/[int commentId](map<string|string[]> headers = {}) returns error?;

    # Update an issue comment
    resource function patch repos/[string owner]/[string repo]/issues/comments/[int commentId](CommentscommentIdBody1 payload, map<string|string[]> headers = {}) returns IssueComment|error;

    # List reactions for an issue comment
    resource function get repos/[string owner]/[string repo]/issues/comments/[int commentId]/reactions(map<string|string[]> headers = {}, ReactionsListForIssueCommentQueries queries) returns Reaction[]|error;

    # Create reaction for an issue comment
    resource function post repos/[string owner]/[string repo]/issues/comments/[int commentId]/reactions(CommentIdReactionsBody1 payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete an issue comment reaction
    resource function delete repos/[string owner]/[string repo]/issues/comments/[int commentId]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # List issue events for a repository
    resource function get repos/[string owner]/[string repo]/issues/events(map<string|string[]> headers = {}, IssuesListEventsForRepoQueries queries) returns IssueEvent[]|error;

    # Get an issue event
    resource function get repos/[string owner]/[string repo]/issues/events/[int eventId](map<string|string[]> headers = {}) returns IssueEvent|error;

    # Get an issue
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber](map<string|string[]> headers = {}) returns Issue|error?;

    # Update an issue
    resource function patch repos/[string owner]/[string repo]/issues/[int issueNumber](IssuesissueNumberBody payload, map<string|string[]> headers = {}) returns Issue|error;

    # Add assignees to an issue
    resource function post repos/[string owner]/[string repo]/issues/[int issueNumber]/assignees(IssueNumberAssigneesBody payload, map<string|string[]> headers = {}) returns Issue|error;

    # Remove assignees from an issue
    resource function delete repos/[string owner]/[string repo]/issues/[int issueNumber]/assignees(IssueNumberAssigneesBody1 payload, map<string|string[]> headers = {}) returns Issue|error;

    # Check if a user can be assigned to a issue
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/assignees/[string assignee](map<string|string[]> headers = {}) returns error?;

    # List issue comments
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/comments(map<string|string[]> headers = {}, IssuesListCommentsQueries queries) returns IssueComment[]|error;

    # Create an issue comment
    resource function post repos/[string owner]/[string repo]/issues/[int issueNumber]/comments(CommentscommentIdBody1 payload, map<string|string[]> headers = {}) returns IssueComment|error;

    # List issue events
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/events(map<string|string[]> headers = {}, IssuesListEventsQueries queries) returns IssueEventForIssue[]|error;

    # List labels for an issue
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/labels(map<string|string[]> headers = {}, IssuesListLabelsOnIssueQueries queries) returns Label[]|error;

    # Set labels for an issue
    resource function put repos/[string owner]/[string repo]/issues/[int issueNumber]/labels(IssueNumberLabelsBody payload, map<string|string[]> headers = {}) returns Label[]|error;

    # Add labels to an issue
    resource function post repos/[string owner]/[string repo]/issues/[int issueNumber]/labels(IssueNumberLabelsBody1 payload, map<string|string[]> headers = {}) returns Label[]|error;

    # Remove all labels from an issue
    resource function delete repos/[string owner]/[string repo]/issues/[int issueNumber]/labels(map<string|string[]> headers = {}) returns error?;

    # Remove a label from an issue
    resource function delete repos/[string owner]/[string repo]/issues/[int issueNumber]/labels/[string name](map<string|string[]> headers = {}) returns Label[]|error;

    # Lock an issue
    resource function put repos/[string owner]/[string repo]/issues/[int issueNumber]/'lock(IssueNumberLockBody payload, map<string|string[]> headers = {}) returns error?;

    # Unlock an issue
    resource function delete repos/[string owner]/[string repo]/issues/[int issueNumber]/'lock(map<string|string[]> headers = {}) returns error?;

    # List reactions for an issue
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/reactions(map<string|string[]> headers = {}, ReactionsListForIssueQueries queries) returns Reaction[]|error;

    # Create reaction for an issue
    resource function post repos/[string owner]/[string repo]/issues/[int issueNumber]/reactions(IssueNumberReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete an issue reaction
    resource function delete repos/[string owner]/[string repo]/issues/[int issueNumber]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # List timeline events for an issue
    resource function get repos/[string owner]/[string repo]/issues/[int issueNumber]/timeline(map<string|string[]> headers = {}, IssuesListEventsForTimelineQueries queries) returns TimelineIssueEvents[]|error;

    # List deploy keys
    resource function get repos/[string owner]/[string repo]/keys(map<string|string[]> headers = {}, ReposListDeployKeysQueries queries) returns DeployKey[]|error;

    # Create a deploy key
    resource function post repos/[string owner]/[string repo]/keys(RepoKeysBody payload, map<string|string[]> headers = {}) returns DeployKey|error;

    # Get a deploy key
    resource function get repos/[string owner]/[string repo]/keys/[int keyId](map<string|string[]> headers = {}) returns DeployKey|error;

    # Delete a deploy key
    resource function delete repos/[string owner]/[string repo]/keys/[int keyId](map<string|string[]> headers = {}) returns error?;

    # List labels for a repository
    resource function get repos/[string owner]/[string repo]/labels(map<string|string[]> headers = {}, IssuesListLabelsForRepoQueries queries) returns Label[]|error;

    # Create a label
    resource function post repos/[string owner]/[string repo]/labels(RepoLabelsBody payload, map<string|string[]> headers = {}) returns Label|error;

    # Get a label
    resource function get repos/[string owner]/[string repo]/labels/[string name](map<string|string[]> headers = {}) returns Label|error;

    # Delete a label
    resource function delete repos/[string owner]/[string repo]/labels/[string name](map<string|string[]> headers = {}) returns error?;

    # Update a label
    resource function patch repos/[string owner]/[string repo]/labels/[string name](LabelsnameBody payload, map<string|string[]> headers = {}) returns Label|error;

    # List repository languages
    resource function get repos/[string owner]/[string repo]/languages(map<string|string[]> headers = {}) returns Language|error;

    # Get the license for a repository
    resource function get repos/[string owner]/[string repo]/license(map<string|string[]> headers = {}) returns LicenseContent|error;

    # Sync a fork branch with the upstream repository
    resource function post repos/[string owner]/[string repo]/merge\-upstream(RepoMergeUpstreamBody payload, map<string|string[]> headers = {}) returns MergedUpstream|error;

    # Merge a branch
    resource function post repos/[string owner]/[string repo]/merges(RepoMergesBody payload, map<string|string[]> headers = {}) returns Commit|error?;

    # List milestones
    resource function get repos/[string owner]/[string repo]/milestones(map<string|string[]> headers = {}, IssuesListMilestonesQueries queries) returns Milestone[]|error;

    # Create a milestone
    resource function post repos/[string owner]/[string repo]/milestones(RepoMilestonesBody payload, map<string|string[]> headers = {}) returns Milestone|error;

    # Get a milestone
    resource function get repos/[string owner]/[string repo]/milestones/[int milestoneNumber](map<string|string[]> headers = {}) returns Milestone|error;

    # Delete a milestone
    resource function delete repos/[string owner]/[string repo]/milestones/[int milestoneNumber](map<string|string[]> headers = {}) returns error?;

    # Update a milestone
    resource function patch repos/[string owner]/[string repo]/milestones/[int milestoneNumber](MilestonesmilestoneNumberBody payload, map<string|string[]> headers = {}) returns Milestone|error;

    # List labels for issues in a milestone
    resource function get repos/[string owner]/[string repo]/milestones/[int milestoneNumber]/labels(map<string|string[]> headers = {}, IssuesListLabelsForMilestoneQueries queries) returns Label[]|error;

    # List repository notifications for the authenticated user
    resource function get repos/[string owner]/[string repo]/notifications(map<string|string[]> headers = {}, ActivityListRepoNotificationsForAuthenticatedUserQueries queries) returns NotificationThread[]|error;

    # Mark repository notifications as read
    resource function put repos/[string owner]/[string repo]/notifications(RepoNotificationsBody payload, map<string|string[]> headers = {}) returns NotificationRead|error?;

    # Get a GitHub Pages site
    resource function get repos/[string owner]/[string repo]/pages(map<string|string[]> headers = {}) returns Page|error;

    # Update information about a GitHub Pages site
    resource function put repos/[string owner]/[string repo]/pages(RepoPagesBody payload, map<string|string[]> headers = {}) returns error?;

    # Create a GitHub Pages site
    resource function post repos/[string owner]/[string repo]/pages(RepoPagesBody1 payload, map<string|string[]> headers = {}) returns Page|error;

    # Delete a GitHub Pages site
    resource function delete repos/[string owner]/[string repo]/pages(map<string|string[]> headers = {}) returns error?;

    # List GitHub Pages builds
    resource function get repos/[string owner]/[string repo]/pages/builds(map<string|string[]> headers = {}, ReposListPagesBuildsQueries queries) returns PageBuild[]|error;

    # Request a GitHub Pages build
    resource function post repos/[string owner]/[string repo]/pages/builds(map<string|string[]> headers = {}) returns PageBuildStatus|error;

    # Get latest Pages build
    resource function get repos/[string owner]/[string repo]/pages/builds/latest(map<string|string[]> headers = {}) returns PageBuild|error;

    # Get GitHub Pages build
    resource function get repos/[string owner]/[string repo]/pages/builds/[int buildId](map<string|string[]> headers = {}) returns PageBuild|error;

    # Create a GitHub Pages deployment
    resource function post repos/[string owner]/[string repo]/pages/deployment(PagesDeploymentBody payload, map<string|string[]> headers = {}) returns PageDeployment|error;

    # Get a DNS health check for GitHub Pages
    resource function get repos/[string owner]/[string repo]/pages/health(map<string|string[]> headers = {}) returns PagesHealthCheck|EmptyObject|error;

    # Enable private vulnerability reporting for a repository
    resource function put repos/[string owner]/[string repo]/private\-vulnerability\-reporting(map<string|string[]> headers = {}) returns error?;

    # Disable private vulnerability reporting for a repository
    resource function delete repos/[string owner]/[string repo]/private\-vulnerability\-reporting(map<string|string[]> headers = {}) returns error?;

    # List repository projects
    resource function get repos/[string owner]/[string repo]/projects(map<string|string[]> headers = {}, ProjectsListForRepoQueries queries) returns Project[]|error;

    # Create a repository project
    resource function post repos/[string owner]/[string repo]/projects(OrgProjectsBody payload, map<string|string[]> headers = {}) returns Project|error;

    # List pull requests
    resource function get repos/[string owner]/[string repo]/pulls(map<string|string[]> headers = {}, PullsListQueries queries) returns PullRequestSimple[]|error?;

    # Create a pull request
    resource function post repos/[string owner]/[string repo]/pulls(RepoPullsBody payload, map<string|string[]> headers = {}) returns PullRequest|error;

    # List review comments in a repository
    resource function get repos/[string owner]/[string repo]/pulls/comments(map<string|string[]> headers = {}, PullsListReviewCommentsForRepoQueries queries) returns PullRequestReviewComment[]|error;

    # Get a review comment for a pull request
    resource function get repos/[string owner]/[string repo]/pulls/comments/[int commentId](map<string|string[]> headers = {}) returns PullRequestReviewComment|error;

    # Delete a review comment for a pull request
    resource function delete repos/[string owner]/[string repo]/pulls/comments/[int commentId](map<string|string[]> headers = {}) returns error?;

    # Update a review comment for a pull request
    resource function patch repos/[string owner]/[string repo]/pulls/comments/[int commentId](CommentscommentIdBody2 payload, map<string|string[]> headers = {}) returns PullRequestReviewComment|error;

    # List reactions for a pull request review comment
    resource function get repos/[string owner]/[string repo]/pulls/comments/[int commentId]/reactions(map<string|string[]> headers = {}, ReactionsListForPullRequestReviewCommentQueries queries) returns Reaction[]|error;

    # Create reaction for a pull request review comment
    resource function post repos/[string owner]/[string repo]/pulls/comments/[int commentId]/reactions(CommentIdReactionsBody2 payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete a pull request comment reaction
    resource function delete repos/[string owner]/[string repo]/pulls/comments/[int commentId]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # Get a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber](map<string|string[]> headers = {}) returns PullRequest|error?;

    # Update a pull request
    resource function patch repos/[string owner]/[string repo]/pulls/[int pullNumber](PullspullNumberBody payload, map<string|string[]> headers = {}) returns PullRequest|error;

    # Create a codespace from a pull request
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/codespaces(PullNumberCodespacesBody payload, map<string|string[]> headers = {}) returns Codespace|error;

    # List review comments on a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/comments(map<string|string[]> headers = {}, PullsListReviewCommentsQueries queries) returns PullRequestReviewComment[]|error;

    # Create a review comment for a pull request
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/comments(PullNumberCommentsBody payload, map<string|string[]> headers = {}) returns PullRequestReviewComment|error;

    # Create a reply for a review comment
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/comments/[int commentId]/replies(CommentIdRepliesBody payload, map<string|string[]> headers = {}) returns PullRequestReviewComment|error;

    # List commits on a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/commits(map<string|string[]> headers = {}, PullsListCommitsQueries queries) returns Commit[]|error;

    # List pull requests files
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/files(map<string|string[]> headers = {}, PullsListFilesQueries queries) returns DiffEntry[]|error;

    # Check if a pull request has been merged
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/merge(map<string|string[]> headers = {}) returns error?;

    # Merge a pull request
    resource function put repos/[string owner]/[string repo]/pulls/[int pullNumber]/merge(PullNumberMergeBody payload, map<string|string[]> headers = {}) returns PullRequestMergeResult|error;

    # Get all requested reviewers for a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/requested_reviewers(map<string|string[]> headers = {}) returns PullRequestReviewRequest|error;

    # Request reviewers for a pull request
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/requested_reviewers(PullNumberRequestedReviewersBody payload, map<string|string[]> headers = {}) returns PullRequestSimple|error;

    # Remove requested reviewers from a pull request
    resource function delete repos/[string owner]/[string repo]/pulls/[int pullNumber]/requested_reviewers(PullNumberRequestedReviewersBody1 payload, map<string|string[]> headers = {}) returns PullRequestSimple|error;

    # List reviews for a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews(map<string|string[]> headers = {}, PullsListReviewsQueries queries) returns PullRequestReview[]|error;

    # Create a review for a pull request
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews(PullNumberReviewsBody payload, map<string|string[]> headers = {}) returns PullRequestReview|error;

    # Get a review for a pull request
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId](map<string|string[]> headers = {}) returns PullRequestReview|error;

    # Update a review for a pull request
    resource function put repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId](ReviewsreviewIdBody payload, map<string|string[]> headers = {}) returns PullRequestReview|error;

    # Delete a pending review for a pull request
    resource function delete repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId](map<string|string[]> headers = {}) returns PullRequestReview|error;

    # List comments for a pull request review
    resource function get repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId]/comments(map<string|string[]> headers = {}, PullsListCommentsForReviewQueries queries) returns ReviewComment[]|error;

    # Dismiss a review for a pull request
    resource function put repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId]/dismissals(ReviewIdDismissalsBody payload, map<string|string[]> headers = {}) returns PullRequestReview|error;

    # Submit a review for a pull request
    resource function post repos/[string owner]/[string repo]/pulls/[int pullNumber]/reviews/[int reviewId]/events(ReviewIdEventsBody payload, map<string|string[]> headers = {}) returns PullRequestReview|error;

    # Update a pull request branch
    resource function put repos/[string owner]/[string repo]/pulls/[int pullNumber]/update\-branch(PullNumberUpdateBranchBody payload, map<string|string[]> headers = {}) returns NotificationRead|error;

    # Get a repository README
    resource function get repos/[string owner]/[string repo]/readme(map<string|string[]> headers = {}, ReposGetReadmeQueries queries) returns ContentFile|error;

    # Get a repository README for a directory
    resource function get repos/[string owner]/[string repo]/readme/[string dir](map<string|string[]> headers = {}, ReposGetReadmeInDirectoryQueries queries) returns ContentFile|error;

    # List releases
    resource function get repos/[string owner]/[string repo]/releases(map<string|string[]> headers = {}, ReposListReleasesQueries queries) returns Release[]|error;

    # Create a release
    resource function post repos/[string owner]/[string repo]/releases(RepoReleasesBody payload, map<string|string[]> headers = {}) returns Release|error;

    # Get a release asset
    resource function get repos/[string owner]/[string repo]/releases/assets/[int assetId](map<string|string[]> headers = {}) returns ReleaseAsset|error?;

    # Delete a release asset
    resource function delete repos/[string owner]/[string repo]/releases/assets/[int assetId](map<string|string[]> headers = {}) returns error?;

    # Update a release asset
    resource function patch repos/[string owner]/[string repo]/releases/assets/[int assetId](AssetsassetIdBody payload, map<string|string[]> headers = {}) returns ReleaseAsset|error;

    # Generate release notes content for a release
    resource function post repos/[string owner]/[string repo]/releases/generate\-notes(ReleasesGenerateNotesBody payload, map<string|string[]> headers = {}) returns ReleaseNotesContent|error;

    # Get the latest release
    resource function get repos/[string owner]/[string repo]/releases/latest(map<string|string[]> headers = {}) returns Release|error;

    # Get a release by tag name
    resource function get repos/[string owner]/[string repo]/releases/tags/[string tag](map<string|string[]> headers = {}) returns Release|error;

    # Get a release
    resource function get repos/[string owner]/[string repo]/releases/[int releaseId](map<string|string[]> headers = {}) returns Release|error;

    # Delete a release
    resource function delete repos/[string owner]/[string repo]/releases/[int releaseId](map<string|string[]> headers = {}) returns error?;

    # Update a release
    resource function patch repos/[string owner]/[string repo]/releases/[int releaseId](ReleasesreleaseIdBody payload, map<string|string[]> headers = {}) returns Release|error;

    # List release assets
    resource function get repos/[string owner]/[string repo]/releases/[int releaseId]/assets(map<string|string[]> headers = {}, ReposListReleaseAssetsQueries queries) returns ReleaseAsset[]|error;

    # Upload a release asset
    resource function post repos/[string owner]/[string repo]/releases/[int releaseId]/assets(byte[] payload, map<string|string[]> headers = {}, ReposUploadReleaseAssetQueries queries) returns ReleaseAsset|error;

    # List reactions for a release
    resource function get repos/[string owner]/[string repo]/releases/[int releaseId]/reactions(map<string|string[]> headers = {}, ReactionsListForReleaseQueries queries) returns Reaction[]|error;

    # Create reaction for a release
    resource function post repos/[string owner]/[string repo]/releases/[int releaseId]/reactions(ReleaseIdReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # Delete a release reaction
    resource function delete repos/[string owner]/[string repo]/releases/[int releaseId]/reactions/[int reactionId](map<string|string[]> headers = {}) returns error?;

    # Get rules for a branch
    resource function get repos/[string owner]/[string repo]/rules/branches/[string branch](map<string|string[]> headers = {}, ReposGetBranchRulesQueries queries) returns RepositoryRuleDetailed[]|error;

    # Get all repository rulesets
    resource function get repos/[string owner]/[string repo]/rulesets(map<string|string[]> headers = {}, ReposGetRepoRulesetsQueries queries) returns RepositoryRuleset[]|error;

    # Create a repository ruleset
    resource function post repos/[string owner]/[string repo]/rulesets(RepoRulesetsBody payload, map<string|string[]> headers = {}) returns RepositoryRuleset|error;

    # List repository rule suites
    resource function get repos/[string owner]/[string repo]/rulesets/rule\-suites(map<string|string[]> headers = {}, ReposGetRepoRuleSuitesQueries queries) returns RuleSuites|error;

    # Get a repository rule suite
    resource function get repos/[string owner]/[string repo]/rulesets/rule\-suites/[int ruleSuiteId](map<string|string[]> headers = {}) returns RuleSuite|error;

    # Get a repository ruleset
    resource function get repos/[string owner]/[string repo]/rulesets/[int rulesetId](map<string|string[]> headers = {}, ReposGetRepoRulesetQueries queries) returns RepositoryRuleset|error;

    # Update a repository ruleset
    resource function put repos/[string owner]/[string repo]/rulesets/[int rulesetId](RulesetsrulesetIdBody1 payload, map<string|string[]> headers = {}) returns RepositoryRuleset|error;

    # Delete a repository ruleset
    resource function delete repos/[string owner]/[string repo]/rulesets/[int rulesetId](map<string|string[]> headers = {}) returns error?;

    # List secret scanning alerts for a repository
    resource function get repos/[string owner]/[string repo]/secret\-scanning/alerts(map<string|string[]> headers = {}, SecretScanningListAlertsForRepoQueries queries) returns SecretScanningAlert[]|error;

    # Get a secret scanning alert
    resource function get repos/[string owner]/[string repo]/secret\-scanning/alerts/[AlertNumber alertNumber](map<string|string[]> headers = {}) returns SecretScanningAlert|error?;

    # Update a secret scanning alert
    resource function patch repos/[string owner]/[string repo]/secret\-scanning/alerts/[AlertNumber alertNumber](AlertsalertNumberBody2 payload, map<string|string[]> headers = {}) returns SecretScanningAlert|error;

    # List locations for a secret scanning alert
    resource function get repos/[string owner]/[string repo]/secret\-scanning/alerts/[AlertNumber alertNumber]/locations(map<string|string[]> headers = {}, SecretScanningListLocationsForAlertQueries queries) returns SecretScanningLocation[]|error;

    # List repository security advisories
    resource function get repos/[string owner]/[string repo]/security\-advisories(map<string|string[]> headers = {}, SecurityAdvisoriesListRepositoryAdvisoriesQueries queries) returns RepositoryAdvisory[]|error;

    # Create a repository security advisory
    resource function post repos/[string owner]/[string repo]/security\-advisories(RepositoryAdvisoryCreate payload, map<string|string[]> headers = {}) returns RepositoryAdvisory|error;

    # Privately report a security vulnerability
    resource function post repos/[string owner]/[string repo]/security\-advisories/reports(PrivateVulnerabilityReportCreate payload, map<string|string[]> headers = {}) returns RepositoryAdvisory|error;

    # Get a repository security advisory
    resource function get repos/[string owner]/[string repo]/security\-advisories/[string ghsaId](map<string|string[]> headers = {}) returns RepositoryAdvisory|error;

    # Update a repository security advisory
    resource function patch repos/[string owner]/[string repo]/security\-advisories/[string ghsaId](RepositoryAdvisoryUpdate payload, map<string|string[]> headers = {}) returns RepositoryAdvisory|error;

    # Request a CVE for a repository security advisory
    resource function post repos/[string owner]/[string repo]/security\-advisories/[string ghsaId]/cve(map<string|string[]> headers = {}) returns record {}|error;

    # List stargazers
    resource function get repos/[string owner]/[string repo]/stargazers(map<string|string[]> headers = {}, ActivityListStargazersForRepoQueries queries) returns StargazerResponse|error;

    # Get the weekly commit activity
    resource function get repos/[string owner]/[string repo]/stats/code_frequency(map<string|string[]> headers = {}) returns CodeFrequencyStat[]|record {}|error?;

    # Get the last year of commit activity
    resource function get repos/[string owner]/[string repo]/stats/commit_activity(map<string|string[]> headers = {}) returns CommitActivity[]|record {}|error?;

    # Get all contributor commit activity
    resource function get repos/[string owner]/[string repo]/stats/contributors(map<string|string[]> headers = {}) returns ContributorActivity[]|record {}|error?;

    # Get the weekly commit count
    resource function get repos/[string owner]/[string repo]/stats/participation(map<string|string[]> headers = {}) returns ParticipationStats|error;

    # Get the hourly commit count for each day
    resource function get repos/[string owner]/[string repo]/stats/punch_card(map<string|string[]> headers = {}) returns CodeFrequencyStat[]|error?;

    # Create a commit status
    resource function post repos/[string owner]/[string repo]/statuses/[string sha](StatusesshaBody payload, map<string|string[]> headers = {}) returns Status|error;

    # List watchers
    resource function get repos/[string owner]/[string repo]/subscribers(map<string|string[]> headers = {}, ActivityListWatchersForRepoQueries queries) returns SimpleUser[]|error;

    # Get a repository subscription
    resource function get repos/[string owner]/[string repo]/subscription(map<string|string[]> headers = {}) returns RepositorySubscription|error;

    # Set a repository subscription
    resource function put repos/[string owner]/[string repo]/subscription(RepoSubscriptionBody payload, map<string|string[]> headers = {}) returns RepositorySubscription|error;

    # Delete a repository subscription
    resource function delete repos/[string owner]/[string repo]/subscription(map<string|string[]> headers = {}) returns error?;

    # List repository tags
    resource function get repos/[string owner]/[string repo]/tags(map<string|string[]> headers = {}, ReposListTagsQueries queries) returns Tag[]|error;

    # List tag protection states for a repository
    resource function get repos/[string owner]/[string repo]/tags/protection(map<string|string[]> headers = {}) returns TagProtection[]|error;

    # Create a tag protection state for a repository
    resource function post repos/[string owner]/[string repo]/tags/protection(TagsProtectionBody payload, map<string|string[]> headers = {}) returns TagProtection|error;

    # Delete a tag protection state for a repository
    resource function delete repos/[string owner]/[string repo]/tags/protection/[int tagProtectionId](map<string|string[]> headers = {}) returns error?;

    # Download a repository archive (tar)
    resource function get repos/[string owner]/[string repo]/tarball/[string ref](map<string|string[]> headers = {}) returns error?;

    # List repository teams
    resource function get repos/[string owner]/[string repo]/teams(map<string|string[]> headers = {}, ReposListTeamsQueries queries) returns Team[]|error;

    # Get all repository topics
    resource function get repos/[string owner]/[string repo]/topics(map<string|string[]> headers = {}, ReposGetAllTopicsQueries queries) returns Topic|error;

    # Replace all repository topics
    resource function put repos/[string owner]/[string repo]/topics(RepoTopicsBody payload, map<string|string[]> headers = {}) returns Topic|error;

    # Get repository clones
    resource function get repos/[string owner]/[string repo]/traffic/clones(map<string|string[]> headers = {}, ReposGetClonesQueries queries) returns CloneTraffic|error;

    # Get top referral paths
    resource function get repos/[string owner]/[string repo]/traffic/popular/paths(map<string|string[]> headers = {}) returns ContentTraffic[]|error;

    # Get top referral sources
    resource function get repos/[string owner]/[string repo]/traffic/popular/referrers(map<string|string[]> headers = {}) returns ReferrerTraffic[]|error;

    # Get page views
    resource function get repos/[string owner]/[string repo]/traffic/views(map<string|string[]> headers = {}, ReposGetViewsQueries queries) returns ViewTraffic|error;

    # Transfer a repository
    resource function post repos/[string owner]/[string repo]/transfer(RepoTransferBody payload, map<string|string[]> headers = {}) returns MinimalRepository|error;

    # Check if vulnerability alerts are enabled for a repository
    resource function get repos/[string owner]/[string repo]/vulnerability\-alerts(map<string|string[]> headers = {}) returns error?;

    # Enable vulnerability alerts
    resource function put repos/[string owner]/[string repo]/vulnerability\-alerts(map<string|string[]> headers = {}) returns error?;

    # Disable vulnerability alerts
    resource function delete repos/[string owner]/[string repo]/vulnerability\-alerts(map<string|string[]> headers = {}) returns error?;

    # Download a repository archive (zip)
    resource function get repos/[string owner]/[string repo]/zipball/[string ref](map<string|string[]> headers = {}) returns error?;

    # Create a repository using a template
    resource function post repos/[string templateOwner]/[string templateRepo]/generate(TemplateRepoGenerateBody payload, map<string|string[]> headers = {}) returns Repository|error;

    # List public repositories
    resource function get repositories(map<string|string[]> headers = {}, ReposListPublicQueries queries) returns MinimalRepository[]|error?;

    # List environment secrets
    resource function get repositories/[int repositoryId]/environments/[string environmentName]/secrets(map<string|string[]> headers = {}, ActionsListEnvironmentSecretsQueries queries) returns ActionsSecretResponse|error;

    # Get an environment public key
    resource function get repositories/[int repositoryId]/environments/[string environmentName]/secrets/public\-key(map<string|string[]> headers = {}) returns ActionsPublicKey|error;

    # Get an environment secret
    resource function get repositories/[int repositoryId]/environments/[string environmentName]/secrets/[string secretName](map<string|string[]> headers = {}) returns ActionsSecret|error;

    # Create or update an environment secret
    resource function put repositories/[int repositoryId]/environments/[string environmentName]/secrets/[string secretName](SecretssecretNameBody6 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete an environment secret
    resource function delete repositories/[int repositoryId]/environments/[string environmentName]/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List environment variables
    resource function get repositories/[int repositoryId]/environments/[string environmentName]/variables(map<string|string[]> headers = {}, ActionsListEnvironmentVariablesQueries queries) returns ActionsVariableResponse|error;

    # Create an environment variable
    resource function post repositories/[int repositoryId]/environments/[string environmentName]/variables(ActionsVariablesBody1 payload, map<string|string[]> headers = {}) returns EmptyObject|error;

    # Get an environment variable
    resource function get repositories/[int repositoryId]/environments/[string environmentName]/variables/[string name](map<string|string[]> headers = {}) returns ActionsVariable|error;

    # Delete an environment variable
    resource function delete repositories/[int repositoryId]/environments/[string environmentName]/variables/[string name](map<string|string[]> headers = {}) returns error?;

    # Update an environment variable
    resource function patch repositories/[int repositoryId]/environments/[string environmentName]/variables/[string name](VariablesnameBody1 payload, map<string|string[]> headers = {}) returns error?;

    # Search code
    resource function get search/code(map<string|string[]> headers = {}, SearchCodeQueries queries) returns CodeSearchResultItemResponse|error?;

    # Search commits
    resource function get search/commits(map<string|string[]> headers = {}, SearchCommitsQueries queries) returns CommitSearchResultItemResponse|error?;

    # Search issues and pull requests
    resource function get search/issues(map<string|string[]> headers = {}, SearchIssuesAndPullRequestsQueries queries) returns IssueSearchResultItemResponse|error?;

    # Search labels
    resource function get search/labels(map<string|string[]> headers = {}, SearchLabelsQueries queries) returns LabelSearchResultItemResponse|error?;

    # Search repositories
    resource function get search/repositories(map<string|string[]> headers = {}, SearchReposQueries queries) returns RepoSearchResultItemResponse|error?;

    # Search topics
    resource function get search/topics(map<string|string[]> headers = {}, SearchTopicsQueries queries) returns TopicSearchResultItemResponse|error?;

    # Search users
    resource function get search/users(map<string|string[]> headers = {}, SearchUsersQueries queries) returns UserSearchResultItemResponse|error?;

    # Get a team (Legacy)
    resource function get teams/[int teamId](map<string|string[]> headers = {}) returns TeamFull|error;

    # Delete a team (Legacy)
    resource function delete teams/[int teamId](map<string|string[]> headers = {}) returns error?;

    # Update a team (Legacy)
    resource function patch teams/[int teamId](TeamsteamIdBody payload, map<string|string[]> headers = {}) returns TeamFull|error;

    # List discussions (Legacy)
    resource function get teams/[int teamId]/discussions(map<string|string[]> headers = {}, TeamsListDiscussionsLegacyQueries queries) returns TeamDiscussion[]|error;

    # Create a discussion (Legacy)
    resource function post teams/[int teamId]/discussions(TeamSlugDiscussionsBody payload, map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # Get a discussion (Legacy)
    resource function get teams/[int teamId]/discussions/[int discussionNumber](map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # Delete a discussion (Legacy)
    resource function delete teams/[int teamId]/discussions/[int discussionNumber](map<string|string[]> headers = {}) returns error?;

    # Update a discussion (Legacy)
    resource function patch teams/[int teamId]/discussions/[int discussionNumber](DiscussionsdiscussionNumberBody payload, map<string|string[]> headers = {}) returns TeamDiscussion|error;

    # List discussion comments (Legacy)
    resource function get teams/[int teamId]/discussions/[int discussionNumber]/comments(map<string|string[]> headers = {}, TeamsListDiscussionCommentsLegacyQueries queries) returns TeamDiscussionComment[]|error;

    # Create a discussion comment (Legacy)
    resource function post teams/[int teamId]/discussions/[int discussionNumber]/comments(DiscussionNumberCommentsBody payload, map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # Get a discussion comment (Legacy)
    resource function get teams/[int teamId]/discussions/[int discussionNumber]/comments/[int commentNumber](map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # Delete a discussion comment (Legacy)
    resource function delete teams/[int teamId]/discussions/[int discussionNumber]/comments/[int commentNumber](map<string|string[]> headers = {}) returns error?;

    # Update a discussion comment (Legacy)
    resource function patch teams/[int teamId]/discussions/[int discussionNumber]/comments/[int commentNumber](DiscussionNumberCommentsBody payload, map<string|string[]> headers = {}) returns TeamDiscussionComment|error;

    # List reactions for a team discussion comment (Legacy)
    resource function get teams/[int teamId]/discussions/[int discussionNumber]/comments/[int commentNumber]/reactions(map<string|string[]> headers = {}, ReactionsListForTeamDiscussionCommentLegacyQueries queries) returns Reaction[]|error;

    # Create reaction for a team discussion comment (Legacy)
    resource function post teams/[int teamId]/discussions/[int discussionNumber]/comments/[int commentNumber]/reactions(CommentNumberReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # List reactions for a team discussion (Legacy)
    resource function get teams/[int teamId]/discussions/[int discussionNumber]/reactions(map<string|string[]> headers = {}, ReactionsListForTeamDiscussionLegacyQueries queries) returns Reaction[]|error;

    # Create reaction for a team discussion (Legacy)
    resource function post teams/[int teamId]/discussions/[int discussionNumber]/reactions(DiscussionNumberReactionsBody payload, map<string|string[]> headers = {}) returns Reaction|error;

    # List pending team invitations (Legacy)
    resource function get teams/[int teamId]/invitations(map<string|string[]> headers = {}, TeamsListPendingInvitationsLegacyQueries queries) returns OrganizationInvitation[]|error;

    # List team members (Legacy)
    resource function get teams/[int teamId]/members(map<string|string[]> headers = {}, TeamsListMembersLegacyQueries queries) returns SimpleUser[]|error;

    # Get team member (Legacy)
    resource function get teams/[int teamId]/members/[string username](map<string|string[]> headers = {}) returns error?;

    # Add team member (Legacy)
    resource function put teams/[int teamId]/members/[string username](map<string|string[]> headers = {}) returns error?;

    # Remove team member (Legacy)
    resource function delete teams/[int teamId]/members/[string username](map<string|string[]> headers = {}) returns error?;

    # Get team membership for a user (Legacy)
    resource function get teams/[int teamId]/memberships/[string username](map<string|string[]> headers = {}) returns TeamMembership|error;

    # Add or update team membership for a user (Legacy)
    resource function put teams/[int teamId]/memberships/[string username](MembershipsusernameBody1 payload, map<string|string[]> headers = {}) returns TeamMembership|error;

    # Remove team membership for a user (Legacy)
    resource function delete teams/[int teamId]/memberships/[string username](map<string|string[]> headers = {}) returns error?;

    # List team projects (Legacy)
    resource function get teams/[int teamId]/projects(map<string|string[]> headers = {}, TeamsListProjectsLegacyQueries queries) returns TeamProject[]|error;

    # Check team permissions for a project (Legacy)
    resource function get teams/[int teamId]/projects/[int projectId](map<string|string[]> headers = {}) returns TeamProject|error;

    # Add or update team project permissions (Legacy)
    resource function put teams/[int teamId]/projects/[int projectId](ProjectsprojectIdBody2 payload, map<string|string[]> headers = {}) returns error?;

    # Remove a project from a team (Legacy)
    resource function delete teams/[int teamId]/projects/[int projectId](map<string|string[]> headers = {}) returns error?;

    # List team repositories (Legacy)
    resource function get teams/[int teamId]/repos(map<string|string[]> headers = {}, TeamsListReposLegacyQueries queries) returns MinimalRepository[]|error;

    # Check team permissions for a repository (Legacy)
    resource function get teams/[int teamId]/repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns TeamRepository|error?;

    # Add or update team repository permissions (Legacy)
    resource function put teams/[int teamId]/repos/[string owner]/[string repo](OwnerrepoBody2 payload, map<string|string[]> headers = {}) returns error?;

    # Remove a repository from a team (Legacy)
    resource function delete teams/[int teamId]/repos/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # List child teams (Legacy)
    resource function get teams/[int teamId]/teams(map<string|string[]> headers = {}, TeamsListChildLegacyQueries queries) returns Team[]|error;

    # Get the authenticated user
    resource function get user(map<string|string[]> headers = {}) returns UserResponse|error?;

    # Update the authenticated user
    resource function patch user(UserBody payload, map<string|string[]> headers = {}) returns PrivateUser|error?;

    # List users blocked by the authenticated user
    resource function get user/blocks(map<string|string[]> headers = {}, UsersListBlockedByAuthenticatedUserQueries queries) returns SimpleUser[]|error?;

    # Check if a user is blocked by the authenticated user
    resource function get user/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # Block a user
    resource function put user/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # Unblock a user
    resource function delete user/blocks/[string username](map<string|string[]> headers = {}) returns error?;

    # List codespaces for the authenticated user
    resource function get user/codespaces(map<string|string[]> headers = {}, CodespacesListForAuthenticatedUserQueries queries) returns CodespaceResponse|error?;

    # Create a codespace for the authenticated user
    resource function post user/codespaces(UserCodespacesBody payload, map<string|string[]> headers = {}) returns Codespace|error;

    # List secrets for the authenticated user
    resource function get user/codespaces/secrets(map<string|string[]> headers = {}, CodespacesListSecretsForAuthenticatedUserQueries queries) returns CodespacesSecretResponse|error;

    # Get public key for the authenticated user
    resource function get user/codespaces/secrets/public\-key(map<string|string[]> headers = {}) returns CodespacesUserPublicKey|error;

    # Get a secret for the authenticated user
    resource function get user/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns CodespacesSecret|error;

    # Create or update a secret for the authenticated user
    resource function put user/codespaces/secrets/[string secretName](SecretssecretNameBody7 payload, map<string|string[]> headers = {}) returns EmptyObject|error?;

    # Delete a secret for the authenticated user
    resource function delete user/codespaces/secrets/[string secretName](map<string|string[]> headers = {}) returns error?;

    # List selected repositories for a user secret
    resource function get user/codespaces/secrets/[string secretName]/repositories(map<string|string[]> headers = {}) returns MinimalRepositoryResponse|error;

    # Set selected repositories for a user secret
    resource function put user/codespaces/secrets/[string secretName]/repositories(SecretNameRepositoriesBody3 payload, map<string|string[]> headers = {}) returns error?;

    # Add a selected repository to a user secret
    resource function put user/codespaces/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove a selected repository from a user secret
    resource function delete user/codespaces/secrets/[string secretName]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Get a codespace for the authenticated user
    resource function get user/codespaces/[string codespaceName](map<string|string[]> headers = {}) returns Codespace|error?;

    # Delete a codespace for the authenticated user
    resource function delete user/codespaces/[string codespaceName](map<string|string[]> headers = {}) returns record {}|error?;

    # Update a codespace for the authenticated user
    resource function patch user/codespaces/[string codespaceName](CodespacescodespaceNameBody payload, map<string|string[]> headers = {}) returns Codespace|error;

    # Export a codespace for the authenticated user
    resource function post user/codespaces/[string codespaceName]/exports(map<string|string[]> headers = {}) returns CodespaceExportDetails|error;

    # Get details about a codespace export
    resource function get user/codespaces/[string codespaceName]/exports/[string exportId](map<string|string[]> headers = {}) returns CodespaceExportDetails|error;

    # List machine types for a codespace
    resource function get user/codespaces/[string codespaceName]/machines(map<string|string[]> headers = {}) returns CodespaceMachineResponse|error?;

    # Create a repository from an unpublished codespace
    resource function post user/codespaces/[string codespaceName]/publish(CodespaceNamePublishBody payload, map<string|string[]> headers = {}) returns CodespaceWithFullRepository|error;

    # Start a codespace for the authenticated user
    resource function post user/codespaces/[string codespaceName]/'start(map<string|string[]> headers = {}) returns Codespace|error?;

    # Stop a codespace for the authenticated user
    resource function post user/codespaces/[string codespaceName]/stop(map<string|string[]> headers = {}) returns Codespace|error;

    # Get list of conflicting packages during Docker migration for authenticated-user
    resource function get user/docker/conflicts(map<string|string[]> headers = {}) returns Package[]|error;

    # Set primary email visibility for the authenticated user
    resource function patch user/email/visibility(EmailVisibilityBody payload, map<string|string[]> headers = {}) returns Email[]|error?;

    # List email addresses for the authenticated user
    resource function get user/emails(map<string|string[]> headers = {}, UsersListEmailsForAuthenticatedUserQueries queries) returns Email[]|error?;

    # Add an email address for the authenticated user
    resource function post user/emails(UserEmailsBody payload, map<string|string[]> headers = {}) returns Email[]|error?;

    # Delete an email address for the authenticated user
    resource function delete user/emails(UserEmailsBody1 payload, map<string|string[]> headers = {}) returns error?;

    # List followers of the authenticated user
    resource function get user/followers(map<string|string[]> headers = {}, UsersListFollowersForAuthenticatedUserQueries queries) returns SimpleUser[]|error?;

    # List the people the authenticated user follows
    resource function get user/following(map<string|string[]> headers = {}, UsersListFollowedByAuthenticatedUserQueries queries) returns SimpleUser[]|error?;

    # Check if a person is followed by the authenticated user
    resource function get user/following/[string username](map<string|string[]> headers = {}) returns error?;

    # Follow a user
    resource function put user/following/[string username](map<string|string[]> headers = {}) returns error?;

    # Unfollow a user
    resource function delete user/following/[string username](map<string|string[]> headers = {}) returns error?;

    # List GPG keys for the authenticated user
    resource function get user/gpg_keys(map<string|string[]> headers = {}, UsersListGpgKeysForAuthenticatedUserQueries queries) returns GpgKey[]|error?;

    # Create a GPG key for the authenticated user
    resource function post user/gpg_keys(UserGpgKeysBody payload, map<string|string[]> headers = {}) returns GpgKey|error?;

    # Get a GPG key for the authenticated user
    resource function get user/gpg_keys/[int gpgKeyId](map<string|string[]> headers = {}) returns GpgKey|error?;

    # Delete a GPG key for the authenticated user
    resource function delete user/gpg_keys/[int gpgKeyId](map<string|string[]> headers = {}) returns error?;

    # List app installations accessible to the user access token
    resource function get user/installations(map<string|string[]> headers = {}, AppsListInstallationsForAuthenticatedUserQueries queries) returns InstallationResponse|error?;

    # List repositories accessible to the user access token
    resource function get user/installations/[int installationId]/repositories(map<string|string[]> headers = {}, AppsListInstallationReposForAuthenticatedUserQueries queries) returns RepositoryResponse|error?;

    # Add a repository to an app installation
    resource function put user/installations/[int installationId]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Remove a repository from an app installation
    resource function delete user/installations/[int installationId]/repositories/[int repositoryId](map<string|string[]> headers = {}) returns error?;

    # Get interaction restrictions for your public repositories
    resource function get user/interaction\-limits(map<string|string[]> headers = {}) returns InteractionLimitResponseAny|error?;

    # Set interaction restrictions for your public repositories
    resource function put user/interaction\-limits(InteractionLimit payload, map<string|string[]> headers = {}) returns InteractionLimitResponse|error;

    # Remove interaction restrictions from your public repositories
    resource function delete user/interaction\-limits(map<string|string[]> headers = {}) returns error?;

    # List user account issues assigned to the authenticated user
    resource function get user/issues(map<string|string[]> headers = {}, IssuesListForAuthenticatedUserQueries queries) returns Issue[]|error?;

    # List public SSH keys for the authenticated user
    resource function get user/keys(map<string|string[]> headers = {}, UsersListPublicSshKeysForAuthenticatedUserQueries queries) returns Key[]|error?;

    # Create a public SSH key for the authenticated user
    resource function post user/keys(UserKeysBody payload, map<string|string[]> headers = {}) returns Key|error?;

    # Get a public SSH key for the authenticated user
    resource function get user/keys/[int keyId](map<string|string[]> headers = {}) returns Key|error?;

    # Delete a public SSH key for the authenticated user
    resource function delete user/keys/[int keyId](map<string|string[]> headers = {}) returns error?;

    # List subscriptions for the authenticated user
    resource function get user/marketplace_purchases(map<string|string[]> headers = {}, AppsListSubscriptionsForAuthenticatedUserQueries queries) returns UserMarketplacePurchase[]|error?;

    # List subscriptions for the authenticated user (stubbed)
    resource function get user/marketplace_purchases/stubbed(map<string|string[]> headers = {}, AppsListSubscriptionsForAuthenticatedUserStubbedQueries queries) returns UserMarketplacePurchase[]|error?;

    # List organization memberships for the authenticated user
    resource function get user/memberships/orgs(map<string|string[]> headers = {}, OrgsListMembershipsForAuthenticatedUserQueries queries) returns OrgMembership[]|error?;

    # Get an organization membership for the authenticated user
    resource function get user/memberships/orgs/[string org](map<string|string[]> headers = {}) returns OrgMembership|error;

    # Update an organization membership for the authenticated user
    resource function patch user/memberships/orgs/[string org](OrgsorgBody1 payload, map<string|string[]> headers = {}) returns OrgMembership|error;

    # List user migrations
    resource function get user/migrations(map<string|string[]> headers = {}, MigrationsListForAuthenticatedUserQueries queries) returns Migration[]|error?;

    # Start a user migration
    resource function post user/migrations(UserMigrationsBody payload, map<string|string[]> headers = {}) returns Migration|error?;

    # Get a user migration status
    resource function get user/migrations/[int migrationId](map<string|string[]> headers = {}, MigrationsGetStatusForAuthenticatedUserQueries queries) returns Migration|error?;

    # Download a user migration archive
    resource function get user/migrations/[int migrationId]/archive(map<string|string[]> headers = {}) returns error?;

    # Delete a user migration archive
    resource function delete user/migrations/[int migrationId]/archive(map<string|string[]> headers = {}) returns error?;

    # Unlock a user repository
    resource function delete user/migrations/[int migrationId]/repos/[string repoName]/'lock(map<string|string[]> headers = {}) returns error?;

    # List repositories for a user migration
    resource function get user/migrations/[int migrationId]/repositories(map<string|string[]> headers = {}, MigrationsListReposForAuthenticatedUserQueries queries) returns MinimalRepository[]|error;

    # List organizations for the authenticated user
    resource function get user/orgs(map<string|string[]> headers = {}, OrgsListForAuthenticatedUserQueries queries) returns OrganizationSimple[]|error?;

    # List packages for the authenticated user's namespace
    resource function get user/packages(map<string|string[]> headers = {}, PackagesListPackagesForAuthenticatedUserQueries queries) returns Package[]|error;

    # Get a package for the authenticated user
    resource function get user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns Package|error;

    # Delete a package for the authenticated user
    resource function delete user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns error?;

    # Restore a package for the authenticated user
    resource function post user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/restore(map<string|string[]> headers = {}, PackagesRestorePackageForAuthenticatedUserQueries queries) returns error?;

    # List package versions for a package owned by the authenticated user
    resource function get user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions(map<string|string[]> headers = {}, PackagesGetAllPackageVersionsForPackageOwnedByAuthenticatedUserQueries queries) returns PackageVersion[]|error;

    # Get a package version for the authenticated user
    resource function get user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns PackageVersion|error;

    # Delete a package version for the authenticated user
    resource function delete user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns error?;

    # Restore a package version for the authenticated user
    resource function post user/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId]/restore(map<string|string[]> headers = {}) returns error?;

    # Create a user project
    resource function post user/projects(UserProjectsBody payload, map<string|string[]> headers = {}) returns Project|error?;

    # List public email addresses for the authenticated user
    resource function get user/public_emails(map<string|string[]> headers = {}, UsersListPublicEmailsForAuthenticatedUserQueries queries) returns Email[]|error?;

    # List repositories for the authenticated user
    resource function get user/repos(map<string|string[]> headers = {}, ReposListForAuthenticatedUserQueries queries) returns Repository[]|error?;

    # Create a repository for the authenticated user
    resource function post user/repos(UserReposBody payload, map<string|string[]> headers = {}) returns Repository|error?;

    # List repository invitations for the authenticated user
    resource function get user/repository_invitations(map<string|string[]> headers = {}, ReposListInvitationsForAuthenticatedUserQueries queries) returns RepositoryInvitation[]|error?;

    # Decline a repository invitation
    resource function delete user/repository_invitations/[int invitationId](map<string|string[]> headers = {}) returns error?;

    # Accept a repository invitation
    resource function patch user/repository_invitations/[int invitationId](map<string|string[]> headers = {}) returns error?;

    # List social accounts for the authenticated user
    resource function get user/social_accounts(map<string|string[]> headers = {}, UsersListSocialAccountsForAuthenticatedUserQueries queries) returns SocialAccount[]|error?;

    # Add social accounts for the authenticated user
    resource function post user/social_accounts(UserSocialAccountsBody payload, map<string|string[]> headers = {}) returns SocialAccount[]|error?;

    # Delete social accounts for the authenticated user
    resource function delete user/social_accounts(UserSocialAccountsBody1 payload, map<string|string[]> headers = {}) returns error?;

    # List SSH signing keys for the authenticated user
    resource function get user/ssh_signing_keys(map<string|string[]> headers = {}, UsersListSshSigningKeysForAuthenticatedUserQueries queries) returns SshSigningKey[]|error?;

    # Create a SSH signing key for the authenticated user
    resource function post user/ssh_signing_keys(UserSshSigningKeysBody payload, map<string|string[]> headers = {}) returns SshSigningKey|error?;

    # Get an SSH signing key for the authenticated user
    resource function get user/ssh_signing_keys/[int sshSigningKeyId](map<string|string[]> headers = {}) returns SshSigningKey|error?;

    # Delete an SSH signing key for the authenticated user
    resource function delete user/ssh_signing_keys/[int sshSigningKeyId](map<string|string[]> headers = {}) returns error?;

    # List repositories starred by the authenticated user
    resource function get user/starred(map<string|string[]> headers = {}, ActivityListReposStarredByAuthenticatedUserQueries queries) returns Repository[]|error?;

    # Check if a repository is starred by the authenticated user
    resource function get user/starred/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # Star a repository for the authenticated user
    resource function put user/starred/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # Unstar a repository for the authenticated user
    resource function delete user/starred/[string owner]/[string repo](map<string|string[]> headers = {}) returns error?;

    # List repositories watched by the authenticated user
    resource function get user/subscriptions(map<string|string[]> headers = {}, ActivityListWatchedReposForAuthenticatedUserQueries queries) returns MinimalRepository[]|error?;

    # List teams for the authenticated user
    resource function get user/teams(map<string|string[]> headers = {}, TeamsListForAuthenticatedUserQueries queries) returns TeamFull[]|error?;

    # List users
    resource function get users(map<string|string[]> headers = {}, UsersListQueries queries) returns SimpleUser[]|error?;

    # Get a user
    resource function get users/[string username](map<string|string[]> headers = {}) returns UserResponse|error;

    # Get list of conflicting packages during Docker migration for user
    resource function get users/[string username]/docker/conflicts(map<string|string[]> headers = {}) returns Package[]|error;

    # List events for the authenticated user
    resource function get users/[string username]/events(map<string|string[]> headers = {}, ActivityListEventsForAuthenticatedUserQueries queries) returns Event[]|error;

    # List organization events for the authenticated user
    resource function get users/[string username]/events/orgs/[string org](map<string|string[]> headers = {}, ActivityListOrgEventsForAuthenticatedUserQueries queries) returns Event[]|error;

    # List public events for a user
    resource function get users/[string username]/events/'public(map<string|string[]> headers = {}, ActivityListPublicEventsForUserQueries queries) returns Event[]|error;

    # List followers of a user
    resource function get users/[string username]/followers(map<string|string[]> headers = {}, UsersListFollowersForUserQueries queries) returns SimpleUser[]|error;

    # List the people a user follows
    resource function get users/[string username]/following(map<string|string[]> headers = {}, UsersListFollowingForUserQueries queries) returns SimpleUser[]|error;

    # Check if a user follows another user
    resource function get users/[string username]/following/[string targetUser](map<string|string[]> headers = {}) returns error?;

    # List gists for a user
    resource function get users/[string username]/gists(map<string|string[]> headers = {}, GistsListForUserQueries queries) returns BaseGist[]|error;

    # List GPG keys for a user
    resource function get users/[string username]/gpg_keys(map<string|string[]> headers = {}, UsersListGpgKeysForUserQueries queries) returns GpgKey[]|error;

    # Get contextual information for a user
    resource function get users/[string username]/hovercard(map<string|string[]> headers = {}, UsersGetContextForUserQueries queries) returns Hovercard|error;

    # Get a user installation for the authenticated app
    resource function get users/[string username]/installation(map<string|string[]> headers = {}) returns Installation|error;

    # List public keys for a user
    resource function get users/[string username]/keys(map<string|string[]> headers = {}, UsersListPublicKeysForUserQueries queries) returns KeySimple[]|error;

    # List organizations for a user
    resource function get users/[string username]/orgs(map<string|string[]> headers = {}, OrgsListForUserQueries queries) returns OrganizationSimple[]|error;

    # List packages for a user
    resource function get users/[string username]/packages(map<string|string[]> headers = {}, PackagesListPackagesForUserQueries queries) returns Package[]|error;

    # Get a package for a user
    resource function get users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns Package|error;

    # Delete a package for a user
    resource function delete users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName](map<string|string[]> headers = {}) returns error?;

    # Restore a package for a user
    resource function post users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/restore(map<string|string[]> headers = {}, PackagesRestorePackageForUserQueries queries) returns error?;

    # List package versions for a package owned by a user
    resource function get users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions(map<string|string[]> headers = {}) returns PackageVersion[]|error;

    # Get a package version for a user
    resource function get users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns PackageVersion|error;

    # Delete package version for a user
    resource function delete users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId](map<string|string[]> headers = {}) returns error?;

    # Restore package version for a user
    resource function post users/[string username]/packages/["npm"|"maven"|"rubygems"|"docker"|"nuget"|"container" packageType]/[string packageName]/versions/[int packageVersionId]/restore(map<string|string[]> headers = {}) returns error?;

    # List user projects
    resource function get users/[string username]/projects(map<string|string[]> headers = {}, ProjectsListForUserQueries queries) returns Project[]|error;

    # List events received by the authenticated user
    resource function get users/[string username]/received_events(map<string|string[]> headers = {}, ActivityListReceivedEventsForUserQueries queries) returns Event[]|error;

    # List public events received by a user
    resource function get users/[string username]/received_events/'public(map<string|string[]> headers = {}, ActivityListReceivedPublicEventsForUserQueries queries) returns Event[]|error;

    # List repositories for a user
    resource function get users/[string username]/repos(map<string|string[]> headers = {}, ReposListForUserQueries queries) returns MinimalRepository[]|error;

    # Get GitHub Actions billing for a user
    resource function get users/[string username]/settings/billing/actions(map<string|string[]> headers = {}) returns ActionsBillingUsage|error;

    # Get GitHub Packages billing for a user
    resource function get users/[string username]/settings/billing/packages(map<string|string[]> headers = {}) returns PackagesBillingUsage|error;

    # Get shared storage billing for a user
    resource function get users/[string username]/settings/billing/shared\-storage(map<string|string[]> headers = {}) returns CombinedBillingUsage|error;

    # List social accounts for a user
    resource function get users/[string username]/social_accounts(map<string|string[]> headers = {}, UsersListSocialAccountsForUserQueries queries) returns SocialAccount[]|error;

    # List SSH signing keys for a user
    resource function get users/[string username]/ssh_signing_keys(map<string|string[]> headers = {}, UsersListSshSigningKeysForUserQueries queries) returns SshSigningKey[]|error;

    # List repositories starred by a user
    resource function get users/[string username]/starred(map<string|string[]> headers = {}, ActivityListReposStarredByUserQueries queries) returns StarredRepositoryResponse|error;

    # List repositories watched by a user
    resource function get users/[string username]/subscriptions(map<string|string[]> headers = {}, ActivityListReposWatchedByUserQueries queries) returns MinimalRepository[]|error;

    # Get all API versions
    resource function get versions(map<string|string[]> headers = {}) returns string[]|error;

    # Get the Zen of GitHub
    resource function get zen(map<string|string[]> headers = {}) returns string|error;
}

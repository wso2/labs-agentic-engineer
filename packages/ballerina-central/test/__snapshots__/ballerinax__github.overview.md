...
| Types | 1,224 declarations (1,101 records, 53 unions, 70 other), not listed here — read one with `type` |

## Next

- `bal-library ops ballerinax/github <path>` — navigate a client's operations
- `bal-library type ballerinax/github <Name> [--deps]` — read a declaration whole
- `bal-library api ballerinax/github` — every declaration, when nothing above answered
...
### Resource functions — 903, call with `->` and a path

**Not listed here** — 903 operations, 180,118 bytes of signatures, over the 100-operation limit. Top-level path segments, with the number of operations under each:
...
- navigate: `bal-library ops ballerinax/github <segment>`
- signatures under a path: `bal-library ops ballerinax/github '<path>' --sigs`

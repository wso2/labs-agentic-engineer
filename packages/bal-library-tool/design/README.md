...
| [0003](decisions/ADR-0003-search-keeps-centrals-relevance.md) | `search` keeps Central's relevance and demotes unadopted packages |
...
| [0016](decisions/ADR-0016-ops-addresses-the-whole-callable-surface.md) | `ops` addresses remote, normal and module-level functions by name, not only resource functions by path — **superseded by 0019**, findings carried forward |
| [0017](decisions/ADR-0017-the-entry-document-is-ordered-to-survive-a-window.md) | `overview` leads with facts, quoted worked code and navigation and ends with the unbounded signature sections; the readme's prose becomes the `guide` verb — *amended 2026-08-17: it generates no signatures at all, and the readme is addressable by chunk* |
...
| [0019](decisions/ADR-0019-the-callable-surface-is-split-by-how-it-is-called.md) | The callable surface is `client`/`class`/`funcs`, split by call form over a derived partition; a wrong kind guess costs one printed line — *supersedes 0016* | *appendix 2026-08-18: the spelling tolerance made uniform — `[owner]`, one-token accessor+path, and `-s` over path segments*
...
- **Four addressed documents in two registers.** A document either *is* Ballerina or it *describes* a
  ...
  blocks `overview` selects out of it under `## Usage` (ADR-0017).

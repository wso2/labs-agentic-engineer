...
// Tiny DSL → Excalidraw scene converter for DOMAIN MODELS (@aep/excalidraw-dsl).
//
// The domain-model dialect declares entities with typed attributes and the
// relations between them; the compiler lays them out (layered, Sugiyama-style)
// and owns every Excalidraw-format concern (required fields, ids, styling).
//
//   entity Order
//     id: uuid
//     total: decimal
...
export function dslToExcalidraw(dsl: string): string {
  const elements = renderDomainModel(parseDomainModelDsl(dsl));
...
export function tryDslToExcalidraw(
  ...
  try {
    const json = dslToExcalidraw(dsl);
    const parsed = JSON.parse(json) as ExcalidrawScene;
    if (parsed.elements.length === 0) {
      return { ok: false, error: 'no entities parsed — expected `entity <Name>` blocks' };
    }
    return { ok: true, json };
  } catch (e) {

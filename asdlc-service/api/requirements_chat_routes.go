...
func registerRequirementsChatRoutes(rt *Router, c controllers.RequirementsChatController) {
	...
	rt.OrgScoped("GET "+prefix+"/chat/baseline/{baselineId}/files/{name}", c.GetBaselineFile)
	rt.OrgScoped("POST "+prefix+"/chat/baseline/{baselineId}/files/{name}/revert", c.RevertBaselineFile)
	rt.OrgScoped("DELETE "+prefix+"/chat/baseline/{baselineId}", c.DropBaseline)
}
